...
void shutdown_my_dcc_recv_socket(void)
{
    ...
    {
        if(FD_ISSET(dcc_conductor->dcc_socket, &read_master)) shutdown((int)dcc_conductor->dcc_socket,(int)2);
        if(FD_ISSET(dcc_conductor->dcc_socket, &read_master)) FD_CLR(dcc_conductor->dcc_socket, &read_master);
        if(FD_ISSET(dcc_conductor->dcc_socket, &write_master)) FD_CLR(dcc_conductor->dcc_socket, &write_master);
...
void shutdown_my_dcc_send_socket(void)
{
    ...
    {
        if(FD_ISSET(dcc_send_conductor->dcc_listen_socket, &read_master)) shutdown((int)dcc_send_conductor->dcc_listen_socket,(int)2);
        if(FD_ISSET(dcc_send_conductor->dcc_listen_socket, &read_master)) FD_CLR(dcc_send_conductor->dcc_listen_socket, &read_master);
        if(FD_ISSET(dcc_send_conductor->dcc_listen_socket, &write_master)) FD_CLR(dcc_send_conductor->dcc_listen_socket, &write_master);
...
void dcc_chat_connect()
{
    ...
    dcc_chat_conductor->dcc_socket = accept(dcc_chat_conductor->dcc_listen_socket, (struct sockaddr *)&dcc_chat_conductor->their_addr, &sin_size);

    IoctlSocket(dcc_chat_conductor->dcc_socket, FIONBIO, (char*)&i);
    IoctlSocket(dcc_chat_conductor->dcc_socket, FIOASYNC, (char*)&i);

    if(!dcc_chat_conductor->str) { dcc_chat_conductor->str=new char[BUFFERSIZE*2]; strcpy(dcc_chat_conductor->str,""); }

    if(dcc_chat_conductor->dcc_socket == -1)
    {
        timestamp_2_string();

        sprintf(buffer3,"%s%sDCC%s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,188,"Unable to establish DCC CHAT Connection"));
    ...
    else
    {
        ...
        if(fdmax<dcc_chat_conductor->dcc_socket) fdmax=dcc_chat_conductor->dcc_socket;
...
void dcc_time_to_send(void)
{
    ...
    sprintf(buffer3,"%s%sDCC%s %s %s [%lu bytes] >> %s. ",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,175,"Finished sending"),dcc_send_conductor->filename,dcc_send_conductor->filesize, dcc_send_conductor->nick);

    dcc_time();

    secs=secs-atoi(dcc_send_conductor->timestarted);

    mins=secs/60; hrs=mins/60; days=hrs/24; secs=secs%60; mins=mins%60; hrs=hrs%24;
    ...
    if(hrs>0) sprintf(buffer2,"%02lu:%02lu:%02lu",hrs,mins,secs);
    else if(mins>0) sprintf(buffer2,"00:%02lu:%02lu",mins,secs);
    else sprintf(buffer2,"00:00:%02lu",secs);


    strcat(buffer3,buffer2);
    //strcat(buffer3,", at an average speed of ");
    strcat(buffer3,". "); strcat(buffer3,GetCatalogStr(catalog,178,"Average speed: ")); strcat(buffer3," ");
...
void accept_dcc(char *b)
{
    ...
    if(DEBUG) printf("trying to accept\n");
    ...
    if(DEBUG) printf("found the dcc conductor entry\n");
    ...
    //if(DEBUG) printf("dcc 6\n");
                        ...
                        {
                            if(DEBUG) printf("examined file for dcc resume:%s\n",dcc_conductor->full_filename_with_path);
                            if(examined_file)
                            {
        //if(DEBUG) printf("dcc 7\n");
                                ...
                                if(ExamineFH(examined_file, a_fib))
                                {
                                    if(a_fib->fib_Size > 0)
                                    {
                                        ...
                                        sprintf(e,"\"%s\" %s",dcc_conductor->full_filename_with_path,GetCatalogStr(catalog,202,"already exists!"));
                                        sprintf(c,"%s: %lu bytes",GetCatalogStr(catalog,203,"Size of existing file"),a_fib->fib_Size);
                                        sprintf(d,"%s: %lu bytes",GetCatalogStr(catalog,204,"Size of file being offered"),dcc_conductor->filesize);
    ...
    if(DEBUG) printf("connect() attempted on dcc socket..\n");
...
void dcc_send_data(int a)
{
    ...
    {
    //printf("sending data\n");
    recv_thing=recv(dcc_send_conductor->dcc_socket, (i_in)dcc_send_conductor->recv_buffer, DCC_RECV_BUFFERSIZE-1, 0);
    ...
    if(!dcc_send_conductor->dcc_file) return;
    ...
    if(dcc_send_conductor->total_recv >= dcc_send_conductor->filesize)
    {
        ...
        if(dcc_send_conductor->connected && !dcc_send_conductor->cancelled)
        {
            ...
            sprintf(buffer3,"%s%sDCC%s SEND: %s %s %s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),dcc_send_conductor->filename,GetCatalogStr(catalog,205,"to"),dcc_send_conductor->nick, GetCatalogStr(catalog,179,"Completed"));
            dcc_time_to_send();
    ...
    else if(recv_thing == -1 || recv_thing == 0)
    {
        ...
        if(DEBUG) printf("error number %ld\n",Errno());
...
int reoffer_dcc(char *networkname, char *string1, char *string2, char *string3, int portnumber)
{
    ...
    if(my_settings.autogethostid) addr=htonl(gethostid());
        else addr=htonl(dcc_addr);
    ...
    //{
        ...
        if(string3[strlen(string3)-count_chars]==':' || string3[strlen(string3)-count_chars]=='/') count_chars--;

        for(a=0; a<=count_chars;a++)
        {
            file_name[a]=string3[strlen(string3)-count_chars+a];
            if(file_name[a] == ' ') file_name[a] = '_';
        }
//        file_name[a] == '\0';
        ...
        if(!examined_file)
        {
            sprintf(buffer3,"%s%sDCC%s SEND: %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,191,"Error, file doesnt exist"));
            add_text_to_current_list(buffer3,9, ACTIVITY_CHAT);
        ...
        if(ExamineFH(examined_file, a_fib))
        {
            ...
            {
                if(!stricmp(status_conductor->networkname,networkname)) break;
                status_conductor=status_conductor->next;
            }
...
int offer_dcc(char *networkname, char *string1, char *string2, char *string3, int portnumber)
{
    ...
    if(!stricmp(string1,"send"))
    {
        ...
        if(my_settings.autogethostid) addr=htonl(gethostid()); //lets use the Local IP addess of this machine
            else if(my_settings.ipaddr_use) //lets use the User Specified IP address
            {
                ...
                {
                    sprintf(buffer3,"%s%sDCC%s Unable to use specified DCC address",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"));
                    add_text_to_current_list(buffer3,9, ACTIVITY_CHAT);
                    return 0;
                }

            }
            else addr=htonl(dcc_addr); //lets use the address automatically fetched from the server upon connection
...
void create_send_dcc(char *nick, char *string3, int filesize, int portnumber)
{
    ...
    {
        if(!dcc_send_conductor->removed) dcc_transfers++;
    ...
    if(!strcmp(nick,"0") && !strcmp(string3,"0")) return;
    ...
    if(DEBUG)
    {
        if(dcc_send_conductor->dcc_listen_socket == -1) printf("unable to create listening socket\nerror number:%li\n",Errno());
        else printf("dcc listen socket created, number: %li\n",dcc_send_conductor->dcc_listen_socket);
    }

    if(fdmax<dcc_send_conductor->dcc_listen_socket) fdmax = dcc_send_conductor->dcc_listen_socket;
    dcc_send_conductor->dcc_socket = -1;
    ...
    if(nick) strcpy(dcc_send_conductor->entry->nick,nick);
    if(nick) strcpy(dcc_send_conductor->nick,nick);
    else strcpy(dcc_send_conductor->entry->nick,"Unknown?");
    strcpy(dcc_send_conductor->entry->percentage,"0%");
    strcpy(dcc_send_conductor->entry->status,GetCatalogStr(catalog,206,"Waiting"));
...
//    file_name[a] == '\0';
    ...
    if(file_name) strcpy(dcc_send_conductor->entry->filename,file_name);
    if(file_name) strcpy(dcc_send_conductor->filename,file_name);

    if(string3) strcpy(dcc_send_conductor->filepath,string3);

    else strcpy(dcc_send_conductor->entry->filename,"Unknown?");
    strcpy(dcc_send_conductor->entry->bytes_transferred,"0");
    strcpy(dcc_send_conductor->entry->cps,"0");
    if(filesize) sprintf(dcc_send_conductor->entry->filesize,"%dkB",filesize/1024);
    if(filesize) dcc_send_conductor->filesize=filesize;
    ...
    if(setsockopt((int)dcc_send_conductor->dcc_listen_socket, (int)SOL_SOCKET, (int)SO_REUSEADDR,&yes,(int)sizeof(yes)) == -1)
    {
        if(DEBUG) printf("unable to reuse address\n");

    }

    //#ifdef __amigaos4__
    if(setsockopt((int)dcc_send_conductor->dcc_listen_socket, (int)SOL_SOCKET, (int)SO_REUSEPORT,&yes,(int)sizeof(yes)) == -1)
    {
        if(DEBUG) printf("unable to reuse port\n");

    }
    //#endif

    if(bind(dcc_send_conductor->dcc_listen_socket, (struct sockaddr *)&dcc_send_conductor->test, sizeof(dcc_send_conductor->test)) == -1)
    {
        if(DEBUG) printf("unable to bind address to socket, error number:%li\n",Errno());

    }

    if(IoctlSocket(dcc_send_conductor->dcc_listen_socket, FIONBIO, (char*)&yes) < 0)
    {
        if(DEBUG) printf("unable to change non-blocking I/O option for socket, error number:%li\n",Errno());
    }
    ...
    sprintf(buffer3,"%s%sDCC%s SEND: %s %s %s %s %s port %i",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,192,"Offering file"),dcc_send_conductor->filepath,GetCatalogStr(catalog,194,"to"),dcc_send_conductor->nick,GetCatalogStr(catalog,193,"on"),portnumber);
    add_text_to_current_list(buffer3,9, ACTIVITY_CHAT);
    ...
    DoMethod((Object*)WookieChat->LV_send_dcc,MUIM_NList_InsertSingle,dcc_send_conductor->entry,MUIV_NList_Insert_Bottom);
...
void create_recv_dcc_chat(char *nick, char *address, char *port)
{
    ...
    {
        if(!stricmp(nick,dcc_chat_conductor->nick) && dcc_chat_conductor->removed==0) break;
...
void initiate_outgoing_dcc_chat(char *nick)
{
    ...
    if(!stricmp(status_conductor->nick,nick)) return;

    if(my_settings.autogethostid) addr=htonl(gethostid()); else addr=htonl(dcc_addr);

    if(my_settings.current_port > atoi(my_settings.end_port)) my_settings.current_port=atoi(my_settings.start_port);

    if(my_settings.current_port < atoi(my_settings.start_port)) my_settings.current_port=atoi(my_settings.start_port);
    ...
    if(DEBUG)
    {
        if(dcc_chat_conductor->dcc_listen_socket == -1) printf("unable to create listening socket\nerror number:%li\n",Errno());
        else printf("dcc listen socket created, number: %li\n",dcc_chat_conductor->dcc_listen_socket);
    }

    if(fdmax<dcc_chat_conductor->dcc_listen_socket) fdmax = dcc_chat_conductor->dcc_listen_socket;
    ...
    if(setsockopt((int)dcc_chat_conductor->dcc_listen_socket, (int)SOL_SOCKET, (int)SO_REUSEADDR,&yes,(int)sizeof(yes)) == -1)
    {
        if(DEBUG) printf("unable to reuse address\n");

    }

    if(setsockopt((int)dcc_chat_conductor->dcc_listen_socket, (int)SOL_SOCKET, (int)SO_REUSEPORT,&yes,(int)sizeof(yes)) == -1)
    {
        if(DEBUG) printf("unable to reuse port\n");

    }

    if(bind(dcc_chat_conductor->dcc_listen_socket, (struct sockaddr *)&dcc_chat_conductor->test, sizeof(dcc_chat_conductor->test)) == -1)
    {
        if(DEBUG) printf("unable to bind address to socket, error number:%li\n",Errno());
    ...
    sprintf(buffer3,"%s%sDCC%s %s %s %s %s port %i",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,195,"Sending DCC CHAT request"),GetCatalogStr(catalog,194,"to"),dcc_chat_conductor->nick,GetCatalogStr(catalog,193,"on"),my_settings.current_port);
    status_conductor->conductor=dcc_chat_conductor->conductor;
    add_text_to_conductor_list(buffer3,9, ACTIVITY_CHAT);
...
void create_recv_dcc(char *nick, char *filename, char *address, unsigned short port, char *filesize)
{
    ...
    {
        if(!dcc_conductor->removed) dcc_transfers++;
        dcc_conductor=dcc_conductor->next;
    }
    ...
    if(!strcmp(nick,"0") && !strcmp(filename,"0")) return;
    ...
    if(fdmax<dcc_conductor->dcc_socket) fdmax=dcc_conductor->dcc_socket;
    if(DEBUG) printf("dcc 4\n");
    ...
    if(nick) strcpy(dcc_conductor->entry->nick,nick);
    if(nick) strcpy(dcc_conductor->nick,nick);
    else strcpy(dcc_conductor->entry->nick,"Unknown?");
    strcpy(dcc_conductor->entry->percentage,"0%");
    strcpy(dcc_conductor->full_filename_with_path,"");

    if(filename) for(unsigned long count=0; count<strlen(filename); count++) { if(filename[count]==' ') filename[count]='_'; }

    if(filename) strcpy(dcc_conductor->entry->filename,filename);
    if(filename) strcpy(dcc_conductor->filename,filename);
    else strcpy(dcc_conductor->entry->filename,"Unknown?");
    strcpy(dcc_conductor->entry->bytes_transferred,"0");
    strcpy(dcc_conductor->entry->cps,"0");
    if(filesize) sprintf(dcc_conductor->entry->filesize,"%dkB",atoi(filesize)/1024);
    if(filesize) dcc_conductor->filesize=atoi(filesize);
    ...
    if(!string1) setmacro((Object*)WookieChat->WI_dcc,MUIA_Window_Open, TRUE); //if the window is already open, dont open it again
    ...
    if(DEBUG) printf("dcc recv entry created with the following details:\nnick:%s\nfilename:%s\naddress:%s\nport:%d\nfilesize:%s\nTime to accept!\n",nick, filename, address, port, filesize);


    if(my_settings.dcc_recv_auto_accept) accept_dcc(string1);
