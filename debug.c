...
**
**  For details and usage read below.
**
**  DISTRIBUTION
**
...
#ifndef DEBUG_CODE_C
#define DEBUG_CODE_C 1

/*************************************************************************

 Function: Dump debug information over the system debug port

 1991 - 2013 by Guido Mersmann

 $VER: Debug 4.00 (23.06.2012)
...
**************************************************************************
 ...
 Changes:
	  ...
	  (09-01-2009) Guido Mersmann

	  Some cleanups

	  (23-06-2013) Guido Mersmann

	  DEBUG_MAXOUTPUTSIZE can now be configured by external define. Using
	  0 as value will disable line size check.
	  ASCII_FILTERCHAR can now be configured by external define.
...
**************************************************************************
 ...
 ToDo:
      ...
      ° There is no direct serial output for Amithlon. If you want to
		...
		comes together. (Will be implemented when needed!)

		NOTE: If you want to redirect the serial output, execute eg.

	    run >nil: sushi >"con:50/50/400/160/sushi/auto" NOPROMPT

		On MorphOS I suggest to use LogTool, which comes with the SDK to
		get realtime output.

 CONFIG:

		When including debug.c from inside a c file use: "#define DEBUG 1"
		before including. From makefile simply add debug.c to object list and
		define -DDEBUG to enable.

		Usage is like with printf(), just check the arguments below. When using
		debug code in makefile it is turned on by default for all objects. You
		can override this default by defining NODEBUG. -DDEBUG -DNODEBUG will
		compile debug output in, but disable by default. You can force output
		again by using #define FORCEDEBUG 1 within your code before including
		the debug.h file.

        To test these functions and give you an example how to use the stuff,
        use the following statement and compile this file stand alone:
		
		"gcc -DDEBUG_TESTCODE debug.c -o testcode"
		or
		"vc  -DDEBUG_TESTCODE debug.c -o testcode"
   ...
   Types and sizes supported by this debug function: ( case independent)
    ...
    without the size indicator these modes are valid. (Note a single 'b' is not allowed here)
					...
					%t  =  wait n seconds by using timer. Useful when searching for a crashpoint.
						   ...
						   (e.g. "Where is the crash?\n" "%4t" creates 4 second delay) Please note
						   that text within the same debug call as the %t argument is called AFTER
						   the delay. You may need to add two debug lines, to see what you are
						   waiting for.
...
#undef debug /* kill prototype macro, if exists again */

/*************************************************************************/

static void IDGB_String         ( char *string, LONG size, ULONG flags, long counter );
static void IDGB_RepeatChar     ( char chr, long count );
static void IDGB_PutString      ( char *string, long count );
static void IDGB_PutChar        ( char chr);
...
#endif  /* HEXDUMP_NIBBLESPERLINE */

/*
** DEBUG_MAXOUTPUTSIZE is quite tricky. It cuts output
** of strings, e.g. when dumping a false string pointer,
** this could end in a gigantic long standing loop.
**
** This define enables a check before output, so instead
** of dumping trillions of chars the limit is set to 100
**
** The silent option allows to just disable output
*/

#ifndef DEBUG_MAXOUTPUTSIZE
#define DEBUG_MAXOUTPUTSIZE 100  /* 0 turns of checking */
#endif  /* DEBUG_MAXOUTPUTSIZE */

#ifndef DEBUG_MAXOUTPUTSIZESILENT
#define DEBUG_MAXOUTPUTSIZESILENT 0  /* 0 turns of warning */
#endif  /* DEBUG_MAXOUTPUTSIZESILENT */

/* ASCII_FILTERCHAR defines the character which is
** used for unprintable characters
*/

#ifndef ASCII_FILTERCHAR
#define ASCII_FILTERCHAR        46
#endif /* ASCII_FILTERCHAR */

/* now some defines to make code more readable
*/
...
/* For 68K we need to create RawPutChar inline depending on compiler */
    ...
    #endif
#endif
/* AmigaOS4 needs special code, too */
#ifdef __amigaos4__
...
*/
#if DEBUG_MAXOUTPUTSIZE
	if( count > DEBUG_MAXOUTPUTSIZE ) {
		#if !DEBUG_MAXOUTPUTSIZESILENT
		debug("\n\nRepeatChar: Warning!! Reduced count from %ld to %ld\n\n", count, DEBUG_MAXOUTPUTSIZE );
		#endif
		count = DEBUG_MAXOUTPUTSIZE;
    }
#endif
...
/* now we can dump the line data as ASCII */
            IDGB_PutChar( 39 );
			while( (long) str < value ) {
                chr = *((BYTE *) str++);
                if( chr != ASCII_RETURN && chr != ASCII_TAB ) {
                    IDGB_PutChar( chr);
                } else {
                    IDGB_PutChar( ASCII_FILTERCHAR );
                }
            }
            IDGB_PutChar( 39 );
...
long arg5 = 0xa50EC05a;
	...
	debug ("Now we wait 5 Seconds before continuing...\n");
	debug ("%5T");
	debug ("... Done!\n");
