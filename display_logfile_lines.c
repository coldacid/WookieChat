...
int display_last_few_lines_of_logfile_conductor()
{
    ...
    if (char_read_storage == -1L)
        return 0;

    if (status_conductor->conductor->name[0] == '#')
        how_many_lines = my_settings.how_many_lines_to_reload_channel;
    else
        how_many_lines = my_settings.how_many_lines_to_reload_private;
    ...
    while (count_the_linefeeds < how_many_lines + 1)
    {
        if (char_read_storage == '\n')
            count_the_linefeeds++;
#ifdef __amiagaos4__
        if(ChangeFilePosition(status_conductor->conductor->log_file, -2, OFFSET_CURRENT))
        {   count_the_linefeeds++; break;}
#else
        if (Seek(status_conductor->conductor->log_file, -2, OFFSET_CURRENT) == -1)
        {
            count_the_linefeeds++;
            break;
        }
...
#else
    if (count_the_linefeeds >= how_many_lines + 1)
        Seek(status_conductor->conductor->log_file, 1, OFFSET_CURRENT);
    else
    {
        Seek(status_conductor->conductor->log_file, -1, OFFSET_CURRENT);
        count = 1;
    ...
    for (;;)
    {
        ...
        if (char_read_storage == '\n')
        {
            ...
            if (ZUNE_SYSTEM == TRUE)
                ...
                strcpy(centry->entry, "         :");
            centry->colour = 9;
            if (centry->colour >= 0 && centry->colour <= 22)
                sprintf(new_array, "\033P[%ld]", custom_pen_colours[centry->colour]);
            else
        ...
        else if (char_read_storage == -1L)
            break;
        else
            buffer_text[a] = char_read_storage;
        a++;
        if (count == count_the_linefeeds)
            break;
