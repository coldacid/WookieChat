...
void acquire_connect_details(char *work_buffer)
{
    ...
    while(string1)
    {
        ...
        {
            if(string1[count]!='\0' && string1[count]!='=' && string1[count]!=' ') parameter[count2]=string1[count];
            else { count++; parameter[count2]='\0'; break; }
        }
        for(count2=0; count<=(LONG)strlen(string1); count2++,count++)
        {
            if(string1[count]!='\0' && string1[count]!=' ') the_value[count2]=string1[count];
            else { count++; the_value[count2]='\0'; break; }
        }

        if(!stricmp(parameter,"SERVER")) strcpy(server_name,the_value);
        else if(!stricmp(parameter,"PORT")) strcpy(port_number,the_value);
        else if(!stricmp(parameter,"SERVER_CHARSET")) strcpy(server_charset,the_value);
        else if(!stricmp(parameter,"NICK_PASSWORD")) strcpy(nick_password,the_value);
        else if(!stricmp(parameter,"SERVER_PASSWORD")) strcpy(server_password,the_value);
        else if(!stricmp(parameter,"AUTOJOINS")) strcpy(auto_joins,the_value);
        else if(!stricmp(parameter,"AUTOCONNECT")) strcpy(auto_connect,the_value);
        else if(!stricmp(parameter,"+") || !stricmp(parameter,"Euro")) strcpy(server_charset,"ISO-8859-1 + Euro");
        else if(!stricmp(parameter,"NICK1")) strcpy(server_nick,the_value);
        else if(!stricmp(parameter,"NICK2")) strcpy(server_nick2,the_value);
        else if(!stricmp(parameter,"NICK3")) strcpy(server_nick3,the_value);
        else if(!stricmp(parameter,"USERNAME")) strcpy(server_user_name,the_value);
        else if(!stricmp(parameter,"REALNAME")) strcpy(server_real_name,the_value);
        else if(!stricmp(parameter,"USE_GLOBAL"))
        {
            if(!strcmp(the_value,"1")) use_global=1;
            else use_global=0;
        ...
        */
        else if(the_value[0]=='\0') strcpy(server_name,string1);
...
int connect2server(char *servername, char *port_number, int typedservercommand, int set_connect_script_name, int connect_in_current_tab, char *requested_remote_charset, int write_new_passwords_to_this_node)
{
    if(use_global)
    {
    ...
    else
    {
        ...
        if(Pro_Charsets_Enabled==FALSE)
        {
            ...
            if(remote_charsets[status_conductor->remote_charset]==NULL) status_conductor->remote_charset=local_charset;
        ...
        else
        {
            ...
            if(remote_charsets[status_conductor->remote_charset]==NULL || count==total_charsets) status_conductor->remote_charset=local_charset;
        }
        ...
        if(set_connect_script_name==1) //do we want to set the connect script name? YES!!
        {
            ...
            ULONG empty = DoMethod((Object*)WookieChat->NLT_Servers, MUIM_NListtree_GetNr, MUIV_NListtree_GetNr_TreeNode_Active, 0);
            int h=empty;

            while(h != 0)
            {
                treenode = (struct MUI_NListtree_TreeNode *) DoMethod((Object*)WookieChat->NLT_Servers, MUIM_NListtree_GetEntry, MUIV_NListtree_GetEntry_ListNode_Root, h, 0);
                if(!treenode) break;
                if(!(strchr(treenode->tn_Name,' ') > 0)) break;
                h=h-1;
            ...
            if(treenode)
            {
                ...
                if(typedservercommand) strcpy(status_conductor->groupname,servername);
            }
            else strcpy(status_conductor->groupname,servername);
        ...
        if(strcmp(status_conductor->groupname,"") != 0) strcpy(status_conductor->networkname,status_conductor->groupname);
        ...
        if(!SocketBase)
        {
            ...
            sprintf(buffer3,"%s%s%s%s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,0,"Error"),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,198,"Please start a TCP/IP stack and try again."));
        ...
        if(DEBUG) printf("server name you're trying to connect to is \"%s\"\n",servername);
        ...
        {
            timestamp_2_string();
            sprintf(buffer3, "%s%s%s%s %s: gethostbyname() %s.",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,0,"Error"),GetCatalogStr(catalog,218,"]"),servername,GetCatalogStr(catalog,184,"Failed"));
            add_text_to_current_list(buffer3,9, ACTIVITY);
        ...
        if(!(status_conductor->portnumber)) status_conductor->portnumber=6667;
        ...
        if(status_conductor->connection_active)
        {
            ...
            sprintf(buffer3, "%s%sConnect%s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,199,"Connecting to a new server"));
            status_conductor->conductor=status_conductor->root;
            while(status_conductor->conductor)
            {
                if(status_conductor->conductor->LV_channel &&
                  (status_conductor->conductor->name[0]=='#' || status_conductor->conductor==status_conductor->root))
                {
        ...
        if(my_settings.use_ident)
        {
            ...
            if(DEBUG)
            {
                if(status_conductor->ident.listen_socket == -1) printf("unable to create listening socket\nerror number:%li\n",Errno());
                else printf("dcc listen socket created, number: %li\n",status_conductor->ident.listen_socket);
            }

            if(fdmax<status_conductor->ident.listen_socket) fdmax = status_conductor->ident.listen_socket;
            ...
            if(setsockopt((int)status_conductor->ident.listen_socket, (int)SOL_SOCKET, (int)SO_REUSEADDR,&yes,(int)sizeof(yes)) == -1)
                if(DEBUG) printf("unable to reuse address\n");

            if(setsockopt((int)status_conductor->ident.listen_socket, (int)SOL_SOCKET, (int)SO_REUSEPORT,&yes,(int)sizeof(yes)) == -1)
                if(DEBUG) printf("unable to reuse port\n");

            if(bind(status_conductor->ident.listen_socket, (struct sockaddr *)&status_conductor->ident.test, sizeof(status_conductor->ident.test)) == -1)
        ...
        if(status_conductor->retry_number==0)
        {
            ...
            //strcpy(status_conductor->nick,nick);
            sprintf(buffer3, "%s%sConnect%s %s %s (%i)",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,200,"Attempting to connect to"),servername,status_conductor->portnumber);
            add_text_to_current_list(buffer3,9, ACTIVITY);
        }
        else
        {
            sprintf(buffer3, "%s%sConnect%s %s %s (%i) %s%i",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,200,"Attempting to connect to"),servername,status_conductor->portnumber,GetCatalogStr(catalog,263,"Retry #"), status_conductor->retry_number);
            add_text_to_conductor_list(buffer3,9, ACTIVITY);
