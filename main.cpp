...
#include "auto_reconnect_server.cpp"
...
//#include "switch_between_tabs.cpp"
#include "coloursettings.cpp"
#include "dcc.cpp"
...
void send_text(char *text2)
{
    ...
    if(DEBUG) printf("Wants to send:%s\n",text2);
	...
	*/
	...
	if(utf8) 
	{
		if((char*)utf8 != (char*)buf)	CodesetsFreeA(utf8,NULL);
		utf8=NULL;
	}
    
    //if((utf8 = CodesetsUTF8Create(CSA_Source,  text2, CSA_Dest, buf, CSA_DestLen, sizeof(buf), TAG_DONE)))
    #ifdef __AROS__
    struct TagItem utf8_conv_taglist[] = { {CSA_Source,  (STACKIPTR)text2}, {CSA_Dest, (STACKIPTR)buf}, {CSA_DestLen, (IPTR)sizeof(buf)}, {TAG_DONE,0} };
    #else
    struct TagItem utf8_conv_taglist[] = { {CSA_Source,  (ULONG)text2}, {CSA_Dest,(ULONG)buf}, {CSA_DestLen, sizeof(buf)}, {TAG_DONE,0} };
    #endif
    ...
    {
        sprintf(buffer3,"%s%s%s%s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,0,"Error"),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,134,"Not connected to a server"));
    ...
    if(status_conductor->connection_active==1)
    {
        ...
        if(n==-1)
        {
            ...
            if(DEBUG) printf("error:%li\nunable to send:%s",Errno(),text);

            sprintf(buffer3,"%s%s%s%s Unable to send text: error number: %li",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,0,"Error"),GetCatalogStr(catalog,218,"]"),Errno());
...
void send_dcc_chat(char *text)
{
    ...
    {
        sprintf(buffer3,"%s%s%s%s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,0,"Error"),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,135,"Not connected to a user"));
        add_text_to_conductor_list(buffer3,9, ACTIVITY_CHAT);
        return;
    }
    ...
    else { if(DEBUG) printf("not connected to dcc chat\n"); }
    ...
    if(n==-1)
    {
	    ...
	    if(Errno() == 57 || Errno() == 32) //Error, not connected!!
        {
            ...
            {
                sprintf(buffer3,"%s%s%s%s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,0,"Error"),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,135,"Not connected to a user"));
                if(dcc_chat_conductor->conductor->LV_channel) add_text_to_conductor_list(buffer3,9, ACTIVITY_CHAT);
...
void send_current(char *text2)
{
    ...
    if(status_current->connection_active)
    {
        ...
        if(n==-1)
        {
            ...
            if(DEBUG) printf("error:%li\nunable to send:%s",Errno(),text);
            sprintf(buffer3,"%s%s%s%s Unable to send text: error number: %li",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,0,"Error"),GetCatalogStr(catalog,218,"]"),Errno());
            if(status_current->conductor->LV_channel) add_text_to_current_list(buffer3,9, ACTIVITY_CHAT);
    ...
    else if(DEBUG) printf("no connection\n");
}
...
#else
        mics=TimerIO->tr_time.tv_micro; secs=TimerIO->tr_time.tv_secs;
#endif
        mins=secs/60; hrs=mins/60; days=hrs/24; secs=secs%60; mins=mins%60; hrs=hrs%24;  // Compute days, hours, etc.
        if(hrs<10) sprintf(timestamp_hrs,"0%ld",hrs); else sprintf(timestamp_hrs,"%ld",hrs);
        if(mins<10) sprintf(timestamp_mins,"0%ld",mins); else sprintf(timestamp_mins,"%ld",mins);
        if(secs<10) sprintf(timestamp_secs,"0%ld",secs); else sprintf(timestamp_secs,"%ld",secs);
...
#endif
        mins=secs/60; hrs=mins/60; days=hrs/24; secs=secs%60; mins=mins%60; hrs=hrs%24;  // Compute days, hours, etc
        if(hrs<10) sprintf(pingtimestamp_hrs,"0%ld",hrs); else sprintf(pingtimestamp_hrs,"%ld",hrs);
        if(mins<10) sprintf(pingtimestamp_mins,"0%ld",mins); else sprintf(pingtimestamp_mins,"%ld",mins);
        if(secs<10) sprintf(pingtimestamp_secs,"0%ld",secs); else sprintf(pingtimestamp_secs,"%ld",secs);
...
void set_channel_clipboard_hook(void)
{
    ...
    if(my_settings.which_clipboard_style==COLUMNS || ALTERNATIVE_CLIPBOARD==1)
    {
        ...
        setmacro((Object*)status_current->current_query->LV_channel,MUIA_NList_CopyEntryToClipHook, &Custom_Clipboard2_Hook); //lets use multicolumn hook
    ...
    else
    {
        ...
        setmacro((Object*)status_current->current_query->LV_channel,MUIA_NList_CopyEntryToClipHook, NULL); //lets use built in clipboard hook
...
int main(int argc, char *argv[])
{
    ...
    if(argc>=2)
    {
        ...
        {
            if(!stricmp((p_in)argv[count],(p_in)"geit")) GEIT=1;
            if(!stricmp((p_in)argv[count],(p_in)"geit2")) GEIT2=1;
            if(!stricmp((p_in)argv[count],(p_in)"geit3")) GEIT3=1;
            if(!stricmp((p_in)argv[count],(p_in)"smalltabs")) SMALLTABS=1;
            if(!stricmp((p_in)argv[count],(p_in)"debug")) DEBUG=1;
            if(!stricmp((p_in)argv[count],(p_in)"raw")) RAW=1;
            if(!stricmp((p_in)argv[count],(p_in)"proxy")) using_a_proxy=TRUE;
            if(!stricmp((p_in)argv[count],(p_in)"nozune")) ZUNE_SYSTEM=FALSE;
            if(!stricmp((p_in)argv[count],(p_in)"zune")) ZUNE_SYSTEM=TRUE;
            if(!stricmp((p_in)argv[count],(p_in)"altclipboard")) ALTERNATIVE_CLIPBOARD=1;
            if(!stricmp((p_in)argv[count],(p_in)"aboutgfx")) my_settings.os3_about_window_gfx=1;
            if(!stricmp((p_in)argv[count],(p_in)"arexx")) USE_AREXX=TRUE;
        }
    }
...
// Setup some MsgPort's if we're going to use AREXX
    if(USE_AREXX==TRUE)
    {
        ...
        else printf("cant create process reply port\n");
    }
//
...
// Create Timers
    if(!TimerWait_Open()) cleanexit((char*)"cant create timers\n");


// Create MUI Subclasses
    #ifdef __AROS__
    mcc = MUI_CreateCustomClass(NULL,MUIC_BetterString,NULL,sizeof(struct InstanceData),(APTR)BetterString_Dispatcher);
    ...
    if(!mcc || !mcc2 || !mcc4 || !mcc5 || !mcc6) cleanexit((char*)GetCatalogStr(catalog,140,"These missing classes can be downloaded from http://www.aminet.net/ or http://os4depot.net. Check the WookieChat readme for direct links to these classes\n"));
//

    WookieChat=CreateApp();
    if(!WookieChat) cleanexit((char*)GetCatalogStr(catalog,133,"cant create application\n"));
...
// Create AREXX Process
    if(USE_AREXX==TRUE)
    {
        ...
        if(DEBUG) printf("Main Task Port: %s\n",maintask_basename);
        ...
        if(app_process_replyport)
        {
            ...
            #else
            CreateNewProcTags(NP_Entry,(APTR)AREXX_Task,NP_Name,"WookieChat_AREXX_Task",NP_StackSize,20000,TAG_DONE);
            #endif
        }
        else printf("problems, AREXX process not started\n");
    }
//
...
// Reload all the old URL's in the URL Grabber window
    ...
    if(urlgrabber_file)
    {
        while(FGets(urlgrabber_file,(b_in)urlgrabber_str, 2000))
        {
            ...
            DoMethod((Object*)WookieChat->LV_urlgrabber,MUIM_NList_InsertSingle,urlgrabber_str,MUIV_NList_Insert_Bottom);
        }
...
//
    ...
    DoMethod((Object*)WookieChat->WI_main,MUIM_Notify,MUIA_Window_Activate,TRUE,WookieChat->WI_main,3,MUIM_Set,MUIA_Window_ActiveObject,(Object*)WookieChat->STR_usertext);
    ...
    if(status_conductor->conductor->LV_nicklist) setmacro((Object*)status_conductor->conductor->LV_nicklist,MUIA_NList_ListBackground, popimage_nicklistbackground);
...
// Set GUI cycle chains for when using the TAB key
    DoMethod((Object *)WookieChat->WI_mainsettings,MUIM_Window_SetCycleChain,
    WookieChat->SETTINGS_Register_Grp,
	WookieChat->WI_mainsettings,
	WookieChat->CH_autojoin_channels_when_kicked,
	WookieChat->CH_auto_open_query_tabs_when_msged,
	WookieChat->STR_label_3,
	WookieChat->CH_label_2,
	WookieChat->CH_label_3,
	WookieChat->STR_label_1,
	WookieChat->STR_label_2,
	WookieChat->STR_label_4,
	WookieChat->STR_label_5,
	WookieChat->CH_label_4,
	WookieChat->CH_label_5,
	WookieChat->STR_label_0,
	WookieChat->CH_label_0,
	WookieChat->STR_PA_label_0,
	WookieChat->BT_label_0,
	WookieChat->BT_label_1,
	WookieChat->BT_label_3,
	WookieChat->BT_label_2,
	WookieChat->STR_highlight_label,
    WookieChat->STR_how_many_lines_to_reload_channel,
    WookieChat->STR_how_many_lines_to_reload_private,
	WookieChat->STR_browser,
    WookieChat->RA_nick_completion_style,
	WookieChat->CH_autoaccept,
    WookieChat->LV_trusted_nicks,
    WookieChat->RA_autoaccept,
    WookieChat->CH_gethostid,
    WookieChat->STR_port1_dcc,
    WookieChat->STR_port2_dcc,
    WookieChat->STR_sound_sample_tabopen,
    WookieChat->RA_sound_sample_tabopen,
    WookieChat->STR_sound_sample_highlight,
    WookieChat->RA_sound_sample_highlight,
    WookieChat->STR_samples_path,
    WookieChat->LV_alias,
    WookieChat->BT_add_alias,
    WookieChat->BT_remove_alias,
    //WookieChat->STR_add_alias,
    NULL);


    DoMethod((Object *)WookieChat->WI_label_2,MUIM_Window_SetCycleChain,
	WookieChat->STR_nickname1,
	WookieChat->STR_nickname2,
	WookieChat->STR_nickname3,
	WookieChat->STR_realname,
	WookieChat->STR_username,
	WookieChat->BT_addserver,
	WookieChat->BT_addgroup,
	WookieChat->BT_delete_server,
	//WookieChat->BT_edit,
	WookieChat->BT_connect,
	WookieChat->BT_connect_tab,
    WookieChat->BT_perform,
    WookieChat->NLT_Servers,
    NULL);


    DoMethod((Object *)WookieChat->WI_quit,MUIM_Window_SetCycleChain,
    WookieChat->BT_quit_yes,
    WookieChat->BT_quit_no,
    NULL);
//
...
// Setup notifications for all our GUI objects
    ...
    DoMethod((Object *)WookieChat->LV_tabs, MUIM_Notify, MUIA_NList_EntryClick, MUIV_EveryTime, (Object*)WookieChat->App,2,MUIM_Application_ReturnID,92);

    DoMethod((Object *)WookieChat->NLT_Servers, MUIM_Notify, MUIA_NList_DoubleClick, TRUE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID, 49);
    DoMethod((Object *)WookieChat->NLT_Servers, MUIM_Notify, MUIA_NListtree_Active, MUIV_EveryTime,(Object*)WookieChat->App,2,MUIM_Application_ReturnID, 53);

    DoMethod((Object*)WookieChat->LV_urlgrabber,MUIM_Notify, MUIA_NList_DoubleClick, TRUE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID, 98);

    DoMethod((Object*)WookieChat->BT_quit_yes,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,90);
	DoMethod((Object*)WookieChat->BT_quit_no,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->WI_quit,3,MUIM_Set,MUIA_Window_Open,FALSE);
                                                                                                                   

	DoMethod((Object*)WookieChat->WI_mainsettings,MUIM_Notify,MUIA_Window_CloseRequest,TRUE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,7);
	DoMethod((Object*)WookieChat->WI_main,MUIM_Notify,MUIA_Window_CloseRequest,TRUE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,MUIV_Application_ReturnID_Quit);
    DoMethod((Object *)MN_quit, MUIM_Notify, MUIA_Menuitem_Trigger,MUIV_EveryTime, (Object*)WookieChat->App,2,MUIM_Application_ReturnID,MUIV_Application_ReturnID_Quit);
	
    DoMethod((Object *)MN_MultiColumnDisplay, MUIM_Notify, MUIA_Menuitem_Trigger,MUIV_EveryTime, (Object*)WookieChat->App,2,MUIM_Application_ReturnID,97);
    DoMethod((Object *)MN_Clipboard, MUIM_Notify, MUIA_Menuitem_Trigger,MUIV_EveryTime, (Object*)WookieChat->App,2,MUIM_Application_ReturnID,94);
    DoMethod((Object *)MN_SaveSettings, MUIM_Notify, MUIA_Menuitem_Trigger,MUIV_EveryTime, (Object*)WookieChat->App,2,MUIM_Application_ReturnID,5);

    DoMethod((Object *)MN_MuteSound, MUIM_Notify, MUIA_Menuitem_Trigger,MUIV_EveryTime, (Object*)WookieChat->App,2,MUIM_Application_ReturnID,SECOND_SET_OF_RETURNIDS+1);
    DoMethod((Object*)WookieChat->WI_label_2,MUIM_Notify,MUIA_Window_CloseRequest,TRUE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,2);
	
	DoMethod((Object*)WookieChat->WI_colour_settings,MUIM_Notify,MUIA_Window_CloseRequest,TRUE,(Object*)WookieChat->WI_colour_settings,3,MUIM_Set,MUIA_Window_Open,FALSE);
    DoMethod((Object*)MN_ColourSettings, MUIM_Notify, MUIA_Menuitem_Trigger,MUIV_EveryTime, (Object*)WookieChat->WI_colour_settings,3,MUIM_Set,MUIA_Window_Open,TRUE);
	DoMethod((Object*)WookieChat->BT_colour_settings_load,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,4);
	DoMethod((Object*)WookieChat->BT_colour_settings_save,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,3);
	DoMethod((Object*)WookieChat->BT_colour_settings_save_as,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,93);
	DoMethod((Object*)WookieChat->BT_colour_settings_apply,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,30);

	DoMethod((Object*)WookieChat->BT_graphical_smileys_preview,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,99);
    DoMethod((Object*)WookieChat->WI_graphical_smileys_preview,MUIM_Notify,MUIA_Window_CloseRequest,TRUE,(Object*)WookieChat->WI_graphical_smileys_preview,3,MUIM_Set,MUIA_Window_Open,FALSE);
	DoMethod((Object*)WookieChat->WI_graphical_smileys_choose,MUIM_Notify,MUIA_Window_CloseRequest,TRUE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,95);
                                               
    DoMethod((Object*)WookieChat->WI_dcc,MUIM_Notify,MUIA_Window_CloseRequest,TRUE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,68);
	DoMethod((Object*)WookieChat->WI_dcc2,MUIM_Notify,MUIA_Window_CloseRequest,TRUE,(Object*)WookieChat->WI_dcc2,3,MUIM_Set,MUIA_Window_Open,FALSE);

    DoMethod((Object*)WookieChat->STR_usertext,MUIM_Notify,MUIA_String_Acknowledge, MUIV_EveryTime,(Object*)WookieChat->App, 2, MUIM_Application_ReturnID,1);

    DoMethod((Object*)WookieChat->BT_addserver,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,50);
    DoMethod((Object*)WookieChat->BT_addgroup,MUIM_Notify,MUIA_Selected,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,51);
    DoMethod((Object*)WookieChat->BT_delete_server,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,52);
    DoMethod((Object*)WookieChat->BT_cancelgroup,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,54);
    DoMethod((Object*)WookieChat->BT_acceptgroup,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,55);
    DoMethod((Object*)WookieChat->BT_perform,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,64);
    DoMethod((Object*)WookieChat->BT_cancelserver,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,56);
    ...
    DoMethod((Object*)WookieChat->BT_connect,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,58);
    DoMethod((Object*)WookieChat->BT_connect_tab,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,59);

    DoMethod((Object*)WookieChat->BT_add_alias,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,81);
    DoMethod((Object*)WookieChat->BT_remove_alias,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,82);
    ...
    DoMethod((Object*)WookieChat->STR_edit_alias_name,MUIM_Notify,MUIA_String_Contents, MUIV_EveryTime,(Object*)WookieChat->App, 2, MUIM_Application_ReturnID,91);
    DoMethod((Object*)WookieChat->STR_edit_alias_command,MUIM_Notify,MUIA_String_Contents, MUIV_EveryTime,(Object*)WookieChat->App, 2, MUIM_Application_ReturnID,91);


    //edit server details in the Server Selection window
    DoMethod((Object*)WookieChat->CH_autoconnect,MUIM_Notify,MUIA_Selected,TRUE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,57);
    DoMethod((Object*)WookieChat->CH_autoconnect,MUIM_Notify,MUIA_Selected,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,57);

    DoMethod((Object*)WookieChat->CH_use_global,MUIM_Notify,MUIA_Selected,TRUE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,57);
    DoMethod((Object*)WookieChat->CH_use_global,MUIM_Notify,MUIA_Selected,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,57);

    if(ZUNE_SYSTEM==TRUE)
    {
        DoMethod((Object*)WookieChat->STR_servername,MUIM_Notify,MUIA_String_Acknowledge, MUIV_EveryTime,(Object*)WookieChat->App, 2, MUIM_Application_ReturnID,57);
        DoMethod((Object*)WookieChat->STR_port,MUIM_Notify,MUIA_String_Acknowledge, MUIV_EveryTime,(Object*)WookieChat->App, 2, MUIM_Application_ReturnID,57);
        DoMethod((Object*)WookieChat->STR_pass,MUIM_Notify,MUIA_String_Acknowledge, MUIV_EveryTime,(Object*)WookieChat->App, 2, MUIM_Application_ReturnID,57);
        DoMethod((Object*)WookieChat->STR_autojoin,MUIM_Notify,MUIA_String_Acknowledge, MUIV_EveryTime,(Object*)WookieChat->App, 2, MUIM_Application_ReturnID,57);
        DoMethod((Object*)WookieChat->STR_nick_registration_command,MUIM_Notify,MUIA_String_Acknowledge, MUIV_EveryTime,(Object*)WookieChat->App, 2, MUIM_Application_ReturnID,57);
        DoMethod((Object*)WookieChat->STR_server_realname,MUIM_Notify,MUIA_String_Acknowledge, MUIV_EveryTime,(Object*)WookieChat->App, 2, MUIM_Application_ReturnID,57);
        DoMethod((Object*)WookieChat->STR_server_username,MUIM_Notify,MUIA_String_Acknowledge, MUIV_EveryTime,(Object*)WookieChat->App, 2, MUIM_Application_ReturnID,57);
        DoMethod((Object*)WookieChat->STR_server_nickname1,MUIM_Notify,MUIA_String_Acknowledge, MUIV_EveryTime,(Object*)WookieChat->App, 2, MUIM_Application_ReturnID,57);
        DoMethod((Object*)WookieChat->STR_server_nickname2,MUIM_Notify,MUIA_String_Acknowledge, MUIV_EveryTime,(Object*)WookieChat->App, 2, MUIM_Application_ReturnID,57);
        DoMethod((Object*)WookieChat->STR_server_nickname3,MUIM_Notify,MUIA_String_Acknowledge, MUIV_EveryTime,(Object*)WookieChat->App, 2, MUIM_Application_ReturnID,57);
    }
    else
    {
        DoMethod((Object*)WookieChat->STR_servername,MUIM_Notify,MUIA_String_Contents, MUIV_EveryTime,(Object*)WookieChat->App, 2, MUIM_Application_ReturnID,57);
        DoMethod((Object*)WookieChat->STR_port,MUIM_Notify,MUIA_String_Contents, MUIV_EveryTime,(Object*)WookieChat->App, 2, MUIM_Application_ReturnID,57);
        DoMethod((Object*)WookieChat->STR_pass,MUIM_Notify,MUIA_String_Contents, MUIV_EveryTime,(Object*)WookieChat->App, 2, MUIM_Application_ReturnID,57);
        DoMethod((Object*)WookieChat->STR_autojoin,MUIM_Notify,MUIA_String_Contents, MUIV_EveryTime,(Object*)WookieChat->App, 2, MUIM_Application_ReturnID,57);
        DoMethod((Object*)WookieChat->STR_nick_registration_command,MUIM_Notify,MUIA_String_Contents, MUIV_EveryTime,(Object*)WookieChat->App, 2, MUIM_Application_ReturnID,57);
        DoMethod((Object*)WookieChat->STR_server_realname,MUIM_Notify,MUIA_String_Contents, MUIV_EveryTime,(Object*)WookieChat->App, 2, MUIM_Application_ReturnID,57);
        DoMethod((Object*)WookieChat->STR_server_username,MUIM_Notify,MUIA_String_Contents, MUIV_EveryTime,(Object*)WookieChat->App, 2, MUIM_Application_ReturnID,57);
        DoMethod((Object*)WookieChat->STR_server_nickname1,MUIM_Notify,MUIA_String_Contents, MUIV_EveryTime,(Object*)WookieChat->App, 2, MUIM_Application_ReturnID,57);
        DoMethod((Object*)WookieChat->STR_server_nickname2,MUIM_Notify,MUIA_String_Contents, MUIV_EveryTime,(Object*)WookieChat->App, 2, MUIM_Application_ReturnID,57);
        DoMethod((Object*)WookieChat->STR_server_nickname3,MUIM_Notify,MUIA_String_Contents, MUIV_EveryTime,(Object*)WookieChat->App, 2, MUIM_Application_ReturnID,57);

    }

    DoMethod((Object*)WookieChat->CYCLE_remote_charset,MUIM_Notify,MUIA_Cycle_Active, MUIV_EveryTime,(Object*)WookieChat->App, 2, MUIM_Application_ReturnID,57);
    if(Pro_Charsets_Enabled==FALSE) setmacro(WookieChat->GR_charsets , MUIA_ShowMe, FALSE);
    if(Pro_Charsets_Enabled==FALSE) setmacro(GR_local_charset, MUIA_ShowMe, FALSE);
    ...
    DoMethod((Object*)WookieChat->STR_user_list_buttons_name,MUIM_Notify,MUIA_String_Contents, MUIV_EveryTime,(Object*)WookieChat->App, 2, MUIM_Application_ReturnID,SECOND_SET_OF_RETURNIDS+2);
    DoMethod((Object*)WookieChat->STR_user_list_buttons_command,MUIM_Notify,MUIA_String_Contents, MUIV_EveryTime,(Object*)WookieChat->App, 2, MUIM_Application_ReturnID,SECOND_SET_OF_RETURNIDS+2);
    DoMethod((Object*)WookieChat->LV_user_list_buttons, MUIM_Notify, MUIA_NList_EntryClick, MUIV_EveryTime, (Object*)WookieChat->App,2,MUIM_Application_ReturnID,SECOND_SET_OF_RETURNIDS+3);
    DoMethod((Object*)WookieChat->BT_user_list_buttons_add,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,SECOND_SET_OF_RETURNIDS+4);
    DoMethod((Object*)WookieChat->BT_user_list_buttons_delete,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,SECOND_SET_OF_RETURNIDS+5);

    DoMethod((Object*)WookieChat->BT_user_list_buttons_move_up,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,SECOND_SET_OF_RETURNIDS+6);
    DoMethod((Object*)WookieChat->BT_user_list_buttons_move_down,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,SECOND_SET_OF_RETURNIDS+7);

    DoMethod((Object*)LV_events, MUIM_Notify, MUIA_NList_EntryClick, MUIV_EveryTime, (Object*)WookieChat->App,2,MUIM_Application_ReturnID,SECOND_SET_OF_RETURNIDS+9);
    DoMethod((Object*)STR_events_script,MUIM_Notify,MUIA_String_Contents, MUIV_EveryTime,(Object*)WookieChat->App, 2, MUIM_Application_ReturnID,SECOND_SET_OF_RETURNIDS+10);
    DoMethod((Object*)STR_events_args,MUIM_Notify,MUIA_String_Contents, MUIV_EveryTime,(Object*)WookieChat->App, 2, MUIM_Application_ReturnID,SECOND_SET_OF_RETURNIDS+10);
    DoMethod((Object*)CYCLE_events,MUIM_Notify,MUIA_Cycle_Active, MUIV_EveryTime,(Object*)WookieChat->App, 2, MUIM_Application_ReturnID,SECOND_SET_OF_RETURNIDS+10);


    DoMethod((Object*)WookieChat->TX_nickname,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->WI_change_nick,3,MUIM_Set,MUIA_Window_Open,TRUE);
    DoMethod((Object*)WookieChat->TX_nickname,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->WI_change_nick,3,MUIM_Set,MUIA_Window_ActiveObject,(Object*)WookieChat->STR_change_nick);

    DoMethod((Object*)WookieChat->BT_change_nick_cancel,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->WI_change_nick,3,MUIM_Set,MUIA_Window_Open,FALSE);
    DoMethod((Object*)WookieChat->BT_change_nick_change,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,SECOND_SET_OF_RETURNIDS+8);
    DoMethod((Object*)WookieChat->STR_change_nick,MUIM_Notify,MUIA_String_Acknowledge, MUIV_EveryTime,(Object*)WookieChat->App, 2, MUIM_Application_ReturnID,SECOND_SET_OF_RETURNIDS+8);
    DoMethod((Object*)WookieChat->WI_change_nick,MUIM_Notify,MUIA_Window_CloseRequest,TRUE,(Object*)WookieChat->WI_change_nick,3,MUIM_Set,MUIA_Window_Open,FALSE);
	//setmacro((Object*)WookieChat->WI_change_nick,MUIA_Window_ActiveObject,(Object*)WookieChat->STR_change_nick);


    DoMethod((Object *)WookieChat->LV_alias, MUIM_Notify, MUIA_NList_EntryClick, MUIV_EveryTime, (Object*)WookieChat->App,2,MUIM_Application_ReturnID,89);
    ...
    */
    ...
    DoMethod((Object*)WookieChat->BT_label_0,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,5);
    DoMethod((Object*)WookieChat->BT_label_1,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,6);
    DoMethod((Object*)WookieChat->BT_label_2,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,7);
    DoMethod((Object*)WookieChat->BT_label_3,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,31);

    //dcc window button notification
    DoMethod((Object*)WookieChat->BT_dcc_accept,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,65);
    DoMethod((Object*)WookieChat->BT_dcc_cancel,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,66);
    DoMethod((Object*)WookieChat->BT_dcc_remove,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,67);

    DoMethod((Object*)WookieChat->BT_dcc_send_cancel,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,75);
    DoMethod((Object*)WookieChat->BT_dcc_send_remove,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,76);
    DoMethod((Object*)WookieChat->BT_dcc_send_reoffer,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,79);
    DoMethod((Object*)WookieChat->BT_dcc_send_new,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->WI_dcc_send,3,MUIM_Set,MUIA_Window_Open,TRUE);

    DoMethod((Object*)WookieChat->BT_dcc_send,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,78);
	DoMethod((Object*)WookieChat->WI_dcc_send,MUIM_Notify,MUIA_Window_CloseRequest,TRUE,(Object*)WookieChat->WI_dcc_send,3,MUIM_Set,MUIA_Window_Open,FALSE);
             
    //dcc exists window buttone notification
    DoMethod((Object*)WookieChat->BT_dcc_resume,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,70);
    DoMethod((Object*)WookieChat->BT_dcc_overwrite,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,71);
    DoMethod((Object*)WookieChat->BT_dcc_abort,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->WI_dcc_file_exists,2,MUIM_Set,MUIA_Window_Open, FALSE);
    DoMethod((Object*)WookieChat->BT_dcc_rename,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,100);


    //ban window close gadget
	DoMethod((Object*)WookieChat->WI_ban,MUIM_Notify,MUIA_Window_CloseRequest,TRUE,(Object*)WookieChat->WI_ban,3,MUIM_Set,MUIA_Window_Open,FALSE);
    DoMethod((Object*)WookieChat->BT_unban,MUIM_Notify,MUIA_Selected,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,19);
    DoMethod((Object*)WookieChat->BT_refreshban,MUIM_Notify,MUIA_Selected,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,18);
    ...
    DoMethod((Object*)MN1_SelectServer, MUIM_Notify, MUIA_Menuitem_Trigger,MUIV_EveryTime, (Object*)WookieChat->WI_label_2,3,MUIM_Set,MUIA_Window_Open,TRUE);

    DoMethod((Object*)MN1_NewTAB, MUIM_Notify, MUIA_Menuitem_Trigger,MUIV_EveryTime,(Object*)WookieChat->App,2,MUIM_Application_ReturnID, 61);
	DoMethod((Object*)MN1_CloseTAB, MUIM_Notify, MUIA_Menuitem_Trigger,MUIV_EveryTime,(Object*)WookieChat->App,2,MUIM_Application_ReturnID, 73);
	DoMethod((Object*)MN_ClearHistory, MUIM_Notify, MUIA_Menuitem_Trigger,MUIV_EveryTime, (Object*)WookieChat->App,2,MUIM_Application_ReturnID, 62);
	DoMethod((Object*)MN_ClearAllHistory, MUIM_Notify, MUIA_Menuitem_Trigger,MUIV_EveryTime, (Object*)WookieChat->App,2,MUIM_Application_ReturnID, 72);
	DoMethod((Object*)MN_SaveHistory, MUIM_Notify, MUIA_Menuitem_Trigger,MUIV_EveryTime, (Object*)WookieChat->App,2,MUIM_Application_ReturnID, 74);
	DoMethod((Object*)MN_MUISettings, MUIM_Notify, MUIA_Menuitem_Trigger,MUIV_EveryTime, (Object*)WookieChat->App,2,MUIM_Application_ReturnID, 63);
    DoMethod((Object*)MN_MainSettings, MUIM_Notify, MUIA_Menuitem_Trigger,MUIV_EveryTime, (Object*)WookieChat->App,2,MUIM_Application_ReturnID, 8);
    DoMethod((Object*)MN_windows_dcc, MUIM_Notify, MUIA_Menuitem_Trigger,MUIV_EveryTime, (Object*)WookieChat->App,2,MUIM_Application_ReturnID, 69);
    DoMethod((Object*)MN_windows_dcc2, MUIM_Notify, MUIA_Menuitem_Trigger,MUIV_EveryTime, (Object*)WookieChat->App,2,MUIM_Application_ReturnID, 80);
	DoMethod((Object*)MN1_NewGTAB, MUIM_Notify, MUIA_Menuitem_Trigger,MUIV_EveryTime,(Object*)WookieChat->App,2,MUIM_Application_ReturnID, 77);
	DoMethod((Object*)MN1_Hide, MUIM_Notify, MUIA_Menuitem_Trigger,MUIV_EveryTime,(Object*)WookieChat->App,3,MUIM_Set,MUIA_Application_Iconified, TRUE);


    DoMethod((Object*)MN_cut,   MUIM_Notify, MUIA_Menuitem_Trigger,MUIV_EveryTime, (Object*)WookieChat->App,2,MUIM_Application_ReturnID, SECOND_SET_OF_RETURNIDS+11);
    DoMethod((Object*)MN_copy,  MUIM_Notify, MUIA_Menuitem_Trigger,MUIV_EveryTime, (Object*)WookieChat->App,2,MUIM_Application_ReturnID, SECOND_SET_OF_RETURNIDS+12);
    DoMethod((Object*)MN_paste, MUIM_Notify, MUIA_Menuitem_Trigger,MUIV_EveryTime, (Object*)WookieChat->App,2,MUIM_Application_ReturnID, SECOND_SET_OF_RETURNIDS+13);


    //ignore notifys
    DoMethod((Object*)MN_ignorelist, MUIM_Notify, MUIA_Menuitem_Trigger,MUIV_EveryTime, (Object*)WookieChat->App,2,MUIM_Application_ReturnID, 83);
	DoMethod((Object*)WookieChat->WI_ignore,MUIM_Notify,MUIA_Window_CloseRequest,TRUE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID, 84);
    DoMethod((Object*)WookieChat->BT_ignore_add,MUIM_Notify,MUIA_Selected,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,85);
    DoMethod((Object*)WookieChat->BT_ignore_edit,MUIM_Notify,MUIA_Selected,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,86);
	DoMethod((Object*)WookieChat->WI_addignore,MUIM_Notify,MUIA_Window_CloseRequest,TRUE,(Object*)WookieChat->WI_addignore,3,MUIM_Set,MUIA_Window_Open,FALSE);
    DoMethod((Object*)WookieChat->BT_addignore,MUIM_Notify,MUIA_Selected,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,87);
    DoMethod((Object*)WookieChat->BT_ignore_remove,MUIM_Notify,MUIA_Selected,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,88);

    //About menu
    DoMethod((Object*)MN_about, MUIM_Notify, MUIA_Menuitem_Trigger,MUIV_EveryTime, (Object*)WookieChat->WI_about,3,MUIM_Set,MUIA_Window_Open,TRUE);
	DoMethod((Object*)WookieChat->WI_about,MUIM_Notify,MUIA_Window_CloseRequest,TRUE,(Object*)WookieChat->WI_about,3,MUIM_Set,MUIA_Window_Open,FALSE);

    //url grabber
    DoMethod((Object*)MN_urlgrabber, MUIM_Notify, MUIA_Menuitem_Trigger,MUIV_EveryTime, (Object*)WookieChat->WI_urlgrabber,3,MUIM_Set,MUIA_Window_Open,TRUE);
	DoMethod((Object*)WookieChat->WI_urlgrabber,MUIM_Notify,MUIA_Window_CloseRequest,TRUE,(Object*)WookieChat->WI_urlgrabber,3,MUIM_Set,MUIA_Window_Open,FALSE);
    DoMethod((Object*)WookieChat->BT_urlgrabber_clear,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->LV_urlgrabber,2,MUIM_NList_Remove,MUIV_NList_Remove_Selected);
    DoMethod((Object*)WookieChat->BT_urlgrabber_clearall,MUIM_Notify,MUIA_Pressed,FALSE,(Object*)WookieChat->LV_urlgrabber,1,MUIM_NList_Clear);

    #ifdef __AROS__
    DoMethod((Object*)MN_urlgrabber, MUIM_Notify, MUIA_Menuitem_Trigger,MUIV_EveryTime, (Object*)WookieChat->WI_urlgrabber,3,MUIM_Set,MUIA_Window_DefaultObject,WookieChat->LV_urlgrabber);
    DoMethod((Object*)MN_urlgrabber, MUIM_Notify, MUIA_Menuitem_Trigger,MUIV_EveryTime, (Object*)WookieChat->WI_urlgrabber,3,MUIM_Set,MUIA_Window_ActiveObject,WookieChat->LV_urlgrabber);
    #endif
...
// Open our servers.txt file and update the Server Selection window
    ...
    */
    ...
    if(my_settings.display_server_window_on_startup) setmacro((Object*)WookieChat->WI_label_2,MUIA_Window_Open, TRUE);
...
// Startup connects
        ...
        ULONG count_nodes = DoMethod((Object*)WookieChat->NLT_Servers, MUIM_NListtree_GetNr, 0, MUIV_NListtree_GetNr_Flag_CountAll);

        for(int count=0, connecting=0; count<(LONG)count_nodes; count++)
        {
            treenode = (struct MUI_NListtree_TreeNode *) DoMethod((Object*)WookieChat->NLT_Servers, MUIM_NListtree_GetEntry, MUIV_NListtree_GetEntry_ListNode_Root, count, 0);
            ...
            if(treenode)
            {
                ...
                else
                {
                    ...
                    if(port_number[0]=='\0') { strcpy(group_name,server_name); }
...
// user has typed some text and press enter
                ...
                if(string123)
                {
                    ...
                    c++;
                    while((test[c]=strtok(NULL,"\r\n\0")) != NULL) { c++; if(c>=1000) break; }
                    int b=c; c=0;

                    //if(b==1) start=TRUE; else start=FALSE;

                    if(status_current) status_conductor=status_current;
                    else status_conductor=status_root;
                        ...
                        if(test[c])
                        {
                            ...
                            if(strlen(test[c]) > 450)
                            {
                                ...
                                while(1)
                                {
                                    ...
                                    do { test2[a]=test[c][b]; a++; b++; } while(test2[a-1] != '\0' && a < 450 && b < e );
                                    test2[a]='\0';
                                        ...
                                        if(string1 && string2 && string3)
                                        {
                                            if(!stricmp(string1,string2))
                                            {
...

                                                {
                                                    ...
                                                    else if(status_current->current_query->queued_messages_total<MAX_QUEUED_MESSAGES)
                                                    {
                                                        if(status_current->current_query->queued_messages[status_current->current_query->queued_messages_total]==NULL)
                                                        {
                                                            status_current->current_query->queued_messages[status_current->current_query->queued_messages_total]=new char[460];
                                                            strcpy(status_current->current_query->queued_messages[status_current->current_query->queued_messages_total],string9);
                                                        }
...

                                    {
                                        if(count>my_settings.number_of_command_aliases)
                                        {
                                            ...
                                            else if(status_current->current_query->queued_messages_total<MAX_QUEUED_MESSAGES)
                                            {
                                                ...
                                                {
                                                    status_current->current_query->queued_messages[status_current->current_query->queued_messages_total]=new char[460];
                                                    strcpy(status_current->current_query->queued_messages[status_current->current_query->queued_messages_total],test2);
                                                }
                                                status_current->current_query->queued_messages_total++;
                                                queued_messages_total++;
                                            }
                                ...
                                for(count=1; count<=my_settings.number_of_command_aliases; count++)
                                {
                                    ...
                                    if(string1 && string2 && string3)
                                    {
                                        if(!stricmp(string1,string2))
                                        {
                                            ...
                                            if(status_current->current_query->queued_messages_total==0 && c==0 || status_current->current_query->name[0]=='$')
                                            {
                                                process_outgoing(string9,1);
                                            }
                                            else
                                ...
                                else
                                {
                                    if(count>my_settings.number_of_command_aliases)
                                    {
                                        if(status_current->current_query->queued_messages_total<MAX_QUEUED_MESSAGES)
                                        {
                                            ...
                                            {
                                                status_current->current_query->queued_messages[status_current->current_query->queued_messages_total]=new char[460];
                                                strcpy(status_current->current_query->queued_messages[status_current->current_query->queued_messages_total],test[c]);
                                            }
                                            status_current->current_query->queued_messages_total++;
                                            queued_messages_total++;
                                        }
                ...
                setmacro((Object*)WookieChat->WI_main,MUIA_Window_ActiveObject,(Object*)WookieChat->STR_usertext);
                if(queued_messages_total>0) make_paste_pause_delay();
...
// save current colours theme as...
                ...
                if(aslresult)
                {
                    ...
                    if(work_buffer[strlen(work_buffer)-1] != ':') strcat(work_buffer,"/");
...
// user has double clicked on an entry in the nicklist
                ...
                if(my_settings.open_private_tab) SQuery(work_entry->name);
                if(my_settings.request_whois_info) SWhoIs(work_entry->name);
                status_conductor=status_current;
...
// add a new server to the Server Selection window
                ...
                "SERVER=server-rename_and_move_me PORT=6667 AUTOCONNECT=No SERVER_CHARSET=UTF-8 SERVER_PASSWORD= NICK_PASSWORD= AUTOJOINS= REALNAME= USERNAME= NICK1= NICK2= NICK3= USE_GLOBAL=1",NULL,
...
// add a new server group to the Server Selection window
            ...
            tree_conductor->t = (struct MUI_NListtree_TreeNode *)DoMethod((Object*)WookieChat->NLT_Servers, MUIM_NListtree_Insert, "NewGroup-Editme", NULL,
                MUIV_NListtree_Insert_ListNode_Root,
                MUIV_NListtree_Insert_PrevNode_Tail, TNF_LIST);
...
//
            ...
            {
                DoMethod((Object*)WookieChat->NLT_Servers, MUIM_NListtree_Remove, MUIV_NListtree_Remove_ListNode_Active, MUIV_NListtree_Remove_TreeNode_Active, 0);
            }
...
// Edit Group or Server entries in the server selection windows nlisttree
                ...
                struct MUI_NListtree_TreeNode *treenode = (struct MUI_NListtree_TreeNode *) DoMethod((Object*)WookieChat->NLT_Servers, MUIM_NListtree_GetEntry, MUIV_NListtree_GetEntry_ListNode_Active, MUIV_NListtree_GetEntry_Position_Active, 0);
                ...
                if(treenode)
                {
                    ...
                    else //else edit a server
                    {
                        ...
                        setmacro((Object*)WookieChat->STR_nick_registration_command,MUIA_String_Contents,nick_password);
                        ...
                        {
                            if(!stricmp(remote_charsets[count],DEFAULT_CHARSET)) count2=count;
...
// Accept group edit change
                ...
                DoMethod((Object*)WookieChat->NLT_Servers, MUIM_NListtree_Rename, MUIV_NListtree_Rename_TreeNode_Active,a, 0);
                setmacro((Object*)WookieChat->WI_edit_group,MUIA_Window_Open, FALSE);
//
            }
...
// Accept server edit change
                ...
                if(DEBUG) printf("new server entry %s\n",work_buffer3);
                #ifndef __AROS__
                if(treenode)
                #endif
                {

                    DoMethod((Object*)WookieChat->NLT_Servers, MUIM_NListtree_Rename, MUIV_NListtree_Rename_TreeNode_Active,work_buffer3, 0);
                }
...
// Connect to server, automatically choose in current tab or new tab
                ...
                if(treenode)
                {
                    ...
                    if(treenode->tn_Name) strcpy(work_buffer,treenode->tn_Name);
                    ...
                    if(work)
                    {
                        ...
                        if(a==0) DoMethod((Object*)WookieChat->App,MUIM_Application_ReturnID,58);
                        else
                        {
                    ...
                    if(!dcc_conductor->removed)
                    {
                        if(find && dcc_conductor->entry)
                        {
                                ...
                                !stricmp(find->port,dcc_conductor->entry->port) ) //experimental, adding port string to entry struct
                            {
                                    ...
                                    {
                                        if(DEBUG) printf("auto renaming incoming file\n");

                                        count=1;

                                        getmacro((Object*)WookieChat->PA_dcc_label_0,MUIA_String_Contents,&string5);

                                        if(string5) strcpy(dcc_conductor->filepath,string5); else strcpy(dcc_conductor->filepath,"");
                                        ...
                                        if(dcc_conductor->filepath[strlen(dcc_conductor->filepath)-1] != '/' && dcc_conductor->filepath[strlen(dcc_conductor->filepath)-1] != ':')
                                            sprintf(dcc_conductor->full_filename_with_path,"%s/%s",dcc_conductor->filepath,dcc_conductor->filename);
                                        else
                                            sprintf(dcc_conductor->full_filename_with_path,"%s%s",dcc_conductor->filepath,dcc_conductor->filename);
                                        ...
                                        DoMethod((Object*)WookieChat->LV_dcc,MUIM_NList_ReplaceSingle,dcc_conductor->entry,a,NOWRAP,ALIGN_LEFT);


                                        //dcc_conductor->accepted=TRUE;

                                        connect(dcc_conductor->dcc_socket, (struct sockaddr*) &dcc_conductor->test, sizeof(struct sockaddr));
...
// Edit perform on connect Script
                ...
                if(treenode)
                {
                    ...
                    if(work)
                    {
                        ...
                        treenode = (struct MUI_NListtree_TreeNode *) DoMethod((Object*)WookieChat->NLT_Servers, MUIM_NListtree_GetEntry, MUIV_NListtree_GetEntry_ListNode_Active, MUIV_NListtree_GetEntry_Position_Head, 0);
                        treenode = (struct MUI_NListtree_TreeNode *) DoMethod((Object*)WookieChat->NLT_Servers, MUIM_NListtree_GetEntry, treenode, MUIV_NListtree_GetEntry_Position_Parent, 0);

                    }

                    if(wookie_folder[strlen(wookie_folder)-1]==':')
                        sprintf(file_name,"run >nil: %s %sconnectscripts/%s",my_settings.text_editor,wookie_folder, treenode->tn_Name);
                    else
                        sprintf(file_name,"run >nil: %s %s/connectscripts/%s",my_settings.text_editor,wookie_folder,treenode->tn_Name);
                        ...
                        if( //!stricmp(find->status,dcc_conductor->entry->status) &&
                            ...
                            !stricmp(find->port,dcc_conductor->entry->port) ) //experimental, adding port string to entry struct
                            {
                                ...
                                if(dcc_prev)
                                {
                                    if(dcc_next)
                                    {
                                        if(DEBUG) printf("dcc_conductor->next isnt false\n");
                                    ...
                                    {
                                        if(DEBUG) printf("dcc_conductor->next IS false\n");
                                        delete dcc_conductor;
                                        dcc_prev->next=NULL;
                                        dcc_conductor=NULL;
                                    }
                                ...
                                {
                                    if(DEBUG) printf("the list forwards, file:%s\n",dcc_conductor->filename);
                    ...
                    if(!dcc_conductor->removed)
                    {
                        ...
                        if(find && dcc_conductor->entry)
                        {
                                ...
                                //!stricmp(find->port,dcc_conductor->entry->port) )
                            {
                                ...
                                sprintf(sendstuff,"PRIVMSG %s :\001DCC RESUME \"%s\" %d %lu\001\r\n",dcc_conductor->nick,dcc_conductor->filename,atoi(dcc_conductor->port), dcc_conductor->total_recv);
                                send_text(sendstuff);
...
//
            ...
            {
                status_conductor=status_current;
                if(strcspn(status_current->current_query->name,status_conductor->chantypes)==0) SPart(NULL); else close_tab();
...
// User selected save history menu item
                    ...
                    if(aslresult)
                    {
                        ...
                        if(newbptr_file)
                        {
                            ...
                            sprintf(buffer3,"%s%sBuffer%s %s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,141,"Saving buffer to file:"),work_buffer);
                            add_text_to_current_list(buffer3,9, ACTIVITY_CHAT);
                            ...
                            for(unsigned int a=0; a<entries; a++)
                            {
                                ...
                                DoMethod((Object*)status_conductor->current_query->LV_channel,MUIM_NList_GetEntry,a, &string2);

                                if(string2) strcpy(work_buffer,string2);
                                ...
                                if(strcmp(work_buffer,work_buffer2))
                                {
                                    ...
                                    if(string1)
                                    {
                                        ...
                                        if(strcmp(string1,""))
                                        {
                                            if(FPuts(newbptr_file,(l_in)string1) != 0) break;
                                            FPutC(newbptr_file,'\n');
                                        }
                                    }
                                }
                        ...
                        if(find && dcc_send_conductor->entry)
                        {
                                ...
                                !stricmp(find->port,dcc_send_conductor->entry->port) )
                            {
                                    ...
                                    if(dcc_send_conductor->entry)
                                    {
                                        ...
                                        //}
                                        DoMethod((Object*)WookieChat->LV_send_dcc,MUIM_NList_ReplaceSingle,dcc_send_conductor->entry,a,NOWRAP,ALIGN_LEFT);
                                    }
                        ...
                        if( //!stricmp(find->status,dcc_send_conductor->entry->status) &&
                            ...
                            !stricmp(find->port,dcc_send_conductor->entry->port) )
                            {
                                    ...
                                    if(dcc_prev)
                                    {
                                        if(dcc_next)
                                        {
                                            if(DEBUG) printf("dcc_send_conductor->next isnt false\n");
                                            dcc_prev->next=dcc_next;
                                            dcc_next->previous=dcc_prev;
                                        ...
                                        {
                                            if(DEBUG) printf("dcc_send_conductor->next IS false\n");
                                            delete dcc_send_conductor;
                                            dcc_prev->next=NULL;
                                            dcc_send_conductor=NULL;
                                        }
                                    ...
                                    for(dcc_send_conductor=dcc_send_root->next; dcc_send_conductor; dcc_send_conductor=dcc_send_conductor->next)
                                    {
                                        if(DEBUG) printf("the list forwards, file:%s\n",dcc_send_conductor->filename);
...
//
            ...
            {
                status_conductor=status_current; SQuery((char*)"-global");
        if(RECENTLY_CREATED_A_TAB==TRUE)
        {
            if(status_current)
            {
...
// DCC SEND button clicked, to initiate DCC SEND
                ...
                {
                    sprintf(buffer3,"%s%sDCC%s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,143,"Please select a file to send, and a nick to offer the file to"));
                    add_text_to_current_list(buffer3,9, ACTIVITY_CHAT);
                }
                else if(!strcmp(string1,""))
                {
                    sprintf(buffer3,"%s%sDCC%s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,144,"Please select a file to send"));
                    add_text_to_current_list(buffer3,9, ACTIVITY_CHAT);
                }
                else if(!strcmp(string2,""))
                {
                    sprintf(buffer3,"%s%sDCC%s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,145,"Please select a nick to offer the file to"));
                    add_text_to_current_list(buffer3,9, ACTIVITY_CHAT);
                }
                        ...
                        if( //!stricmp(find->status,dcc_send_conductor->entry->status) &&
                            ...
                            !stricmp(find->port,dcc_send_conductor->entry->port) )
                            {
                                ...
                                while(dcc_send_conductor)
                                {
                                    ...
                                    {
                                        offer_dcc(dcc_send_conductor->networkname, (char*)"SEND", dcc_send_conductor->nick, dcc_send_conductor->filepath,atoi(dcc_send_conductor->port));
...
//
            ...
            else if(result == 81) //add an alias
            {
                ...
                DoMethod((Object*)WookieChat->LV_alias,MUIM_NList_InsertSingle,work_buffer,MUIV_NList_Insert_Bottom);
            }
...
// EDIT an ignore entry in the ignore list
                ...
                if(string4)
                {
                    ...
                    string1=strtok(string7," ");
                    if(string1) setmacro((Object*)WookieChat->STR_addignore,MUIA_String_Contents,string1);

                    string2=strtok(NULL," ");
                    if(string2) { if(!stricmp(string2,"ignore")) setmacro((Object*)WookieChat->CH_addignore_privmsg,MUIA_Selected,TRUE);
                    else setmacro((Object*)WookieChat->CH_addignore_privmsg,MUIA_Selected,FALSE); }

                    string2=strtok(NULL," ");
                    if(string2) { if(!stricmp(string2,"ignore")) setmacro((Object*)WookieChat->CH_addignore_ctcp,MUIA_Selected,TRUE);
                    else setmacro((Object*)WookieChat->CH_addignore_ctcp,MUIA_Selected,FALSE); }

                    string2=strtok(NULL," ");
                    if(string2) { if(!stricmp(string2,"ignore")) setmacro((Object*)WookieChat->CH_addignore_dcc,MUIA_Selected,TRUE);
                    else setmacro((Object*)WookieChat->CH_addignore_dcc,MUIA_Selected,FALSE); }
...
// "Add/Edit Ignore" window ACTION button
                ...
                if(DEBUG) printf("add or edit ignore? %s\n", string1);
                if(!stricmp(string1,"edit") || !stricmp(string1,GetCatalogStr(catalog,274,"Edit")))
                {
                    ...
                    if(string4)
                    {
                        if(stricmp(string4,""))
                        {
                            ...
                            if(string1) strcat(urlgrabber_str,"ignore "); else strcat(urlgrabber_str,"allow ");
                            if(string2) strcat(urlgrabber_str,"ignore "); else strcat(urlgrabber_str,"allow ");
                            if(string3) strcat(urlgrabber_str,"ignore "); else strcat(urlgrabber_str,"allow ");
                            ...
                            for(count=0; count<=(LONG)entries; count++)
                            {
                                ...
                                if(string5)
                                {
                                    ...
                                    {
                                        DoMethod((Object*)WookieChat->LV_ignore,MUIM_NList_ReplaceSingle,urlgrabber_str,count, NOWRAP, ALIGN_LEFT);
                ...
                else if(!stricmp(string1,"add") || !stricmp(string1,GetCatalogStr(catalog,273,"Add")))
                {
                    ...
                    if(string4)
                    {
                        if(stricmp(string4,""))
                        {
                            ...
                            DoMethod((Object*)WookieChat->LV_ignore,MUIM_NList_InsertSingle,urlgrabber_str,MUIV_NList_Insert_Bottom);
...
// user selected yes to quit
                ...
                while(status_conductor)
                {
                    if(status_conductor->connection_active) send_text(buffer3);
                    status_conductor=status_conductor->next;
...
// Accept the alias edit change
                ...
                DoMethod((Object*)WookieChat->LV_alias,MUIM_NList_GetEntry,MUIV_NList_GetEntry_Active,&string1);
                if(string1) strcpy(old_alias_entry,string1);
                    ...
                    if(string3)
                    {
                       if(string1 && string2)
                        ...
                        if(!stricmp(old_alias_entry,string3))
                            DoMethod((Object*)WookieChat->LV_alias,MUIM_NList_ReplaceSingle,work_buffer,count,NULL,ALIGN_LEFT);
...
// double click in LV_channel. Lets see if its a URL we can visit!
                ...
                DoMethod((Object*)status_current->current_query->LV_channel,MUIM_NList_TestPos,MUI_MAXMAX,MUI_MAXMAX,last_clicked_res);
                if(last_clicked_res)
                {
                    DoMethod((Object*)status_current->current_query->LV_channel,MUIM_NList_GetEntry,last_clicked_res->entry,&string1);

                    if(string1)
                    {
                        strcpy(work_buffer,string1);

                        if(DEBUG) printf("double clicked buffer:%s\n",work_buffer);
                        ...
                        if(string1) string1=doubleclick_url_action(string1, last_clicked_res->char_number, last_clicked_res->column);
                        if(string1)
                        {
                            if(!strcmp(my_settings.browser,""))
                            {
                                sprintf(buffer3,"%s%s%s%s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,0,"Error"),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,136,"Please go to the \"Main Settings\" window, and choose a browser for WookieChat to open web sites with"));
                            ...
                            else
                            {
                                ...
                                if(DEBUG) printf("end result, now visiting %s\n",string1);
...
// double click in URL Grabber window
                DoMethod((Object*)WookieChat->LV_urlgrabber,MUIM_NList_GetEntry,MUIV_NList_GetEntry_Active,&string1);

                if(string1)
                {

                    if(DEBUG) printf("URL clicked:%s\n",string1);
...
// open the "graphical smileys theme" preview window
                        ...
                        if(work_buffer[0]!='#')
                        {
                            ...
                            if(string1)
                            {
                                ...
                                if(preview_smilies[preview_total_smileys].icon)
                                    DoMethod((Object*)WookieChat->LV_graphical_smileys_preview,MUIM_NList_UseImage,preview_smilies[preview_total_smileys].icon, preview_total_smileys, 0L);
                            ...
                            while(string1)
                            {
                                sprintf(preview_smilies[preview_total_smileys].ascii[preview_smilies[preview_total_smileys].ascii_total],"%s ",string1);
                                if(preview_smilies[preview_total_smileys].ascii_total==0) sprintf(preview_smilies[preview_total_smileys].remove_ascii,"%s",string1);
                                preview_smilies[preview_total_smileys].ascii_total++;
                                string1=strtok(NULL,"\n ");
                                if(preview_smilies[preview_total_smileys].ascii_total>=MAXIMUM_SMILEY_ASCII) { break; }

                            }
                            if(preview_total_smileys>=MAXIMUM_SMILEYS) break;
                        }
                    }
                ...
                for(count4=1; count4<=preview_total_smileys && count4<=MAXIMUM_SMILEYS; count4++)
                {
                    ...
                    for(count3=0; count3<preview_smilies[count4].ascii_total && count3<=MAXIMUM_SMILEY_ASCII; count3++)
                    {
                    ...
                    DoMethod((Object*)WookieChat->LV_graphical_smileys_preview,MUIM_NList_InsertSingle,centry,MUIV_NList_Insert_Bottom);
...
//
            ...
            else if(result == 100) //rename the incoming dcc transfer because it already exists
            {
                ...
                filerequester=(struct FileRequester*)AllocAslRequestTags(ASL_FileRequest,ASLFR_Window,my_window,ASLFR_PopToFront,TRUE,ASLFR_DoPatterns, TRUE,ASLFR_DoMultiSelect,FALSE,ASLFR_InitialDrawer,my_settings.download_path,ASLFR_InitialFile,dcc_conductor->entry->filename,TAG_DONE, NULL);
                #else
                filerequester=(struct FileRequester*)AllocAslRequestTags(ASL_FileRequest,ASLFR_DoPatterns, TRUE,ASLFR_DoMultiSelect,FALSE,ASLFR_InitialDrawer,my_settings.download_path,ASLFR_InitialFile,dcc_conductor->entry->filename,TAG_DONE, NULL);
                #endif
            ...
            else if(result >= SECOND_SET_OF_RETURNIDS && result < AREXX_MENU_VALUES)
            {
                ...
                else if(result == 2) //user is typing in the User List Buttons config
                {
                    ...
                    getmacro((Object*)WookieChat->STR_user_list_buttons_command,MUIA_String_Contents,&string2);
...
//#define USERLIST_COMMAND_SIZE 800
                        ...
                        DoMethod((Object*)WookieChat->LV_user_list_buttons,MUIM_NList_GetEntry,MUIV_NList_GetEntry_Active,&string3);
                        getmacro((Object*)WookieChat->LV_user_list_buttons,MUIA_NList_Entries,&entries);
                        if(string3)
                        {
                            for(unsigned int count=0; count<=entries; count++)
                            {
                                DoMethod((Object*)WookieChat->LV_user_list_buttons,MUIM_NList_GetEntry,count,&string4);
                                if(string4)
                                {
                                    //printf("string4:%s\n",string4);
                                    ...
                                    {
                                        DoMethod((Object*)WookieChat->LV_user_list_buttons,MUIM_NList_ReplaceSingle,work_buffer,count,NOWRAP,ALIGN_LEFT);
                                        break;
                                    }
                                }
                ...
                else if(result == 3) //user clicked on an entry in the User List Buttons config
                {
                    ...
                    DoMethod((Object*)WookieChat->LV_user_list_buttons,MUIM_NList_GetEntry,MUIV_NList_GetEntry_Active,&string1);
                    if(string1)
                    {
                        strcpy(work_buffer,string1);
                        ...
                        if(string1) setmacro((Object*)WookieChat->STR_user_list_buttons_name,MUIA_String_Contents,string1);
                            else setmacro((Object*)WookieChat->STR_user_list_buttons_name,MUIA_String_Contents,"");
                        if(string2) setmacro((Object*)WookieChat->STR_user_list_buttons_command,MUIA_String_Contents,string2);
                            else setmacro((Object*)WookieChat->STR_user_list_buttons_command,MUIA_String_Contents,"");
                ...
                else if(result == 4) //User List Buttons config: Add New Entry
                {
                    ...
                    if(entries<MAX_BUTTONS)
                    {
                        ...
                        DoMethod((Object*)WookieChat->LV_user_list_buttons,MUIM_NList_InsertSingle,work_buffer,MUIV_NList_Insert_Bottom);
                    }
                }
                else if(result == 5) //User List Buttons config: Delete Entry
                {

                    DoMethod((Object*)WookieChat->LV_user_list_buttons,MUIM_NList_Remove,MUIV_NList_Remove_Active);
                    ...
                    if(string3)
                    {
                        for(unsigned int count=0; count<=entries; count++)
                        {
                            ...
                            if(string4)
                            {
                                ...
                                {
                                    if(count==0) break;
                                    DoMethod((Object*)WookieChat->LV_user_list_buttons,MUIM_NList_Exchange,count,count-1);
                                    break;
                                }
                            }
                ...
                else if(result == 9) //user clicked on an entry in the EVENTS config
                {
                    ...
                    if(string1)
                    {
                        ...
                        if(string1) setmacro((Object*)CYCLE_events,MUIA_Cycle_Active,atoi(string1));
                            else setmacro((Object*)CYCLE_events,MUIA_Cycle_Active,0);

                        if(string2) setmacro((Object*)STR_events_script,MUIA_String_Contents,string2);
                            else setmacro((Object*)STR_events_script,MUIA_String_Contents,"");

                        if(string3) setmacro((Object*)STR_events_args,MUIA_String_Contents,string3);
                            else setmacro((Object*)STR_events_args,MUIA_String_Contents,"");
                ...
                else if(result == 10) //user is typing in the EVENTS config
                {
                    ...
                    if(string1)
                    {
                        ...
                        DoMethod((Object*)LV_events,MUIM_NList_GetEntry,MUIV_NList_GetEntry_Active,&string3);
                        ...
                        if(string3)
                        {
                            for(unsigned int count=0; count<=entries; count++)
                            {
                                ...
                                if(string4)
                                {
                                    ...
                                    if(!stricmp(string3,string4))
                                    {
                                        if(count==0) strcpy(insert_string2,"Highlight\033");
                                        else if(count==1) strcpy(insert_string2,"Connect\033");
                                        else if(count==2) strcpy(insert_string2,"Disconnect\033");
                                        else if(count==3) strcpy(insert_string2,"Join\033");
                                        else if(count==4) strcpy(insert_string2,"Part\033");
                                        else if(count==5) strcpy(insert_string2,"Quit\033");
                                        else if(count==6) strcpy(insert_string2,"DCC RECV Offered\033");
                                        else if(count==7) strcpy(insert_string2,"DCC RECV Finished\033");
                                        else if(count==8) strcpy(insert_string2,"DCC SEND Finished\033");
                                        else if(count==9) strcpy(insert_string2,"Channel Message\033");
                                        else if(count==10) strcpy(insert_string2,"Private Message\033");
                                        else if(count==11) strcpy(insert_string2,"Notice\033");
                                        else if(count==12) strcpy(insert_string2,"CTCP Request\033");
                                        else if(count==13) strcpy(insert_string2,"Kick\033");
                                        else if(count==14) strcpy(insert_string2,"Mode Change\033");
                                        else if(count==15) strcpy(insert_string2,"Invite\033");

                                        else strcpy(insert_string2,"(blank)\033");

                                        strcat(insert_string2,work_buffer);

                                        DoMethod((Object*)LV_events,MUIM_NList_ReplaceSingle,insert_string2,count,NOWRAP,ALIGN_LEFT);
                                        break;
                                    }
                                }
                ...
                else if(result == 11) //Edit Menu Item, cut
                {
                    ...
                    if(active) DoMethod((Object*)active, MUIM_BetterString_DoAction, MUIV_BetterString_DoAction_Cut);
                ...
                else if(result == 12) //Edit Menu Item, copy
                {
                    ...
                    Object *active;
                    getmacro((Object*)WookieChat->WI_main, MUIA_Window_ActiveObject,&active);
                    if(active) DoMethod((Object*)active, MUIM_BetterString_DoAction, MUIV_BetterString_DoAction_Copy);

                }
                else if(result == 13) //Edit Menu Item, paste
                {
                    if(DEBUG) printf("paste\n");
                    Object *active;
                    getmacro((Object*)WookieChat->WI_main, MUIA_Window_ActiveObject,&active);
                    if(active) DoMethod((Object*)active, MUIM_BetterString_DoAction, MUIV_BetterString_DoAction_Paste);
                ...
                else if(result >= 100) //User List Button: CLicked!
                {
                    ...
                    if(strstr(buttons[result].command,"%s") ||strstr(buttons[result].command,"%h")) // if this code is selected, then we want to do a command on each selected nick
                    {
                        ...
                        {
                            if(buttons[result].command[0] != '/') strcpy(work_buffer,"/"); else strcpy(work_buffer,"");
                            ...
                            DoMethod((Object*)status_conductor->conductor->LV_nicklist,MUIM_NList_NextSelected,&id);

                            DoMethod((Object*)status_conductor->conductor->LV_nicklist,MUIM_NList_GetEntry,id, &work_entry);
                            ...
                            for(unsigned int count=0,count2=0; count<strlen(work_buffer); count++, count2++)
                            {
                                ...
                                if(work_buffer[count]=='%')
                                {
                                    ...
                                    {
                                        for(int count5=0; count5<status_conductor->conductor->nicks; count5++)
                                        {

                                            if(!stricmp(status_conductor->conductor->nicklist[count5].name, work_entry->name))
                                            {

                                                if(status_conductor->conductor->nicklist[count5].hostname)
                                                {
                                                    strcat(work_buffer2,status_conductor->conductor->nicklist[count5].hostname);
...
// Execute an AREXX script
                ...
                {
                    #ifdef __AROS__
                    struct TagItem my_arexx_command_taglist[] = { {NP_CurrentDir,(IPTR)arexx_dir_lock}, {TAG_DONE, 0} };
                    #else
                    struct TagItem my_arexx_command_taglist[] = { {NP_CurrentDir,(ULONG)arexx_dir_lock}, {TAG_DONE, (ULONG)0} };
                    #endif
...
// Insert a graphical smiley ascii into the chat entry string gadget
                ...
                if(position!=0)
                {
                    do { tabwork2_string[b]=tabwork_string[a]; b++; a++; } while(a<=(LONG)position);
                    b=b-1; tabwork2_string[b]='\0';
                }
                else
                {
                    b=0; tabwork2_string[b]='\0';
                }
                a=position; b=0;

                if(position!=(LONG)strlen(tabwork_string))
                {
                    do{ tabwork3_string[b]=tabwork_string[a]; b++; a++; } while(a<=(LONG)strlen(tabwork_string));
                }

                tabwork3_string[b]='\0';

                sprintf(tabwork_string,"%s%s %s",tabwork2_string,smilies[count].remove_ascii,tabwork3_string);
                setmacro((Object*)WookieChat->STR_usertext,MUIA_String_Contents,tabwork_string);
                setmacro((Object*)WookieChat->STR_usertext,MUIA_String_BufferPos,position+strlen(smilies[count].ascii[0]));
...
// Mode gadgets have been clicked. Lets send the commands to the server now
            else if(status_current)
            {
                ...
                if(status_conductor && current_query)
                {
                    ...
                    { if(!status_conductor->conductor->mode_T) { sprintf(sendstuff, "mode %s +t\r\n", status_conductor->conductor->name); send_text(sendstuff); } }

                    else if(result == 11) //mode n button hit
                    { if(!status_conductor->conductor->mode_N) { sprintf(sendstuff, "mode %s +n\r\n", status_conductor->conductor->name); send_text(sendstuff); } }

                    else if(result == 12) //mode s button hit
                    { if(!status_conductor->conductor->mode_S) { sprintf(sendstuff, "mode %s +s\r\n", status_conductor->conductor->name); send_text(sendstuff); } }

                    else if(result == 13) //mode i button hit
                    { if(!status_conductor->conductor->mode_I) { sprintf(sendstuff, "mode %s +i\r\n", status_conductor->conductor->name); send_text(sendstuff); } }

                    else if(result == 14) //mode p button hit
                    { if(!status_conductor->conductor->mode_P) { sprintf(sendstuff, "mode %s +p\r\n", status_conductor->conductor->name); send_text(sendstuff); } }

                    else if(result == 15) //mode m button hit
                    { if(!status_conductor->conductor->mode_M) { sprintf(sendstuff, "mode %s +m\r\n", status_conductor->conductor->name); send_text(sendstuff); } }

                    else if(result == 16) //toggle limit mode gadget hit
                    {
                        if(!status_conductor->conductor->mode_L)
                        {
                            STRPTR work; getmacro((Object*)status_conductor->conductor->STR_limit,MUIA_String_Contents, &work);
                    ...
                    else if(result == 17) //toggle keyword mode gadget hit
                    {
                        if(!status_conductor->conductor->mode_K)
                        {
                            ...
                            STRPTR work; getmacro((Object*)status_conductor->conductor->STR_keyword,MUIA_String_Contents, &work);
                    ...
                    else if(result == 18) //get list of banmasks for current channel
                    {
                        ...
                        sprintf(ban_window_title,"%s %s",GetCatalogStr(catalog,146,"List of banmasks for channel"),status_conductor->conductor->name);
                    ...
                    else if(result == 19) //"unban" button clicked for the banmask window
                    {
                        ...
                        DoMethod((Object*)WookieChat->LV_ban,MUIM_NList_GetEntry,MUIV_NList_GetEntry_Active,&string1);
                        ...
                        if(string2)
                        {
                            ...
                            sprintf(buffer3,"%s%sMode%s %s %s %s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,147,"Removing banmask"),string2,GetCatalogStr(catalog,155,"for channel"),status_conductor->conductor->name);
                    ...
                    { if(status_conductor->conductor->mode_T) { status_conductor->conductor->mode_T=0; sprintf(sendstuff, "mode %s -t\r\n", status_conductor->conductor->name); send_text(sendstuff); } }

                    else if(result == 21) //mode n button hit
                    { if(status_conductor->conductor->mode_N) { status_conductor->conductor->mode_N=0; sprintf(sendstuff, "mode %s -n\r\n", status_conductor->conductor->name); send_text(sendstuff); } }

                    else if(result == 22) //mode s button hit
                    { if(status_conductor->conductor->mode_S) { status_conductor->conductor->mode_S=0; sprintf(sendstuff, "mode %s -s\r\n", status_conductor->conductor->name); send_text(sendstuff); } }

                    else if(result == 23) //mode i button hit
                    { if(status_conductor->conductor->mode_I) { status_conductor->conductor->mode_I=0; sprintf(sendstuff, "mode %s -i\r\n", status_conductor->conductor->name); send_text(sendstuff); } }

                    else if(result == 24) //mode p button hit
                    { if(status_conductor->conductor->mode_P) { status_conductor->conductor->mode_P=0; sprintf(sendstuff, "mode %s -p\r\n", status_conductor->conductor->name); send_text(sendstuff); } }

                    else if(result == 25) //mode m button hit
                    { if(status_conductor->conductor->mode_M) { status_conductor->conductor->mode_M=0; sprintf(sendstuff, "mode %s -m\r\n", status_conductor->conductor->name); send_text(sendstuff); } }

                    else if(result == 27) //toggle limit mode gadget
                    {
                        if(DEBUG) printf("1:trying to remove limit mode\n");
                        if(status_conductor->conductor->mode_L)
                        {
                            status_conductor->conductor->mode_L=0;
                            if(DEBUG) printf("2:trying to remove limit mode\n");
                    ...
                    {
                            STRPTR work; getmacro((Object*)status_conductor->conductor->STR_topic,MUIA_String_Contents, &work);
...
//
        ...
        if(running && signal) select_result=WaitSelect(fdmax+1, &read_fds, &write_fds, NULL, NULL, &waitsignals);
        ...
        if(waitsignals & arexx_wants_to_send_signal) //lets send some text
        {
            if(USE_AREXX==TRUE)
            {
                while((incoming_message=(struct XYMessage*)GetMsg(send_text_replyport)))
                {
                    ...
                    {
                        if(DEBUG) printf("send 2 : recieving - command:%s\n",string_to_send2);
...
// Update the dcc transfers window entries every 2 seconds!
            ...
            while(GetMsg(Timer2MP))
            {
                    ...
                    if(!dcc_conductor->removed && !dcc_conductor->completed)
                    {
                        if(dcc_conductor->entry)
                        {
                            ...
                            if(dcc_conductor->total_recv > 0)
                                sprintf(dcc_conductor->entry->bytes_transferred,"%lukB",dcc_conductor->total_recv/1024);
                            else
                            ...
                            //    seconds_left = (bytes_left/1024) / ((dcc_conductor->total_recv / current_time)/1024);
                                ...
                                if(work1 && work2) seconds_left=work1/work2;
                            }

                            mins=seconds_left/60; hrs=mins/60; seconds_left=seconds_left%60; mins=mins%60; // hrs=hrs%24;

                            if(hrs>0) sprintf(dcc_conductor->entry->timeleft,"%02lu:%02lu:%02lu",hrs,mins,seconds_left);
                            else if(mins>0) sprintf(dcc_conductor->entry->timeleft,"00:%02lu:%02lu",mins,seconds_left);
                            else sprintf(dcc_conductor->entry->timeleft,"00:00:%02lu",seconds_left);


                            if(!stricmp(dcc_conductor->entry->status,GetCatalogStr(catalog,148,"Transferring")))
                                DoMethod((Object*)WookieChat->LV_dcc,MUIM_NList_ReplaceSingle,dcc_conductor->entry,a,NOWRAP,ALIGN_LEFT);
                ...
                while(dcc_send_conductor)
                {
                    ...
                    if(!dcc_send_conductor->removed && !dcc_send_conductor->completed)
                    {
                        if(dcc_send_conductor->entry)
                        {
                            ...
                            if(dcc_send_conductor->total_recv > 0)
                                sprintf(dcc_send_conductor->entry->bytes_transferred,"%lukB",dcc_send_conductor->total_recv/1024);
                            else
                            ...
                            mins=seconds_left/60; hrs=mins/60; seconds_left=seconds_left%60; mins=mins%60; // hrs=hrs%24;
                            if(hrs>0) sprintf(dcc_send_conductor->entry->timeleft,"%02lu:%02lu:%02lu",hrs,mins,seconds_left);
                            else if(mins>0) sprintf(dcc_send_conductor->entry->timeleft,"00:%02lu:%02lu",mins,seconds_left);
                            else sprintf(dcc_send_conductor->entry->timeleft,"00:00:%02lu",seconds_left);

                            if(!stricmp(dcc_send_conductor->entry->status,GetCatalogStr(catalog,148,"Transferring")))
                                DoMethod((Object*)WookieChat->LV_send_dcc,MUIM_NList_ReplaceSingle,dcc_send_conductor->entry,a,NOWRAP,ALIGN_LEFT);
                    ...
                    if(!dcc_send_conductor->removed)
                    {
                        if(dcc_send_conductor->connected) dcc_send_data(a);
                        //dcc_send_data(a);
                        a++;
                    }
                ...
                if(delay_b4_ping_server_count>=SECONDS_TO_WAIT_BEFORE_PING_SERVER)
                {
                    ...
                    for(status_conductor=status_root; status_conductor; status_conductor=status_conductor->next)
                    {
                        if(status_conductor->connection_active) // && !status_conductor->waiting_for_ping)
                        {
                            ...
                            if(DEBUG) printf("sending ping to server..\n");
                        }
            ...
            #endif
            {
                ...
                for(status_conductor=status_root; status_conductor; status_conductor=status_conductor->next)
                {
                    ...
                    if(status_conductor->connection_active==0 && status_conductor->quit_requested==FALSE && status_conductor->trying_to_reconnect==TRUE)
                    {
                        ...
                        if(status_conductor->retry_number<my_settings.Maximum_Retries+1)
                        {
                            ...
                            connect2server(status_conductor->servername, string7, 1, 0, DONT_CONNECT_IN_CURRENT_TAB,NULL,0);
                        ...
                        else
                        {
                            ...
                            {
                    			sprintf(buffer3,"%s%sConnect%s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,279,"Unable to connect, giving up"));
                                add_text_to_conductor_list(buffer3,9, ACTIVITY_CHAT);
                ...
                if(start_reconnect_delay_timer) automatically_reconnect_server(START_DELAY);
                status_conductor=status_root;
...
//
            ...
            for(status_conductor=status_root; status_conductor; status_conductor=status_conductor->next)
            {
                    for(status_conductor->conductor=status_conductor->root; status_conductor->conductor; status_conductor->conductor=status_conductor->conductor->next)
                    {
                        if(status_conductor->conductor->queued_messages[0]!=NULL)
                        {
                            ...
                            {
                                if(status_conductor->conductor->queued_messages[count]!=NULL && status_conductor->conductor->queued_messages[count+1]!=NULL)
                                {
                                    strcpy(status_conductor->conductor->queued_messages[count],status_conductor->conductor->queued_messages[count+1]);
                                }
                            }
                            ...
                            if(DEBUG) printf("tab queued messages left %d\n", status_conductor->conductor->queued_messages_total);
                            if(DEBUG) printf("total queued messages left %d\n", queued_messages_total);
        ...
        if(GetMsg(Timer4MP))
        {
            ...
            if(hrs<10) sprintf(timestamp_hrs,"0%ld",hrs); else sprintf(timestamp_hrs,"%ld",hrs);
            if(mins<10) sprintf(timestamp_mins,"0%ld",mins); else sprintf(timestamp_mins,"%ld",mins);
            sprintf(buffer3,"%s%s%s%s * * %d-%d-%d * *",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,344,"Info"), GetCatalogStr(catalog,218,"]"),clockdata->year,clockdata->month,clockdata->mday); //,timestamp_hrs,timestamp_mins);
        ...
        if(select_result>0) do_waitselect_code(); //iterate through all the sockets and read/write to them
