...
int do_waitselect_code(void)
{
    ...
    for(int i = 0; i <= fdmax; i++)
    {
        ...
        if(FD_ISSET(i, &read_fds))
        {
            ...
            while(i != status_conductor->a_socket && i != status_conductor->ident.a_socket && i != status_conductor->ident.listen_socket)
            {
            ...
            if(i==status_conductor->a_socket)
            {
                    ...
                    if(count>0) recv_thing=recv(status_conductor->a_socket, (i_in)status_conductor->buffer, count-1, 0);
                    if(recv_thing != -1)
                    {
                        status_conductor->buffer[recv_thing]='\0';
            			strcat(status_conductor->str,status_conductor->buffer);
                       ...
                       || (status_conductor->connection_active==1 && (recv_thing == -1 && Errno() == 32)) ) //broken pipe
                    {
                        ...
                        {
                            if(!status_conductor->connection_active && status_conductor->conductor->queued_messages[0]!=NULL)
                            {
                        ...
                        if( (my_settings.events[DISCONNECT].use_when == 1 && is_window_active()) || (my_settings.events[DISCONNECT].use_when == 2 && !is_window_active()) || my_settings.events[DISCONNECT].use_when == 3 )
                        {
                            strcpy(target_nick,"");
                            sprintf(buffer3, "%sServer%s %s",GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,183,"Disconnected"));

                            create_arexx_event_string(my_settings.events[DISCONNECT].arexx_script,my_settings.events[DISCONNECT].arexx_script2);
                            Execute(event_string,0,0);
                        }


                        sprintf(buffer3, "%s%sServer%s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,183,"Disconnected"));
                        status_conductor->conductor=status_conductor->root;
                        while(status_conductor->conductor)
                        {
                            if(status_conductor->conductor->LV_channel)
                            {
                                if(status_conductor->conductor->name[0]=='#' || status_conductor->conductor==status_conductor->root) add_text_to_conductor_list(buffer3,9, ACTIVITY);
                            }
                    ...
                    else if(!status_conductor->connection_active && recv_thing>0)
                    {
                        ...
                        sprintf(buffer3, "%s%sServer%s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,182,"Connected"));
                        if(DEBUG) printf("connected!!\n");

                        add_text_to_conductor_list(buffer3,9, ACTIVITY);

                        if(status_conductor->user_name[0]=='\0')    strcpy(status_conductor->user_name,"mrnobody");
                        if(uhostname[0]=='\0')   strcpy(uhostname,"test");
                        if(uservername[0]=='\0') strcpy(uservername,"test2");
                        if(status_conductor->real_name[0]=='\0')    strcpy(status_conductor->real_name,"mr nobody");

                        if(status_conductor->pass[0] == '\0')
                        {
                            sprintf(sendstuff, "NICK %s\r\nUSER %s %s %s :%s\r\n", status_conductor->nick, status_conductor->user_name, uhostname, uservername, status_conductor->real_name);
                        }
                        else
                        {

                            sprintf(sendstuff, "PASS %s\r\nNICK %s\r\nUSER %s %s %s :%s\r\n", status_conductor->pass,status_conductor->nick, status_conductor->user_name, uhostname, uservername, status_conductor->real_name);
                        }
                        send_text(sendstuff);

                        if( (my_settings.events[CONNECT].use_when == 1 && is_window_active()) || (my_settings.events[CONNECT].use_when == 2 && !is_window_active()) || my_settings.events[CONNECT].use_when == 3 )
                        {
                            sprintf(buffer3, "%sServer%s %s",GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,182,"Connected"));

                            strcpy(target_nick,"");
                            create_arexx_event_string(my_settings.events[CONNECT].arexx_script,my_settings.events[CONNECT].arexx_script2);
                            Execute(event_string,0,0);
                        }
                    ...
                    {
            			sprintf(buffer3,"%s%sConnect%s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,368,"Unable to connect"));
                        add_text_to_conductor_list(buffer3,9, ACTIVITY);
                        if(status_conductor)
                        {
            ...
            else if(i==status_conductor->ident.listen_socket)
            {
                    ...
                    if(status_conductor->ident.listen_socket != -1)
                    {
                        ...
                        status_conductor->ident.a_socket = accept(status_conductor->ident.listen_socket, (struct sockaddr *)&status_conductor->ident.their_addr, &sin_size);
                        if(DEBUG) printf("trying to accept connection for ident\n");

                        if(FD_ISSET(status_conductor->ident.listen_socket, &read_master)) FD_CLR(status_conductor->ident.listen_socket, &read_master);

                        status_conductor->ident.listen_socket=-1;

                        if(status_conductor->ident.a_socket!= -1)
                        {
                            if(DEBUG) printf("ident connection successful!\n");
                            FD_SET(status_conductor->ident.a_socket, &read_master);
                            if(fdmax<status_conductor->ident.a_socket) fdmax=status_conductor->ident.a_socket;
                        }
                    }
            ...
            else if(i==status_conductor->ident.a_socket)
            {
                    ...
                    if(status_conductor->ident.a_socket!= -1)
                    {
                        ...
                        recv_thing=recv(status_conductor->ident.a_socket, (i_in)status_conductor->ident.buffer, BUFFERSIZE-1, 0);
                        if(recv_thing != -1) status_conductor->ident.buffer[recv_thing]='\0';
                        ...
                        if(string1)
                        {
                            ...
                            if(DEBUG) printf("incoming ident text:%s\n",status_conductor->ident.buffer);

                            sprintf(string10,"%s : USERID : AMIGAOS : %s\r\n",string1,username);
                            if(DEBUG) printf("now attempting to send the following ident string:%s\n",string10);
                            ...
                            if(FD_ISSET(status_conductor->ident.a_socket, &read_master)) FD_CLR(status_conductor->ident.a_socket, &read_master);
            ...
            while(dcc_conductor)
            {
                ...
                if(i == dcc_conductor->dcc_socket) break;
            ...
            if(dcc_conductor)
            {
                ...
                if(i == dcc_conductor->dcc_socket)
                {
                    ...
                    if(DEBUG) printf("dcc recv socket has been hit!\n");
                    ...
                    if(!dcc_conductor->connected)
                    {
                        ...
                        if(DEBUG) printf("dcc recv socket: not connected yet, so lets open the file..\n");

                        //if(dcc_conductor->filename)
                        if(stricmp(dcc_conductor->full_filename_with_path,""))
                        {
                            if(DEBUG) printf("dcc recv socket: filename okay?\n");
                            ...
                            {
                                dcc_conductor->dcc_file = Open(dcc_conductor->full_filename_with_path,MODE_NEWFILE);

                            }
                            else
                            {
                                dcc_conductor->dcc_file = Open(dcc_conductor->full_filename_with_path,MODE_READWRITE);

                            }

                            if(dcc_conductor->dcc_file)
                            {
                                if(DEBUG) printf("dcc recv socket: save file has been opened successfully..\n");

                                Close(dcc_conductor->dcc_file);
                                #ifdef __MORPHOS__
                                sprintf(buffer2,"%s <%s> @ %s",dcc_conductor->nick,(i_in)Inet_NtoA(inet_addr((UBYTE*)dcc_conductor->address)),dcc_conductor->networkname);
                                #elif __AROS__
                                sprintf(buffer2,"%s <%s> @ %s",dcc_conductor->nick,(i_in)Inet_NtoA(inet_addr((const char*)dcc_conductor->address)),dcc_conductor->networkname);
                                #else
                            ...
                            {
                                printf("%s\n",GetCatalogStr(catalog,185,"unable to create new file for incoming transfer"));
                                //if(dcc_conductor->dcc_file) Close(dcc_conductor->dcc_file);
                            }

                        }

                        if(DEBUG) printf("dcc recv socket: replacing File Transfer entry\n");

                        if(dcc_conductor->entry)
                        {
                            strcpy(dcc_conductor->entry->status,GetCatalogStr(catalog,148,"Transferring"));
                    ...
                    recv_thing=recv(dcc_conductor->dcc_socket, (i_in)dcc_conductor->buffer, DCC_RECV_BUFFERSIZE-1, 0);

                    if(recv_thing == 0)
                    {

                            if(DEBUG) printf("dcc recv socket: connection has been closed somehow for some reason. could be an error, could be completed\n");
                            ...
                            if(dcc_conductor->connected && !dcc_conductor->cancelled)
                            {
                                ...
                                if(dcc_conductor->total_recv>=dcc_conductor->filesize)
                                {
                                    if(dcc_conductor->entry)
                                    {
                                        ...
                                        strcpy(dcc_conductor->entry->status,GetCatalogStr(catalog,179,"Complete")); //"Completed");
                                ...
                                else
                                {
                                    if(dcc_conductor->entry)
                                    {
                                        ...
                                        strcpy(dcc_conductor->entry->status,GetCatalogStr(catalog,180,"Incomplete")); //"Incomplete");
                                    ...
                                    sprintf(buffer3,"%s%sDCC%s SEND: %s %s %s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),dcc_conductor->filename,GetCatalogStr(catalog,173,"offered from"),dcc_conductor->nick,GetCatalogStr(catalog,180,"Incomplete"));
                            ...
                            else if(dcc_conductor->cancelled)
                            {
                                ...
                                if(dcc_conductor->entry)
                                {
                                    strcpy(dcc_conductor->entry->status,GetCatalogStr(catalog,181,"Cancelled"));
                                    float pct=0;
                                ...
                                DoMethod((Object*)WookieChat->LV_dcc,MUIM_NList_ReplaceSingle,dcc_conductor->entry,a,NOWRAP,ALIGN_LEFT);
                                sprintf(buffer3,"%s%sDCC%s SEND: %s %s %s *%s*",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),dcc_conductor->filename,GetCatalogStr(catalog,173,"offered from"),dcc_conductor->nick,GetCatalogStr(catalog,181,"Cancelled"));
                            ...
                            else
                            {
                                ...
                                if(dcc_conductor->entry)
                                {
                                    strcpy(dcc_conductor->entry->status,GetCatalogStr(catalog,184,"Failed"));
                                    float pct=0;
                                ...
                                DoMethod((Object*)WookieChat->LV_dcc,MUIM_NList_ReplaceSingle,dcc_conductor->entry,a,NOWRAP,ALIGN_LEFT);
                                sprintf(buffer3,"%s%sDCC%s SEND: %s %s %s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),dcc_conductor->filename,GetCatalogStr(catalog,173,"offered from"),dcc_conductor->nick,GetCatalogStr(catalog,184,"Failed"));
                    ...
                    if(recv_thing != -1)
                    {
                        ...
                        if(DEBUG) printf("dcc recv socket: received %lu bytes so far..\n",dcc_conductor->total_recv);
            ...
            while(dcc_send_conductor)
            {
                ...
                if(i == dcc_send_conductor->dcc_socket) break;
            ...
            while(dcc_send_conductor)
            {
                ...
                if(i == dcc_send_conductor->dcc_listen_socket) break;
            ...
            if(dcc_send_conductor)
            {
                ...
                if(i == dcc_send_conductor->dcc_listen_socket)
                {
                    ...
                    {
                        if(!stricmp(status_conductor->networkname,dcc_send_conductor->networkname)) break;
                        status_conductor=status_conductor->next;
                        if(!status_conductor)
                        {
                            status_conductor=status_root;
                    ...
                    if(!dcc_send_conductor->connected && (dcc_send_conductor->total_recv != dcc_send_conductor->filesize))
                    {
                        if(dcc_send_conductor->filename)
                        {
                            ...
                            {
                                sprintf(buffer3,"%s%sDCC%s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,186,"unable to open requested file"));
                        ...
                        if(dcc_send_conductor->entry)
                        {
                            ...
                            strcpy(dcc_send_conductor->entry->status,GetCatalogStr(catalog,148,"Transferring")); //"Transferring");
                        ...
                        if(dcc_send_conductor->dcc_socket == -1)
                        {
                            ...
                            sprintf(buffer3,"%s[DCC%s %s %s %s %s error number:%li",timestamp,GetCatalogStr(catalog,218,"]"), dcc_send_conductor->filename,GetCatalogStr(catalog,205,"to"),dcc_send_conductor->nick,GetCatalogStr(catalog,184,"failed"),Errno());
                            //add_text_to_current_list(buffer3,9, ACTIVITY_CHAT);
                            add_text_to_conductor_list(buffer3, 9, ACTIVITY_CHAT);

                            if(FD_ISSET(dcc_send_conductor->dcc_listen_socket, &read_master)) FD_CLR(dcc_send_conductor->dcc_listen_socket, &read_master);
                        }
                        else
                        {
                            timestamp_2_string();

                            if(dcc_send_conductor->entry && (dcc_send_conductor->total_recv != dcc_send_conductor->filesize))
                            {
                            ...
                            {
                                sprintf(buffer3,"%s%sDCC%s SEND: %s %s %s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,187,"Started file transfer:"),dcc_send_conductor->filename,GetCatalogStr(catalog,205,"to"),dcc_send_conductor->nick);
                                //add_text_to_current_list(buffer3,9, ACTIVITY_CHAT);
                                add_text_to_conductor_list(buffer3, 9, ACTIVITY_CHAT);

                                if( (my_settings.events[DCC_SEND_FINISHED].use_when == 1 && is_window_active()) || (my_settings.events[DCC_SEND_FINISHED].use_when == 2 && !is_window_active()) || my_settings.events[DCC_SEND_FINISHED].use_when == 3 )
                                {
                                    strcpy(target_nick,"");
                                    create_arexx_event_string(my_settings.dcc_send_finished.arexx_script,my_settings.dcc_send_finished.arexx_script2);
                                    Execute(event_string,0,0);
                                }
                            ...
                            if(fdmax<dcc_send_conductor->dcc_socket) fdmax=dcc_send_conductor->dcc_socket;
                            ...
                            #else
                            if(send_thing != actualLength) Seek(dcc_send_conductor->dcc_file,dcc_send_conductor->total_recv,OFFSET_BEGINNING);
                            #endif
                            ...
                            if(dcc_send_conductor->total_recv == dcc_send_conductor->filesize)
                            {
                                ...
                                if(dcc_send_conductor->connected && !dcc_send_conductor->cancelled)
                                {
                                    ...
                                    if(dcc_send_conductor->entry)
                                    {
                                        ...
                                        strcpy(dcc_send_conductor->entry->status,GetCatalogStr(catalog,179,"Complete"));//"Completed");
                                    ...
                                    //sprintf(buffer3,"%s%sDCC%s SEND: %s to %s complete",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),dcc_send_conductor->filename,dcc_send_conductor->nick);
                        	    ...
                        	    if(Errno() == 57 || Errno() == 34 || Errno() == 32) //Error, not connected!!
                                {
                                    ...
                                    if(dcc_send_conductor->connected)
                                    {
                                        ...
                                        if(dcc_send_conductor->entry)
                                        {
                                            ...
                                            strcpy(dcc_send_conductor->entry->status,GetCatalogStr(catalog,180,"Incomplete"));//"Incomplete");
                                            strcpy(dcc_send_conductor->entry->timeleft,"N/A");//Done");
                                            float pct=0;
                                        ...
                                        sprintf(buffer3,"%s%sDCC%s SEND: %s %s %s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),dcc_send_conductor->filename,GetCatalogStr(catalog,205,"to"),dcc_send_conductor->nick,GetCatalogStr(catalog,180,"Incomplete"));
            ...
            while(dcc_chat_conductor)
            {
                ...
                if(i == dcc_chat_conductor->dcc_socket || i == dcc_chat_conductor->dcc_listen_socket) break;
                dcc_chat_conductor=dcc_chat_conductor->next;
                if(!dcc_chat_conductor) break;

            }

            if(dcc_chat_conductor)
            {
                if(DEBUG) printf("dcc chat conductor being processed\n");
                ...
                if(i==dcc_chat_conductor->dcc_socket)
                {
                    ...
                    recv_thing=recv(dcc_chat_conductor->dcc_socket, (i_in)dcc_chat_conductor->recv_buffer, BUFFERSIZE-1, 0);
                    if(recv_thing == 0)
                    {
                        ...
                        if(dcc_chat_conductor->connected)
                        {
                            ...
                            sprintf(buffer3, "%s%sDCC%s DCC CHAT %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,183,"Disconnected"));
                            status_conductor->conductor=dcc_chat_conductor->conductor;
                            add_text_to_conductor_list(buffer3,9, ACTIVITY_CHAT);
                            if(FD_ISSET(dcc_chat_conductor->dcc_socket, &read_master)) FD_CLR(dcc_chat_conductor->dcc_socket, &read_master);
                            ...
                            if(dcc_chat_conductor->next)
                            {
                                ...
                                if(dcc_chat_conductor) delete dcc_chat_conductor;
                    ...
                    else if(recv_thing>0)
                    {
                        ...
                        if(!dcc_chat_conductor->connected)
                        {
                            ...
                            sprintf(buffer3,"%s%sDCC%s DCC CHAT %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,182,"Connected"));
                            status_conductor->conductor=dcc_chat_conductor->conductor;
                            add_text_to_conductor_list(buffer3,9, ACTIVITY_CHAT);
        ...
        if(FD_ISSET(i, &write_fds))
        {
            ...
            while(dcc_chat_conductor)
            {
                if(i == dcc_chat_conductor->dcc_socket) break;
                dcc_chat_conductor=dcc_chat_conductor->next;
                if(!dcc_chat_conductor) break;
            ...
            if(dcc_chat_conductor)
            {
                ...
                if(i==dcc_chat_conductor->dcc_socket)
                {
                    ...
                    if(!dcc_chat_conductor->connected)
                    {
                        ...
                        sprintf(buffer3,"%s%sDCC%s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,189,"DCC CHAT connection established"));
                        status_conductor->conductor=dcc_chat_conductor->conductor;
                        add_text_to_conductor_list(buffer3,9, ACTIVITY);
                        //strcpy(dcc_chat_conductor->str,"");
                    ...
                    if(FD_ISSET(dcc_chat_conductor->dcc_socket, &write_master)) FD_CLR(dcc_chat_conductor->dcc_socket, &write_master);
