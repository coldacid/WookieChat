...
int free_graphical_smilies(struct query_window *new_tab)
{
    ...
    if(DEBUG) printf("freeing all graphical smilies() so they can be used in a new tab..\n");
    ...
    if(my_settings.graphical_nicklist)
        setmacro((Object*)new_tab->LV_nicklist,MUIA_ShortHelp,GetCatalogStr(catalog,319,"Graphical Usermodes Key:\nGreen = Channel Operators or @\nBlue = HalfOps or \%\nYellow = Voiced Users or +"));
    else
...
int use_graphical_smilies(struct query_window *new_tab)
{
    ...
    if(DEBUG) printf("use_graphical_smilies()\n");
...
void exit_delete_smiley_objects()
{
    ...
    if(DEBUG) printf("exit_delete_smiley_objects()\n");
	...
	while ((member_object = NextObject( (Object**)&object_state )) )
	{
        if(member_object)
        {
            ...
            MUI_DisposeObject((Object*)member_object); member_object=NULL;
        }
    }
    ...
    for(count=1; count<=total_smileys; count++)
    {
        ...
        if(smilies[count].icon)           MUI_DisposeObject((Object*)smilies[count].icon);
    ...
    for(count=1; count<=preview_total_smileys; count++)
    {
        ...
        if(preview_smilies[count].icon)   MUI_DisposeObject((Object*)preview_smilies[count].icon);
...
void delete_smiley_objects()
{
    ...
    if(status_conductor)
    {
        ...
        if(status_conductor->conductor) work_query=status_conductor->conductor;
    ...
    for(status_conductor=status_root; status_conductor; status_conductor=status_conductor->next)
    {
        for(status_conductor->conductor=status_conductor->root; status_conductor->conductor; status_conductor->conductor=status_conductor->conductor->next)
        {
            ...
            {
                if(DEBUG) printf("delete_smiley_objects()\n");
    ...
    for(count=0; count<=2; count++)
    {
        if(graphical_nicklist[count].icon)
        {
            if(DEBUG) printf("delete_smiley_objects() 2\n");
    ...
    if(status_conductor) status_conductor->conductor=status_conductor->root;
}
...
int load_graphical_smilies(void)
{
    ...
    if(DEBUG) printf("opening graphical smilies\n");
                ...
                if(work_buffer[0]!='#')
                {
                    ...
                    if(string1)
                    {
                        ...
                        */
                        ...
                        setmacro((Object*)smilies[total_smileys].choose_icon,MUIA_InputMode,MUIV_InputMode_RelVerify);
                        DoMethod((Object*)smilies[total_smileys].choose_icon,MUIM_Notify,MUIA_Selected,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,GRAPHICAL_SMILEY_VALUES+total_smileys);
                            ...
                            if(DoMethod((Object*)GR_string_gadget,MUIM_Group_InitChange))
                            {
                            	...
                            	while ((member_object = NextObject( (Object**)&object_state )) )
                            	{
                                    if(member_object == WookieChat->smiley_choose_icon)
                                    {
                                        ...
                                        if(member_object) DoMethod((Object*)member_object,MUIM_KillNotify,MUIA_Selected);

                                        DoMethod((Object*)GR_string_gadget, OM_REMMEMBER, (Object*)member_object);
                                    }
                                }

                                //attempt to free up images when quit
                                if(WookieChat->smiley_choose_icon) MUI_DisposeObject((Object*)WookieChat->smiley_choose_icon);
                                ...
                                */
                                ...
                                if(WookieChat->smiley_choose_icon)
                                {
                                    setmacro((Object*)WookieChat->smiley_choose_icon,MUIA_InputMode,MUIV_InputMode_RelVerify);

                                    DoMethod((Object*)GR_string_gadget,OM_ADDMEMBER, (Object*)WookieChat->smiley_choose_icon);

                                    DoMethod((Object*)WookieChat->smiley_choose_icon,MUIM_Notify,MUIA_Selected,FALSE,(Object*)WookieChat->App,2,MUIM_Application_ReturnID,95);
                                }
                            	DoMethod((Object*)GR_string_gadget,MUIM_Group_ExitChange);
                            }

                            if(DEBUG) printf("setup our CHOOSER icon..\n");
                        ...
                        if(smilies[total_smileys].choose_icon)
                            DoMethod((Object*)WookieChat->GR_graphical_smileys_choose, OM_ADDMEMBER, (Object*)smilies[total_smileys].choose_icon);
                    ...
                    while(string1)
                    {
                        sprintf(smilies[total_smileys].ascii[smilies[total_smileys].ascii_total],"%s ",string1);
                        if(smilies[total_smileys].ascii_total==0) sprintf(smilies[total_smileys].remove_ascii,"%s",string1);
                        smilies[total_smileys].ascii_total++;
                        string1=strtok(NULL,"\n ");
                        if(smilies[total_smileys].ascii_total>=MAXIMUM_SMILEY_ASCII) { /*smilies[total_smileys].ascii_total--;*/ break; }

                    }
                    if(total_smileys>=MAXIMUM_SMILEYS) break;
                }
            }
...
char * remove_smiley(char *work_buffer, char *ascii_smiley,int smiley_gfx_number)
{
    ...
    while(graphical_string1)
    {
        ...
        strcpy(graphical_string1,""); strcat(graphical_string1,ascii_smiley);
...
void insert_smiley(char *ascii_smiley,int smiley_gfx_number)
{
    ...
    {
        if(centry->entry[a]!=ascii_smiley[b]) break;
