...
int search_for_highlight_words(char *stringSearch, char *buffer3)
{
    ...
    if(strstr(string2,status_conductor->nick))
    {
        ...
        (my_settings.play_sound_when_highlight == 3 && status_conductor->conductor!=status_current->current_query) ||
        (my_settings.play_sound_when_highlight == 3 && !string1))
        {

            //play_external_sound_replayer_highlight();
            if(!muted_sound && o2) DoMethod (o2,DTM_TRIGGER,NULL,STM_PLAY,NULL);
    ...
    for(unsigned int a=0; a!=strlen(string2)+1; a++) string2[a] = ToLower(string2[a]);
