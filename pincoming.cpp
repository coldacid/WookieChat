...
#ifdef __AROS__
struct TagItem my_incoming_charset1_taglist[] = { {CSA_Source, (IPTR)buffer2 },
    {CSA_SourceCodeset, (IPTR)cs}, {CSA_DestCodeset, (IPTR)charsets[local_charset]}, {TAG_DONE, 0} };

struct TagItem my_incoming_charset2_taglist[] = { {CSA_Source, (IPTR)buffer2 },
    {CSA_SourceCodeset, (IPTR)cs}, {CSA_DestCodeset, (IPTR)local_charsets[local_charset]}, {TAG_DONE, 0} };

struct TagItem my_incoming_charset3_taglist[] = { {CSA_Source, (IPTR)buffer2},{CSA_CodesetFamily, CSV_CodesetFamily_Latin},{CSA_FallbackToDefault, TRUE},{TAG_DONE,0} };
#else
struct TagItem my_incoming_charset1_taglist[] = { {CSA_Source, (ULONG)buffer2 },
    {CSA_SourceCodeset, (ULONG)cs}, {CSA_DestCodeset, (ULONG)charsets[local_charset]}, {TAG_DONE, 0} };

struct TagItem my_incoming_charset2_taglist[] = { {CSA_Source, (ULONG)buffer2 },
    {CSA_SourceCodeset, (ULONG)cs}, {CSA_DestCodeset, (ULONG)local_charsets[local_charset]}, {TAG_DONE, 0} };

struct TagItem my_incoming_charset3_taglist[] = { {CSA_Source, (ULONG)buffer2},{CSA_CodesetFamily, CSV_CodesetFamily_Latin},{CSA_FallbackToDefault, TRUE},{TAG_DONE,0} };
#endif
...
void add_unread_lines_num(void)
{
    ...
    if(status_conductor->conductor->number_of_lines_unread<MAXIMUM_UNREAD_LINES_VALUE)
    {
        ...
        //button style tabs
        sprintf(channel_display,"%s%s %d",NewPreParse_NewText,status_conductor->conductor->displayed_name,status_conductor->conductor->number_of_lines_unread);
...
void onjoin_clone_detection(char *nick,char *hostname)
{
    ...
    if(nick && hostname)
    {
        ...
        sprintf(buffer3,"%s%s%s%s %s %s: %s ",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,286,"Clones"),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,287,"Detected clones from"),string1, nick);
        ...
        if(found_clones==TRUE) add_text_to_conductor_list((char*)buffer3, 9, ACTIVITY);
...
BOOL user_being_ignored(char *nick, char *hostname, int type)
{
    ...
    if(type==PRIVMSG) ignore_privmsg = FALSE;
    else if(type==CTCP)    ignore_ctcp    = FALSE;
    else if(type==DCC)     ignore_dcc     = FALSE;

    char string7[800];

    strcpy(string7,nick);
    for(unsigned count3=0; count3<strlen(string7); count3++) string7[count3]=ToLower(string7[count3]);
    //for(unsigned count3=0; count3<strlen(nick); count3++) nick[count3]=ToLower(nick[count3]);
    for(unsigned count3=0; count3<strlen(hostname); count3++) hostname[count3]=ToLower(hostname[count3]);
...
//
//    if(clicked_char_number == -1) return NULL;
//
//    unsigned int aa=0;
//    unsigned int bb=0;
//    unsigned int cc=0;
...
//
//    /*if(column==2)
//        clicked_char_number=clicked_char_number+10-2;
//    else
//        clicked_char_number=clicked_char_number-10+1;
//      */
//
//    if(DEBUG) printf("buffer1:%s\n",buffer1);
//    if(DEBUG) printf("clicked_char_number:%d\n",clicked_char_number);
...
//
//            if(urlvisit_str[num_url_string][aa]==' ' || urlvisit_str[num_url_string][aa]==')' || urlvisit_str[num_url_string][aa]=='\'')
//            {
...
//
//        for(aa=strlen(buffer1); aa>0; aa--)
//        {
//            if(buffer1[aa]==' ') end_character=aa;
//            if(aa==clicked_char_number) break;
//        }
//
//        for( ; aa>=0; aa--)
//        {
...
//        start_character=aa;
//
//        for(count=0,count2=start_character; count2<=end_character; count++,count2++)
//        {
//            urlvisit_str[num_url_string][count]=buffer1[count2];
//        }
//        urlvisit_str[num_url_string][count]='\0';
//
//        if(DEBUG) printf("start_char:%d,",start_character);
//        if(DEBUG) printf("end_char:%d\n",end_character);
...
//        urlvisit_str[num_url_string][aa]='\0';
//        /*
//        if(column==2 && my_settings.use_column_display==TRUE)
//        {
//            printf("attempting to open url 1\n");
//            if(DEBUG) printf("urlvisit_str:%s\n",urlvisit_str[num_url_string]);
//            if(DEBUG) printf("column:%d\n",column);
//            if(DEBUG) printf("bb:%d\n",bb);
//
//        }
//        else if(column==0 && my_settings.use_column_display==FALSE)
//        {
//            printf("attempting to open url 2\n");
//            if(DEBUG) printf("urlvisit_str:%s\n",urlvisit_str[num_url_string]);
//            if(DEBUG) printf("column:%d\n",column);
//            if(DEBUG) printf("bb:%d\n",bb);
//        }
...
//
//
//        /*if(strncmp(urlvisit_str[num_url_string],"http://",6)==0 || strncmp(urlvisit_str[num_url_string],"https://",6)==0 || strncmp(urlvisit_str[num_url_string],"www",2)==0 )
//        {
//
...
//                if(bb>=cc)
//                //if((bb==(ULONG)clicked_char_number || bb > (ULONG)clicked_char_number) && (bb < strlen(urlvisit_str[num_url_string])) && strcspn(urlvisit_str[num_url_string],status_current->chantypes)!=0)
//                {
//                    if(DEBUG) printf("url action 1\n");
...
//                //if(DEBUG) printf("trying url action 1\n");
//
//                if((bb==(ULONG)clicked_char_number || bb > (ULONG)clicked_char_number) && strcspn(urlvisit_str[num_url_string],status_current->chantypes)!=0) // urlvisit_str[num_url_string][0] != '#')
//                {
//                    if(DEBUG) printf("url action 1\n");
//                    return urlvisit_str[num_url_string];
//                }
...
//                //if(DEBUG) printf("trying url action 2\n");
//
//                if((bb+8==(ULONG)clicked_char_number || bb+8 > (ULONG)clicked_char_number) && strcspn(urlvisit_str[num_url_string],status_current->chantypes)!=0) //urlvisit_str[num_url_string][0] != '#')
//                {
//                    if(DEBUG) printf("url action 2\n");
//                    return urlvisit_str[num_url_string];
//                }
...
void search_for_url_then_add(char *buffer1)
{
    ...
    if(!my_settings.urlgrabber) return; //is the url grabber switched off?
    ...
    while(bb<strlen(buffer1))
    {
        ...
        if(strncmp(urlgrabber_str,"http://",6)==0 || strncmp(urlgrabber_str,"https://",6)==0 || strncmp(urlgrabber_str,"www",2)==0) // || strchr(urlgrabber_str,'@'))
        {
            ...
            if(aaa==entries)
            {
                DoMethod((Object*)WookieChat->LV_urlgrabber,MUIM_NList_InsertSingle,urlgrabber_str,MUIV_NList_Insert_Top);
...
int add_text_to_conductor_list(char *buffer1, LONG colour, int activitylevel)
{
    ...
    do
    {
        if(buffer1[a]=='\003')
        {
            ...
            {
                a++;
                if(buffer1[a] - '0' >= 0 && buffer1[a] - '0' <= 9) a++;
            }
            else if(buffer1[a] - '0' == 1)
            {
                a++;
                if(buffer1[a] - '0' >= 0 && buffer1[a] - '0' <= 5) a++;
            }
    ...
    if(!status_conductor)
    { ReleaseSemaphore((struct SignalSemaphore*)slist); return 0; }
    ...
    if(!status_conductor->conductor->log_file && ((strcspn(status_conductor->conductor->name,status_conductor->chantypes)==0 && my_settings.log_channels) || my_settings.log_private))
    {
        ...
        if(status_conductor->conductor->log_file)
        {
            ...
            sprintf(buffer_text,"* * %s %d-%d-%d, at %s:%s * *",GetCatalogStr(catalog,156,"Logging started at"),clockdata->year,clockdata->month,clockdata->mday,timestamp_hrs,timestamp_mins);
    ...
    if(status_conductor->conductor->log_file
    && ((strcspn(status_conductor->conductor->name,status_conductor->chantypes)==0 && my_settings.log_channels) || (strcspn(status_conductor->conductor->name,status_conductor->chantypes)!=0 && my_settings.log_private) ))
    {
...
int add_text_to_channels_query(char *buffer1, LONG colour, int activitylevel)
{
    ...
    if(!status_conductor->conductor->LV_channel) return 0;
    ...
    */
	...
	DoMethod((Object*)status_conductor->conductor->LV_channel,MUIM_NList_InsertSingle,centry,MUIV_NList_Insert_Bottom);
...
void process_incoming()
{
    ...
    while(1)
    {
        ...
        {
            james[a]=status_conductor->str[b]; a++; b++;
            james[a]=status_conductor->str[b]; a++; b++;
    ...
    if(pch)
	{
		...
		if(RAW) printf("before conversion %s\n",buffer2);

        if((cs = CodesetsFindBestA(my_incoming_charset3_taglist)))
        {
            #ifdef __AROS__
            struct TagItem my_incoming_charset2_taglist[] = { {CSA_Source, (IPTR)buffer2 }, {CSA_SourceCodeset, (IPTR)cs}, {CSA_DestCodeset, (IPTR)local_charsets[local_charset]}, {TAG_DONE, 0} };
            #else
            struct TagItem my_incoming_charset2_taglist[] = { {CSA_Source, (ULONG)buffer2 }, {CSA_SourceCodeset, (ULONG)cs}, {CSA_DestCodeset, (ULONG)local_charsets[local_charset]}, {TAG_DONE, 0} };
            #endif
            ...
            if(FindUTF8Chars(buffer2))
            {
        	    ...
        	    {
                    #ifdef __AROS__
                    struct TagItem my_incoming_charset4_taglist[] = { {CSA_Source, (IPTR)buffer2 },
	                	            {CSA_SourceCodeset, (IPTR)remote_charsets[0]}, /*{CSA_DestCodeset, (ULONG)local_charsets[local_charset]},*/ {TAG_DONE, 0} };
			        
                    #else
		            struct TagItem my_incoming_charset4_taglist[] = { {CSA_Source, (ULONG)buffer2 },
	                	            {CSA_SourceCodeset, (ULONG)remote_charsets[0]}, /*{CSA_DestCodeset, (ULONG)local_charsets[local_charset]},*/ {TAG_DONE, 0} };
			        #endif
			         
	        	    text3 = CodesetsUTF8ToStrA(my_incoming_charset4_taglist);

                	if(DEBUG) printf("codesets 2\n");
            
            	}
            }
            else
            {
                if(DEBUG) printf("codesets 3\n");
        ...
        else
        {
            #ifdef __AROS__
            struct TagItem my_incoming_charset4_taglist[] = { {CSA_Source, (IPTR)buffer2 },
                {CSA_SourceCodeset, (IPTR)remote_charsets[status_conductor->remote_charset]}, {CSA_DestCodeset, (IPTR)local_charsets[local_charset]}, {TAG_DONE, 0} };
            #else
            struct TagItem my_incoming_charset4_taglist[] = { {CSA_Source, (ULONG)buffer2 },
                {CSA_SourceCodeset, (ULONG)remote_charsets[status_conductor->remote_charset]}, {CSA_DestCodeset, (ULONG)local_charsets[local_charset]}, {TAG_DONE, 0} };
            #endif
            ...
            if(1) //if(status_conductor->remote_charset==0)
            {
                ...
                if(DEBUG) printf("codesets 4\n");

            }
            else
            {
                if(DEBUG) printf("codesets 5\n");
        ...
        if(text3) strcpy(buffer2,text3);

        if(text3) CodesetsFreeA(text3, NULL);
		text3=NULL;
        ...
        if(!(incoming_1)) { incoming_1=new char[1]; strcpy(incoming_1,""); }
        if(!(incoming_2)) { incoming_2=new char[1]; strcpy(incoming_2,""); }
        if(!(incoming_3)) { incoming_3=new char[1]; strcpy(incoming_3,""); }

		if(DEBUG && incoming_3) printf("1:%s, 2:%s, 3:%s\n",incoming_1, incoming_2, incoming_3);
		...
		if(!status_conductor->conductor) status_conductor->conductor=status_conductor->root;
		...
		else if(!strcmp(incoming_2,"482"))
		{
            ...
            if(string1 && string2)
            {
    			if(!stricmp((p_in)string1,(p_in)status_conductor->nick)) sprintf(buffer3,"%s%s%s%s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,0,"Error"),GetCatalogStr(catalog,218,"]"),string2);
    			else sprintf(buffer3,"%s%s%s%s%s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,0,"Error"),GetCatalogStr(catalog,218,"]"),string1, string2);
		...
		else if(!strcmp(incoming_2,"001"))
		{
            ...
            if(strcmp(status_conductor->groupname,"") != 0)
            {
                ...
                if(ZUNE_SYSTEM==TRUE)
                    sprintf(status_conductor->root->nlist_tab_title.entry,"         \033c\033E%s",status_conductor->networkname);
                else
                    sprintf(status_conductor->root->nlist_tab_title.entry,"         :\033c\033E%s",status_conductor->networkname);
                status_conductor->root->nlist_tab_title.colour=23;
                DoMethod((Object*)WookieChat->LV_tabs,MUIM_NList_ReplaceSingle,&status_conductor->root->nlist_tab_title,status_conductor->root->nlist_tab_title_number);
            }
            else
            {
            ...
            if(my_settings.no_server_tabs==0) strcpy(status_conductor->root->displayed_name,"Status");
            ...
            if(status_conductor->networkname[0] == '\0') strcpy(status_conductor->networkname, status_conductor->servername);
			...
			sprintf(buffer3,"%s%s%s%s %s %s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,359,"Server"),GetCatalogStr(catalog,218,"]"),incoming_1, incoming_2, incoming_3);
            status_conductor->conductor=status_conductor->root;
                    ...
                    if(strtok(NULL,"@"))
                    {
                        ...
                        if(string3)
                        {
                            //if(strchr(string3,'.'))
                            {
                                ...
                                {
                                    sprintf(buffer3,"%s%sDCC%s Unable to use obtain own address for use in DCC connections",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"));
            ...
            for(status_conductor->conductor=status_conductor->root; status_conductor->conductor; status_conductor->conductor=status_conductor->conductor->next)
            {
                if(strcspn(status_conductor->conductor->name,status_conductor->chantypes)==0)
                {
                    if(!stricmp(status_conductor->conductor->keyword,""))
                        ...
                        sprintf(buffer3, "JOIN %s %s\r\n",status_conductor->conductor->name,status_conductor->conductor->keyword);
                    send_text(buffer3);
                }
            }
	    ...
	    else if(!strcmp(incoming_2,"WALLOPS"))
		{
			...
			{ string7[b]=incoming_1[a]; a++; b++; }	string7[b]='\0';

            b=0;

            if(incoming_3[0]==':') a=1; else a=0;

			while(incoming_3[a] != '\0')
			{ string9[b]=incoming_3[a]; b++; a++; }	string9[b]='\0';
        ...
        else if(!strcmp(incoming_2,"INVITE"))
		{
            ...
            for(a=1; a<strlen(incoming_1); a++)
            {
                ...
                if(string7[a-1]=='!') { string7[a-1]='\0'; break; }
            }

            if(string1 && string2)
            {
                sprintf(buffer3, "%s%s%s%s %s%s %s %s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,800,"Invite"),GetCatalogStr(catalog,218,"]"),string1,GetCatalogStr(catalog,157,"has been invited to channel"),string2, GetCatalogStr(catalog,158,"by user"),string7);
    			add_text_to_conductor_list((char*)buffer3, 14, ACTIVITY);

                if( (my_settings.events[INVITE].use_when == 1 && is_window_active()) || (my_settings.events[INVITE].use_when == 2 && !is_window_active()) || my_settings.events[INVITE].use_when == 3 )
                {
                    sprintf(buffer3, "%s%s%s %s%s %s %s %s",GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,800,"Invite"),GetCatalogStr(catalog,218,"]"),string1,GetCatalogStr(catalog,157,"has been invited to channel"),string2, GetCatalogStr(catalog,158,"by user"),string7);

                    strcpy(target_nick,string1);
                    create_arexx_event_string(my_settings.events[INVITE].arexx_script, my_settings.events[INVITE].arexx_script2);
	    ...
	    else if(!strcmp(incoming_2,"005"))
		{
            ...
            if(string1 && string2)
            {
    			if(!stricmp((p_in)string1,(p_in)status_conductor->nick)) sprintf(buffer3,"%s%s%s%s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,359,"Server"),GetCatalogStr(catalog,218,"]"),string2);
    			else sprintf(buffer3,"%s%s%s%s %s %s",timestamp, GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,359,"Server"),GetCatalogStr(catalog,218,"]"),string1, string2);

    			add_text_to_conductor_list((char*)buffer3, colour, ACTIVITY);

                if(stricmp((p_in)string1,(p_in)status_conductor->nick)) strcpy(status_conductor->nick,string1);
			...
			if(work)
			{
				...
				while(work[a1] != ' ')
				{
					   nick_length_char[b1]=work[a1]; a1++; b1++;
				}
                ...
                if(string1 && string2)
                {
        			if(!strcmp(string1,status_conductor->nick)) sprintf(buffer3,"%s%s%s%s %s",timestamp, GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,359,"Server"),GetCatalogStr(catalog,218,"]"),string2);
        			else sprintf(buffer3,"%s%s%s%s %s %s",timestamp, GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,359,"Server"),GetCatalogStr(catalog,218,"]"),string1, string2);
            ...
            if(work)
            {
				...
				while(work[a1] != ' ')
				{
					   status_conductor->networkname[b1]=work[a1]; a1++; b1++;
				}
				status_conductor->networkname[b1] = '\0';

                if(status_conductor->networkname[0] != '\0')     {  sprintf(string7, "\033I[2:00000000,00000000,00000000] %s",status_conductor->networkname); } //keep the proper network name in the button instead of "status"
                ...
                if(!strpbrk(status_conductor->root->nlist_tab.entry,"#&"))
                {
                    ...
                    if(my_settings.no_server_tabs==0 && my_settings.use_nlistobject_for_tabs==0) strcpy(status_conductor->root->displayed_name, status_conductor->networkname);
        ...
        else if(!strcmp(incoming_2,"315")) // End of /WHO List
        {
            ...
            //lets find clones!
             if(string1)
             {
                ...
                for(status_conductor->conductor=status_conductor->root; status_conductor->conductor; status_conductor->conductor=status_conductor->conductor->next)
                {
                    if(!stricmp(status_conductor->conductor->name,string1)) break;
                }
                ...
                if(status_conductor->conductor)
                {
                    ...
                    for(int count2=0; count2<status_conductor->conductor->nicks; count2++)
                    {
                        ...
                        if(status_conductor->conductor->nicklist[count2].hostname)
                        {
                            ...
                            sprintf(buffer3,"%s%s%s%s %s %s: ",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,286,"Clones"),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,287,"Detected clones from"),string1);
                        ...
                        if(count3>1)
                        {
                            if(count_clones==MAX_CLONE_FIELDS) break;

                            for(a=0; a<MAX_CLONE_FIELDS; a++)
                            {
                                if(!stricmp(clone_results[a],buffer3)) break;
                            }
...
//1:NO.AmigaWorld.Net, 2:615, 3:jahcman jahcman :is using modes +i +
            ...
            if(string1) { string2=strtok(NULL,""); } //is using modes (whatever)

            if(string1 && string2)
            {
                sprintf(buffer3,"%s%sMode%s %s%s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"), string1, string2);
                ...
                !strcmp(incoming_2,"378") || !strcmp(incoming_2,"616") || !strcmp(incoming_2,"614") ) // WHOIS RESULT DISPLAY
		{
            ...
            {
                sprintf(buffer3,"%s%sWhois%s %s User is away %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),work1,string1);
            ...
            else if(!strcmp(incoming_2,"317")) //seconds idle, signon time
            {
                ...
                for(count=0; count2<(LONG)strlen(string7); count++,count2++)
                {
                    ...
                    if(string10[count]==' ') { string10[count]='\0'; count++; count2++; break; }
                }

                for(count=0; count2<(LONG)strlen(string7); count++, count2++)
                {
                    string11[count]=string7[count2];
                    if(string11[count]==' ') { string11[count]='\0'; count++; break; }
                ...
                //if(string10 && string11)
                {
                    ...
                    sprintf(buffer3,"%s%sWhois%s Idle: %s, Signed on at: %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"), string10, asctime(timeinfo));
            ...
            else
            {
                ...
                if(string1)
                    sprintf(buffer3,"%s%sWhois%s %s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"), work1, string1);
                else
                    sprintf(buffer3,"%s%sWhois%s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"), work1);
		...
		else if(!strcmp(incoming_2,"305") || !strcmp(incoming_2,"306")) // away messages
		{
            ...
            if(string1)
            {
                sprintf(buffer3,"%s%sAway%s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),string1);
                add_text_to_conductor_list(buffer3, 9, ACTIVITY);
            }
        ...
        else if(!strcmp(incoming_2,"367")) //an incoming banmask for a channel
        {
            ...
            if(string1 && string2 && string3)
            {
                ...
                DoMethod((Object*)WookieChat->LV_ban,MUIM_NList_InsertSingle,buffer3,MUIV_NList_Insert_Bottom,ALIGN_LEFT);
		...
		else if(!strcmp(incoming_2,"341")) //message telling you that you have just invited someone to a channel
		{
            ...
            if(string1 && string2)
            {
                sprintf(buffer3, "%s%sInvite%s %s %s %s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,159,"You have invited"),string1,GetCatalogStr(catalog,160,"to channel"),string2);
    			add_text_to_conductor_list((char*)buffer3, 14, ACTIVITY);
            }
		...
		else if(!strcmp(incoming_2,"321")) //a list of channels on this network is about to be sent to us
		{
            ...
            for(a=0; a!=strlen(incoming_1)+1; a++) rawservername[a]=incoming_1[a+1]; //servername

            status_conductor=status_root;
            do
            {
                if(!status_conductor) break;
                if(!stricmp(status_conductor->servername,rawservername)) break;
            ...
            do
            {
                ...
                if(!stricmp(status_conductor->conductor->name,"*Channel_List")) break;
                status_conductor->conductor=status_conductor->conductor->next;

            }while(1);

            if(!status_conductor->conductor) create_new_tab((char*)"*Channel_List", 1, 2); //open a private query tab to display channel names in
            ...
            if(string1) add_text_to_channels_query(string1,9, ACTIVITY);
		...
		{
            strtok(incoming_3," ");
            char *channel; channel=strtok(NULL," ");
            char *modes; modes=strtok(NULL, " ");
            char *mode_params; mode_params=strtok(NULL,"\0");
            char *work; work=strtok(mode_params, " ");
            int a=0;
        ...
        else if( !strcmp(incoming_2,"353") ) // NAMES list
    	{
            ...
            while(strcspn(incoming_3,status_conductor->chantypes)!=(ULONG)a) a++;

            //while(incoming_3[a] != '#' && incoming_3[a] != '&') a++;

            while(incoming_3[a] != ' ')
    	    { string7[b]=incoming_3[a]; a++; b++; }    string7[b]='\0'; a=a+2; b=0;
            ...
            for(status_conductor->conductor=status_conductor->root; status_conductor->conductor; status_conductor->conductor=status_conductor->conductor->next)
            {
                if(!stricmp(string7,status_conductor->conductor->name)) break;
            }
            ...
            if(!status_conductor->conductor)
            {
                ...
                sprintf(buffer3,"%s%sNames%s %s %s:",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,208,"Users in channel"),string7);
                add_text_to_current_list(buffer3, 9, ACTIVITY);

                sprintf(buffer3,"%s%sNames%s %s:",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),string8);
                add_text_to_current_list(buffer3, 9, ACTIVITY);
            ...
            else  //we're in the channel we're getting the list from, so lets update the nicklist structure with their names
            {
                ...
                if(work)
                {
                    ...
                    {
            	        if(work[0] == status_conductor->user_modes[count].symbol[0]) result=1;

                    }
                    if(result)
                    {
            	        mode_work[0]=work[0]; mode_work[1]='\0';
                    	a=0;
                ...
                if(status_conductor->conductor)
                {
                    if(status_conductor->conductor->nicks) count=status_conductor->conductor->nicks-1; else count=-1;
                }
                ...
                while(work)
        	    {
                    ...
                    {
            	        if(work[0] == status_conductor->user_modes[count4].symbol[0]) result=1;
		...
		else if(!strcmp(incoming_2,"332")) // Topic
		{
			...
			int a=0, b=0;
			while(incoming_3[b] != ' ')
			{ string7[a]=incoming_3[b]; a++; b++; }	string7[a]='\0'; b++; a=0;

			while(incoming_3[b] != ' ')
			{ string8[a]=incoming_3[b]; a++; b++; }	string8[a]='\0'; b=b+2; a=0;

			while(incoming_3[b] != '\0')
			{ string9[a]=incoming_3[b]; a++; b++; } string9[a]='\0';
			...
			sprintf(buffer3,"%s%sTopic%s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),string9);
            ...
            if(status_conductor->conductor)
            {
                ...
                DoMethod((Object*)status_conductor->conductor->STR_topic,MUIM_Notify,MUIA_String_Acknowledge, MUIV_EveryTime, (Object*)WookieChat->App,2,MUIM_Application_ReturnID,29);

                add_text_to_conductor_list((char*)buffer3, 16, ACTIVITY);

                strcpy(status_conductor->conductor->topic,string9);
                setmacro((Object*)status_conductor->conductor->STR_topic,MUIA_ShortHelp,&status_conductor->conductor->topic);
		...
		else if(!strcmp(incoming_2,"333")) // Topic set by
		{
			...
			while(incoming_3[b] != ' ')
			{ string8[a]=incoming_3[b]; a++; b++; }	string8[a]='\0'; b++; a=0;

			while(incoming_3[b] != ' ')
			{ string9[a]=incoming_3[b]; a++; b++; }	string9[a]='\0'; b++; a=0;

			while(incoming_3[b] != '\0')
			{ string10[a]=incoming_3[b]; a++; b++; } string10[a]='\0';

            time_t rawtime=atoi(string10);
            struct tm *timeinfo;
            timeinfo=localtime(&rawtime);

			sprintf(buffer3,"%s%sTopic%s %s %s %s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,161,"set by"),string9, GetCatalogStr(catalog,162,"on"),asctime(timeinfo));
    		...
    		if(status_conductor->conductor) add_text_to_conductor_list((char*)buffer3, 16, ACTIVITY);
                else add_text_to_current_list((char*)buffer3,16, ACTIVITY);
		...
		else if( !strcmp(incoming_2,"433") && incoming_3[0] == '*') // Nick in use msg when connecting to server
		{
            ...
            if(!stricmp((p_in)status_conductor->nick,(p_in)status_conductor->initial_nick) && status_conductor->nick2[0]!='\0')
            {
                strcpy(newnick,status_conductor->nick2);
            }

            else if(!stricmp((p_in)status_conductor->nick,(p_in)status_conductor->nick2) && status_conductor->nick3[0]!='\0')
            {
			...
			sprintf(buffer3,"%s%sNick%s Nick %s %s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),status_conductor->nick,GetCatalogStr(catalog,163,"in use, retrying with"),newnick);
		...
		else if(!strcmp(incoming_2,"JOIN"))
		{
			...
			while(incoming_1[a] != '!')
			{ string7[b]=incoming_1[a]; a++; b++; } string7[b]='\0'; b=0; a++;

			while(incoming_1[a] != '\0')
			{ string8[b]=incoming_1[a]; a++; b++; }	string8[b]='\0'; a=0; b=0;
			...
			sprintf(buffer3,"%s%s%s%s %s (%s)",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,164,"Join"),GetCatalogStr(catalog,218,"]"),string7,string8);
            ...
            if(my_settings.no_server_tabs || (!my_settings.no_server_tabs && status_conductor->root->server_tab==0))
            {
                for(status_conductor->conductor=status_conductor->root; status_conductor->conductor; status_conductor->conductor=status_conductor->conductor->next)
                {
                    if(strcspn(status_conductor->conductor->name,status_conductor->chantypes)==0) break;
                    if(status_conductor->conductor->name[0]=='#') break;
                    if(status_conductor->conductor->name[0]=='!') break;
            ...
            if(!status_conductor->conductor)
            {
                ...
                if(!status_conductor->conductor)
                {
                    ...
                    for(status_conductor->conductor=status_conductor->root; status_conductor->conductor; status_conductor->conductor=status_conductor->conductor->next)
                    {
                        ...
                        if(status_conductor->conductor->BT_querybutton && status_conductor->conductor->LV_nicklist && strcspn(status_conductor->conductor->name,status_conductor->chantypes)!=0) break;
                ...
                else
                {
        			...
        			if( !stricmp((p_in)status_conductor->nick,(p_in) string7) )
        			{
        				...
        				else sprintf(buffer3,"%s%s%s%s %s %s..",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,164,"Join"),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,209,"Auto-rejoining"),string9);
            ...
            else
            {
    			...
    			else
    			{
                    ...
                    if(status_conductor->conductor)
                    {
                        ...
                        if( (my_settings.events[JOIN].use_when == 1 && is_window_active()) || (my_settings.events[JOIN].use_when == 2 && !is_window_active()) || my_settings.events[JOIN].use_when == 3 )
                        {
                            strcpy(target_nick,string7);
                            create_arexx_event_string(my_settings.events[JOIN].arexx_script, my_settings.events[JOIN].arexx_script2);
                            Execute(event_string,0,0);
                        }


                        if(my_settings.clone_detection) { onjoin_clone_detection(string7,string8); }

                        strcpy(new_entry2.modes,"");
                        strcpy(new_entry2.name,string7);
                        if(!new_entry2.hostname) new_entry2.hostname=new char[HOSTNAME_STRING_SIZE];
                        strcpy(new_entry2.hostname,string8);

                        strcpy(status_conductor->conductor->nicklist[status_conductor->conductor->nicks].name,new_entry2.name);
                        ...
                        if(!status_conductor->conductor->nicklist[status_conductor->conductor->nicks].hostname)
                            status_conductor->conductor->nicklist[status_conductor->conductor->nicks].hostname=new char[HOSTNAME_STRING_SIZE];

                        strcpy(status_conductor->conductor->nicklist[status_conductor->conductor->nicks].hostname,new_entry2.hostname);
                        ...
                        if(status_conductor->conductor && status_conductor->conductor->nicks==0)
                        {
                            ...
                            sprintf(buffer3,"%s%s%s%s %s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,164,"Join"),GetCatalogStr(catalog,218,"]"),string7,GetCatalogStr(catalog,299,"has returned to IRC.."));
                            add_text_to_conductor_list((char*)buffer3, 2, ACTIVITY);
                        }
		...
		else if(!strcmp(incoming_2,"KICK"))
		{
            ...
            if(string3 && string4)
            {
                ...
                {
                    string5[a]='(';
                    sprintf(buffer3,"%s%sKick%s %s %s %s %s)",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),string4,GetCatalogStr(catalog,167,"was kicked by"),string7,string5);
                }
                else
                {
                    sprintf(buffer3,"%s%sKick%s %s %s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),string4,GetCatalogStr(catalog,167,"was kicked by"),string7);
    			...
    			if(status_conductor->conductor)
    			{
        			...
        			{
        				sprintf(buffer3,"%s%sKick%s %s %s %s)",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,168,"You were kicked by"),string7,string5);
                    ...
                    if( (my_settings.events[KICK].use_when == 1 && is_window_active()) || (my_settings.events[KICK].use_when == 2 && !is_window_active()) || my_settings.events[KICK].use_when == 3 )
                    {

            			if(string5)
                        {
                            string5[a]='(';
                            sprintf(buffer3,"%sKick%s %s %s %s %s)",GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),string4,GetCatalogStr(catalog,167,"was kicked by"),string7,string5);
                        }
                        else
                            sprintf(buffer3,"%sKick%s %s %s %s",GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),string4,GetCatalogStr(catalog,167,"was kicked by"),string7);


            			if( !stricmp((p_in)string4,(p_in)status_conductor->nick) )
            				sprintf(buffer3,"%sKick%s %s %s %s)",GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,168,"You were kicked by"),string7,string5);


                        strcpy(target_nick,string7);
                        create_arexx_event_string(my_settings.events[KICK].arexx_script, my_settings.events[KICK].arexx_script2);
                        Execute(event_string,0,0);
                    }
		...
		else if(!strcmp(incoming_2,"NOTICE"))
		{
			...
			while(incoming_3[a] != ' ')
			{ string9[b]=incoming_3[a]; b++; a++; }	string9[b]='\0'; a=a+2; b=0;
            ...
            if(!ignore_privmsg)
            {
    			...
    			if( strstr(string10,"\001PING") && string10[strlen(string10)-1] == '\001' )
    			{
        			...
        			if(a==1) sprintf(buffer3,"%s%sPing%s PING Reply %s %s, %i second",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,210,"from"),string7,a);
                    else sprintf(buffer3,"%s%sPing%s PING Reply %s %s, %i seconds",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,210,"from"),string7,a);
                    colour=6;



                }
    			else if((strstr(string10,"\001VERSION") || strstr(string10,"\001version")) && string10[strlen(string10)-1] == '\001' )
    			{
    				strtok(string10," ");
    				string5=strtok(NULL,"");
                    ...
                    if(string5)
                    {
                        ...
                        sprintf(buffer3,"%s%sVersion%s %s VERSION Reply: %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),string7,string5);
                        colour=6;
                    }

    			}
    			else if((strstr(string10,"\001TIME") || strstr(string10,"\001time")) && string10[strlen(string10)-1] == '\001' )
    			{
    				strtok(string10," ");
    				string5=strtok(NULL,"");
                    ...
                    {
        				string5[strlen(string5)-1]='\0';
                        sprintf(buffer3,"%s%sTime%s %s TIME Reply: %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),string7,string5);
                        colour=6;
                    }
    			}
                ...
                if( (my_settings.events[NOTICE].use_when == 1 && is_window_active()) || (my_settings.events[NOTICE].use_when == 2 && !is_window_active()) || my_settings.events[NOTICE].use_when == 3 )
                {
                    strcpy(buffer3,buffer3+strlen(timestamp));
                    strcpy(target_nick,string7);
                    create_arexx_event_string(my_settings.events[NOTICE].arexx_script, my_settings.events[NOTICE].arexx_script2);
                    Execute(event_string,0,0);
                }
		...
		else if( !strcmp(incoming_2,"MODE") )
		{
            ...
            if( (my_settings.events[MODE_CHANGE].use_when == 1 && is_window_active()) || (my_settings.events[MODE_CHANGE].use_when == 2 && !is_window_active()) || my_settings.events[MODE_CHANGE].use_when == 3 )
            {

                if(string5)
        			sprintf(buffer3,"%sMode%s%s %s: %s %s",GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),string1, GetCatalogStr(catalog,211,"sets mode"),string4, string5);
    			else
                    sprintf(buffer3,"%sMode%s%s %s: %s",GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),string1, GetCatalogStr(catalog,211,"sets mode"),string4);

                strcpy(target_nick,string1);
                create_arexx_event_string(my_settings.events[MODE_CHANGE].arexx_script, my_settings.events[MODE_CHANGE].arexx_script2);
                Execute(event_string,0,0);
            }


            if(string5)
    			sprintf(buffer3,"%s%sMode%s%s %s: %s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),string1, GetCatalogStr(catalog,211,"sets mode"),string4, string5);
			else
                sprintf(buffer3,"%s%sMode%s%s %s: %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),string1, GetCatalogStr(catalog,211,"sets mode"),string4);
            ...
            //if(string1 && string2 && string3 && string4)
            {
                ...
                if(status_conductor->conductor) // && string5)
                {
        			while(string4[a] != '\0')
        			{
        				...
        				{
        					if(string4[a] == '+') a++;
        					if(string4[a+1] != '-') option=1; else option=0;

                            for(int count=1; count<=status_conductor->max_modes; count++)
                            {
                                if(string4[a] == status_conductor->user_modes[count].mode[0])
                                    {   ReplaceNicksWithVoiceOps(work, string4[a],1 ); break; }
                            }
        				...
        				{
        					if(string4[a] == '-') a++;
        					if(string4[a+1] != '-') option=0; else option=1;

                            for(int count=1; count<=status_conductor->max_modes; count++)
                            {
                                if(string4[a] == status_conductor->user_modes[count].mode[0])
                                {    ReplaceNicksWithVoiceOps(work, string4[a],0 ); break; }
                            }
		...
		else if( !strcmp(incoming_2,"PART") )
		{
            ...
            if(string4)
            {
                ...
                {
                    string4[0]='(';
                    sprintf(buffer3,"%s%s%s%s %s (%s) %s)",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,165,"Part"),GetCatalogStr(catalog,218,"]"),string7,string8,string4);
                }
                else
                sprintf(buffer3,"%s%s%s%s %s (%s) (%s)",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,165,"Part"),GetCatalogStr(catalog,218,"]"),string7,string8,string4);
            }
            else sprintf(buffer3,"%s%s%s%s %s (%s)",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,165,"Part"),GetCatalogStr(catalog,218,"]"),string7,string8);
            ...
            if(string3)
            {
        		...
        		if(status_conductor->conductor && string7 != status_conductor->nick)
        		{
                    ...
                    if( (my_settings.events[PART].use_when == 1 && is_window_active()) || (my_settings.events[PART].use_when == 2 && !is_window_active()) || my_settings.events[PART].use_when == 3 )
                    {
                        strcpy(target_nick,string7);
                        create_arexx_event_string(my_settings.events[PART].arexx_script,my_settings.events[PART].arexx_script2);
                        Execute(event_string,0,0);
                    }
		...
		else if( !strcmp(incoming_2,"QUIT") )
		{
			...
			while(incoming_1[a] != '\0')
			{ string8[b]=incoming_1[a]; a++; b++; }	string8[b]='\0'; a=0;

			while(incoming_3[a] != '\0')
			{ string9[a]=incoming_3[a+1]; a++; } string9[a]='\0';


			status_conductor->conductor=status_conductor->root;

			sprintf(buffer3,"%s%s%s%s %s (%s) (%s)",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,166,"Quit"),GetCatalogStr(catalog,218,"]"),string7,string8,string9);
            if( (my_settings.events[QUIT].use_when == 1 && is_window_active()) || (my_settings.events[QUIT].use_when == 2 && !is_window_active()) || my_settings.events[QUIT].use_when == 3 )
            {
                strcpy(target_nick,string7);
                create_arexx_event_string(my_settings.events[QUIT].arexx_script,my_settings.events[QUIT].arexx_script2);
                Execute(event_string,0,0);
            }
			...
			while(status_conductor->conductor)
			{
    			//if(string7)
                {
                    ...
                    {
            			sprintf(buffer3,"%s%s%s%s %s %s.. (%s)",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,166,"Quit"),GetCatalogStr(catalog,218,"]"),string7,GetCatalogStr(catalog,298,"has left IRC.."),string9);
                        //RemoveNick(string7);
		...
		else if( !strcmp(incoming_2,"NICK") )
		{
            ...
            {
    			while(incoming_3[a] != '\0')
    			{ string9[a]=incoming_3[a]; a++; } string9[a]='\0';

            }

			sprintf(buffer3,"%s%sNick%s %s %s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),string7,GetCatalogStr(catalog,169,"is now known as"),string9);
			...
			while(status_conductor->conductor)
			{
				...
				if(status_conductor->conductor->LV_nicklist) ChangeNick(string7, string9, buffer3);

                if(status_conductor->conductor->BT_querybutton && !stricmp((p_in)status_conductor->conductor->name,(p_in)string7) )
                {
		...
		else if(!strcmp(incoming_2,"PRIVMSG"))
		{
            ...
            if(status_conductor->conductor) sprintf(buffer3,"%s<%s%s> %s",timestamp,modes,target_nick, string10);
			else if(!status_conductor->conductor && my_settings.open_query_on_privmsg==0) sprintf(buffer3,"%s*%s* %s",timestamp,target_nick, string10);
			else if(!status_conductor->conductor && my_settings.open_query_on_privmsg==1) sprintf(buffer3,"%s<%s%s> %s",timestamp,modes,target_nick, string10);
            ...
            if(!ignore_privmsg)
            {
    			...
    			if(!status_conductor->conductor && my_settings.open_query_on_privmsg==1 && strstr(string10,"\001")==NULL && strcspn(string9,status_conductor->chantypes)!=0)
                {
                    char work[100];
                    strcpy(work,string7);
                    ...
                    if((!string1 && my_settings.play_sound_when_tabopen == 2) || my_settings.play_sound_when_tabopen == 1)
                    {
                        if(!muted_sound)
    			...
    			if( strstr(string10,"\001ACTION") && string10[strlen(string10)-1] == '\001' )
    			{
        			...
        			if(!status_conductor->conductor && my_settings.open_query_on_privmsg==1 && strcspn(string9,status_conductor->chantypes)!=0) //string9[0] != '#')
                        create_new_tab(string7, 0, 2);
    				...
    				string5=strtok(NULL,"");
                    if(!(string5)) { string5=new char[1]; strcpy(string5,""); }

    				string5[strlen(string5)-1]='\0';


    				if(status_conductor->conductor) sprintf(buffer3,"%s* %s %s",timestamp,string7, string5);
    				else sprintf(buffer3,"%s*%s* * %s %s",timestamp,string7, string7, string5);

                    colour=7;


                if( (my_settings.events[CHANNEL_MESSAGE].use_when == 1 && is_window_active()) || (my_settings.events[CHANNEL_MESSAGE].use_when == 2 && !is_window_active()) || my_settings.events[CHANNEL_MESSAGE].use_when == 3 )
                {
                        char backup[800];
                        ...
                        strcpy(target_nick,string7);
                        create_arexx_event_string(my_settings.events[CHANNEL_MESSAGE].arexx_script, my_settings.events[CHANNEL_MESSAGE].arexx_script2);
                        Execute(event_string,0,0);
                ...
                else if( (my_settings.events[CHANNEL_MESSAGE].use_when == 1 && is_window_active()) || (my_settings.events[CHANNEL_MESSAGE].use_when == 2 && !is_window_active()) || my_settings.events[CHANNEL_MESSAGE].use_when == 3 )
                {
            ...
            if(!ignore_ctcp)
            {
    			...
    			if( strstr(string10,"\001SOUND") && string10[strlen(string10)-1] == '\001' )
    			{
    				...
    				sprintf(buffer3,"%s%sSound%s %s %s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),string7, GetCatalogStr(catalog,170,"plays"),string5);
    			...
    			else if( (strstr(string10,"\001VERSION") || strstr(string10,"\001version")) && string10[strlen(string10)-1] == '\001' )
    			{
                    ...
                    if(strcspn(string9,status_conductor->chantypes)==0)
        				...
        				sprintf(buffer3,"%s%sCTCP%s CTCP VERSION %s %s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"), string9, GetCatalogStr(catalog,210,"from"), string7);
                    else
        				sprintf(buffer3,"%s%sCTCP%s CTCP VERSION %s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"), GetCatalogStr(catalog,210,"from"), string7);
    			...
    			else if( (strstr(string10,"\001PING") || strstr(string10,"\001ping")) && string10[strlen(string10)-1] == '\001' )
    			{

                    //if(string9[0]=='#')
                    if(strcspn(string9,status_conductor->chantypes)==0)
        				sprintf(buffer3,"%s%sCTCP%s CTCP PING %s %s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"), string9, GetCatalogStr(catalog,210,"from"), string7);
                    else
        				sprintf(buffer3,"%s%sCTCP%s CTCP PING %s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"), GetCatalogStr(catalog,210,"from"), string7);
    			...
    			else if( (strstr(string10,"\001TIME") || strstr(string10,"\001time")) && string10[strlen(string10)-1] == '\001' )
    			{
                    //if(string9[0]=='#')
                    if(strcspn(string9,status_conductor->chantypes)==0)
        				sprintf(buffer3,"%s%sCTCP%s CTCP TIME %s %s %s",timestamp,GetCatalogStr(catalog,217,"["), GetCatalogStr(catalog,218,"]"),string9, GetCatalogStr(catalog,210,"from"),string7);
                    else
        				sprintf(buffer3,"%s%sCTCP%s CTCP TIME %s %s",timestamp,GetCatalogStr(catalog,217,"["), GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,210,"from"),string7);
                    ...
                    sprintf(sendstuff, "NOTICE %s :\001TIME %d-%s-%d %s:%s\001\r\n",string7,clockdata->mday,month,clockdata->year,timestamp_hrs,timestamp_mins);
                ...
                {
                        string10[0]=' '; string10[strlen(string10)-1]=' ';
        				sprintf(buffer3,"%s%sCTCP%s UNKNOWN CTCP%s%s %s",timestamp,GetCatalogStr(catalog,217,"["), GetCatalogStr(catalog,218,"]"),string10, GetCatalogStr(catalog,210,"from"),string7);
                        colour=6;
                    is_it_a_ctcp=TRUE;
                    channel_message=TRUE;
                ...
                if( (my_settings.events[CTCP_REQUEST].use_when == 1 && is_window_active()) || (my_settings.events[CTCP_REQUEST].use_when == 2 && !is_window_active()) || my_settings.events[CTCP_REQUEST].use_when == 3 )
                {
                    if(is_it_a_ctcp==TRUE)
                    {
                        ...
                        strcpy(buffer3,buffer3+strlen(timestamp));
                        strcpy(target_nick,string7);
                        create_arexx_event_string(my_settings.events[CTCP_REQUEST].arexx_script, my_settings.events[CTCP_REQUEST].arexx_script2);
                        Execute(event_string,0,0);
            ...
            if(!ignore_dcc)
            {
    			...
    			{
                    strtok(string10," "); strtok(NULL, " "); //move strtok position past "DCC RESUME"
                    ...
                    if(string1 && string2 && string3)
                    {
                        ...
                        {
                            if(!dcc_send_conductor->cancelled && !dcc_send_conductor->removed && !stricmp(dcc_send_conductor->filename, string1) )
                            {
                                ...
                                #else
                                if(dcc_send_conductor->dcc_file) Seek(dcc_send_conductor->dcc_file,dcc_send_conductor->total_recv,OFFSET_BEGINNING);
                                #endif
                                sprintf(buffer3,"%s%sDCC%s %s %s >> %s @ %s bytes",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,259,"RESUME file"),dcc_send_conductor->filename,dcc_send_conductor->nick, string3);

                        if( (my_settings.events[DCC_RECV_OFFERED].use_when == 1 && is_window_active()) || (my_settings.events[DCC_RECV_OFFERED].use_when == 2 && !is_window_active()) || my_settings.events[DCC_RECV_OFFERED].use_when == 3 )
                        {
                            strcpy(target_nick,string7);

                            create_arexx_event_string(my_settings.events[DCC_RECV_OFFERED].arexx_script,my_settings.events[DCC_RECV_OFFERED].arexx_script2);
                            Execute(event_string,0,0);
                        }
                ...
                else if( strstr(string10,"\001DCC ACCEPT") && string10[strlen(string10)-1] == '\001' ) //incoming dcc resume
    			{
                    ...
                    if(strchr(string10,'\"'))
                    {
                        strtok(string10," "); strtok(NULL, "\""); //move strtok position past "DCC ACCEPT"
                        work1=strtok(NULL,"\"");
                        for(count=0; count<(LONG)strlen(work1); count++) { if(work1[count]==' ') work1[count]='_'; }
                    ...
                    if(work1)
                    {
                        ...
                        {
                            if(work1[b] != '\"') { work1[a]=work1[b]; a++; b++; }
                            else b++;
                        }
                        ...
                        while(dcc_conductor)
                        {
                            ...
                            if(!dcc_conductor->removed && !stricmp(dcc_conductor->nick, string7) && !stricmp(dcc_conductor->filename, work1) )
                            {
                                connect(dcc_conductor->dcc_socket, (struct sockaddr*) &dcc_conductor->test, sizeof(struct sockaddr));
                                colour=9;
                                sprintf(buffer3,"%s%sDCC%s %s %s %s %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),GetCatalogStr(catalog,171,"Attempting to resume"),dcc_conductor->filename,GetCatalogStr(catalog,210,"from"),dcc_conductor->nick); //,dcc_conductor->address,dcc_conductor->port);
                            }
                ...
                else if( strstr(string10,"\001DCC CHAT") && string10[strlen(string10)-1] == '\001' )
    			{
                        ...
                        if(work1 && work2 && work3)
                        {
                            ...
                            {
                                colour=9;
                                sprintf(buffer3,"%s%sDCC%s%s - %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),string7, GetCatalogStr(catalog,188,"Unable to establish DCC CHAT Connection"));
                            }
                            else
                            {
                                create_recv_dcc_chat(string7,work2,work3);
                                colour=9;
                                sprintf(buffer3,"%s%sDCC%s%s - %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),string7, GetCatalogStr(catalog,172,"Attempting to accept DCC CHAT request"));
                            }
                        }
                ...
                else if( strstr(string10,"\001DCC SEND") && string10[strlen(string10)-1] == '\001' )
    			{
                    ...
                    if(DEBUG) printf("incoming file transfer detected! now chopping up details into individual strings, for sending to our create_recv_dcc() function..\n");

                    if(!strchr(string10,'"'))
                    {
                        strtok(string10," "); strtok(NULL, " "); //move strtok position past "DCC SEND"
                    ...
                    {
                        if(work3) work6=atoi(work3); //port number
                        else work6=2000;

                        create_recv_dcc(string7,work1,work2,work6,work7);

                        sprintf(buffer3,"%s%sDCC%s SEND: %s %s %s port %i",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),work1,GetCatalogStr(catalog,173,"offered from"),string7,work6);
                        colour=9;
            ...
            if(  !(ignore_ctcp && (strstr(string10,"\001")))
              && !(ignore_privmsg && (colour == 8 || colour == 7))
              && !(ignore_dcc && colour == 9))

            {
                ...
                if(status_conductor->conductor)
    			{
                    if(strcspn(string7,status_conductor->chantypes)!=0) update_nicks_hostname(string7, string8);
                    ...
                    if(strcspn(string9,status_conductor->chantypes)!=0 && play_tab_open_sample==FALSE)
                    {
                        ...
                        (my_settings.play_sound_when_newmsg == 1 && (status_conductor->conductor != status_current->current_query)) ||
                        (!string1 && my_settings.play_sound_when_newmsg == 1))
                        {
                            if(!muted_sound && o3) DoMethod (o3,DTM_TRIGGER,NULL,STM_PLAY,NULL);
                    ...
                    if( (colour==7 && search_for_highlight_words(string5,buffer3)) || (search_for_highlight_words(string10,buffer3)) )
                    {
                        channel_message=TRUE;
            			if(status_conductor->conductor)
                        {
                        ...
                        if( (my_settings.events[HIGHLIGHT].use_when == 1 && is_window_active()) || (my_settings.events[HIGHLIGHT].use_when == 2 && !is_window_active()) || my_settings.events[HIGHLIGHT].use_when == 3 ) Execute(event_string,0,0);
                ...
                else
                {
                    ...
                    if(strcspn(string9,status_conductor->chantypes)!=0 && play_tab_open_sample==FALSE)
                    {
                        ...
                        if((!string1 && my_settings.play_sound_when_newmsg == 3) || my_settings.play_sound_when_newmsg == 2 ||
                        (!string1 && my_settings.play_sound_when_newmsg == 1))
                        {
                            //play_external_sound_replayer_newmsg();
                        	if(!muted_sound && o3) DoMethod (o3,DTM_TRIGGER,NULL,STM_PLAY,NULL);
                    ...
                    if(colour==7 && search_for_highlight_words(string5,buffer3))           add_text_to_current_list((char*)buffer3, colour, ACTIVITY_HIGHLIGHT);
    				else if(search_for_highlight_words(string10,buffer3))  				   add_text_to_current_list((char*)buffer3, colour, ACTIVITY_HIGHLIGHT);
                    else                                                                   add_text_to_current_list((char*)buffer3, colour, ACTIVITY_CHAT);
                ...
                if( (my_settings.events[PRIVATE_MESSAGE].use_when == 1 && is_window_active()) || (my_settings.events[PRIVATE_MESSAGE].use_when == 2 && !is_window_active()) || my_settings.events[PRIVATE_MESSAGE].use_when == 3 )
                {
                    ...
                    if(channel_message==FALSE)
                    {
                        ...
                        create_arexx_event_string(my_settings.events[PRIVATE_MESSAGE].arexx_script, my_settings.events[PRIVATE_MESSAGE].arexx_script2);
                        Execute(event_string,0,0);
		...
		else if(!strcmp(incoming_2,"TOPIC")) //changing the topic
		{
			...
			sprintf(buffer3,"%s%sTopic%s %s %s '%s'",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,218,"]"),string7, GetCatalogStr(catalog,174,"changes the topic to"), string10);
        ...
        {                                                               //404 cant send to channel
            ...
            if(string2)
            {
    			sprintf(buffer3,"%s%s%s%s %s",timestamp,GetCatalogStr(catalog,217,"["), GetCatalogStr(catalog,0,"Error"), GetCatalogStr(catalog,218,"]"),string2);
    			add_text_to_current_list((char*)buffer3, 9, ACTIVITY);
            }
                    ...
                    if(status_conductor->conductor->BT_querybutton)
                    {
                        ...
                        if((find=strstr(buffer3,"/QUOTE PASS")))
                        {
                            ...
                            if(string2)
                            {
                    			...
                    			sprintf(buffer3,"%s%s%s%s Automatically sending: %s",timestamp,GetCatalogStr(catalog,217,"["),GetCatalogStr(catalog,359,"Server"),GetCatalogStr(catalog,218,"]"), sendstuff);
