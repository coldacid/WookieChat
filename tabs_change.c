...
void check_column_size()
{
    ...
    if (my_settings.use_column_display == 1)
    {
        ...
        col1 = DoMethod((Object*) status_current->current_query->LV_channel, MUIM_NList_ColWidth, 1,
                MUIV_NList_ColWidth_Get);
        col2 = DoMethod((Object*) status_current->current_query->LV_channel, MUIM_NList_ColWidth, 2,
                MUIV_NList_ColWidth_Get);
            ...
            while ((member_object = NextObject((Object**) &object_state)))
            {
                ...
                if (member_object == status_current->current_query->GR_tabs)
                    DoMethod((Object*) status_current->current_query->GR_nicklist_and_tabs, OM_REMMEMBER,
                            (Object*) status_current->current_query->GR_tabs);
    ...
    if (status_conductor)
    {
        ...
        if (1)
        {
            ...
            if (status_current)
            {
                if (status_current->current_query)
                {
                    success2 = DoMethod((Object*) status_conductor->conductor->GR_nicklist_and_tabs,
            ...
            {
                {
                    ...
                    if (my_settings.use_nlistobject_for_tabs)
                    {
                        ...
                        if (is_it_there == FALSE && !GEIT2)
                        {
                            ...
                            if (GEIT)
                                printf("adding listview nlist object to the group\n");
...
void kill_my_notifys(void)
{
    ...
    if (status_current->current_query->BT_mode_N)
        DoMethod((Object*) status_current->current_query->BT_mode_N, MUIM_KillNotify, MUIA_Selected);
    if (status_current->current_query->BT_mode_T)
        DoMethod((Object*) status_current->current_query->BT_mode_T, MUIM_KillNotify, MUIA_Selected);
    if (status_current->current_query->BT_mode_S)
        DoMethod((Object*) status_current->current_query->BT_mode_S, MUIM_KillNotify, MUIA_Selected);
    if (status_current->current_query->BT_mode_I)
        DoMethod((Object*) status_current->current_query->BT_mode_I, MUIM_KillNotify, MUIA_Selected);
    if (status_current->current_query->BT_mode_P)
        DoMethod((Object*) status_current->current_query->BT_mode_P, MUIM_KillNotify, MUIA_Selected);
    if (status_current->current_query->BT_mode_M)
        DoMethod((Object*) status_current->current_query->BT_mode_M, MUIM_KillNotify, MUIA_Selected);
    if (status_current->current_query->BT_mode_K)
        DoMethod((Object*) status_current->current_query->BT_mode_K, MUIM_KillNotify, MUIA_Selected);
    if (status_current->current_query->STR_keyword)
        DoMethod((Object*) status_current->current_query->STR_keyword, MUIM_KillNotify, MUIA_String_Acknowledge);
    if (status_current->current_query->BT_mode_L)
        DoMethod((Object*) status_current->current_query->BT_mode_L, MUIM_KillNotify, MUIA_Selected);
    if (status_current->current_query->STR_limit)
        DoMethod((Object*) status_current->current_query->STR_limit, MUIM_KillNotify, MUIA_String_Acknowledge);
    ...
    //if(stricmp(my_string1,status_conductor->nick_string)) //if the displayed nick is different to whats in the server group, lets update it
    {
        if (DoMethod((Object*) GR_string_gadget, MUIM_Group_InitChange))
        {
            ...
            if (status_conductor->conductor->your_current_mode[0] == ' '
                    || status_conductor->conductor->your_current_mode[0] == '\0')
            {
            ...
            else
            {
                ...
                {
                    if (status_conductor->conductor->your_current_mode[0] == '@')
                        sprintf(status_conductor->nick_string, " \033I[5:progdir:smilies/nicklist/ops] %s ",
                                status_conductor->nick);
                    else if (status_conductor->conductor->your_current_mode[0] == '%')
                        sprintf(status_conductor->nick_string, " \033I[5:progdir:smilies/nicklist/half_ops] %s ",
                                status_conductor->nick);
                    else if (status_conductor->conductor->your_current_mode[0] == '+')
                        sprintf(status_conductor->nick_string, " \033I[5:progdir:smilies/nicklist/voice] %s ",
                                status_conductor->nick);
                    else
                        sprintf(status_conductor->nick_string, " %c%s ",
                                status_conductor->conductor->your_current_mode[0], status_conductor->nick);

                }
                else
                    sprintf(status_conductor->nick_string, " %c%s ", status_conductor->conductor->your_current_mode[0],
                            status_conductor->nick);
...
void activate_tab_button()
{
    ...
    {
        if (GEIT)
            printf("cant activate tab! (error #1)\n");
        return;
    }
    if (!status_conductor->conductor)
    {
        if (GEIT)
            printf("cant activate tab! (error #2)\n");
        return;
    }

    if (NEWDEBUG)
        printf("activating tab\n");
    ...
    {
        if (status_conductor->conductor->BT_querybutton)
            setmacro((Object*) status_conductor->conductor->BT_querybutton, MUIA_Text_PreParse, "\033c\0332");
        ...
        DoMethod((Object*) status_conductor->conductor->BT_querybutton, MUIM_Notify, MUIA_Selected, TRUE,
                (Object*) WookieChat->App, 2, MUIM_Application_ReturnID, status_conductor->conductor->iv);
...
void setup_notifys(void)
{
    ...
    setmacro((Object*) status_current->current_query->STR_keyword, MUIA_String_Contents,
            status_current->current_query->keyword);
    ...
    setmacro((Object*) status_current->current_query->STR_limit, MUIA_String_Contents,
            status_current->current_query->limit);

    if (status_current->current_query->BT_mode_N)
        DoMethod((Object*) status_current->current_query->BT_mode_N, MUIM_Notify, MUIA_Selected, TRUE,
                (Object*) WookieChat->App, 2, MUIM_Application_ReturnID, 11);
    if (status_current->current_query->BT_mode_N)
        DoMethod((Object*) status_current->current_query->BT_mode_N, MUIM_Notify, MUIA_Selected, FALSE,
                (Object*) WookieChat->App, 2, MUIM_Application_ReturnID, 21);

    if (status_current->current_query->BT_mode_T)
        DoMethod((Object*) status_current->current_query->BT_mode_T, MUIM_Notify, MUIA_Selected, TRUE,
                (Object*) WookieChat->App, 2, MUIM_Application_ReturnID, 10);
    if (status_current->current_query->BT_mode_T)
        DoMethod((Object*) status_current->current_query->BT_mode_T, MUIM_Notify, MUIA_Selected, FALSE,
                (Object*) WookieChat->App, 2, MUIM_Application_ReturnID, 20);

    if (status_current->current_query->BT_mode_S)
        DoMethod((Object*) status_current->current_query->BT_mode_S, MUIM_Notify, MUIA_Selected, TRUE,
                (Object*) WookieChat->App, 2, MUIM_Application_ReturnID, 12);
    if (status_current->current_query->BT_mode_S)
        DoMethod((Object*) status_current->current_query->BT_mode_S, MUIM_Notify, MUIA_Selected, FALSE,
                (Object*) WookieChat->App, 2, MUIM_Application_ReturnID, 22);

    if (status_current->current_query->BT_mode_I)
        DoMethod((Object*) status_current->current_query->BT_mode_I, MUIM_Notify, MUIA_Selected, TRUE,
                (Object*) WookieChat->App, 2, MUIM_Application_ReturnID, 13);
    if (status_current->current_query->BT_mode_I)
        DoMethod((Object*) status_current->current_query->BT_mode_I, MUIM_Notify, MUIA_Selected, FALSE,
                (Object*) WookieChat->App, 2, MUIM_Application_ReturnID, 23);

    if (status_current->current_query->BT_mode_P)
        DoMethod((Object*) status_current->current_query->BT_mode_P, MUIM_Notify, MUIA_Selected, TRUE,
                (Object*) WookieChat->App, 2, MUIM_Application_ReturnID, 14);
    if (status_current->current_query->BT_mode_P)
        DoMethod((Object*) status_current->current_query->BT_mode_P, MUIM_Notify, MUIA_Selected, FALSE,
                (Object*) WookieChat->App, 2, MUIM_Application_ReturnID, 24);

    if (status_current->current_query->BT_mode_M)
        DoMethod((Object*) status_current->current_query->BT_mode_M, MUIM_Notify, MUIA_Selected, TRUE,
                (Object*) WookieChat->App, 2, MUIM_Application_ReturnID, 15);
    if (status_current->current_query->BT_mode_M)
        DoMethod((Object*) status_current->current_query->BT_mode_M, MUIM_Notify, MUIA_Selected, FALSE,
                (Object*) WookieChat->App, 2, MUIM_Application_ReturnID, 25);

    if (status_current->current_query->BT_mode_K)
        DoMethod((Object*) status_current->current_query->BT_mode_K, MUIM_Notify, MUIA_Selected, TRUE,
                (Object*) WookieChat->App, 2, MUIM_Application_ReturnID, 17);
    if (status_current->current_query->BT_mode_K)
        DoMethod((Object*) status_current->current_query->BT_mode_K, MUIM_Notify, MUIA_Selected, FALSE,
                (Object*) WookieChat->App, 2, MUIM_Application_ReturnID, 28);

    if (status_current->current_query->STR_keyword)
        DoMethod((Object*) status_current->current_query->STR_keyword, MUIM_Notify, MUIA_String_Acknowledge,
                MUIV_EveryTime, (Object*) WookieChat->App, 2, MUIM_Application_ReturnID, 17);

    if (status_current->current_query->BT_mode_L)
        DoMethod((Object*) status_current->current_query->BT_mode_L, MUIM_Notify, MUIA_Selected, TRUE,
                (Object*) WookieChat->App, 2, MUIM_Application_ReturnID, 16);
    if (status_current->current_query->BT_mode_L)
        DoMethod((Object*) status_current->current_query->BT_mode_L, MUIM_Notify, MUIA_Selected, FALSE,
                (Object*) WookieChat->App, 2, MUIM_Application_ReturnID, 27);

    if (status_current->current_query->STR_limit)
        DoMethod((Object*) status_current->current_query->STR_limit, MUIM_Notify, MUIA_String_Acknowledge,
                MUIV_EveryTime, (Object*) WookieChat->App, 2, MUIM_Application_ReturnID, 16);
...
void remove_tab_listview2()
{
    ...
    if (!status_conductor->conductor)
        return;
    if (status_conductor->conductor->removed == 1)
        return;
        ...
        if (member_object == status_conductor->conductor->GR_tabs) // && member_object != status_current->current_query->GR_tabs)
        {
            ...
            //if(DoMethod((Object*)status_conductor->conductor->GR_listviews,MUIM_Group_InitChange))
            {
                //if(DoMethod((Object*)status_conductor->conductor->GR_nicklist_and_tabs,MUIM_Group_InitChange))
                {
                    ...
                    if (GEIT)
                        printf("remove2 3-5 \n");
                    DoMethod((Object*) status_conductor->conductor->GR_nicklist_and_tabs, OM_REMMEMBER,
                            (Object*) status_conductor->conductor->GR_tabs);
    ...
    if (DoMethod((Object*) GROUP_ROOT_0, MUIM_Group_InitChange))
    {
        ...
        if (DoMethod((Object*) GR_top, MUIM_Group_InitChange))
        {
            ...
            while (running1)
            {
                ...
                b++;
                if (running1 && status_conductor->conductor->next)
                    status_conductor->conductor = status_conductor->conductor->next;
                else
                    running1 = 0;
        ...
        DoMethod((Object*) status_current->current_query->LV_nicklist, MUIM_NList_ReplaceSingle, work_list_entry, 0,
                NOWRAP, ALIGN_LEFT);
...
int switch_between_tabs_with_keys(int result)
{
    ...
    if (DEBUG)
        printf("switching with keys to tab: %i\n", result);
    //kill_my_notifys();
...
int switch_between_tabs_with_nlisttabclick(LONG result, BOOL saving_prefs)
{
    ...
    for (count = 0, status_conductor = status_root; status_conductor; status_conductor = status_conductor->next)
    {
        ...
        for (status_conductor->conductor = status_conductor->root; status_conductor->conductor;
                status_conductor->conductor = status_conductor->conductor->next)
        {
            if (result == status_conductor->conductor->nlist_tab_number)
            { /*if(NEWDEBUG) printf("found %d\n",result);*/
                break;
            }
    ...
    if (DoMethod((Object*) GROUP_ROOT_0, MUIM_Group_InitChange))
    {
        ...
        if (DoMethod((Object*) GR_top, MUIM_Group_InitChange))
        {
            ...
            for (count = 0, status_conductor = status_root; status_conductor; status_conductor = status_conductor->next)
            {
                ...
                {
                    if (result == status_conductor->conductor->nlist_tab_number
                            && status_conductor->conductor->BT_querybutton)
                    {
                        activate_tab_button();
                        kill_my_notifys();
                        setup_notifys();
