...
void change_window_titlebar_text(void)
{
    ...
    if(status_conductor->conductor==status_current->current_query)
    {
        if(status_conductor->conductor->nicks==0)
            sprintf(windowtitlestring,"WookieChat - %s %s %s - %s", status_conductor->nick,GetCatalogStr(catalog,177,"connected to"),status_conductor->servername,status_conductor->conductor->name);
        else
            sprintf(windowtitlestring,"WookieChat - %s %s %s - %s [%i %s]", status_conductor->nick,GetCatalogStr(catalog,177,"connected to"),status_conductor->servername,status_conductor->conductor->name, status_conductor->conductor->nicks,GetCatalogStr(catalog,257,"users"));
...
void DisplayNicks(void)
{
    ...
    if(!status_conductor) return;
    if(!status_conductor->conductor) return;
    if(!status_conductor->conductor->LV_nicklist) return;
    ...
    while(a<status_conductor->conductor->nicks)
    {
        ...
        if(new_entry->name)
        {
            if(new_entry->modes[0] == '@') status_conductor->conductor->nicks_ops++;
        }
...
void AddNick(char *nick, char *hostname)
{
    ...
    while(a < entries && b != 1)
    {
        ...
        {
	        if(work_entry->modes[0] == status_conductor->user_modes[count].symbol[0]) { result=1; break; }

        }

        if(result==1) a++; else b=1;
    ...
    while(a != entries)
    {
        ...
        if(work_entry && work_entry3) { if(stricmp((p_in)work_entry3->name,(p_in)work_entry->name) < 0) break; } else break;
        a++;
    }

    DoMethod((Object*)status_conductor->conductor->LV_nicklist,MUIM_NList_InsertSingle,work_entry3,a,WRAPCOL0,ALIGN_LEFT);

    change_window_titlebar_text();

    if(work_entry3->hostname) delete [] work_entry3->hostname; work_entry3->hostname=NULL;
    delete work_entry3; work_entry3=NULL;
            ...
            if(option==1)
            {
                for(int count=1; count<=status_conductor->max_modes; count++)
                {
                    ...
                    //printf("2giving status:%s. %s\n",status_conductor->nick,string1);
                        ...
                        {
                            status_conductor->conductor->your_current_mode[0]=status_conductor->user_modes[count].symbol[0];
                            status_conductor->conductor->your_current_mode[1]='\0';
                            update_your_nickname_text_label();
                        }
    ...
    for(a=0; a<entries; a++)
    {
        ...
        if(string1 && status_conductor->conductor->nicklist[a].name)
        {
            if(!stricmp(string1,status_conductor->conductor->nicklist[a].name))
            {
                ...
                if(status_conductor->conductor->nicklist[a].modes[0]=='@') status_conductor->conductor->nicks_ops--;

                while(a+1 < entries)
                {
                    strcpy(status_conductor->conductor->nicklist[a].name, status_conductor->conductor->nicklist[a+1].name);
                    strcpy(status_conductor->conductor->nicklist[a].modes, status_conductor->conductor->nicklist[a+1].modes);

                    if(status_conductor->conductor->nicklist[a].hostname && status_conductor->conductor->nicklist[a+1].hostname)
                    {
                        strncpy(status_conductor->conductor->nicklist[a].hostname, status_conductor->conductor->nicklist[a+1].hostname,HOSTNAME_STRING_SIZE-1);
                ...
                strcpy(status_conductor->conductor->nicklist[a].modes," ");
                if(status_conductor->conductor->nicklist[a].hostname) strcpy(status_conductor->conductor->nicklist[a].hostname,"");
