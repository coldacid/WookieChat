## Version $VER: WookieChat.catalog 3.0 (05.06.2013)
## Languages english deutsch español français greek italiano magyar norsk polski svenska
## Codeset english 0
## Codeset deutsch 0
## Codeset español 0
## Codeset français 0
## Codeset greek 10
## Codeset italiano 0
## Codeset magyar 0
## Codeset norsk 0
## Codeset polski 5
## Codeset svenska 0
## SimpleCatConfig CharsPerLine 200
## autonum 1
;
MSG_ERROR (1//)
Error
Fehler
ERROR
Erreur
ÓöÜëìá
Errore
Hiba
Feil
B³±d
Fel
;
MSG_BANMASK
Ban mask
Bann Maske
Máscara de Prohibir
Masque de bannissement
ÌÜóêá Ban
Maschera Ban
Ban maszk
Banmaske
Maska blokady
Ban mask
;
MSG_BANSETBY
Set by
erstellt von
Establecido por
Positionné par
Ïñéóèİí áğü
Eseguito da
Beállító
Satt av
Ustawiona przez
Satt av
;
MSG_BANDATEPLACED
Date placed
Datum gesetzt
Fecha
Date
Ïñéóèİí çìåñ/íßá
Data stabilita
Dátum
Dato satt
Data umieszczenia
Ban datum
;
MSG_MODE_X
Close the current tab.
Diesen Tabulator Schliessen.
Cerrar la pestaña actual.
Fermer l'onglet courant.
Êëåßóéìï ôçò ôùñéíŞò êáñôİëáò.
Chiudi questo Tab.
Jelenlegi panelt bezár.
Steng denne taben.
Zamyka aktualn± zak³adkê.
Stäng flik.
;
MSG_TOPIC
This is the channels current topic.
Das ist das Grundthema.
La temática actual del canal.
Sujet courant du canal.
Ôï èİìá ôïõ êáíáëéïı.
Questo Ã¨ l'oggetto del canale.
A csatornák jelenlegi témája.
Dette er kanalens overskrift.
To jest temat aktualnego kana³u.
Detta är kanalens nuvarande ämne (topic).
;
MSG_MODE_T
Topic protection. This setting prevents regular users from changing the channel topic.
Themenschutz.\nSie müssen Operator sein, um das Thema ändern zu können.
Proteger temática. Evita a usuarios regulares\nque cambien la temática del canal.
Mode « Protection du sujet ». Dans ce mode les utilisateurs normaux ne sont pas autorisés à changer le sujet du canal.
Ğñïöıëáîç èİìáôïò. ÁõôŞ ç åğéëïãŞ áğáãïñåıåé óôïõò áğëïıò ÷ñŞóôåò íá ôï áëëÜîïõí.
Protezione del titolo, Questa impostazione impedisce agli utenti normali di cambiare il titolo del canale.
Téma rögzítése. Ez a beállítás az általános felhasználók számára megakadályozza a témaváltoztatást.
Overskriftsbeskyttelse. Denne forhindrer ordinære brukere fra å forandre kanaloverskriften.
Zabezpieczenie tematu. To ustawienie zapobiega zmianom tematu przez normalnych u¿ytkowników.
Topic protection. Hindrar vanliga användare från att ändra kanalens ämne.
;
MSG_MODE_N
No messaging. This setting forces users to be in the channel to be able to talk in it.
Keine Nachrichten.\nNachrichten von Clients ausserhalb des Kanals verbieten.
No mensajes. Obliga a los usuarios del\ncanal a permitir hablar en el mismo.
Mode « Pas de message ». Dans ce mode les utilisateurs extérieurs ne peuvent pas envoyer de messages sur le canal.
×ùñßò ìçíıìáôá. ÁõôŞ ç åğéëïãŞ áíáãêÜæåé ôïõò ÷ñŞóôåò íá ğáñåõñßóêïíôáé óôï êáíÜëé ãéá íá ìéëïıí óå áõôü.
Niente messaggi. Questa impostazione permette agli utenti nel canale di chattare.
Nincs üzenõ. Csak a csatornában lévõ felhasználók szólhatnak bele.
Ingen beskjeder. Dette tvinger brukerne til å være i kanalen for å kunne snakke i den.
Przesy³anie wiadomo¶ci zabronione. To ustawienie wymusza na u¿ytkownikach obecno¶æ na kanale aby móc na nim rozmawiaæ.
No messaging. Kräver att användaren finns i kanalen för att kunna skriva till den.
;
MSG_MODE_S
Secret mode. If this is set, the channel is invisible until you have joined.
Geheim-Modus.\nWenn gesetzt, ist der Kanal bis zum Eintreten unsichtbar.
Modo secreto. Hace al canal "invisible"\nhasta que accede a él.
Mode « Secret ». Dans ce mode, le canal est invisible jusqu'à ce que vous l'ayez rejoint.
Êñõöü. Áí ïñéóôåß áõôŞ ç åğéëïãŞ, ôï êáíÜëé åßíáé êñõöü ìİ÷ñé íá ìğåé êÜğïéïò ìİóá.
ModalitÃ  segreta. Se abilitata, il canale rimarrÃ  invisibile fino a che non vi si entri.
Titkosított. Beállításával a csatorna láthatatlan amíg nem csatlakozol.
Hemmelig modus. Hvis denne er satt er kanalen usynlig inntil du har entret den.
Tajny. Je¶li ta opcja jest w³±czona, kana³ jest niewidoczny, dopóki u¿ytkownik siê do niego nie do³±czy.
Secret mode. Om satt, är kanalen osynlig tills du gått in i den.
;
MSG_MODE_I
Invite only. Only users who have been invited to the channel may join.
Nur für eingeladene Benutzer.
Sólo invitados. Permite acceder unicamente\na usuarios que han sido invitados.
Mode « Invitations seules ». Dans ce mode, seuls les utilisateurs y ayant été invités peuvent rejoindre le canal.
Ìİóù ğñüóêëçóçò. Ìğïñïıí íá ìğïõí ìüíï ïé ğñïóêåêëçìİíïé ÷ñŞóôåò.
Solo invito. Disponibile ai soli utenti invitati nel canale.
Csak meghívóval. Csatlakozni csak a meghívottak részére lehetséges.
Kun invitasjon. Kun brukere som er invitert til kanalen kan entre den.
Dostêpny tylko za zaproszeniem. Tylko u¿ytkownicy, którzy zostan± zaproszeni bêd± mogli siê do³±czyæ.
Invite only. Bara inbjudna användare får komma in.
;
MSG_MODE_P
Private mode. If set, channel topic is not shown on global channel lists.
Privat-Modus.\nWenn gesetzt, wird das Thema dieses Kanals nicht in der allgemeinen Kanalliste angezeigt.
Modo privado. La temática del canal no se\nmuestra en la lista global de canales.
Mode « Privé ». Dans ce mode, le sujet du canal n'apparaît pas dans les listes de canaux globales.
Ğñïóùğéêü. ÅÜí İ÷åé ïñéóèåß, ôï èİìá ôïõ êáíáëéïı äåí åìöáíßæåôáé óôéò ãåíéêİò ëßóôåò ôùí êáíáëéşí.
ModalitÃ Â privata. Se abilitata, il titolo del canale Ã¨ invisibile nell' elenco globale dei canali.
Privát. Beállításával a csatorna témája nem jeleneik meg a globális csatorna listában.
Privat modus. Hvis denne er satt vises ikke kanalen i globale kanallister.
Prywatny. Je¶li ta opcja jest w³±czona, temat kana³u nie zostanie wy¶wietlony na globalnej li¶cie kana³ów.
Private mode. Om satt, kommer kanalens ämne inte att visas i globala listningar.
;
MSG_MODE_M
Moderated mode. Only users with operator, halfop or voice status (thats @ % or +) may talk in channel when set.
Moderierter Kanal.\nNur Nutzer mit Operator, Halfop oder Voice Status (das sind @ % oder +) dürfen in diesem Kanal sprechen, wenn gesetzt.
Modo moderado. Pueden hablar solamente\noperadores, medio-operadores o con voz\n(op[@], halfop[%%], voice[+]).
Mode « Modéré ». Dans ce mode, seuls les utilisateurs avec le statut d'opérateur, demi-opérateur ou voice (i.e. @ % ou +) peuvent parler.
ÓõíôçñçôéêŞ êáôÜóôáóç. Ìüíï ïé äéá÷åéñéóôİò, çìéäéá÷åéñéóôİò êáé ïé ÷ñŞóôåò ğïõ İ÷ïõí ïìéëßá (ïé @ % Ş +) ìğïñïıí íá ìéëŞóïõí óå áõôü ôï êáíÜëé.
ModalitÃ Â operatore. Solo utenti con stato di operatore, semioperatore o voce (cioÃ¨ @ % o +) potranno parlare.
Moderált. Csak az operator, halfop vagy voice (vagyis @ % vagy +) státuszban lehet beszélni a csatornában ennek bekapcsolásával.
Moderert modus. Bare brukere som er operatør, halvoperatør eller som har voice-status (de med @ % eller +) kan prate i kanalen.
Tryb moderowany. Po w³±czeniu tej opcji, tylko u¿ytkownicy ze statusem operatora, pó³operatora lub g³osem (@ % lub +) mog± rozmawiaæ na kanale.
Moderated mode. Bara användare med operator, halfop eller voice (dvs. @ % or +) får tala i kanalen.
;
MSG_MODE_B
Click to view a list of all banmasks for this channel.
Klicke um eine komplette Liste aller Bannmasken für diesen Kanal zu sehen.
Pulse para ver la lista de todas\nlas prohibiciones en este canal.
Cliquez pour voir une liste de tous les masques de ban de ce canal.
ĞáôŞóôå åäş ãéá íá äåßôå üëåò ôéò ban ìÜóêåò ãéá ôï êáíÜëé.
Clicca per vedere la lista completa delle maschere di Ban per questo canale.
Kattints ide a csatornához tartozó teljes banmaszk listáért.
Klikk for å vise en liste over alle banmasker for denne kanalen.
Naci¶nij, aby zobaczyæ listê wszystkich masek blokad dla tego kana³u.
Klicka för att visa en lista över kanalens ban villkor.
;
MSG_MODE_K
Keyword mode. When this is set, users need to supply a keyword (password) to enter.
Passwort-Modus, wurde er gesetzt, benötigen Nutzer ein Passwort um einzutreten.
Modo palabra clave. El usuario\ndebe indicar una palabra clave\n(contraseña) para entrar.
Mode « Clef ». Dans ce mode, les utilisateurs doivent fournir une clef (mot de passe) pour entrer.
Ìå êùäéêü. ÅÜí İ÷åé ïñéóèåß, ïé ÷ñŞóôåò ğñİğåé íá åéóÜãïõí ìéá ëİîç êëåéäß (êùäéêü) ãéá íá åéóİëèïõí.
ModalitÃ  parola chiave. Se impostata, gli utenti potranno accedere solo tramite password.
Kulcsszóval. Beállításával a felhasználól egy kulcsszó (jelszó) megadására kötelezettek.
Passordmodus. Når denne er satt, må brukere gi et passord for å entre kanalen.
Tryb z has³em. Je¶li ta opcja jest w³±czona, u¿ytkownicy bêd± musieli podaæ has³o, aby móc siê do³±czyæ.
Keyword mode. Om satt, behöver användare ge ett lösenord för att komma in.
;
MSG_MODE_K_STRING
Current channel keyword.
gegenwärtiges Kanal Kennwort.
Contraseña actual del canal.
Clef actuelle.
Ëİîç êëåéäß ôïõ êáíáëéïı.
Attuale parola chiave del canale.
Jelenlegi csatorna kulcsszava.
Aktiv kanals passord..
Aktualne has³o kana³u.
Gällande lösenord.
;
MSG_MODE_L
Enforce a limit to how many users can currently join the channel.
Limitiert die Anzahl der Nutzer für diesen Kanal.
Forzar el límite de usuarios\nposibles a acceder al canal.
Mode « Limite ». Dans ce mode, une limite au nombre d'utilisateurs pouvant actuellement rejoindre le canal est positionnée.
ÁëëáãŞ ôïõ ïñßïõ ôùí ÷ñçóôşí ğïõ ìğïñïıí íá åéóİëèïõí óôï êáíÜëé.
Limita il numero di utenti abilitati ad accedere al canale.
Korlátozás bevezetése a csatornához jelenleg csatlakoztatható létszámban.
Setter en grense på hvor mange brukere som kan entre kanalen.
Ustala limit maksymalnie po³±czonych u¿ytkowników.
Sätt en gräns för hur många användare som kan komma in i kanalen.
;
MSG_MODE_L_STRING
Current user limit.
Gegenwärtiges Benutzerlimit.
Límite actual de usuarios.
Limite d'utilisateurs actuelle.
¼ñéï ÷ñçóôşí.
Limite utenti.
Létszámkorlát.
Aktuell grense på brukere.
Obecny limit u¿ytkowników.
Nuvarande max användarantal.
;
MSG_HOSTNAME_STR
This is the hostname of the user you are currently talking to.
Das ist der Host-Name des Nutzers mit dem gerade gechattet wird.
Nombre del anfitrión del usuario\ncon el que está hablando.
C'est le nom de machine de l'utilisateur avec qui vous discutez actuellement.
Åßíáé ôï üíïìá host ôïõ ÷ñŞóôç ìå ôïí ïğïßï ìéëÜôå.
Questo Ã¨ l'hostname dell'utente con cui stai parlando.
Beszélgetõ partner host neve.
Dette er hostnavnet på brukeren som du snakker med.
Jest to nazwa wêz³a u¿ytkownika, z którym aktualnie rozmawiasz.
Detta är värdnamnet till den du pratar med just nu.
;
MSG_ERROR_NLIST
Please download and install the NList MUI Class from aminet\n
Bitte die MUI Klasse 'nlist.mcc'aus dem Aminet laden und installieren\n
Descargue e instale la clase NList de MUI de AMINET.\n
Veuillez télécharger et installer la classe MUI NList depuis Aminet\n
ÊáôåâÜóôå êáé åãêáôáóôŞóôå ôçí NList MUI Class áğü ôï aminet\n
Per favore scarica ed installa la classe MUI NList da Aminet\n
Kérlek töltsd le és telepítsd az NList MUI osztályt az aminetrõl\n
Vennligst last ned og installer NList MUI klassen fra aminet\n
Proszê pobraæ i zainstalowaæ klasê MUI NList z Aminetu\n
Ladda ned och installera MUI-klassen NList från aminet\n
;
MSG_GENERAL
General
Allgemein
General
Général
ÃåíéêÜ
Generale
Általános
Generell
G³ówne
Generellt
;
MSG_LOGGING
Logging
Mitschnitt
Registrar
Logging
ÊáôáãñáöŞ
Protocollo
Naplózás
Logging
Raportowanie
Logga
;
MSG_SOUNDS
Sounds
Klänge
Sonidos
Sons
¹÷ïé
Suoni
Hangok
Lyder
D¼wiêki
Ljud
;
MSG_ALIASES
Aliases
Aliases
Aliases
Alias
Óõíşíõìá
Alias (sinonimi)
Helyettesítõk
Aliaser
Aliasy
Alias
;
MSG_ABOUT
About...
Über ...
Acerca de...
À propos...
Ğİñé ôïõ...
Informazioni...
Névjegy...
Om...
O programie...
Om ...
;
MSG_YES
Yes
Ja
Sí
Oui
Íáé
Si
Igen
Ja
Tak
Ja
;
MSG_NO
No
NEIN
No
Non
¼÷é
No
Nem
Nei
Nie
Nej
;
MSG_DCC_CHOOSE_FILENAME
File To Send
Datei zum Senden
Elija fichero a enviar
Fichier à envoyer
Áñ÷åßï ãéá áğïóôïëŞ
File da inviare
Küldendõ fájl
Fil å sende
Plik do wys³ania
Fil att skicka
;
MSG_BUTTON_SEND_FILE
Send File
Sende Datei
Enviar fichero
Envoyer le fichier
ÁğïóôïëŞ áñ÷åßïõ
Invia file
Fájl küldése
Send fil
Wy¶lij plik
Skicka fil
;
MSG_DCC_SEND_WINDOW
DCC SEND File
DCC SEND File
DCC SEND
Envoi de fichier par DCC SEND
DCC SEND áñ÷åßïõ
Invia file tramite DCC
DCC SEND fájlküldés
DCC SEND fil
DCC SEND
DCC SEND Fil
;
MSG_REOFFER
Reoffer File
Biete die Datei erneut an
Reintentar
Renvoyer le fichier
ÅğáíáğñïóöïñÜ áñ÷åßïõ
Rioffri il file
Fájlt újra ajánl
Send på nytt
Zaoferuj plik ponownie
Erbjud fil igen
;
MSG_CANCEL_SEND
Cancel Send
Senden abbrechen
Cancelar envío
Annuler l'envoi
Áêıñùóç áğïóôïëŞò
Interrompi l'invio
Küldés megszakítása
Avbryt sending
Poniechaj wysy³ania
Avbryt sändning
;
MSG_REMOVE
Remove
Entfernen
Eliminar
Supprimer
Áöáßñåóç
Rimuovi
Eltávolítás
Fjern
Usuñ
Ta bort
;
MSG_RESUME
Resume
Anfügen
Reanudar
Reprendre
Óõíİ÷éóç
Continua
Folytatás
Fortsett
Wznów
Återuppta
;
MSG_OVERWRITE
Overwrite
Überschreiben
Sobreescribir
Écraser
ÅğáíåããñáöŞ
Sovrascrivi
Felülírás
Skriv over
Nadpisz
Skriv över
;
MSG_ABORT
Abort
Abbrechen
Cancelar
Annuler
Ìáôáßùóç
Interrompi
Megszakítás
Stopp!
Przerwij
Avbryt
;
MSG_ALREADY_EXISTS_WINDOW_TITLE
Incoming File already exists
Ankommende Datei existiert bereits
WookieChat: Aviso
Fichier en réception déjà existant
Ôï åéóåñ÷üìåíï áñ÷åßï õğÜñ÷åé Şäç
File in ricezione giÃ Â esistente
Beérkezõ fájl már létezik
Innkommene fil eksisterer allerede
Przychodz±cy plik ju¿ istnieje
Inkommande fil finns redan
;
MSG_CHOOSE_DOWNLOAD_PATH
Choose a path to save your downloads to.
Wähle einen Pfad um Deine Downloads zu speichern.
Senda para las descargas.
Choisissez un chemin pour enregistrer vos téléchargements.
Åğéëİîôå ôïí êáôÜëïãï üğïõ èá áğïèçêåıïíôáé üóá êáôåâÜæåôå.
Scegli il percorso dove salvare i file ricevuti.
Válaszd ki a letöltések helyét.
Velg stinavn for å lagre dine filer til.
Wybierz ¶cie¿kê dostêpu zapisywania pobieranych plików.
Välj sökväg att spara fil på.
;
MSG_ACCEPT
Accept
Akzeptieren
Aceptar
Accepter les modifications
Áğïäï÷Ş
Accetta
Elfogad
Godta
Akceptuj
Acceptera
;
MSG_CANCEL_TRANSFER
Cancel
Abbrechen
Cancelar
Annuler
Áêıñùóç
Annulla
Mégsem
Avbryt
Poniechaj
Avbryt
;
MSG_PREFS_NICK_COMPLETION_STYLE
Nick Completion Style
Nicknamens Vervollständigung wie :
Estilo en los aliases:
Style de complétion de pseudo
Óôõë óõìğëŞñùóçò øåõäşíõìïõ
Stile di completamento del Nickname
Nick kiegészítési stílus
Nicknavn sluttføringsstil
Styl uzupe³niania pseudonimów
Stil för nick-komplettering
;
MSG_PREFS_NICK_COMPLETION_STYLE_HELP
AmIRC style puts a colon and a space after users nicks if used at the beginning of a line, or just a space afterwards anywhere else. mIRC style: No colon or spaces are used, only the nick is displayed.
AmIRC Stil: Setzt hinter den Nicknamen einen Doppelpunkt und ein Leerzeichen am Satzanfang oder ein Leerzeichen im Satz.\nmIRC Stil: Kein Doppelpunkt oder Leerzeichen, nur der Name wird \
vervollständigt.
AmIRC pone dos puntos y un espacio después\ndel alias, si está al inicio de una línea,\no un espacio si está en otra parte. mIRC\nno añade nada, sólo muestra el alias.
Le style AmIRC ajoute deux-points et un espace après le pseudo lorsqu'il est utilisé en début de ligne, juste un espace après ailleurs. Le style mIRC : pas de deux-points ni d'espace, juste le pseudo.
Ôï óôõë ôïõ AmIRC ôïğïèåôåß ìßá Üíù-êÜôù ôåëåßá êáé İíá êåíü ìåôÜ ôï øåõäşíõìï ôïõ ÷ñŞóôç åÜí âñßóêåôáé óôçí áñ÷Ş ìéáò ãñáìçò, Ş áğëüò İíá êåíü ïğïõäŞğïôå áëëïı. Ôï mIRC óôõë: Ìüíï ôï øåõäşíõìï \
åìöáíßæåôáé, ÷ùñßò êåíü Ş Üíù-êÜôù ôåëåßá íá áêïëïõèåß.
Lo stile AmIRC pone dei doppi punti ed uno spazio dopo il nick dell'utente quando usato all'inizio linea o solo uno spazio in qualsiasi altra parte. \nStile mIRC: senza doppi punti o spazi, verra \
visualizzato solo il nick.
AmIRC stílus egy kettõspontot és egy szóközt illeszt a nick után ha egy sor elején került felhasználásra, vagy csak egy szóközt bárhol másutt. mIRC stílus: nem használ kettõspontot vagy szóközt, csak \
a nick jelenik meg.
AmIRC sluttføringsstil: Et kolon og et åpenrom legges til etter brukers nicknavn hvis det brukes på begynnelsen av en linje, eller bare et åpenrom bak hvis brukt ellers i linjen. mIRC \
sluttføringsstil: Verken kolon eller åpenrom brukes, kun nicknavn vises.
Styl AmIRC-a umieszcza dwukropek i  spacjê po pseudonimie u¿ytkowników je¶li zostanie u¿yte na pocz±tku linii, lub po prostu spacjê po pseudonimie w ka¿dym innym miejscu. Styl mIRC-a: bez dwukropka i \
odstêpu, tylko pseudonim jest dope³niany.
AmIRC sätter ett kolon och mellanslag efter ett nick i början på en rad, eller bara ett mellanslag efter överallt annars. mIRC: Inget kolon eller mellanslag, bara nicket visas.
;
MSG_ASK
Ask
Nachfragen
Preguntar
Demander
Ìå åñşôçóç
Chiedi
Kérés
Spør
Zapytaj
Fråga
;
MSG_PREFS_AUTO_REJOIN
Auto-rejoin channels when kicked.
Automatischer Wiedereintritt in Kanal nachdem man gekickt wurde.
Auto-reacceder al ser expulsado.
Rejoindre les canaux automatiquement après un kick.
Áõôüìáôç åğáíåßóïäïò óôï êáíÜëé óå ğåñßğôùóç ğåôÜãìáôïò.
Riaccedi automaticamente al canale se sei stato espulso.
Automatikus újracsaktlakozás kirúgáskor.
Re-entre kanaler automatisk ved kick.
Automatycznie przy³±czaj po wyrzuceniu.
Återanslut automatiskt efter kick.
;
MSG_PREFS_AUTO_OPEN_TAB
Auto-open private query tabs when messaged.
Automatisch Tabs öffnen, wenn Nachricht kommt.
Abrir pestaña privada al recibir mensaje.
Ouverture automatique des onglets de messagerie privée lors des messages.
Áõôüìáôï Üíïéãìá ğñïóùğéêŞò êáñôİëáò óõíïìéëßáò óå ğåñßğôùóç ğïõ ëÜâåôå ìŞíõìá.
Apri automaticamente dei Tab di richiesta quando ricevi un messaggio.
Automatikus privát lekérdezõ nyitás üzenet érkezésekor.
Auto-åpne private spørre-tab'er ved beskjeder.
Automatycznie otwieraj zak³adkê rozmów prywatnych po otrzymaniu wiadomo¶ci.
Öppna flik automatiskt vid privat meddelande.
;
MSG_PREFS_MAX_LINES_BUFFER
Maximum number of lines to keep in buffer.
Maximale Zeilenzahl des Puffers.
Líneas máximas de búfer.
Nombre maximum de lignes à conserver.
Ìİãéóôïò áñéèìüò ãñáììşí ãéá ôçí ğñïóùñéíŞ ìíŞìç.
Massimo numero di linee da mantenere nel buffer.
A buffer-ben tartott sorok maximális száma.
Maksimalt antall linjer å lagre i buffer.
Maksymalna liczba linii przechowywanych w buforze.
Max antal rader att hålla i bufferten.
;
MSG_PREFS_LOG_CHANNELS
Log channels
Schneide Kanäle mit
Canales
Historiser les canaux
ÊáôáãñáöŞ êáíáëéşí
Log canali
Csatornák naplózása
Logge kanaler
Raportuj kana³y
Logga kanaler
;
MSG_PREFS_LOG_PRIVATE
Log private chats
Schneide private Kanäle mit
 Charlas privadas
Historiser les discussions privées
ÊáôáãñáöŞ ğñïóùğéêşí óõíïìéëßşí
Log delle chats private
Privát beszélgetések naplózása
Logge private samtaler
Raportuj rozmowy prywatne
Logga privata konversationer
;
MSG_DEFAULT_KICK_MESSAGE
Default kick message
Kick Meldung
Al expulsar
Message de kick par défaut
ÌŞíõìá ğåôÜãìáôïò
Messaggio espulsione (kick)
Alap kick üzenet
Standard beskjed ved kick
Domy¶lna wiadomo¶æ przy wyrzuceniu
Standard kick-meddelande
;
MSG_DEFAULT_BAN_MESSAGE
Default ban message
Bann Nachricht
Mensaje al prohibir
Message de bannissement par défaut
ÌŞíõìá ban
Messaggio esclusione (ban)
Alap ban üzenet
Standard beskjed ved ban
Domy¶lna wiadomo¶æ przy blokadzie
Standard ban-meddelande
;
MSG_QUIT_MESSAGE
Quit message
Meldung beim Beenden
Al salir:
Message par défaut pour quitter IRC
ÌŞíõìá åîüäïõ
Messaggio di terminazione (quit)
Üzenet kilépéskor
Avslutningsbeskjed
Wiadmomo¶æ przy zakoñczeniu pracy
Avslutningsmeddelande
;
MSG_PART_MESSAGE
Part message
Meldung beim Verlassen
Al abandonar:
Message par défaut pour partir du canal
ÌŞíõìá İíôáîçò
Messaggio di uscita (part)
Üzenet szétbomláskor
Avslutningsbeskjed for kanal
Wiadomo¶æ przy od³±czeniu
Lämna-meddelande
;
MSG_REQUEST_WHOIS
Request whois information
Frage nach 'whois' Information
Información del usuario (WHOIS)
Demande l'information whois
Áßôçóç ğëçñïöïñéşí whois
Richiesta Chi Ã¨ (WHOIS)
Whois (Ki vagy?) információ kérése
Etterspør whois-informasjon
¯±daj informacji "whois"
Begär whois-information
;
MSG_OPEN_TAB
Open private query tab
Öffne privaten Tab
Abrir pestaña privada
Ouvrir un onglet de requête privée
¶íïéãìá ğñïóùğéêŞò êáñôİëáò óõíïìéëßáò
Apri TAB privato
Privát panelt nyit
Åpne privat spørre-tab
Otwórz zak³adkê rozmowy prywatnej
Öppna privat flik
;
MSG_DOUBLECLICK_GROUP_FRAME_TITLE
Nicklist doubleclick actions
Doppelklick Aktion in der Nicknamensliste
Doble Pulsación Sobre Aliases
Action du double-clic sur la liste de pseudos
Åíİñãåéá äéğëïı ğáôŞìáôïò óôçí ëßóôá øåõäùíıìùí
Azioni del doppio click nella lista Nick
Nick lista duplaklikk események
Dobbelklikk-hendelser for nickliste
Dzia³anie dwukliku na li¶cie pseudonimów
Dubbelklicksfunktioner för nicklistan
;
MSG_CUSTOM_TIMESTAMPS (57//300)
Customise your timestamps! use these codes: %h for the hour, %m for the minute, and %s for the second. This is useful to see what time an event happened (such as a user joining, or speaking).
Einstellen des Uhrzeitformats. %h für Stunden, %m für Minuten und %s für Sekunden. Das ist nützlich, um zu sehen, wann ein Ereignis stattfand.(wie Login/logout, oder sprechen).
Configurar el formato de la hora.\nÚtil para saber cuando pasó un evento\n(acceder a un canal, hablar,...).\n\033b%h\033n - hora; \033b%m\033n - minutos; \033b%s\033n - segundos.
Personnalisez l'horodatage. Utilisez ces codes %h pour l'heure, %m pour les minutes et %s pour les secondes. C'est utile pour retrouver à quelle heure un évènement est survenu (comme l'arrivée d'un \
utilisateur ou une annonce).
ÁëëÜîôå ôçí åìöÜíéóç ôùí åíäåßîåùí ÷ñüíïõ! ×ñçóéìïğïéŞóôå ôï %h ãéá ôçí şñá, ôï %m ãéá ôá ëåğôÜ êáé ôï %s ãéá ôá äåõôåñüëåğôá. ×ñŞóéìï, ãéá íá ğáñáêïëïõèåßôå ğüôå İãéíå İíá óõìâÜí, üğùò ğüôå ìğŞêå \
óôï êáíÜëé êÜğïéïò ÷ñŞóôçò Ş ğüôå åßğå êÜôé.
Customizza il formato della data e dell'ora! Utilizza %h per l'ora, %m per i minuti e %%s per i secondi. Ãˆ utile per vedere quando Ã¨ avvenuto un evento (p.es. l'accesso od un messaggio di un \
utente).
Idõlenyomat alakja! Használd órához a %h, perchez a %m és másodperchez a %s jelzõt. Hasznos, ha látható az események idõpontja (egy felhasználó mikor csatlakozott, beszélt).
Lag ditt eget tidsstempel! Bruk %h for timer, %m for minutter og %s for sekunder. Dette anvendes for å se tidspunktet for en spesiell hendelse (som f.eks. en bruker som entret eller snakket).
Spersonalizuj swoje oznaczenia czasowe! U¿yj %h dla godzin, %m - minut i %s - sekund. Jest to u¿yteczne, je¶li chcesz wiedzieæ kiedy co¶ siê wydarzy³o (np. u¿ytkownik siê do³±czy³ lub co¶ \
powiedzia³).
Modifiera tidstämplingen. Använd %h för timmar, %m för minuter, och %s för sekunder. Användbart för att se när något hände (såsom när en användare kom in, eller talade).
;
MSG_TIMESTAMP_FORMAT
Timestamp Format
Uhrzeitformat
 Formato:
Format de l'horodatage
ÌïñöŞ İíäåéîçò ÷ñüíïõ
Formato data e ora
Idõlenyomat alakja
Format for tidsstempel
Format oznaczeñ czasowych
Tidstämplingsformat
;
MSG_HIGHLIGHT
Words to Highlight
Worte zum Hervorheben
Resaltar:
Mots à mettre en évidence
Ëİîåéò ãéá ìáñêÜñéóìá
Parole da evidenziare
Kiemelendõ szavak
Ord som skal høylyses
S³owa do pod¶wietlania
Ord att markera
;
MSG_QUIT_REQUESTER
Quit Program?
Programm beenden?
¿Salir de WookieChat?
Quitter le programme ?
Êëåßóéìï ğñïãñÜììáôïò?
Sei sicuro di voler uscire da WookieChat?
Program elhagyása?
Avslutte programmet?
Skoñczyæ pracê?
Avsluta programmet?
;
MSG_FILE_TO_SEND
File to send
Datei zum Senden
Fichero:
Fichier à envoyer
Áñ÷åßï ğñïò áğïóôïëŞ
Fila da inviare
Küldendõ fájl
Fil å sende
Plik do wys³ania
Fil att skicka
;
MSG_WINDOW_TITLE_DCC_INCOMING
Incoming File Transfers...
Ankommender Dateitransfer ...
Recibir ficheros...
Transferts des fichiers entrants...
ÌåôáöïñÜ åéóåñ÷üìåíïõ áñ÷åßïõ...
Trasmissione dati in ricevimento...
Beérkezõ fájl átvitel...
Innkommende filoverføringer...
Pliki przychodz±ce...
Inkommande fil ...
;
MSG_WINDOW_TITLE_DCC_OUTGOING
Outgoing File Transfers...
Ausgehender Dateitransfer ...
Enviar ficheros...
Transferts des fichiers sortants...
ÌåôáöïñÜ åîåñ÷üìåíïõ áñ÷åßïõ...
Trasmissione dati uscita...
Kimenõ fájl átvitel...
Utgående filoverføringer...
Pliki wychodz±ce...
Utgående fil ...
;
MSG_HIGHLIGHT_TITLE
Highlight
Hervorheben
Lista De Palabras A Resaltar
Mettre en évidence
ÌáñêÜñéóìá
Evidenzia
Kiemelés
Høylyse
Pod¶wietlanie
Markera titel
;
MSG_PREFS_USE_TIMESTAMPS
Use timestamps
Zeige Uhrzeit
Mostrar
Utiliser l'horodatage
×ñŞóç İíäåéîçò ÷ñüíïõ
Mostra data e ora
Idõlenyomat használata
Bruk tidsstempel
U¿ywaj oznaczeñ czasowych
Använd tidstämpling
;
MSG_TIMESTAMPS_TITLE
Timestamps
Uhrzeit
Hora
Horodatage
Åíäåßîçò ÷ñüíïõ
Data e ora
Idõlenyomatok
Tidsstempler
Oznaczenia czasowe
Tidstämpling
;
MSG_TEXT_EDITOR_HELP
This text editor is used when you edit your "Perform on Connect" scripts (from the Server Selection window).
Dieser Texteditor wird benutzt, wenn Verbindungsskripte editiert werden (Vom Serverauswahlfenster/Starte bei Verbindung).
Editor de scripts al pulsar el\nbotón "Al conectar realizar..."\nde la ventana/lista de servidores.
Cet éditeur est utilisé lorsque vous éditez vos scripts « Exécuter à la connexion » (dans la fenêtre Sélection de Serveur).
Ï êåéìåíïãñÜöïò ÷ñçóéìïğïéåßôáé üôáí èİëåôå íá áëëÜîåôå ôéò åíôïëİò ğïõ åêôåëïıíôáé êáôÜ ôçí óıíäåóç (áğü ôï ğáñÜèõñï åğéëïãŞò äéáêïìéóôŞ).
Questo editor viene usato per modificare il tuo script "Azioni durante la connessione".
A "Kapcsolódáskor Végrehajtandó" szkript készítéséhez használt szövegszerkesztõ (a Kiszolgáló választása ablakban).
Denne teksteditoren brukes når du editerer dine "Utføre ved oppstart"-handlinger (fra Servervalg-vinduet).
Ten edytor tekstu zustanie u¿yty do edycji skryptu "Wykonaj po po³±czeniu" (z okna wyboru serwera).
Denna text editor används när du ändrar i "Perform on Connect"-script (från servervals-fönstret).
;
MSG_TEXT_EDITOR
Text Editor
Texteditor
Editor:
Éditeur de texte
ÊåéìåíïãñÜöïò
Editor di testo
Szövegszerkesztõ
Teksteditor
Edytor tekstu
Text Editor
;
MSG_WEB_BROWSER_LABEL
Web Browser
Web Browser
Navegador:
Navigateur Web
Web Browser
Web Browser
Böngészõ
Nettleser
Przegl±darka stron WWW
Webbläsare
;
MSG_WEB_BROWSER_HELP
Choose a web browser to use with WookieChat's "URL Grabber" Window. Double clicking an address there will load in this chosen browser.
Einen Webbrowser wählen, um ihn mit WookieChats "URL Grabber"zu nutzen. Bei einem Doppelklick auf eine Adresse wird automatisch in den voreingestellen Browser geladen.
Navegador que usa WookiChat en el "Gestor URLs"\nal pulsar dos veces en una de sus entradas.
Choisissez le navigateur à utiliser dans la fenêtre « Récupérateur d'URL » de WookieChat. Un double-clic sur une adresse la chargera dans le navigateur choisi.
Åğéëİîôå ôïí web browser ãéá íá ÷ñçóéìïğïéåßôå ìå ôï ğáñÜèõñï "URL Grabber" ôïõ WookieChat. Ìå äéğëü êëéê óå ìéá äéåıèõíóç èá ìåôáöåñèåß óôï åğéëåãìİíï web browser.
Seleziona un browser da usare con l'URL Grabber di WookieChat. Doppio click sull'indirizzo caricherÃ  l'indirizzo nel browser prescelto.
A WookieChat "URL Gyûjtõ" ablak használatához válassz egy internet böngészõt. Ez a böngészõ lesz indítva egy címen duplát kattintva.
Velg en nettleser til bruk for WookieChat sin "URL-fanger". Ved å dobbelklikke en adresse der vil siden lastes i den valgte nettleseren.
Wybierz przegl±darkê stron WWW, która bêdzie wykorzystywana w oknie "Kolekcjonera adresów URL". Dwuklik na adresie w tym oknie wczyta go w wybranej przegl±darce.
Välj en webbläsare att använda med WookieChat's "URL Grabber"-fönster. Dubbelklicka på en adress där och den laddas i den valda läsaren.
;
MSG_WEB_BROWSER_REQUESTER
Choose a browser
Einen Browser wählen
Elija navegador
Sélectionnez un navigateur
Åğéëİîôå İíáí browser
Scegli un browser
Böngészõ választása
Velg en nettleser
Wybierz przegl±darkê
Välj en webbläsare
;
MSG_SAVE
Save
Speichern
Guardar
Enregistrer
ÁğïèŞêåõóç
Salva
Mentés
Lagre
Zapisz
Spara
;
MSG_USE
Use
Benutzen
Usar
Utiliser
×ñŞóç
Usa
Használat
Bruk
U¿yj
Använd
;
MSG_CANCEL
Cancel
Abbrechen
Cancelar
Annuler
Áêıñùóç
Annulla
Mégsem
Avbryt
Poniechaj
Avbryt
;
MSG_DCC_AUTO_ACCEPT
Auto accept incoming DCC File Transfers
Automatisch eintreffenden DCC Datei-Transfer akzeptieren
Auto-aceptar transferencias de ficheros DCC
Accepter automatiquement les transferts de fichier DCC entrants
Áõôüìáôç áğïäï÷Ş åéóåñ÷ïìİíùí áğïóôïëşí áñ÷åßùí
Accetta automaticamente la ricezione di file DCC
Beérkezõ DCC fájl átvitelek automatikus elfogadása
Auto-godta innkommende DCC filoverføringer
Automatycznie akceptuj przychodz±ce transfery plików DCC
Acceptera inkommande DCC-filer automatiskt
;
MSG_DCC_ACTION
Action to take if file exists
Wenn Datei vorhanden, folgendes wählen:
Acción si el fichero existe:
Action si le fichier existe déjà
Åíİñãåéá ğïõ èá åêôåëåóôåß İáí ôï áñ÷åßï õğÜñ÷åé
Azione da eseguire se il file Ã¨ giÃ  esistente
Esemény már létezõ fájl esetén
Aksjon hvis filen eksisterer
Co zrobiæ, gdy plik ju¿ istnieje
Att göra om fil redan finns
;
MSG_DCC_LOCAL_ADDRESS
Use local address for DCC connections
Lokale Adresse für DCC Verbindungen benutzen
Usar dirección local para conexiones DCC
Utiliser l'adresse locale pour les connexions DCC
×ñŞóç ôïğéêŞ äéåıèõíóçò ãéá ôéò DCC óõíäİóåéò
Usa indirizzo locale per connessioni DCC
Helyi cím használata DCC kapcsolatokhoz
Bruk lokale adresser for DCC-forbindelser
U¿yj adresu lokalnego dla po³±czeñ DCC
Använd lokal adress för DCC-anslutningar
;
MSG_PORT_RANGE_START
DCC Port range from
DCC Port Bereich von
Rango puerto DCC de:
DCC sur les ports
ÃêÜìá DCC èõñşí áğü
Dimensione porta DCC da
DCC port sáv eleje
DCC portområde fra
Zakres portów DCC
DCC port-område börjar
;
MSG_PORT_RANGE_END
to
bis
 a:
 à
İùò
fino a
vége
til
 do
DCC port-område slutar
;
MSG_PORT_RANGE_START_HELP
This is the start of the port range for incoming DCC connections. This is configurable so you can choose which ports to use and open up on any firewall you might have.
Dieses ist der Startbereich für ankommende DCC Verbindungen. Das ist konfigurierbar, so kann gewählt werden, welcher Port offen ist und mit jeder Firewall funktioniert.
Inicio del rango de puertos para conexiones\nDCC. Es configurable y puede elegir el\nrango que desee usar y habilitarlos en su\ncortafuegos (firewall).
Il s'agit début de l'intervalle de ports autorisés pour les connexions DCC entrantes. C'est configurable, vous pouvez donc sélectionner quels ports utiliser et ouvrir sur les pare-feu que vous pouvez \
avoir.
Ç áñ÷Ş ôçò ãêÜìáò èõñşí ãéá ôéò åéóåñ÷üìåíåò DCC óõíäİóåéò. Ìğïñåßôå íá ôéò áëëÜîåôå ìå óêïğü íá ñõèìßóåôå ğïéåò ğüñôåò èá áíïßîåôå óôï firewall ğïõ ìğïñåß íá ÷ñçóéìïğïéåßôå.
Questo Ã¨ l'inizio del port range per trasmissioni DCC in entrata. Ãˆ configurabile cosÃ¬ puoi scegliere quali porte usare e aprire nelle firewall del tuo sistema.
Itt kezdõdik a beérkezõ DCC kapcsolatok port sávja. Mindez beállítható, hogy az esetleges tûzfalon megnyitott portok legyenek használva.
Dette er starten på et portområde for innkommende DCC-forbindelser. Dette er konfigurerbart slik at du kan velge hvilke porter du vil bruke, og åpne opp en eventuell brannmur du har.
Jest to pocz±tkowy numer zakresu portów dla przychodz±cych po³±czeñ DCC. Mo¿esz wybraæ których portów chcesz u¿ywaæ, lub które porty masz otwarte na zaporze ogniowej.
Här är början på portområdet för inkommande DCC-anslutningar. Detta är konfigurerbart så att du kan välja vilka portar att använda och öppna i brandväggen.
;
MSG_PORT_RANGE_END_HELP
This is the end of the port range for incoming DCC connections. This is configurable so you can choose which ports to use and open up on any firewall you might have.
Dieses ist der Endbereich für ankommende DCC Verbindungen. Das ist konfigurierbar, so kann gewählt werden, welcher Port offen ist und mit jeder Firewall funktioniert.
Fin del rango de puertos para conexiones\nDCC. Es configurable y puede elegir el\nrango que desee usar y habilitarlos en\nsu cortafuegos (firewall).
Il s'agit de la fin de l'intervalle de ports autorisés pour les connexions DCC entrantes. C'est configurable, vous pouvez donc sélectionner quels ports utiliser et ouvrir sur les pare-feu que vous \
pouvez avoir.
Ç ôåëéêŞ ôéìŞ ôçò ãêÜìáò èõñşí ãéá ôéò åéóåñ÷üìåíåò DCC óõíäİóåéò. Ìğïñåßôå íá ôéò áëëÜîåôå ìå óêïğü íá ñõèìßóåôå ğïéåò ğüñôåò èá áíïßîåôå óôï firewall ğïõ ìğïñåß íá ÷ñçóéìïğïéåßôå.
Questa Ã¨ la fine del port range per trasmissioni DCC riceventi. Ãˆ configurabile cosÃ¬ puoi scegliere quali ports usare e aprirli nelle firewall del tuo sistema.
Ez a vége a beérkezõ DCC kapcsolatok port sávjának. Mindez beállítható, hogy az esetleges tûzfalon megnyitott portok legyenek használva.
Dette er slutten på et portområde for innkommende DCC-forbindelser. Dette er konfigurerbart slik at du kan velge hvilke porter du vil bruke, og åpne opp en eventuell brannmur du har.
Jest to koñcowy numer zakresu portów dla przychodz±cych po³±czeñ DCC. Mo¿esz wybraæ których portów chcesz u¿ywaæ, lub które porty masz otwarte na zaporze ogniowej.
Här är slutet på portområdet för inkommande DCC-anslutningar. Detta är konfigurerbart så att du kan välja vilka portar att använda och öppna i brandväggen.
;
MSG_NEVER
Never
Nie
Nunca
Jamais
Ğïôİ
Mai
Soha
Aldri
Nigdy
Aldrig
;
MSG_ALWAYS
Always
Immer
Siempre
Toujours
ĞÜíôá
Sempre
Mindig
Alltid
Zawsze
Alltid
;
MSG_ONLY_WHEN_WINDOW_INACTIVE
Only when window is inactive
Nur wenn Fenster inaktiv
Sólo si la ventana está inactiva
Seulement lorsque la fenêtre est inactive
Ìüíï üôáí ôï ğáñÜèõñï åßíáé áíåíåñãü
Solo se la finestra Ã¨ inattiva
Csak akkor ha az ablak inaktív
Bare når vindu er inaktivt
Tylko, gdy okno jest nieaktywne
Bara när fönstret är inaktivt
;
MSG_SOUND_SAMPLE_TAB_OPEN
Sample to play when a tab opens
Klang wird beim Öffnen von Tabs gespielt
Al abrir pestaña:
Échantillon à jouer lorsqu'un onglet s'ouvre
Ç÷ïò ğïõ èá áêïıãåôáé üôáí ìéá íİá êáñôİëá áíïßîåé
Suono da riprodurre quando si apre un Tab
Hangminta lejátszása panel megnyitásakor
Lyd å spille når tab åpner
D¼wiêk odtwarzany przy otwieraniu zak³adki
Ljud att spela när en flik öppnas
;
MSG_PLAY_SAMPLE
Play sample
Klang abspielen
Reproducir:
Jouer
ÁíáğáñáãùãŞ Ş÷ïõ
Riproduci suono
Hangminta lejátszása
Spill lyd
Odtwórz d¼wiêk
Spela ljud
;
MSG_CHOOSE_FILENAME
Choose a filename
Dateinamen auswählen
Elija fichero
Veuillez sélectionner un fichier
ÅğéëïãŞ ïíüìáôïò áñ÷åßïõ
Scegli un nome per il file
Válassz egy fájlnevet
Velg et filnavn
Wybierz plik
Välj en fil
;
MSG_SOUND_SAMPLE_HIGHLIGHT
Sample to play on highlight
Klang der bei markiertem Text abgespielt wird
Al resaltar:
Échantillon à jouer sur mise en évidence
Ç÷ïò ğïõ èá áêïıãåôáé üôáí åìöáíéóôåß ìáñêáñéóìİíç ëİîç
Suono da riprodurre per testo evidenziato
Kiemeléskor lejátszott hangminta
Lyd å spille ved høylysing
D¼wiêk odtwarzany przy pod¶wietlaniu
Ljud att spela vid markering
;
MSG_SAMPLES_PATH
Samples Path for CTCP SOUND
Klang Pfad für CTCP Sound
Senda sonidos:
Chemin des échantillons pour la commande CTCP SOUND
ÖÜêåëïò ìå ç÷ïõò ãéá CTCP SOUND
Percorso per suoni CTCP
Hangminta elérési út a CTCP SOUND-hoz
Stinavn for lyder ved CTCP SOUND
¦cie¿ka dostêpu dla CTCP SOUND
Ljudsökväg för CTCP SOUND
;
MSG_SAMPLES_PATH_HELP
When you recieve a CTCP SOUND request, WookieChat will\nlook in this path for sound samples.
Bei Empfang einer CTCP Anfrage, sucht WookieChat in diesem Pfad nach der Tondatei.
Al recibir una petición CTCP SOUND,\n'WookieChat' escoje los sonidos de\nesta senda.
Lors de la réception d'une requête CTCP SOUND, WookieChat\nrecherchera dans cet emplacement pour les échantillons.
¼ôáí ëçöèåß ìßá êëŞóç ãéá CTCP SOUND, WookieChat èá øÜîåé\nóå áõôüí ôïí êáôÜëïãï ãéá Ş÷ïõò.
Quando ricevi una richiesta CTPS SOUND, WookieChat cercherÃ¡Â \nun suono in questo percorso.
Amikor CTCP SOUND kérés érkezik, WookieChat ezt az elérési\nutat nézi át hangmintákért.
Når du mottar en forespørsel CTCP SOUND, vil WookieChat\nse etter lyder i denne mappen.
Gdy otrzymasz ¿±danie CTCP SOUND, program\nprzeszuka podan± ¶cie¿kê w poszukiwaniu d¼wiêków.
När du får en CTCP SOUND-förfrågan, kommer WookieChat att leta i denna sökvägen efter ljudfiler.
;
MSG_CHOOSE_DIR
Choose a directory
Ein Verzeichnis auswählen
Elija directorio
Veuillez sélectionner un tiroir
ÅğéëïãŞ êáôáëüãïõ
Scegli un cassetto
Válassz egy könyvtárat
Velg en mappe
Wybierz katalog
Välj katalog
;
MSG_LOGFILE_RELOADING_LABEL
Num of lines to display from the end of logfiles when opening a tab\n(Put 0 to disable this feature).
Anzahl der Zeilen die vom Ende eines Logfiles in ein neues Tab übernommen werden. (0 um Auszuschalten).
\033cLíneas a mostrar del final de los registros\nal abrir una pestaña (0 para desactivar).
Nombre de lignes à recharger de l'historique lors de l'ouverture d'un onglet\n(Mettre 0 pour désactiver cette fonction).
Áñéèìüò ãñáììşí ğïõ èá åìöáíßæïíôáé áğü ôï ôİëïò ôùí áñ÷åßùí êáôáãñáöŞò\nêáôÜ ôï Üíïéãìá ìéáò êáñôİëáò. ÅéóÜãåôå 0 ãéá áğåíåñãïğïßçóç.
Numero di linee da riprendere dalla fine del logfile nel nuovo Tab\n(inserisci 0 per disabilitare questa funzione).
A naplófájl végérõl megjelenített sorok száma egy panel megnyitásakor\n(0 értékkel ez a lehetõség kikapcsolható).
Antall linjer å vise fra slutten av loggfiler når man åpner en tab\n(Sett 0 for å la denne muligheten være).
Liczba ostatnich linii z pliku raportu do wy¶wietlenia przy otwieraniu zak³adki\n(0 - wy³±czaa tê opcjê).
Antal rader att visa från slutet av loggfilerna när en flik öppnas (sätt till 0 för att stänga av funktionen).
;
MSG_CHANNEL
Channel
Kanal
Canal:
Canal
ÊáíÜëé
Canale
Csatorna
Kanal
Kana³
Kanal
;
MSG_CHANNEL_LOG_RELOAD_HELP
How many lines to reload from the end of log files when you join a channel.
Anzahl der Textreihen die vom Ende eines Logfiles übernommen werden, wenn ein neuer Kanal betreten wird.
Número de líneas a recargar del final\ndel registro al acceder a un canal.
Nombre de ligne à recharger de l'historique lors de l'entrée dans un canal.
Ğüóåò ãñáììİò íá åìöáíéóôïıí áğü ôï ôİëïò ôïõ áñ÷åßïõ êáôáãñáöŞò ìüëéò åéóÜãåóôå óå êÜğïéï êáíÜëé.
QuantitÃ Â di linee da ricaricare dalla fine del logfile quando accedi ad un canale.
A naplófájl végérõl megjelenített sorok száma egy csatornához csatlakozáskor.
Antall linjer å laste på nytt fra slutten av loggfiler når du entrer en kanal.
Ile linii od koñca pliku raportu wczytaæ podczas do³±czania do kana³u.
Hur många rader att återladda från slutet av loggfilerna när du ansluter till en kanal.
;
MSG_PRIVATE
Private tab
Privater Tab
 Pestaña privada:
Onglet privé
ĞñïóùğéêŞ êáñôİëá
Tab privato
Privát panel
Privat tab
Zak³adka prywatna
Privat flik
;
MSG_PRIVATE_LOG_RELOAD_HELP
How many lines to reload from the end of log files when you open a private query tab to another user.
Anzahl der Textreihen die vom Ende eines Logfiles übernommen werden, wenn ein privater Kanal betreten wird.
Número de líneas a recargar del final del\nregistro al abrir una pestaña privada.
Nombre de lignes à recharger de l'historique lors de l'ouverture d'un onglet privé avec un autre utilisateur.
Ğüóåò ãñáììİò íá åìöáíéóôïıí áğü ôï ôİëïò ôïõ áñ÷åßïõ êáôáãñáöŞò ìüëéò áíïßîåôå ìßá ğñïóùğéêŞ êáñôİëá óõíïìéëßáò ìå Üëëùí ÷ñŞóôç.
QuantitÃ  di linee da ricaricare dalla fine del log file quando apri un canale privato.
A naplófájl végérõl megjelenített sorok száma egy másik felhasználó privát paneljének megnyitásakor.
Antall linjer å laste på nytt fra slutten av loggfiler når du åpner en privat spørretab mot en annen bruker.
Ile linii od koñca pliku raportu wczytaæ podczas rozpoczêcia rozmowy z innym u¿ytkownikiem.
Hur många rader att återladda från slutet av loggfilerna när du öppnar en privat flik till en annan användare.
;
MSG_LOG_RELOADING_TITLE
Log Reloading
Mitschnitt Neuladen
Recarga De Registro
Rechargement d'historique
Åğáíáöüñôùóç áñ÷åßïõ êáôáãñáöŞò
Ricaricare il file di protocollo
Napló újraolvasás
Laster logg på nytt
Wczytywanie raportów
Ladda logg igen
;
MSG_ALIAS_HELP
Enter an alias name, then the command you want it to use\nE.g. With this line added to the alias list..\n/j /join\n..then the "/j" command will be replaced with "/join".
Alias Namen eintragen, dann das gewünschte Kommando.\nZ.B. Mit diesem Text: \n/j /join\n... wird das "/j" durch ein "/join" ersetzt.
Alias para comandos. Ejemplo: \033b/j /join\033n,\ncon lo que escribiendo sólo \033i/j <canal> \033n\npodrá acceder al <canal> en lugar de\nescribir \033i/join <canal>.\033n.
Entrez un nom d'alias, puis la commande à lui associer.\nPar exemple, la ligne « /j /join » permet d'utiliser la commande « /j » à la place de « /join ».
ÅéóÜãåôå İíá óõíşíõìï üíïìá, êáé ìåôÜ ôçí åíôïëŞ ğïı èİëåôå íá ÷ñçóéìïğïéçèåß.\nĞ.÷. Ìå áõôŞ ôçí ãñáììŞ..\n/j /join\n..ç åíôïëŞ "/j" áíôéêáèßóôáíôáé áğü ôçí åíôïëŞ "/join".
Inserisci un alias, poi il comando che vuoi assegnare\nes.: inserisci \n/j /join\n.. e l'alias /j verra sostituito con il commando /join.
Adj meg egy helyettesítõ nevet, majd a parancsot amit használni kívánsz hozzá\npl.: a helyettesítõ listában...\n/j /join\n ... akkor a /j parancs felváltja  a /join parancsot.
Skriv inn et alias-navn, deretter kommandoen du vil som skal brukes\nF.eks. om du legger til denne linjen\n/j /join\n..da vil "/j"-kommandoen bli erstattet med "/join".
Wprowad¼ nazwê aliasu, a nastêpnie polecenie, które ma byæ wykonywane\nNp. po dodaniu tej linii do listy aliasów\n/j /join\npolecenie "/j" bêdzie zastêpowane przez "/join".
Skriv in ett aliasnamn, sedan kommandot du vill använda\n dvs. Med denna raden adderas till aliaslistan..\n/j /join\n..sedan "/j" kommandot kommer att ersättas med "/join".
;
MSG_BUTTON_ADD_ALIAS
Add alias >>
Alias hinzufügen >>
Añadir alias >>
Ajouter Alias >>
Ğñïóè. óõíşíõìïõ >>
Aggiungi alias >>
Helyettesítõt hozzáad >>
Legg til alias >>
Dodaj alias >>
Lägg till alias >>
;
MSG_BUTTON_REMOVE_ALIAS
Remove
Entfernen
Eliminar
Supprimer
Áöáßñåóç
Rimuovi
Eltávolít
Fjern
Usuñ
Ta bort alias
;
MSG_SETTINGS_WINDOW_TITLE
Settings
Einstellungen
Ajustes
Réglages
Ñõèìßóåéò
Impostazioni
Beállítások
Innstillinger
Ustawienia
Inställningar
;
MSG_COLOR_SETTINGS_WINDOW_TITLE
Color settings...
Farbeinsteller ...
Colores...
Réglages des couleurs...
×ñùìáôéêİò ñõèìßóåéò...
Impostazione colori...
Szín beállítások...
Fargeinnstillinger...
Ustawienia kolorów...
Färginställningar ...
;
MSG_MENU_SELECT_SERVER
Select server...
Server auswählen ...
Elegir servidor...
Choisir un serveur...
ÅğéëïãŞ äéáêïìéóôŞ...
Scegli server...
Kiszolgáló választása ...
Velg server...
Wybierz serwer...
Välj server ...
;
MSG_MENU_CLOSE_TAB
Close current tab
Aktiven Tab schliessen
Cerrar pestaña actual
Fermer l'onglet actif
Êëåßóéìï êáñôİëáò
Chiudi questo Tab
Jelenlegi panel bezárása
Lukk aktiv tab
Zamknij aktualn± zak³adkê
Stäng gällande flik
;
MSG_MENU_NEW_SERVER_TAB
New server tab
Neuen Server Tab öffnen
Nueva pestaña
Nouvel onglet serveur
Íİá êáñôİëá äéáêïìéóôŞ
Apri un Tab "server"
Új kiszolgáló panel
Ny servertab
Nowa zak³adka serwera
Ny serverflik
;
MSG_MENU_OPEN_GLOBAL_TAB
Open global query tab
Globalen Tab öffnen
Abrir pestaña global
Ouvrir onglet de requête global
¶íïéãìá ãåíéêŞò êáñôİëáò
Apri un tab "globale"
Globális panel megnyitása
Åpne global spørretab
Otwórz globaln± zak³adkê rozmów
Öppna flik för global förfrågan
;
MSG_MENU_QUIT
Quit...
Programm beenden ...
Salir...
Quitter...
¸îïäïò...
Esci...
Kilépés...
Avslutte...
Skoñcz...
Avsluta ...
;
MSG_MENU_SAVE_BUFFER
Save buffer as...
Fensterinhalt speichern als ...
Guardar búfer como...
Enregistrer le tampon sous...
ÁğïèŞêåõóç ğñïó. ìíŞìçò ùò...
Salva il buffer come...
Buffer mentése, mint...
Lagre buffer som...
Zapisz bufor jako...
Spara buffert som ...
;
MSG_MENU_MAIN_TITLE
Main
Hauptmenü
Principal
Principal
Êõñßùò
Menu principale
Fõmenü
Fil
G³ówne
Huvudrubrik
;
MSG_MENU_MAIN_SETTINGS
Main settings...
Haupteinstellungen ...
Globales...
Réglages principaux...
Êıñéåò ñõèìßóåéò...
Impostazioni principali...
Elsõdleges beállítások...
Innstillinger...
Ustawienia programu...
Huvudinställningar ...
;
MSG_MENU_MULITCOLUMN_DISPLAY
Multi-column chat display
Extra Spalte für Anzeige
Múltiples columnas
Affichage Multi-colonnes
ÅìöÜíéóç ğïëëşí óôçëşí
Schermo chat multicolonna
Több-oszlopos csevegõ kijelzõ
Multikolonne snakke-visning
Wy¶wietlaj rozmowy w wielu kolumnach
Visning med flera kolumner
;
MSG_URL_GRABBER
URL Grabber...
URL Greifer ...
Gestor URLs...
Récupérateur d'URL...
URL Grabber...
URL Grabber...
URL Gyûjtõ...
URL-fanger...
Kolekcjoner adresów URL...
URL-Grabber ...
;
MSG_SERVER_NAME
Server Name
Server Name
Servidor:
Nom du serveur
¼íïìá äéáêïìéóôŞ
Nome del server
Kiszolgáló neve
Servernavn
Nazwa serwera
Servernamn
;
MSG_PORT_NUMBER
Port Number
Port Nummer
Puerto:
Numéro de port
Áñéèìüò èıñáò
Numero porta
Port száma
Portnummer
Numer portu
Portnummer
;
MSG_SERVER_PASSWORD
Server Password
Server Passwort
Contraseña:
Mot de passe du serveur
Êùäéêüò äéáêïìéóôŞ
Password server
Kiszolgáló jelszó
Serverpassord
Has³o do serwera
Serverlösen
;
MSG_WINDOW_EDIT_SERVER
Edit Server
Server ändern
Editar servidor
Éditer le serveur
Åğåî. äéáêïìéóôŞ
Modifica server
Kiszolgáló módosítása
Editere server
Edytuj serwer
Redigera server
;
MSG_GROUP_NAME
Group Name
Gruppen-Name
Nombre grupo:
Nom de groupe
¼íïìá ïìÜäáò
Nome del gruppo
Csoport név
Gruppenavn
Nazwa grupy
Gruppnamn
;
MSG_PERFORM_ON_CONNECT
Perform on connect...
Bei Verbindung starten ...
Al conectar realizar...
Exécuter à la connexion...
Åíİñãåéá êáôÜ ôçí óıíäåóç...
Esegui alla connessione...
Kapcsolódáskor Végrehajtandó...
Utfør ved forbindelse...
Wykonaj po po³±czeniu...
Utför vid anslutning ...
;
MSG_EDIT_GROUP
Edit Group...
Gruppe ändern ...
Editar grupo...
Éditer le groupe...
Åğåî. ïìÜäáò...
Modifica gruppo...
Csoport szerkesztése...
Editere gruppe...
Edytuj grupê...
Redigera grupp ...
;
MSG_REAL_NAME
Real Name
Realname
 Nombre real:
Nom réel
Ïíïìáôåğşíõìï
Nome reale
Valódi név
Virkelig navn
Prawdziwe imiê
Verkligt namn
;
MSG_USER_NAME
User Name
Nutzer Name
Usuario:
Nom d'utilisateur
¼íïìá ÷ñŞóôç
Nome utente
Felhasználónév
Brukernavn
Nazwa u¿ytkownika
Användarnamn
;
MSG_NEW_SERVER
New Server
Neuer Server
Nuevo servidor
Nouveau serveur
Íİïò äéáêïìéóôŞò
Nuovo server
Új kiszolgáló
Ny server
Nowy serwer
Ny server
;
MSG_NEW_GROUP
New Group
Neue Gruppe
Nuevo grupo
Nouveau groupe
Íİá ïìÜäá
Nuovo gruppo
Új csoport
Ny gruppe
Nowa grupa
Ny grupp
;
MSG_EDIT_ENTRY
Edit Entry...
Eintrag ändern ...
Editar entrada...
Éditer l'entrée...
Åğåî. åéóáãùãŞò...
Modifica...
Módosítás...
Editere valg...
Edytuj wpis...
Redigera ...
;
MSG_DELETE_ENTRY
Delete Entry
Eintrag löschen
Borrar entrada
Effacer l'entrée
ÄéáãñáöŞ åéóáãùãŞò
Cancella
Törlés
Ødelegg valg
Usuñ wpis
Ta bort
;
MSG_CONNECT_TO_SERVER
Connect to server
Mit Server verbinden
Conectar a servidor
Connexion au serveur
Óıíäåóç óôïí äéáêïìéóôŞ
Connetti al server
Csatlakozás
Forbinde til server
Po³±cz z sererem
Anslut till server
;
MSG_CONNECT_TO_SERVER_IN_NEW_TAB
Connect in new tab
In neuem Tab verbinden
Conec. en nueva pestaña
Connexion dans un nouvel onglet
Óıíäåóç óå íİá êáñôİëá
Connetti in un nuovo Tab
Csatlakozás új panelen
Forbinde i ny tab
Po³±cz w nowej zak³adce
Anslut i ny flik
;
MSG_WINDOW_SELECT_SERVER
Select server
Server wählen
WookieChat: Elija servidor
Sélection de serveur
ÅğéëïãŞ äéáêïìéóôŞ
Seleziona server
Kiszolgáló választása
Velg server
Wybierz serwer
Välj server
;
MSG_UNBAN
Unban
Verbannung aufheben
Eliminar entrada
Débannir (Unban)
Unban
Ritira Ban
Ban eltávolítás
Uban
Unban
Unban
;
MSG_REFRESH_BAN_LIST
Refresh banlist
Bannliste erneuern
Refrescar lista
Rafraîchir la liste de bannissement
Áíáíİùóç ban ëßóôáò
Aggiorna la banlist
Ban lista frissítése
Oppfrisk banliste
Od¶wie¿ listê blokad
uppdatera banlistan
;
MSG_UNABLE_TO_CREATE_APP
Unable to create app\n
Anlegen nicht möglich
No se ha podido crear aplicación.\n
Impossible de créer l'application\n
Áäıíáôç ç åìöÜíéóç ôçò åöáñìïãŞò\n
Impossibile creare applicazione
Alkalmazás nem hozható létre\n
Kan ikke åpne applikasjon\n
Nie mo¿na utworzyæ applikacji\n
Kan inte skapa app\n
;
MSG_NOT_CONNECTED
Not connected to a server.
Nicht mit Server verbunden.
No está conectado a un servidor.
Non connecté à un serveur.
Ìç óõíäåäåìİíï óå äéáêïìéóôŞ.
Non connesso al server.
Nincs kiszolgálóhoz csatlakozva.
Ingen forbindelse til server.
Nie po³±czony z serwerem.
Inte ansluten till server.
;
MSG_NOT_CONNECTED_TO_USER
Not connected to a user.
Nicht mit Nutzer verbunden.
No está conectado a un usuario.
Non connecté à un utilisateur.
Ìç óõíäåäåìİíï óå ÷ñŞóôç.
Non connesso all'utente.
Nincs felhasználóhoz csatlakozva.
Ingen forbindelse til bruker.
Nie po³±czony z u¿ytkownikiem.
Inte ansluten till användare.
;
MSG_NO_BROWSER_SELECTED
Please go to the "Main Settings" window, and choose a browser for WookieChat to open web sites with.
Bitte in den Grundeinstellungen einen Browser auswählen um ausgewählte Seiten zu öffnen.
Elija el navegador que 'WookieChat'\nusará con páginas web en\n"Ajustes->Globales.../General".
Veuillez aller dans la fenêtre « Réglages principaux », et sélectionner le navigateur avec lequel WookieChat doit ouvrir les sites Web.
Ğçãáßíôå óôéò "Êıñéåò Ñõèìßóåéò" êáé åğéëİîôå İíáí web browser ìå ôï ïğïßï èá áíïßãïõí ïé éóôïóåëßäåò.
Ritorna alle "impostazioni principali" e scegli un browser per WookieChat.
Kérlek menj az "Elsõdleges Beállítások" ablakhoz és válassz egy böngészõt, amellyel a WookieChat megnyithat weboldalakat.
Vennligst gå til "Innstillinger" - vinduet, og velg en nettleser for WookieChat.
Proszê przej¶æ do okna ustawieñ programu i wybraæ przegl±darkê, która ma byæ wykorzystywana do otwierania stron WWW.
Gå till "Huvudinställningar", och välj en webbläsare att öppna webbsidor med.
;
MSG_ERROR_MISSING_MCC_BETTERSTRING
Please download and install the BetterString.mcc MUI Class.
Bitte MUI Klasse 'BetterString.mcc'  runterladen und installieren.
Descarge e instale la clase de MUI 'BetterString.mcc'.
Veuillez télécharger et installer la classe MUI BetterString.
ÊáôåâÜóôå êáé åãêáôáóôŞóôå ôï BetterString.mcc MUI Class.
Si prega di scaricare ed installare la classe MUI "BetterString.mcc".
Kérlek töltsd és telepítsd a BetterString.mcc MUI osztályt.
Vennligst last ned og installer MUI-klassen BetterString.mcc.
Proszê pobraæ i zainstalowaæ klasê MUI BetterString.mcc.
Ladda ned och installera MUI-klassen BetterString.mcc.
;
MSG_ERROR_MISSING_MCC_NLIST
Please download and install the NList.mcc MUI Class.
Bitte MUI Klasse 'NList.mcc' runterladen und installieren.
Descarge e instale la clase de MUI 'NList.mcc'.
Veuillez télécharger et installer la classe MUI NList.
ÊáôåâÜóôå êáé åãêáôáóôŞóôå ôï NList.mcc MUI Class.
Si prega di scaricare ed installare la classe MUI "NList.mcc".
Kérlek töltsd és telepítsd az NList.mcc MUI osztályt.
Vennligst last ned og installer MUI-klassen NList.mcc.
Proszê pobraæ i zainstalowaæ klasê MUI NList.mcc.
Ladda ned och installera MUI-klassen NList.mcc.
;
MSG_ERROR_MISSING_MCC_NLISTVIEW
Please download and install the NListview.mcc MUI Class.
Bitte MUI Klasse 'NListview.mcc' runterladen und installieren.
Descarge e instale la clase de MUI 'NListview.mcc'.
Veuillez télécharger et installer la classe NListview.
ÊáôåâÜóôå êáé åãêáôáóôŞóôå ôï NListview.mcc MUI Class.
Si prega di scaricare ed installare la classe MUI "NListview.mcc".
Kérlek töltsd és telepítsd az NListview.mcc MUI osztályt.
Vennligst last ned og installer MUI-klassen NListview.mcc.
Proszê pobraæ i zainstalowaæ klasê MUI NListview.mcc.
Ladda ned och installera MUI-klassen NListview.mcc.
;
MSG_ERROR_MISSING
These missing classes can be downloaded from http://www.aminet.net/ or http://os4depot.net. Check the WookieChat readme for direct links to these classes\n
Fehlende Klasse kann unter http://www.aminet.net/ oder http://os4depot.net geladen werden. Bitte WookieChat readme prüfen für direkte Links zu den Klassen\n
Las clases que faltan, puede descargarlas de 'http://www.aminet.net/' o 'http://os4depot.net'. Lea "readme.txt" para los enlaces directos a esas clases.\n
Ces classes manquantes peuvent être téléchargées sur http://www.aminet.net/ ou http://os4depot.net. Veuillez vous reporter au fichier LisezMoi de WookieChat pour les liens directs vers ces classes.\n
Ôá classes áõôÜ ğïõ ëåßğïõí ìğïñïıí íá êáôåâïıí áğü ôï http://www.aminet.net/ Ş ôï http://os4depot.net. Äåßôå ôï readme ôïõ WookieChat ãéá ôïõò áğ' åõèåßáò óõíäİóìïõò\n
Le classi MUI mancanti possono essere scaricate da http://www.aminet.net/ o http://os4depot.net. Per ulteriori informazioni puoi leggere anche il file readme di WookieChat per verificare i link \
diretti alle varie classi\n
Ezek a hiányzó osztályok letölthetõek a következõ helyekrõl: http://www.aminet.net/ vagy http://os4depot.net. Ellenõrizd a WookieChat dokumentációt a közvetlen linkekhez\n
Disse manglende klassene kan lastes ned fra http://www.aminet.net/ eller http://os4depot.net/. Les WookieChat readme-filene for direkte linker til disse klassene\n
Te brakuj±ce klasy mo¿esz pobraæ z http://www.aminet.net/ lub http:/os4depot.net/. W pliku "readme" programu znajdziesz bezpo¶rednie odsy³acze do tych klas\n
De saknade klasserna kan laddas ned från http://www.aminet.net/ eller http://os4depot.net. Se WookieChat readme för direktlänkar till klasserna\n
;
MSG_SAVING_BUFFER_TO
Saving buffer to file:
Puffer speichern unter:
Guardando búfer en
Sauvegarde du tampon dans le fichier :
ÁğïèŞêåõóç ğñïó. ìíŞìçò óå áñ÷åßï:
Salvare buffer come file:
Buffer mentése fájlba:
Lagrer buffer til fil:
Zapisywanie bufora do pliku:
Spara buffer till fil
;
MSG_CANCELLED
Cancelled
Abgebrochen
Cancelado
Annulé
Áêõñşèçêå
Annullato
Megszakítva
Avbrutt
Poniechano
Avbruten
;
MSG_DCC_SEND_MESSAGE
Please select a file to send, and a nick to offer the file to.
Bitte Datei zum Senden auswählen und den Empfänger.
Elija fichero a enviar y a quien (alias) desea ofrecerlo.
Veuillez sélectionner un fichier à envoyer, et indiquer le pseudo à qui offrir le fichier.
Åğéëİîôå ôï ğñïò áğïóôïëŞ áñ÷åßï êáé ôï øåõäşíõìï ğïõ èá ôï ëÜâåé.
Seleziona il file e un nick a cui trasmetterlo.
Válassz egy fájlt elküldésre és egy nick-et, akinek a fájl felkínáható.
Vennligst velg en fil som skal sendes, og et nicknavn som skal tilbys filen.
Proszê wybraæ plik do wys³ania, oraz pseudonim u¿ytkownika, dla którego chcesz zaoferowaæ plik.
Välj en fil att skicka, och ett nick att skicka till.
;
MSG_DCC_SEND_MESSAGE2
Please select a file to send
Bitte Datei zum Senden Auswählen
Elija fichero a enviar
Veuillez sélectionner un fichier à envoyer
Åğéëİîôå İíá áñ÷åßï ãéá áğïóôïëŞ
Seleziona il file da trasmettere
Válassz egy fájlt elküldésre
Vennligst velg en fil å sende
Proszê wybraæ plik do wys³ania
Välj en fil att skicka
;
MSG_DCC_SEND_MESSAGE3
Please select a nick to offer the file to.
Bitte Empfänger auswählen.
Elija a quien (alias) desea enviar el fichero.
Veuillez indiquer le pseudo à qui offrir le fichier.
Åğéëİîôå İíá øåõäşíõìï ğïõ èá ğáñáëÜâåé ôï áñ÷åßï.
Seleziona un nick a cui trasmettere il file.
Válassz egy nick-et, akinek a fájl felajánlható.
Vennligst velg et nicknavn å tilby filen til.
Proszê wybraæ pseudonim u¿ytkownika, dla którego chcesz zaoferowaæ plik.
Välj ett nick att skicka till.
;
MSG_WINDOW_TITLE_BANMASKS
List of banmasks for channel
Liste der Bannmasken im Kanal
Lista de prohibiciones del canal
Liste des masques de bannissement pour le canal
Ëßóôá ôùí ban ìáóêşí ôïõ êáíáëéïı
Lista delle banmask per il canale
A csatorna banmaszk listája
Liste over banmasker for kanal
Lista masek blokad dla kana³u
Lista på kanalens banmasker
;
MSG_REMOVING_BAN
Removing banmask
Bannmasken entfernen
Eliminando prohibiciones
Suppression du masque de bannissement
Áöáßñåóç ban ìÜóêáò
Rimuovi banmask
Banmaszk eltávolítása
Fjerner banmaske
Usuwanie maski blokady
Ta bort banmask
;
MSG_TRANSFERRING
Transferring
Übertragen
Transfirendo
Transfert en cours
ÌåôáöïñÜ
Trasferimento dati
Átvitel
Overfører
Przesy³anie
Överför
;
MSG_SYNTAX
Syntax
Schreibweise
Sintáxis
Syntaxe
Óıíôáîç
Sintassi
Szintaxis
Syntaks
Sk³adnia
Syntax
;
MSG_STATUS
Status
Status
Estado
Statut
ÊáôÜóôáóç
Stato
Állapot
Status
Status
Status
;
MSG_SENT
Amount sent
Bereits übertragen
Enviado
Envoyé
Áğåóôáëìİíï ìİãåèïò
QuantitÃ Â trasmessa
Elküldött mennyiség
Sendt
Wys³ano
Skickat
;
MSG_FILE_SIZE
Size
Größe
Tamaño
Taille
Ìİãåèïò
Dimensione
Méret
Filstørrelse
Wielko¶æ
Storlek
;
MSG_TIME_LEFT
Time Left
Vergangene Zeit
Restan
Temps restant
×ñüíïò ğïõ áğïìİíåé
Tempo rimanente
Hátralévõ idõ
Gjenværende tid
Pozosta³o czasu
Tid kvar
;
MSG_RECIEVED
Recieved
Empfangen
Recibido
Reçu
ĞáñåëŞöèçóáí
Ricevuto
Fogadott
Mottatt
Odebrano
Mottaget
;
MSG_REMOVING_BAN_FOR_CHANNEL
for channel\x20
Für Kanal\x20
para el canal\x20
pour le canal\x20
ãéá ôï êáíÜëé\x20
Per canale\x20
csatornának\x20
for kanal\x20
dla kana³u\x20
För kanal\x20
;
MSG_LOGGING_STARTED_AT
Logging started at\x20
Mitschnitt begonnen bei\x20
Registro iniciado\x20
Historisation débutée à\x20
Ç êáôáãñáöŞ Üñ÷éóå óôéò\x20
Protocollo iniziato al\x20
Naplózás megkezdõdött\x20
Logging startet\x20
Raportowanie rozpoczêto o\x20
Loggning startade\x20
;
MSG_HAS_BEEN_INVITED_TO_CHANNEL
has been invited to channel\x20
wurde in den Kanal eingeladen\x20
ha sido invitado la canal\x20
a été invité sur le canal\x20
ğñïóåêëŞèåé óôï êáíÜëé\x20
Ãˆ stato invitato nel canale\x20
meghívást kapott ebbe a csatornába:\x20
har blitt invitert til kanal\x20
zosta³ zaproszony na kana³\x20
har blivit inbjuden till kanal\x20
;
MSG_INVITED_BY_USER
by user
von User
por
par l'utilisateur
áğü ôïí ÷ñŞóôç
da utente
meghívó:
av bruker
przez u¿ytkownika
av användare
;
MSG_YOU_HAVE_INVITED
you have invited\x20
du wurdest eingeladen\x20
ha invitado a\x20
vous avez invité\x20
ğñïóêáëİóáôå\x20
Sei stato invitato\x20
meghívásod küldõje:\x20
du har invitert\x20
zosta³e¶ zaproszony\x20
Du har bjudit in\x20
;
MSG_TO_CHANNEL
to channel\x20
in den Kanal\x20
al canal\x20
sur le canal\x20
óôï êáíÜëé\x20
nel canale\x20
ebbe a csatornába:\x20
til kanal\x20
na kana³\x20
till kanal\x20
;
MSG_TOPIC_SET_BY
set by\x20
von\x20
establecida por\x20
entré par\x20
ìğŞêå áğü ôïí\x20
di\x20
beállította:\x20
satt av\x20
ustawiony przez\x20
satt av\x20
;
MSG_TOPIC_DATE
on\x20
am\x20
el\x20
le\x20
óôéò\x20
al\x20
ekkor:\x20
på\x20
dnia\x20
den\x20
;
MSG_NICK_IN_USE_RETRYING_WITH
in use, retrying with\x20
Wird benutzt, wieder versuchen mit\x20
en uso, reintentándolo con\x20
est déjà utilisé, nouvelle tentative avec\x20
óå ÷ñŞóç, ğñïóğÜèåéá ìå ôï\x20
in uso, riprova con\x20
már hasznát, próbálkozom ezzel:\x20
i bruk, prøver igjen med\x20
jest ju¿ u¿ywany, próbujê ponownie u¿ywaj±c\x20
är i bruk, försöker med\x20
;
MSG_JOIN
Join
Verbunden
Acceder
Entrée
¸íôáîç
Entra
Csatlakozás
ankommer
Join
Anslut
;
MSG_PART
Part
Part
Abandonar
Sortie
Ìİñïò
Part
Szétbomlás
forlater
Part
Lämna
;
MSG_QUIT
Quit
Beenden
Salir
Quitte
¸îïäïò
Esci
Kilépés
Avslutte
Quit
Avsluta
;
MSG_USER_WAS_KICKED_BY
was kicked by
wurde gekickt von
fue expulsado por
a été kické par
êëùôóŞèçêå áğü ôïí
Ãˆ stato espulso da
-et kirúgta:
ble kicket av
zosta³ wyrzucony przez
utsparkad av
;
MSG_YOU_WERE_KICKED_BY
You were kicked by
Du wurdest gekickt von
Ha sido expulsado por
Vous avez été kické par
êëùôóŞèçêåò áğü ôïí
Sei stato espulso da
Téged kirúgott:
Du ble kicket av
Zosta³e¶ wyrzucony przez
Du har blivit utsparkad av
;
MSG_IS_NOW_KNOWN_AS
is now known as\x20
ist bekannt als\x20
cambia el alias por\x20
est maintenant connu sous le nom de\x20
åßíáé ãíùóôüò ùò\x20
Ãˆ conosciuto come\x20
ezután\x20
er nå kjent som\x20
jest teraz znany jako\x20
är nu känd som\x20
;
MSG_PLAYS
plays
Spielt
reproducir
joue
ğáßæåé
Suona
lejátsza ezt:
spiller
odtwarza
spelar
;
MSG_ATTEMPTING_TO_RESUME
Attempting to resume
Versuchen wieder aufzunehmen
Intentando reanudar
Tentative de reprise
ĞñïóğÜèåéá åğáíÜêôçóçò
Tentativo di riconnessione
Megpróbálom folytatni a
Forsøker å fortsette
Próba wznowienia
Försöker återuppta
;
MSG_ATTEMPT_TO_ACCEPT
Attempting to accept DCC CHAT request
Versuchen DCC Chat Anfrage anzunehmen
Intentando aceptar petición DCC CHAT
Tentative d'acceptation d'une requête DCC CHAT
ĞñïóğÜèåéá áğïäï÷Şò êëŞóçò ãéá DCC CHAT
Tentativo di accettazione della richiesta DCC CHAT
Kísérlet DCC CHAT kérés elfogadására
Forsøker å godta forespørsel om DCC CHAT
Próba zaakceptowania ¿±dania DCC CHAT
Försöker acceptera DCC-chat
;
MSG_OFFERED_FROM
offered from
Angeboten von
ofrecido por
offerte par
ğïõ ğñïóöİñèçêå áğü ôïí
offerta da
fájlt felajánlotta:
tilbudt fra
zaoferowany przez
Erbjuden av
;
MSG_TOPIC_CHANGED
changes the topic to
Thema ändern auf
cambia la temática a
change le sujet en
áëëÜæåé ôï èİìá óå
Cambia topic in
megváltoztatta a témát erre:
forandrer overskrift til
zmieni³ temat na
ändrar topic till
;
MSG_FINISHED_SENDING
Finished sending\x20
Senden beendet\x20
Finalizado envío\x20
Fin de l'envoi de\x20
Ôİëïò áğïóôïëŞò\x20
Trasmissione terminata\x20
Küldés befejezve\x20
Sluttført sending av\x20
Zakoñczono wysy³anie\x20
Sändning slutförd\x20
;
MSG_FINISHED_RECIEVING
Finished recieving
Empfang beendet
Finalizada recepción
Fin de la réception de
Ôİëïò ëŞøçò
Ricezione terminata
Fogadás befejezve
Sluttført mottak av
Zakoñczono odbieranie
Mottagning slutförd
;
MSG_CONNECTED_TO
connected to\x20
Verbunden mit\x20
conectado a\x20
connecté à\x20
óõíäİèçêå óôï\x20
Connesso a\x20
Kapcsolódva ide:\x20
tilkoblet til\x20
po³±czony z\x20
Ansluten till\x20
;
MSG_AT_AN_AVERAGE_SPEED_OF
Average speed:
Durchschnittsgeschwindigkeit:
CPS:
Vitesse moyenne :
Ôá÷ıôçôá êáôÜ ìİóù üñï:
VelocitÃ Â media:
Átlagos sebesség:
Gjennomsnittlig fart
¦rednia prêdko¶æ:
Genomsnittslig hastighet
;
MSG_DCC_COMPLETE
Complete
Komplett
Completado
Complet
Ïëüêëçñï
Completo
Befejezve
Ferdig
zakoñczono pomy¶lnie
Komplett
;
MSG_DCC_INCOMPLETE
Incomplete
Nicht  komplett
Incompleto
Incomplet
ÇìéôåëŞò
Incompleto
Hiányos
Uferdig
niekompletny
Icke komplett
;
MSG_DCC_CANCELLED
Cancelled
Abgebrochen
Cancelado
Annulé
Áêõñşèçêå
Interrotto
Megszakított
Avbrutt
poniechano
Avbruten
;
MSG_CONNECTED
Connected
Verbunden
Conectado
Connecté
Óõíäåäåìİíï
Connesso
Kapcsolódva
Tilkoblet
po³±czono
Ansluten
;
MSG_DISCONNECTED
Disconnected
Nicht verbunden
Desconectado
Déconnecté
Áğïóõíäåäåìİíï
Disconnesso
Leválasztva
Avkoblet
roz³±czono
Bortkopplad
;
MSG_DCC_FAILED
Failed
Fehlgeschlagen
fallido
Échec
Áğİôõ÷å
Fallito
Sikertelen
Feilet
nie powiod³o siê
Misslyckades
;
MSG_CREATE_FILE_FAILED
unable to create new file for incoming transfer
Erstellen einer neuen Datei für ankommenden Transfer nicht möglich
no se ha podido crear fichero recibido
Impossible de créer un nouveau fichier pour le transfert entrant
áäıíáôç ç äçìéïõñãßá ôïõ íİïõ áñ÷åßïõ ãéá ôçí åéóåñ÷üìåíç ìåôáöïñÜ
Impossibile creare nuovo file per la corrente ricezione
nem tudom létrehozni az érkezõ átvitelhez a fájlt
Kan ikke skape ny fil for innkommende overførsel
nie mo¿na utworzyæ nowego pliku dla przesy³anego pliku
Kunde inte skapa ny fil för inkommande sändning
;
MSG_OPEN_FILE_FOR_DCC_SEND_FAILED
unable to open requested file
Öffnen der gewählten Datei nicht möglich
no se ha podido abrir fichero elegido
Impossible d'ouvrir le fichier demandé
áäıíáôï ôï Üíïéãìá ôïõ áñ÷åßïõ
Impossibile aprire il file richiesto
a kért fájlt nem tudom megnyitni
Kan ikke åpne forespurt fil
nie mo¿na otworzyæ ¿±danego pliku
Kunde inte öppna begärd fil
;
MSG_DCC_STARTED_FILE_TRANSFER
Started file transfer:
Dateitransfer gestartet:
Iniciada transferencia:
Transfert de fichier commencé :
Åêêßíçóç ìåôáöïñÜò áñ÷åßïõ:
Iniziata la trasmissione:
Elkezdett fájl átvitel:
Startet filoverførsel:
Rozpoczêto przesy³anie pliku:
Påbörjade filöverföring:
;
MSG_UNABLE_TO_CONNECT
Unable to establish DCC CHAT Connection
DCC Verbindung nicht möglich
No se ha podido establecer DCC CHAT
Impossible d'étable une connexion DCC CHAT
Áäıíáôç ç åğßôåõîç óıíäåóçò DCC CHAT
Inabile di effettuare la connessione DCC CHAT
DCC CHAT kapcsolatot nem tudok létrehozni
Kan ikke opprette forbindelse for DCC CHAT
Nie mo¿na ustanowiæ po³±czenia DCC CHAT
Kunde inte etablera anslutning för DCC-chat
;
MSG_CONNECTED_DCC_CHAT
DCC CHAT Connection Established
DCC Chat Verbindung hergestellt
Establecida DCC CHAT
Connexion DCC CHAT établie
Ç óıíäåóç DCC CHAT åğéôåı÷èåé
Connessione DCC CHAT effettuata
DCC CHAT kapcsolat létrehozva
Forbindelse opprettet for DCC CHAT
Ustanowiono po³±czenie DCC CHAT
Anslutning för DCC-chat etablerad
;
MSG_TRANSFER_TIME
Transfer time:
Übertragungsdauer:
Estimado
Temps de transfert :
×ñüíïò ìåôáöïñÜò:
Tempo di trasferimento:
Átviteli idõ:
Overføringstid:
Czas przesy³ania
Överföringstid:
;
MSG_FILE_DOESNT_EXIST
Error, file doesnt exist
Fehler, Datei existiert nicht
¡ERROR! Fichero no existe
Erreur, le fichier n'existe pas
ÓöÜëìá, ôï áñ÷åßï äåí õğÜñ÷åé
Errore, il File non esiste
Hiba, a fájl nem létezik
Feil, filen eksisterer ikke
B³±d, plik nie istnieje
Fel, fil finns inte
;
MSG_OFFERING_FILE
Offering file
Datei anbieten
Ofreciendo
Offre du fichier
ĞñïóöïñÜ áñ÷åßïõ
Offerta File
Fájl felajánlása
Tilbyr fil
Oferowanie pliku
Erbjuder fil
;
MSG_OFFERED_ON
on
auf
en
le
óå
alle
itt:
på
wykorzystuj±c
via
;
MSG_OFFERED_TO
to
an
a
à
óôïí
a
neki:
til
do
till
;
MSG_DCC_CHAT_REQUEST
Sending DCC CHAT request
DCC Chat-Anfrage senden
Enviando petición DCC CHAT
Envoi d'une requête DCC CHAT
ÁğïóôïëŞ êëŞóçò DCC CHAT
Invio richiesta DCC CHAT
DCC CHAT kérés küldése
Sender forespørsel om DCC CHAT
Wysy³anie ¿±dania DCCCHAT
Skickar förfrågan om DCC-chat
;
MSG_COLOUR_SETTINGS_LOAD_ERROR
Unable to load colour settings, using default settings. To avoid seeing this message again, create a drawer called 'Colours' inside WookieChat's drawer, then open and close the Colour Settings window \
the next time you run WookieChat.
Farbeinstellungen konnten nicht geladen werden, darum werden Grundeinstellungen benutzt. Um diese Anzeige nicht wieder zu sehen, bitte einen Ordner namens 'Colours' im Verzeichnis WookieChat \
erstellen, dann beim nächsten Start von WookieChat den Farbeinsteller einmal öffnen und schließen.
No se han podido cargar los ajustes de colores,\nusando ajustes por omisión. Para evitar este\naviso, cree un cajón llamado "Colours" en\ndonde reside 'WookieChat' y reinícielo.
Impossible de charger les réglages de couleur, utilisation des défauts. Pour éviter de revoir ce message, veuillez créer un tiroir nommé 'Colours' dans celui de WookieChat, puis ouvrez et fermez la \
fenêtre des réglages de couleur lors de votre prochaine utilisation de WookieChat.
Áäıíáôç ç öüñôùóç ôùí ÷ñùìáôéêşí ñõèìßóåùí, ãßíåôáé ÷ñŞóç ğñïåğéëåãìİíùí. Ãéá íá ìç îáíáäåßôå áõôü ôï ìŞíõìá, äçìéïõñãŞóôå İíáí öÜêåëï ïíüìáôé 'Colours' ìİóá óôïí êáôÜëïãï ôïõ WookieChat. ¸ğåéôá \
áíïßîôå êáé êëåßóôå ôï ğáñÜèõñï ôùí ÷ñùìáôéêşí ñõèìßóåùí ôçí åğüìåíç öïñÜ ğïõ èá îáíáôñİîåôå ôï WookiChat.
Impossibile caricare le impostazione dei colori, verrÃ  utilizzato lo schema di default. Per evitare questo\nmessaggio, crea un cassetto 'Colors' nel cassetto di WookieChat\npoi apri e richiudi la \
finestra delle impostazioni dei colori la prossima volta che avvii WookieChat.
A színbeállítások nem tölthetõek be, alapértékek kerülnek felhasználásra. A hiba elkerüléséhez hozz létre egy "Colours" fiókot a WookieChat fiókban, majd a program legközelebbi futtatásakor nyisd meg \
és zárd be a Színbeállítások ablakot.
Kan ikke hente fargeinnstillinger, bruker standard innstillinger. For å unngå å se denne beskjeden igjen, lag en mappe kallt 'Colours' inne i WookieChat sin mappe, åpne og avslutte deretter \
Fargeinnstillinger-vinduet neste gang du kjører WookieChat.
Nie mo¿na wczytaæ ustawieñ kolorów, wykorzystane zostan± ustawienia domy¶lne. Aby zapobiec ponownemu pojawieniu siê tego komunikatu, utwórz katalog "Clours" wewn±trz katalogu z programem, nastêpnie \
przy kolejnym uruchomieniu programu otwórz i zamknij okno ustawieñ kolorów.
Kunde inte ladda färginställningarna, använder standard istället. För att slippa se detta meddelande igen, skapa en låda kallad 'Colours' i WookieChat's låda, öppna och stäng Färginställningsfönstret \
nästa gång du kör WookieChat.
;
MSG_COLOUR_SETTINGS_SAVE_ERROR
Unable to save your colour settings. Make sure you have a drawer called 'Colours' inside WookieChat's drawer, for it to save your colour preferences to.
Kann Farbeinstellungen nicht speichern. Bitte sicherstellen, dass ein Verzeichnis namens 'Colours' im Ordner WookieChat vorhanden ist um die Einstellungen sichern zu können.
No se han podido guardar los ajustes de colores.\nAsegúrese de tener un cajón llamado "Colours"\nen donde reside 'WookieChat' para poder guardarlos.
Impossible d'enregistrer vos réglages de couleur. Veuillez vous assurer qu'un tiroir nommé 'Colours' existe dans celui de WookieChat pour que vos réglages puissent y être enregistrés.
Áäıíáôç ç áğïèŞêåõóç ôùí ÷ñùìáôéêşí óáò ñõèìßóåùí. Óéãïõñåõôåßôå üôé İ÷åôå İíáí öÜêåëï ìå ôï üíïìá 'Colours' óôïí êáôÜëïãï ôïõ WookieChat, üğïõ èá óùèïıí ïé ÷ñùìáôéêİò óáò ñõèìßóåéò.
Impossibile salvare le impostazioni dei colori. Assicurati di avere un cassetto chiamato 'Colours'\nnel cassetto WookieChat dove verranno salvate le impostazioni.
Nem tudom elmenteni a színbeállításaidat. Gyõzõdj meg róla, hogy a WookieChat fiók tartalmaz-e "Colours" fiókot a színbeállítások tárolására.
Kan ikke lagre dine fargeinnstillinger. Vær sikker på at du har en mappe kallt 'Colours" inne i WookieChat sin mappe, slik at dine fargepreferanser blir lagret.
Nie mo¿na zapisaæ Twoich ustawieñ kolorów. Upewnij siê, i¿ posiadasz katalog "Colours" w katalogu z programem, aby mo¿na by³o w nim zapisaæ Twoje ustawienia kolorów.
Kunde inte spara färginställningarna. Försäkra dig om att du har en låda kallad 'Colours' i WookieChat's låda, att spara inställningarna i.
;
MSG_UNABLE_TO_OPEN_TCPIP_STACK
Please start a TCP/IP stack and try again.
Bitte TCP/IP Stack starten und noch einmal versuchen.
¡Debe tener activa una pila TCP/IP.
Veuillez démarrer la pile TCP/IP et réessayer.
ÎåêéíŞóôå ôï TCP/IP êáé îáíáğñïóğáèŞóôå.
Avvia una connessione TCP/IP e riprova.
Kérlek indíts el egy hálózatkezelõt, majd próbálkozz újra.
Vennligst start en TCP/IP-stakk og prøv igjen.
Proszê uruchomiæ stos TCP/IP i spróbowaæ ponownie.
Starta TCP/IP-stacken och försök igen.
;
MSG_CONNECT_TO_A_NEW_SERVER
Connecting to a new server
Mit neuem Server verbinden
Conectando a un nuevo servidor
Connexion à un nouveau serveur
Óıíäåóç óå äéáêïìéóôŞ
Connessione ad un nuovo server
Csatlakozás új kiszolgálóhoz
Oppretter forbindelse til ny server
£±czenie z nowym serwerem
Ansluter till ny server
;
MSG_ATTEMPTING_TO_CONNECT_TO_SERVER
Attempting to connect to\x20
Versuche zu verbinden mit\x20
Intentando conectarse a\x20
Tentative de connexion vers\x20
ĞñïóğÜèåéá óıíäåóçò óôï\x20
Sto provando a connettermi a\x20
Csatlakozási kísérlet ide:\x20
Forsøker å forbinde til\x20
Próba po³±czenia z\x20
Försöker ansluta till\x20
;
MSG_CONNECTING
Connecting
Verbindung aufnehmen
Conectando
Connexion en cours
Óıíäåóç
Connessione
Kapcsolódás
Oppretter forbindelse
£±czenie
Ansluter
;
MSG_FILE_ALREADY_EXISTS
already exists!
Existiert bereits!
ya existe.\n
existe déjà !
õğÜñ÷åé Şäç
gia esistente!
már létezik!
eksisterer allerede!
ju¿ istnieje!
finns redan!
;
MSG_INCOMING_FILE_SIZE
Size of existing file
Größe der existierenden Datei
\033cFichero existente
Taille du fichier existant
ôï ìİãåèïò ôïõ õğáñêôïı áñ÷åßïõ
Dimensione del file esistente
A meglévõ fájl mérete
Størrelse på eksisterende fil
Wielko¶æ istniej±cego pliku
Storlek på filen som redan finns
;
MSG_FILE_SIZE_OFFERED
Size of file being offered
Größe der angebotenen Datei
\033cFichero ofrecido
Taille du fichier offert
Ôï ìİãåèïò ôïõ áñ÷åßïõ ğïõ ğñïóöİñèçêå
Dimensione del file offerto
A felajánlott fájl mérete
Størrelse på tilbudt fil
Wielko¶æ zaoferowanego pliku
Storlek på erbjuden fil
;
MSG_TO
to
an
a
a
óôï
a
neki:
til
do
till
;
MSG_DCC_WAITING
Waiting
Warte
Esperando
En attente
ÁíáìïíŞ
In attesa
Várakozik
Venter
Oczekiwanie
Väntar
;
MSG_UPDATING_NAMES_LIST
Updating Names list for channel
Namensliste des Kanals wird neu geladen
Actualizando lista de nombres del canal
Mise à jour de la liste des noms pour le canal
Áíáíİùóç ëßóôáò ïíïìÜôùí ãéá ôï êáíÜëé
Aggiornamento della lista nomi per il canale
A következõ csatorna névlistájának frissítése:
Oppdaterer navneliste for kanal
Uaktualnianie listy pseudonimów dla kana³u
Uppdaterar kanalens namnlista
;
MSG_USERS_IN_CHANNEL
Users in channel
Anwesende im Kanal
Usuarios en el canal
Utilisateurs dans le canal
×ñŞóôåò óôï êáíÜëé
Utenti nel canale
Csatorna felhasználói
Brukere i kanal
U¿ytkownicy na kanale
Användare i kanalen
;
MSG_AUTO_REJOINING
Auto-rejoining\x20
Neuverbindung\x20
Auto-reacceder\x20
Reconnexion automatique\x20
Áõôï-İíôáîç\x20
Riconnessione\x20
Automatikus újrakapcsolódás\x20
Auto-entrer\x20
Automatyczne do³±czanie\x20
Återansluter automatiskt\x20
;
MSG_FROM
from
von
de
depuis
áğü
da
tõle:
fra
od
från
;
MSG_SETS_MODE
sets mode
Modus einstellen
establece
positionne le mode
ïñßæåé ôïí ôñüğï
Impostazione modalitÃ 
módbeállítása
setter modus
ustawi³ tryb
sätter mod
;
MSG_PLAYING
playing
Spiele
reproduciendo
joue
ğáßæåé
Suonando
lejátszom
spiller
odtwarzanie
spelar
;
MSG_SENDING_CTCP
Sending CTCP
CTCP senden
Enviando CTCP
Envoi CTCP
ÁğïóôïëŞ CTCP
Invio CTCP
CTCP küldés
Sender CTCP
Wysy³anie CTCP
Skickar CTCP
;
MSG_WINDOWS_MENU
Windows
Fenster
Ventanas
Fenêtres
ĞáñÜèõñá
Finestre
Ablakok
Vindu
Okna
Fönster
;
MSG_FILENAME
Filename
Dateiname
Fichero
Nom de fichier
¼íïìá áñ÷åßïõ
Nome file
Fájlnév
Filnavn
Nazwa pliku
Filnamn
;
MSG_NICKNAMES
Nick Names
Spitznamen
Aliases:
Pseudonymes
Øåõäşíõìá
NickName
Álnevek
Nicknavn
Pseudonimy
Nicks
;
MSG_OPENING_BRACKET
[
[
[
[
[
[
[
[
[
[
;
MSG_CLOSING_BRACKET
]
]
]
]
]
]
]
]
]
]
;
MSG_NICKLIST_OPTIONS
Nicklist Options
Nutzer Listen Option
Opciones Aliases
Options liste des pseudos
Åğéëïãİò ëßóôáò øåõäùíıìùí
Opzioni Nicklist
Becenév opciók
Nicklistevalg
Opcje listy pseudonimów
Val Nicklista
;
MSG_WHOIS_MENU_ITEM
Whois
Whois
WHOIS
Whois
Ğïéüò åßíáé
Whois
Kicsoda
Whois
Whois
Whois
;
MSG_QUERY_MENU_ITEM
Open Query
Open Query
Preguntar
Ouvrir une requête
¶íïéãìá åñùôŞìáôïò
Apri query
Panelt nyit
Åpne spørring
Otwórz zak³adkê rozmowy
Öppna förfrågan
;
MSG_GLOBAL_QUERY_MENU_ITEM
Open Global Query
Open Global Query
Preguntar Global
Ouvrir une requête globale
¶íïéãìá ãåíéêïı åñùôŞìáôïò
Apri richiesta globale
Globális panelt nyit
Åpne global spørring
Otwórz globaln± zak³adkê rozmów
Öppna Global förfrågan
;
MSG_CTCP_MENU_TITLE
CTCP
CTCP
CTCP
CTCP
CTCP
CTCP
CTCP
CTCP
CTCP
CTCP
;
MSG_PING_MENU_ITEM
Ping
Ping
PING
Ping
Ping
Ping
Ping
Ping
Ping
Ping
;
MSG_VERSION_MENU_ITEM
Version
Version
Versión
Version
¸êäïóç
Versione
Verzió
Version
Wersja
Version
;
MSG_TIME_MENU_ITEM
Time
Zeit
Hora
Time
×ñüíïò
Ora
Idõ
Time
czas
Tid
;
MSG_DIRECT_CONNECTION_MENU_TITLE
Direct Connection
Direkte Verbindung
Conexión Directa
Connexion directe
Áğåõèåßáò óıíäåóç
Connessione diretta
Közvetlen kapcsolat
Direkte forbindelse
Bezpo¶rednie po³±czenie
Direktanslutning
;
MSG_SEND_FILE_MENU_ITEM
Send file
Datei senden
Enviar fichero
Envoyer un fichier
ÁğïóôïëŞ áñ÷åßïõ
Invio file
Fájl küldése
Send fil
Wy¶lij plik
Skicka fil
;
MSG_CHAT_MENU_ITEM
Chat
Chat
Charlar
Chat
Óõíïìéëßá
Chat
Csevegõ
Chat
Rozmowa
Chat
;
MSG_CONTROL_MENU_TITLE
Control
Kontrolle
Control
Contrôle
Åğéëïãİò
Controllo
Vezérlõ
Kontroll
Kontrola
Kontroll
;
MSG_OP_MENU_ITEM
Op
OP
OP
Op
Äéá÷
OP
Op
Op
Op
Op
;
MSG_DEOP_MENU_ITEM
DeOp
DeOp
Quitar OP
DeOp
Áê. Äéá÷
DeOP
Nem Op
DeOp
DeOp
DeOp
;
MSG_HALFOP_MENU_ITEM
HalfOp
HalfOp
HalfOP
HalfOp
Ìéóüò äéá÷
HalfOp
HalfOp
HalfOp
HalfOp
HalfOp
;
MSG_DEHALFOP_MENU_ITEM
DeHalfOp
DeHalfOp
Quitar HalfOP
DeHalfOp
Áê. Ìéó. Äéá÷
DeHalfOp
Nem HalfOp
DeHalfOp
DeHalfOp
DeHalfOp
;
MSG_VOICE_MENU_ITEM
Voice
Voice
Voz
Voice
Ïìéëßá
Voce
Voice
Voice
Voice
Voice
;
MSG_DEVOICE_MENU_ITEM
DeVoice
DeVoice
Quitar voz
DeVoice
Áê. Ïìéëßáò
DeVoice
Nem Voice
DeVoice
DeVoice
DeVoice
;
MSG_KICK_MENU_ITEM
Kick
Kick
Expulsar
Kick
ÊëùôóéÜ
Kick
Kirúg
Kick
Kick
Kicka
;
MSG_BAN_MENU_ITEM
Ban
Bannen
Prohibir
Ban
Ban
Ban
Banol
Ban
Ban
Ban
;
MSG_SENDER
Sender
Sender
Remitente
Expéditeur
Áğïóôïëİáò
Mittente
Küldi
Avsender
Nadawca
Avsändare
;
MSG_RECIEVER
Reciever
Empfänger
Destinatario
Destinataire
Áğïäï÷İáò
Destinatario
Fogadja
Mottaker
Odbiorca
Mottagare
;
MSG_CHANNEL_NAME_HELP
<channelname>
<Kanalname>
<canal>
<nom_du_canal>
<êáíÜëé>
<nome canale>
<csatorna neve>
<kanalnavn>
<nazwa kana³u>
<kanalnamn>
;
MSG_NICK_HELP
<nick>
<Nutzer>
<alias>
<pseudo>
<øåõäşíõìï>
<nick>
<becenév>
<nick>
<pseudonim>
<nick>
;
MSG_COMMAND_NAME_HELP
<command>
<Kommando>
<comando>
<commande>
<åíôïëŞ>
<comando>
<parancs>
<kommando>
<polecenie>
<kommando>
;
MSG_SOUND_SAMPLE_NAME_HELP
<sound_sample_name>
<Sound Schnipsel Name>
<sonido>
<nom_echantillon>
<üíïìá áñ÷åßïõ Ş÷ïõ>
<nome_sound_sample>
<hangminta neve>
<lyd_sample_navn>
<nazwa pliku d¼wiêkowego>
<ljud_snutt_namn>
;
MSG_SOUND_BAN_MASK_HELP
<ban_mask>
<ban_mask>
<máscara_prohibir>
<masque_bannissement>
<ìÜóêá ban>
<ban_mask>
<banmaszk>
<ban_maske>
<maska blokady>
<ban_mask>
;
MSG_MESSAGE_HELP
<message>
<message>
<mensaje>
<message>
<ìŞíõìá>
<messaggio>
<üzenet>
<message>
<wiadomo¶æ>
<meddelande>
;
MSG_NEW_NICK_HELP
<new_nick>
<neuer Nutzername>
<nuevo_alias>
<nouveau_pseudo>
<íİï øåõäşíõìï>
<nuovo_nick>
<új becenév>
<nytt_nick>
<nowy pseudonim>
<nytt_nick>
;
MSG_TOPIC_HELP
<new_topic>
<neues Thema>
<nueva_temática>
<nouveau_sujet>
<íİï èİìá>
<nuovo_titolo>
<új téma>
<ny_overskrift>
<nowy temat>
<ny_topic>
;
MSG_SERVERNAME_HELP
<servername>
<Servername>
<servidor>
<nom_serveur>
<äéáêïìéóôŞò>
<nome_server>
<kiszolgáló neve>
<servernavn>
<nazwa serwera>
<servernamn>
;
MSG_PORT_HELP
<port>
<Port>
<puerto>
<port>
<èıñá>
<port>
<port>
<port>
<port>
<port>
;
MSG_SWEAR_FILTER
SwearFilter
SwearFilter
Filtro palabrotas
Filtre de jurons
Ößëôñï âñéóéşí
SwearFilter
Szitokszûrõ
BanneFilter
FiltrPrzekleñstw
SvärFilter
;
MSG_ENABLED
Enabled
Eingeschaltet
Activado
Activé
Åíåñãü
Abilitato
engedélyezve
Aktivert
w³±czony
Till
;
MSG_DISABLED
Disabled
Abgeschaltet
Desactivado
Désactivé
Áíåíåñãü
Disabilitato
letiltva
Deaktivert
wy³±czony
Från
;
MSG_NICK_OR_CHANNEL_HELP
<nick or channel>
<Nutzer oder Kanal>
<alias o canal>
<pseudo ou canal>
<øåõäşíõìï Ş êáíÜëé>
<nick o canale>
<becenév vagy csatorna>
<nick eller kanal>
<pseudonim lub kana³>
<nick eller kanal>
;
MSG_FILENAME_HELP
<filename>
<Dateiname>
<fichero>
<fichier>
<üíïìá áñ÷åßïõ>
<nome_file>
<fájlnév>
<filnavn>
<nazwa pliku>
<filnamn>
;
MSG_ACTION_HELP
<action>
<Aktion>
<acción>
<action>
<åíİñãåéá>
<azione>
<cselekmény>
<aksjon>
<dzia³anie>
<action>
;
MSG_USERS
users
Anwesend
usuarios
utilisateurs
<÷ñŞóôåò>
Utenti
felhasználó
brukere
u¿ytkowników
användare
;
MSG_HIGHLIGHT_STRINGGADGET_HELP_BUBBLE
Enter words here for WookieChat to watch for. Lines of text will appear in the highlight colour if one of these words is found. Enter the words you want to highlight, separated by spaces.
Worte die Wookiechat beobachten soll, hier eintragen. Wenn die entsprechenden Worte gefunden wurden, wird die ganze Zeile gekennzeichnet. Mehrere Worte werden nur durch ein Leerzeichen getrennt.
Palabras con las que se resaltarán las líneas\nen las que aparecen. Puede añadir tantas como\ndesee, separándolas por espacios.
Entrez les mots, séparés par des virgules, que WookieChat doit scruter. Les lignes dans lesquelles l'un de ces mots apparaîtra seront affichées avec la couleur de mise en évidence.
ÅéóÜãåôå ëİîåéò ğïõ èá ğáñáêïëïõèåß ôï WookieChat. Ïé ãñáììİò êåéìİíïõ èá åìöáíéóôïıí ìå ôï ÷ñşìá ôïíßóìáôïò áí ìßá áğü áõôİò ôéò ëİîåéò âñåèåß. ÅéóÜãåôå ôéò ëİîåéò ğïõ èİëåôå íá ôïíßóåôå, ÷ùñéóìİíåò \
ìå êåíü.
Inserisci qui le parole che WookieChat dovrÃ Â filtrare. Le linee di testo appariranno evidenziate se una di queste parole sarÃ Â presente. Inserisci le parole separate da spazi.
Adj meg szavakat, melyeket a WookieChat figyel. Ezen szavakat tartalmazó sorok a kiemelt színnel jelennek meg. Add meg a kiemelendõ szavakat, szóközkkel elválasztva.
Skriv inn ord som WookieChat skal se etter. Tekslinjene med de aktuelle ordene vil bli highlightet. Skriv inn ordene du vil ha highlightet, separert med åpenrom.
Tutaj wpisz s³owa, na które ma program zwracaæ uwagê. Linia tekstu zostanie wy¶wietlona w kolorze pod¶wietlenia, je¶li choæ jedno z tych s³ów zostanie odnalezione. Wprowad¼ s³owa, które chcesz \
pod¶wietlaæ oddzielaj±c je spacjami.
Skriv ord här för WookieChat att observera. Om ett av dessa ord upptäcks kommer meningen att markeras. Skriv in de ord du vill markera, separerade med mellanslag.
;
MSG_RESUME_FILE
RESUME file
Datei anfügen
REANUDAR fichero
REPRENDRE fichier
Óõíİ÷éóç ôïõ áñ÷åßïõ
Riavvia file
Fájl folytatása
Fortsett filen
Wznowienie pliku
Återuppta fil
;
MSG_PREFS_SERVERS_GROUP
Servers
Server
Servidores
Serveurs
Äéáêïìéóôİò
Server
Kiszolgálók
Servere
Serwery
Servrar
;
MSG_MAX_RECONNECT_RETRIES
Maximum attempts to reconnect after non-requested disconnection
Maximale Neuverbindungs-Versuche nach unfreiwilliger Trennung
Intentos máx. reconectar al auto-desconectarse
Nombre de tentatives de reconnexion maximum après une déconnexion impromptue
Ìİãéóôïò áñéèìüò ğñïóğáèåéşí åğáíáóıíäåóçò ìåôÜ áğü áğïóıíäåóç
Limite max tentativi di riconnessione dopo una sconnessione non richiesta
Újracsatlakozási próbálkozások száma egy váratlan kapcsolat bomlás után
Maksimalt antall reoppkoblingsforsøk etter ikke etterspurt utkobling
Maksymalna liczba prób ponowienia po nieporz±danym od³±czeniu
Max antal försök att återansluta efter oavsiktlig nedkoppling
;
MSG_RECONNECT_DELAY
Delay (in seconds) in between reconnection attempts
Wartezeit (in Sekunden) zwischen Neuverbindungsversuchen
Espera entre reconexiones (segundos)
Délai (en secondes) entre les tentatives de reconnexion
Êáèõóôİñçóç (óå äåõô.) ìåôáîı ôùí ğñïóğáèåéşí åğáíáóıíäåóçò
Ritardo in secondi tra i tentativi di riconnessione
Késleltetés (másodpercekben) az újracsatlakozási kísérletek között
Pause (i sekunder) mellom reoppkoblingsforsøk
Opó¼nienie (w sekundach) pomiêdzy kolejnymi próbami ponownego po³±czenia
Fördröjning (i sekunder) mellan återanslutningsförsök
;
MSG_RETRY
Retry #
Versuche #
Reintento #
Essai #
ĞñïóğÜèåéá #
Tentativo #
Próbálkozás #
Gjenta #
Próba #
Försök #
;
MSG_IGNORE_MASK
Ignore Mask
Ignorieren Maske
Máscara a Ignorar
Ignore Mask
ÌÜóêáò áãíüçóçò
Ignora Mask
Figyelmen kívül
Ignorer-maske
Maska ignorowania
Ignoreringsregel
;
MSG_IGNORE_TEXT
Text
Text
Texto
Texte
Êåßìåíï
Testo
Szöveg
Tekst
Tekst
Text
;
MSG_SERVERS_WINDOW_SERVERS
Servers
Server
Servidor
Serveurs
Äéáêïìéóôİò
Servers
Kiszolgálók
Servere
Serwery
Servrar
;
MSG_SERVERS_WINDOW_PORT
Port
Port
Puerto
Port
Èıñá
Port
Port
Port
Port
Port
;
MSG_SERVERS_WINDOW_AUTOCONNECT
Auto-Connect
Automatische Verbindung
Auto-Conectar
Connexion Automatique
Áõôï-İíôáîç
Auto-connessione
Autom. kapcsolódás
Auto-oppkoble
Po³±cz automatycznie
Anslut automatiskt
;
MSG_SERVERS_WINDOW_PASSWORD
Password
Passwort
Contraseña
Mot de passe
êùäéêüò
Password
Jelszó
Passord
Has³o
Lösenord
;
MSG_IGNORE_LONG_DETAILED_HELP
Hostmarks look like this: nick!userid@hostname\ni.e.«Join» jahc (~jahc00@jahc.Amigaworld.Net)\njahc is the nick, jahc00 is the userid, and jahc.Amigaworld.Net is the hostname\nTo ignore this person, \
you could insert an ignore entry of: jahc*!*@*\nThis will ignore all people with a nick starting with the letters jahc.\nThe use of wildcards makes this ignore command very powerful.\nThings like this \
are possible: guru*editatio*!*userid@*.se\nor if you want to keep it simple, you could just use: jahc*
Hostmarks look like this: nick!userid@hostname\ni.e.«Join» jahc (~jahc00@jahc.Amigaworld.Net)\njahc is the nick, jahc00 is the userid, and jahc.Amigaworld.Net is the hostname\nTo ignore this person, \
you could insert an ignore entry of: jahc*!*@*\nThis will ignore all people with a nick starting with the letters jahc.\nThe use of wildcards makes this ignore command very powerful.\nThings like this \
are possible: guru*editatio*!*userid@*.se\nor if you want to keep it simple, you could just use: jahc*
\033cLas marcas de anfitrión suelen ser: alias!usuarioID@anfitrión\nEjemplo: .. [Acceder] jahc (~jahc00@Amigaworld.Net) ..\n\033bjahc\033n es el alias, \033bjahc00\033n el usuarioID y \
\033bAmigaworld.Net\033n el anfitrión.\nPara ignorar a esta persona, puede insertar la cadena: \033bjahc*!*@*\033n\nEl uso de comodines hace que este comando sea muy potente.\nPuede hacer cosas como: \
\033bguru*editatio*!*usuarioID@*.se\033n\no si desea hacerlo más simple: \033broberto*\033n
Les masques d'hôte ressemblent à ceci : \npseudo!utilisateur@machine\ni.e. [Entrée] jahc (~jahc00@jahc.Amigaworld.Net)\njahc est le pseudo, jahc00 l'utilisateur et\njahc.Amigaworld.Net le nom d'hôte.\n\
Pour ignorer cette personne, vous pouvez insérer\nle masque jahc*!*, cela ignorera toute personne\ndont le pseudo commence par 'jahc'.\nL'utilisation de jokers rend cette commande très\npuissante, des \
choses telles que guru*editatio*!*userid@*.se\nsont possibles ou pour faire plus simple vous pouvez\nutiliser roberto*
Ôï ìáñêÜñéóìá ãßíåôáé êÜğùò İôóé: nick!userid@hostname\nğ.÷. «Join» jahc (~jahc00@jahc.Amigaworld.Net)\nôï jahc åßíáé ôï øåõäşíõìï, ôï jahc00 åßíáé ôï üíïìá ÷ñŞóôç, åíş ôï jahc.Amigaworld.Net åßíáé ôï \
hostname\nÃéá íá áãíïŞóåôå áõôü ôï Üôïìï, èá ìğïñïıóáôå íá åéóÜãåôå ìßá êáôá÷şñçóç áãíüçóçò üğùò: jahc*!*@*\nÁõôü èá áãíïŞóåé üëïõò ìå øåõäşíõìï ğïõ áñ÷ßæåé ìå ôá ãñÜììáôá jahc.\nÇ ÷ñŞóç ôùí ìğáëáíôİñ \
êÜíåé áõôŞ ôç ëåéôïõñãåßá ğïëı äõíáôŞ. Ìğïñåßôå íá ÷ñçóéìïğïéŞóåôå ğñÜãìáôá üğùò: guru*editatio*!*userid@*.se\nŞ áí èİëåôå íá ôï êñáôŞóåôå áğëü, ìğïñåßôå íá ãñÜøåôå áğëÜ: roberto*
Le Hostmarks sono composte come segue: nick!userid@hostname\ni.e.[Join] jahc (~jahc00@jahc.Amigaworld.Net)\njahc Ã¨ il nick, jahc00 Ã¨ la userid e jahc.Amigaworld.Net Ã¨ la persona\nda ignorare \
dall'hostname, puoi inserire un ignore tipo: jahc*!*@*\nQuesto ignorerÃ Â  tutte le persone con un nick che inizia con le lettere jahc.\nL'uso delle WildCards rende il comando ignore molto \
potente.\nSono possibili anche se seguenti combinazioni: guru*editatio*!*userid@*.se\no se vuoi renderti la vita piÃ¹ semplice potresti usare: roberto*
A hoszt jelzõk így néznek ki: becenév!azonosító@hosztnév\ni.e«Join» jahc (~jahc00@jahc.Amigaworld.Net)\njahc az becenév, jahc00 az azonosító és jahc.Amigaworld.Net a hoszt név\nBeszúrva egy mellõzõ \
tagok, ez a személy mellõzhetõ: jahc*!*@*\nEz mindenkit mellõzni fog akinek az álneve jahc, vagy azzal kezdõdik.\nA helyettesítõ karakterek használata igen hatékonnyá teszi ezt a parancsot.\nAkár a \
következõket is megtehetjük: guru*editatio*!*userid@*.se\nvagy ha egyszerüsíteni akarjuk, akár csak enyivel: jahc*
Hostmerkene ser slik ut: nick!userid@hostname\nfor eks. «Join» jahc (~jahc00@jahc.Amigaworld.Net)\njahc er nicket, jahc00 er brukerens id, og jahc.Amigaworld.Net er hostnavnet\nFor å overse denne \
personen, kan du sette en ignoremaske som dette: jahc*!*@*\nDette vil overse alle som har et nick som starter med "jahc".\nBruken av wildcards gjør denne ignoremasken veldig kraftfull.\nTing som dette \
er mulig: guru*editatio*!*userid@*.se\neller hvis du vil gjøre det enkelt, kan du bare bruke for eksempel: roberto*
Maska wygl±da tak: pseudonim!identyfikator@wêze³\nnp. [Join] jahc (~jahc00@jahc.Amigaworld.net)\njahc to pseudonim, jahc00 to identyfikator\na jahc.Amigaworld.net to nazwa wêz³a.\nAby ignorowaæ tê \
osobê, mo¿esz dodaæ do listy jahc*!*@*\nWookieChatten sposób bêd± ignorowane wszystkie osoby\nz pseudonimem zaczynaj±cym siê od liter jahc.\nU¿ywanie znaków zastêpczych pozwala na tworzenie z³o¿onych \
regu³.\nMo¿na np. utworzyæ tak± regu³ê: guru*editatio*!*userid@*.se\nlub je¶li chcesz sobie upro¶ciæ ¿ycie, mo¿esz u¿yæ po prostu: jahc*
Värdinfo ser ut så här: nick!userid@hostname\ndvs.Join jahc (~jahc00@jahc.Amigaworld.Net)\njahc är nicket, jahc00 är anvädarid och jahc.Amigaworld.Net är värdnamnet\nFör att ignorera användaren kan du \
skriva in jahc*!*@*\nDetta ignorerar alla nick som börjar med bokstäverna jahc.\nMed wildcards (*) blir detta kommando mycket kraftfullt.\nMöjliga kombinationer: guru*editatio*!*userid@*.se\neller för \
att göra det enkelt:  roberto*
;
MSG_IGNORE_ENTRY
Ignore Entry:
Ignoriere Eintrag:
Ignorar:
Ignorer :
Êáôá÷şñçóç áãíüçóçò:
Elemento da ignorare
Mellõzõ bejegyzés
Ignoremaske:
Ignoruj:
Ignorera enligt:
;
MSG_IGNORE_EVENTS
Which events to ignore:
Welche Ereignisse sollen ignoriert werden:
Eventos a ignorar:
Évènements à ignorer :
Ãåãïíüôá ğïõ èá áãíïïıíôáé:
Quale evento ignorare:
Mely eseményt kell mellõzni:
Hendelser å overse:
Które zdarzenia ignorowaæ:
Händelser att ignorera
;
MSG_IGNORE_ADD
Add
Hinzufügen
Añadir
Ajouter
Ğñüóèåóç
Aggiungi
Hozzáad
Legge til
Dodaj
Lägg till
;
MSG_IGNORE_EDIT
Edit
Ändern
Editar
Éditer
Åğåîåñãáóßá
Modifica
Módosít
Editere
Edytuj
Ändra
;
MSG_IGNORE_REMOVE
Remove
Entfernen
Eliminar
Supprimer
Áöáßñåóç
Rimuovi
Eltávolít
Fjerne
Usuñ
Ta bort
;
MSG_AUTO_CONNECT_OPTION
Automatically connect here when WookieChat is started
Diesen Server beim Start von WookieChat automatisch verbinden
Conectarse automáticamente al inicio
Connexion automatique à ce serveur au démarrage de WookieChat
Áõôüìáôç óıíäåóç óå áõôü êáôÜ ôï îåêßíçìá ôïõ WookieChat
Connettiti automaticamente qui quando WookieChat viene lanciato
Automatikusan csatlakozik ide a WookieChat indításakor
Koble automatisk til her når WookieChat starter
Po³±cz automatycznie po uruchomieniu programu
Anslut hit automatiskt när WookieChat startas
;
MSG_DISPLAY_SERVER_SELECTION_WINDOW
Display Server Window upon startup:
Serverfenster beim Start zeigen:
Mostrar esta ventana al inicio
Afficher la fenêtre des serveurs au lancement :
ÅìöÜíéóç ğáñáèıñïõ äéáêïìéóôşí êáôÜ ôçí İíáñîç:
Mostra finestra server all'avvio:
Kiszolgáló ablak megjelenítése indításkor:
Vis servervindu ved oppstart:
Wy¶wietlaj listê serwerów przy uruchomieniu:
Visa serverfönstret vid uppstart:
;
MSG_UNABLE_TO_RECONNECT_TO_SERVER
Unable to connect, giving up!
Keine Verbindung möglich! Abbruch!
No se ha podido conectar; abandonando!
Impossible de se connecter, connexion abandonnée!
Áäıíáôç ç óıíäåóç. Ôá ğáñáôÜù!
Connessione impossibile, rinuncio!
Nem csatlakozik, feladom!
Kunne ikke koble opp, gir opp!
Nie mo¿na uzyskaæ po³±czenia, zaprzestano prób!
Anslutning misslyckades, ger upp!
;
MSG_COMMAND_ALIAS_WINDOW_COLUMN1_TITLE
Alias
Alias
Alias
Alias
Óõíşíõìï
Alias
Helyettesítõ
Alias
Alias
Alias
;
MSG_COMMAND_ALIAS_WINDOW_COLUMN2_TITLE
Command
Kommando
Comando
Commande
ÅíôïëŞ
Comando
Parancs
Kommando
Polecenie
Kommando
;
MSG_EDIT_ALIAS
Edit...
Ändern ...
Editar...
Éditer...
Åğåîåñãáóßá...
Modifica...
Módosít...
Edit...
Edytuj...
Ändra...
;
MSG_EDIT_ALIAS_CHANGE
Change
Übernehmen
Cambiar
Changer
ÁëëáãŞ
Cambia
Változtat
Forandre
Zmieñ
Byt
;
MSG_EDIT_ALIAS_CANCEL
Cancel
Abbrechen
Cancelar
Annuler
Áêıñùóç
Annulla
Mégsem
Avbryt
Poniechaj
Avbryt
;
MSG_MENU_ITEM_CLONE_DETECTION
Clone detection
Doppelerkennung
Detectar duplicados
Détection de clone
ÁíáæŞôçóç êëşíïõ
Clone detection
Klón érzékelés
Klonedetektering
Wykrywaj klony
Klondetektering
;
MSG_CLONES
Clones
Doppel
Duplicados
Clones
Êëşíïé
Cloni
Klónok
Kloner
Klony
Kloner
;
MSG_CLONE_DETECTED
Detected clones from
Doppel gefunden von
Detectados duplicados de
Clone(s) détecté(s) pour
Âñİèçêáí êëşíïé áğü
Trovato cloni da
Felderített klónok innen:
Kloner detektert
Wykryto klony z
Detekterat kloner från
;
MSG_SAVE_SETTINGS
Save settings
Einstellungen speichern
Guardar ajustes
Enregistrer les réglages
ÁğïèŞêåõóç ñõèìßóåùí
Salva impostazioni
Beállítások mentése
Lagre innstillinger
Zapisz ustawienia
Spara inställningar
;
MSG_SPLIT_UP_LOGS_PREFS_LABEL
Split up logfiles into smaller chunks
Logdateien in kleinere Stücke splitten
Separar registros en ficheros pequeños
Découper les fichiers d'historique en petits morceaux
Äéá÷ùñéóìüò ôùí áñ÷åßùí êáôáãñáöŞò óå ìéêñüôåñá êïììÜôéá
Spezzetta i file del protocollo in parti minori
Naplófájlok felbontása kisebb barabokra
Splitt opp loggfiler i mindre biter
Dziel pliki raportów na mniejsze kawa³ki
Dela upp loggfiler i mindre stycken
;
MSG_LOGSIZE_PREFS_LABEL
If the above is enabled, choose a log file size (in kbytes)
Wenn obiges aktiviert ist, bitte die Größe der Logdatei angeben (in kbytes)
Tamaño de los ficheros pequeños (kB):
Choisissez la taille des fichiers (en kilo Octets)
ÅÜí ôï ğáñáğÜíù åßíáé åíåñãïğïéçìİíï, åğéëİîôå ôï ìİãåèïò ôïõ áñ÷åßïõ (óå kbytes)
....e  scegli la dimensione del logfile\nse l'opzione sopra Ã¨ abilitata (in kilobyte)
Ha a fenti bekapcsolva, akkor a naplófájl mérete (kbájtban)
Hvis denne er aktiv, velg størrelse på loggfil (i kilobytes)
Je¶li opcja powy¿ej jest zaznaczona, podaj wielko¶æ pliku raportu (w kB)
Om du valt alternativet ovan, ange storlek på loggfilerna   (i kilobyte)
;
MSG_SPLIT_UP_LOGS_PREFS_HELP
When a tab is opened, the logfile will be examined. If it is over a certain size, then it will be renamed with the date appended to the end, and WookieChat will resume logging in a new empty logfile. \
This is implemented for performance reasons. It can take a few seconds to reload large logfiles that are a few MB in size.
Sobald ein Tab geöffnet ist, wird die Logdatei überprüft. Wenn sie über der definierten Größe ist, wird sie umbenannt, mit dem Datum am Ende, und WookieChat schreibt in einer neuen leeren Datei \
weiter. Dieses wurde aus Leistungsgründen eingeführt. Es kann einige Sekunden dauern, bis große Logdateien mit einigen MB geladen werden.
Al abrir una nueva pestaña, se examina el fichero de\nregistro. Si su tamaño es mayor que el especificado,\neste se renombra con la fecha y se crea uno nuevo.\nEsto se hace por razones de rendimiento \
(pueden\npasar varios segundos al cargar uno muy grande).
Lorsqu'un onglet est ouvert, le fichier d'historique sera examiné. S'il dépasse une certaine taille, il sera renommé en ajoutant la date à la fin, et WookieChat recommencera l'historisation dans un \
nouveau fichier vide. Cela a été implémenté pour des raisons de performance : cela peut prendre plusieurs secondes de recharger un fichier faisant plusieurs MO.
¼ôáí ìéá êáñôİëá åßíáé áíïé÷ôŞ, ôï áñ÷åßï êáôáãñáöŞò åîåôÜæåôáé. ÅÜí åßíáé ìåãáëıôåñï áğü İíá óõãêåêñéìİíï ìİãåèïò, ôüôå èá ìåôïíïìáóôåß ìå ôçí çìåñïìçíßá ôïğïèåôçìİíç óôï ôİëïò, êáé ôï WookieChat èá \
óõíå÷ßóåé ôçí êáôáãñáöŞ óå íİï Üäåéï áñ÷åßï. Áõôü äçìéïõñãŞèçêå ãéá ëüãïõò áğüäïóçò. Ğáßñíåé êÜğïéï ÷ñüíï ç åğáíáöüñôùóç ìåãÜëùí áñ÷åßùí êáôáãñáöŞò ğïõ İ÷ïõí ìİãåèïò êÜğïéá MB.
Se un Tab viene aperto, il logfile verrÃ  esaminato. Se questo supera una certa dimensione allora verrÃ  rinominato secondo la data associata, dopodichÃ¨ WookieChat riprenderÃ Â a protocollare in un \
nuovo file. Questo aumenterÃ  la prestazioni. Potrebbe verificarsi un attesa di qualche secondo per la riapertura di logfile di grandi dimensioni (in MB).
Amikor egy panel megnyílik, a naplófájl átvizsgálásra kerül. Ha nagyobb egy bizonyos méretnél, átnevezésre kerül a dátummal kiegészítve. Ezután a WookieChat folytatja a naplózást egy új üres fájlba. \
Teljesítmény iránti megfontolásból került beépítésre ez a funkció, mivel néhány másodpercet igénybe vehet egy több MB méretû naplófájl betöltése.
Når en tab åpnes, vil loggfilen bli undersøkt. Hvis den er over en viss størrelse, vil den bli omdøpt med datoen lagt til slutten, og WookieChat vil fortsette å logge til en ny loggfil. Dette er \
implementert for effektivitetsgrunner. Det kan ta noen sekunder for å reloade store loggdiler som er noen MB i størrelse.
Podczas otwierania zak³adki zostanie sprawdzony plik raportu. Je¶li jego wielko¶æ przekracza okre¶lon± warto¶æ, jego nazwa zostanie zmieniona poprzez dodanie na koñcu obecnej daty, a program bêdzie \
kontynuowa³ raportowanie do nowego, pustego pliku raportu. Opcja ta zosta³a dodana w celu poprawienia wydajno¶ci. Wczytywanie plików raportów zajmuj±cych kilka megabajtów mo¿e zajmowaæ kilka sekund.
När en ny flik öppnas, kommer loggfilen att undersökas. Om den är över en given storlek, kommer den att få ett nytt namn, med dagens datum lagd till slutet, och WookieChat fortsätter att logga i en ny \
tom fil. Detta är gjort av prestandaskäl. Det kan ta ett par sekunder att ladda om stora loggfiler.
;
MSG_NO_SERVER_TABS_PREFS
Disable server tabs
Keine Server Tabs benutzen
 No mostrar servidores
Désactiver onglets serveur
Disable server tabs
Disabilita i Tab server
Kiszolgáló panelek tiltása
Hindre server tabs
Wy³±cz zak³adki serwerów
Använd inte serverflikar
;
MSG_HIDE_WINDOW
Hide
Verbergen
Iconificar
Masquer
Hide
Nascondi
Elrejt
Gjem
Ikonifikuj
Göm
;
MSG_PREFS_DCC_ADDRESS
Specify DCC address?
DCC Adresse eingeben?
Dirección DCC específica
Spécifier l'adresse DCC
Specify DCC address?
Specifica indirizzo DCC?
DCC cím meghatározása?
Spesifisere DCC adresse
Okre¶l adres dla po³±czeñ DCC
Specificera DCC adress?
;
MSG_PREFS_DCC_ADDRESS_HELP
WookieChat attempts to obtain your address automatically for outgoing DCC connections. If this address is incorrect, you can specify it here.
WookieChat versucht die Adresse für ausgehende DCC Verbindungen automatisch zu ermitteln. Sollte diese Adresse fehlerhaft sein, so kann hier eine eingegeben werden.
WookieChat intenta obtener su dirección de forma\nautomática para conexiones DCC de salida. Si\nesta es incorrecta, puede especificar una aquí.
WookieChat tente d'obtenir votre adresse automatiquement pour les connexions DCC sortantes. Si cette adresse est incorrecte, vous pouvez la modifier ici.
WookieChat attempts to obtain your address automatically for outgoing DCC connections. If this address is incorrect, you can specify it here.
WookieChat tenterÃ  automaticamente di ottenere il tuo indirizzo per i trasferimenti DCC. Se l'indirizzo Ã¨ scorretto, puoi specificarlo qui.
Kimenõ DCC kapcsolathoz a WookieChat megpróbálja automatikusan beszerezni a címedet. Ha ez a cím helytelen, itt megadhatod a megfelelõt.
WookieChat prøver å oppnå din adresse automatisk for utgående DCC forbindelser. Hvis denne adressen ikke er korrekt, kan du spesifisere den her.
Program spróbuje automatycznie okre¶liæ Twój adres dla wychodz±cych po³±czeñ DCC. Je¶li ten adres jest nieprawid³owy, mo¿esz okre¶liæ w³asny adres tutaj.
WookieChat försöker hämta din adress automatiskt för utgående DCC-anslutningar. Om adressen är fel, kan du ändra den här.
;
MSG_PREFS_TOGGLE_BUTTON_TABS
Use button-style tabs
Tabs im Knopfstil benutzen
Canales/servidores como botones
Utiliser les onglets par bouton
Use button-style tabs
Usa registri in stile pulsanti
Panel váltás kapcsolókkal
Bruk knappestil tabber
U¿ywaj zak³adek w postaci przycisków
Använd knappliknande flikar
;
MSG_PREFS_TOGGLE_BUTTON_TABS_HELP
Turn off this option to use Listview-style tabs (the listview is better having a large amount of tabs open, such as 20 or 30).
Um Tabs im Listenmodus zu erhalten, diese Option abschalten (Der Listenmodus ist optimaler, wenn viele Tabs, so zwischen 20 und 30, offen sind).
Desactivado muestra canales/servidores en una lista\nen vez de botones (permitiendo "ver" más entradas).
Désactiver cette option pour utiliser les onglets par liste (la liste est plus adaptée lorsqu'un grand nombre d'onglets (comme 20 ou 30) est ouvert).
Turn off this option to use Listview-style tabs (the listview is better having a large amount of tabs open, such as 20 or 30).
Disabilita questa opzione per usare i registri in stile lista (ciÃ² renderÃ  la lista piÃ¹ perspicua se ne hai piÃ¹ di 20 aperti).
Az opciót kikapcsolva a megnyitott panelek egy listában jelennek meg (nagyobb mennyiségû pl. 20-30 panel esetén ez jobban áttekinthetõ).
Skru av denne muligheten for å bruke Listview-stil tabber  (listview er bedre når du har mange tabber åpne, så som 20 eller 30).
Wy³±cz tê opcjê, aby wy¶wietlaæ zak³adki w postaci listy (widok listy jest wygodniejszy przy wiêkszej ilo¶ci otwartych zak³adek).
Välj bort detta alternativet för att se flikarna som listvy (listvyn passar bättre när många flikar är öppna, som 20 eller 30).
;
MSG_USER_HAS_QUIT_IRC
has left IRC..
hat das IRC verlassen...
abandona el IRC...
a quitté l'IRC...
has left IRC..
Ã¨ uscito da IRC...
elhagyta a csatornát..
har avsluttet IRC...
opu¶ci³ IRC-a...
har lämnat IRC...
;
MSG_USER_HAS_RETURNED_TO_IRC
has returned to IRC...
ist ins IRC zurückgekehrt ...
vuelve al IRC...
est revenu sur l'IRC...
has returned to IRC..
Ã¨ rientrato su IRC...
visszatért a csatornába..
har returnert til IRC...
powróci³ na IRC-a...
har kommit tillbaka till IRC...
;
MSG_CHECKBOX_ENABLE_GRAPHICAL_SMILEYS
Enable Graphical Smileys
Grafische Smilies aktivieren
Emoticonos gráficos
Activer les Émoticônes graphiques
Enable Graphical Smileys
Usa faccine grafiche
Grafikus hangulatjelek
Tillate grafiske smileys
W³±cz graficzne emotikonki
Aktivera grafiska Smileys
;
MSG_BACKGROUND_COLOUR_FOR_GRAPHICAL_SMILEYS
Background Colour for Smileys
Hintergrund für Smilies
 Fondo de emoticonos:
Couleur d'arrière-plan des Émoticônes
Background Colour for Smileys
Colore sfondo per le faccine
Háttérszín a hangulatjelekhez
Bakgrunnsfarge for smileys
Kolor t³a emotikonek
Bakgrundsfärg för Smileys
;
MSG_GRAPHICAL_SMILEY_SHORTHELP
This option will display common ASCII "smiley faces" such as :) and :D with graphical pictures.
Diese Option stellt die wichtigsten ASCII "Smilie-Gesichter" wie :) und :D als Grafik dar.
Mostrar los emoticonos :) :/ como gráficos.
Avec cette option les Émoticônes courants ASCII comme :) et :D seront remplacés par des images.
This option will display common ASCII "smiley faces" such as :) and :D with graphical pictures.
Questa opzione visualizzerÃ  le faccine in modo standard p.es. :) e :D in modo grafico.
Ez a lehetõség a hagyományos ASCII "hangulatjeleket", mint pl. :) és :D grafikus képekkel jeleníti meg.
Dette vil vise vanlige ASCII "smilefjes" som :) og :D med grafiske bilder.
Opcja ta zast±pi najczê¶ciej spotykane emotikonki tekstowe, takie jak :) i :D odpowiednimi ich obrazkami.
Smileys visas med ASCII-kod "smiley faces" såsom :) och :D med grafiska figurer.
;
MSG_CHECKBOX_EXTERNAL_PLAYER_FOR_CTCP_SOUND
Use External Replayer For CTCP SOUND
Externen Abspieler für CTCP-Klang benutzen
Programa externo para CTCP SOUND
Utiliser un lecteur de son externe pour CTCP SOUND
Use External Replayer For CTCP SOUND
Usa replayer esterno per CTCP SOUND
Külsõ lejátszó használata a CTCP SOUND-hoz
Bruk ekstern avspiller for CTCP SOUND
U¿ywaj zewnêtrznego odtwarzacza dla ¿±dañ CTCP SOUND
Använd extern spelare för CTCP SOUND
;
MSG_EXTERNAL_PLAYER_FOR_CTCP_SOUND
Sound Player
Abspieler für Klänge
Reproductor:
Lecteur de son
Sound Player
Sound Player
Lejátszó
Lydavspiller
Odtwarzacz d¼wiêku
Ljudspelare
;
MSG_DOWNLOAD_PATH
Download Path:
Pfad zum Laden der Dateien:
Descargas:
Chemin de téléchargement :
Download Path:
Percorso per il download
Letöltés helye:
Nedlastingssti
¦cie¿ka pobierania
Sökväg för nedladdning
;
MSG_LABEL_LISTVIEW_TABS_WEIGHT
Listview tabs vertical weight
Feldgröße im Listenmodus
Altura de la lista de canales/servidores (pixels)
Taille des onglets en mode liste
Listview tabs vertical weight
Dimensioni e larghezza per i Tab in stile lista
Panel lista mérete
Listview-stil tabs størrelse/vekt
Rozmiar listy zak³adek
Storlek på listvyflikarna
;
MSG_GROUP_FRAME_TITLE_TAB_PREFERENCES
Tab Preferences
Tab Einstellungen
Mostrar Canales/Servidores
Préférences des onglets
Tab Preferences
Impostazioni Tab
Panel beállítások
Innstillinger
Preferencje zak³adek
Inställningar för flikar
;
MSG_PREFS_HELP_BUBBLE_LISTVIEW_TABS_WEIGHT
If you have disabled button style tabs, you can configure the size of the "tabs listview".
Wenn die Knopfansicht der Kanäle abgeschaltet ist, kann  hier die Größe des Kanalanzeigefeldes angegeben werden.
Altura de la lista de canales/servidores,\nsi ha elegido no mostrarlos como botones.
Lorsque les onglets mode « bouton » sont désactivés, il est possible de configurer la taille des « onglets mode liste ».
If you have disabled button style tabs, you can configure the size of the "tabs listview".
Se hai disabilitato i Tab in stile pulsante, puoi configurare la dimensione dei \n"tabs stile lista".
Ha kikapcsolta a kapcsolókkal történõ panel váltást, akkor itt határozható meg a "panel lista" mérete.
Hvis du ikke har knappestil tabs, kan du konfigurere størrelsen av "tabs listview".
Je¶li wy³±czy³e¶ reprezentacjê zak³adek poprzez przyciski, mo¿esz skonfigurowaæ wielko¶æ "listy zak³adek".
Om du har inaktiverat flikar med knapputseende, kan du konfigurera "tabs listview".
;
MSG_PREFS_HELP_BUBBLE_NO_SERVER_TABS
You can turn off keeping a dedicated tab for server messages to conserve memory and valuable screen space.
Um Bildschirmplatz und Speicher zu sparen, kann dieser Tab abgeschaltet werden.
No mostrar el servidor en pestaña/lista, ahorrando\nespacio en la ventana principal y memoria.
Vous pouvez désactiver l'affichage d'un onglet spécifique aux messages du serveur pour économiser la mémoire et l'espace sur l'écran.
You can turn off keeping a dedicated tab for server messages to conserve memory and valuable screen space.
Puoi omettere un Tab per i messaggi dal server in modo da non sprecare memoria e spazio su schermo.
Kikapcsolható a kiszolgáló üzeneteinek fenntartott panel, hogy ne foglaljon memóriát és értékes képernyõterületet.
Du kan skru av dedikert tab for serverbeskjeder for å spare minne og verdifull skjermplass.
W celu zaoszczêdzenia pamiêci oraz cennego miejsca na ekranie mo¿esz wy³±czyæ wy¶wietlanie dedykowanej zak³adki na wiadomo¶ci serwera.
Du kan stänga särskild flik för servermeddelanden för att spara skärmyta.
;
MSG_PREFS_GRAPHICAL_NICKLIST_CHECKBOX
Enable Graphical Nicklist
Grafischen Zusatz bei Nicknamen anzeigen
 Lista de alias con colores
Activer la liste de pseudos graphique
Enable Graphical Nicklist
Abilita modalitÃ  grafica per la lista nick
Grafikus becenév lista
Tillate grafisk nickliste
W³±cz graficzn± listê pseudonimów
Aktivera grafisk nicklista
;
MSG_PREFS_GRAPHICAL_NICKLIST_HELP_BUBBLE
This will display the three most common nicklist usermodes in a graphical way.\nOperators (or @) are Green, Halfops (or \% ) are Blue, Voiced users (or +) are Yellow\n(This feature is reliant on \
picture datatypes).
Das zeigt die drei häufigsten Nutzermodi auf grafische Weise an.\nOperatoren (oder @) sind grün, HalbOperatoren (oder % ) sind blau, sprachberechtigte Nutzer (oder +) sind gelb.\n(Diese Funktion hängt \
vom Picture Datentyp ab).
Mostrar a operarios, medio-operarios y con\nvoz con colores en lugar de los típicos\ncaracteres @, % + (requiere picture.datatype).
Les trois modes utilisateur les plus courants seront affichés de manière graphique.\nLes Opérateurs (ou @) en vert, les Demi-Opérateurs (ou \%) en bleu et les Voice (ou +) en jaune\n(Cette fonction \
dépend des datatypes des formats d'image).
This will display the three most common nicklist usermodes in a graphical way.\nOperators (or @) are Green, Halfops (or \% ) are Blue, Voiced users (or +) are Yellow\n(This feature is reliant on \
picture datatypes).
Questo visualizzerÃ  i tre piÃ¹ comuni modi d'uso della nicklist in modalitÃ  grafica\nOperatori (o @) sono verdi, Mezzioperatori (o \% ) sono blu, Voiced users (o +) sono gialli \n(Questa funzione \
dipende dai datatype installati).
A három leggyakoribb felhasználó típust grafikusan jeleníti meg.\nOperátorok (@) zöldek, Halfop-ok ( \% ) kékek,Voice felhasználók (+) sárgák\n(Ez a tulajdonság a picture datatype-ra épül).
Dette vil vise de tre mest vanlige brukermodusene for nicklister på en grafisk måte.\nOper (eller @) er grønn, halvoper (eller \% ) er blå, voicede brukere (eller +) er gul\n(Denne opsjonen avhenger \
av datatyper r grafikk).
Pozwoli to na wy¶wietlanie trzech najpopularniejszych trybów u¿ytkowników na li¶cie pseudonimów przy pomocy grafiki.\nOperatorzy (lub @) s± zieloni, pó³operatorzy (lub \%) s± niebiescy, u¿ytkownicy z \
przyznanym g³osem (lub +) s± ¿ó³ci.\n(opcja ta wymaga zainstalowanych datatype'ów).
Visar de tre vanligaste användartyperna på ett grafiskt vis.\nOperator (eller @) är gröna, Halvop (eller \% ) är Blåa, användare med röst (Voice eller +) är gula\n(har beroende i datatyperna för \
bilder).
;
MSG_PREFS_COLUMN_WIDTH_TIMESTAMP
Column width (in Pixels) for timestamps
Spaltenbreite (in Pixeln) für Zeiteinträge
Anchura columna Fecha/Hora (pixels)
Largeur de la colonne horodatage (en pixels)
Column width (in Pixels) for timestamps
Larghezza colonna (in Pixels) per timestamps
Oszlop szélesség (képpontokban) az idõlenyomathoz
Kolonnebredde (i piksler) for tidsstempel
Szeroko¶æ kolumny (w pikselach) oznaczeñ czasowych
Kolumnbredd (i pixlar) för tidstämpel
;
MSG_PREFS_COLUMN_WIDTH_MIDDLE_NICKS
Column width (in Pixels) for the middle/nicks column
Spaltenbreite (in Pixeln) für die Mitte/Nickamen
Anchura columna alias (pixels)
Largeur de la colonne des pseudos (en pixels)
Column width (in Pixels) for the middle/nicks column
Larghezza (in Pixels) per le colonne centrale/nick
A középsõ/becenév oszlop szélessége (képontokban)
Kolonnebredde (i piksler) for den midtre/nick-kolonnen
Szeroko¶æ kolumny (w pikselach) pseudonimów
Bredd (i pixlar) för nick-/mittkolumnen
;
MSG_PREFS_DIVIDER_BAR_SPACE
Amount of space surrounding columns divider bar
Wert des Abstandes zu den Spalten
Espacio libre entre columnas y texto (pixels)
Espace autour du séparateur de colonnes
Amount of space surrounding columns divider bar
Spazio circostante le colonne divisorie
Az oszlopokat elválasztó hézag mérete
Mengde av plass som omgir avdeler-baren for kolonner
Ilo¶æ miejsca otaczaj±cego belkê rozdzielaj±c± kolumny
Plats runt kolumndelaren
;
MSG_PREFS_MULTICOLUMN_NICK_DISPLAY
Remove < and > from around nicks
Entferne '<' und '>' um Nicknamen
Eliminar '<' y '>' de los alias
Ne pas afficher < et > autour des pseudos
Remove < and > from around nicks
Rimuovi < e > attorno ai nick
< és > jelek eltávolítása a becenevekbõl
Fjerne < og > fra nick
Usuwaj nawiasy otaczaj±ce pseudonimy
Ta bort < och > runt nick
;
MSG_GROUP_FRAME_TITLE_MULTICOLUMN_DISPLAY_SETTINGS
Multicolumn display settings
Spalteneinstellungen
Al Mostrar V. Charla En Columnas
Réglages de l'affichage multi colonnes
Multicolumn display settings
Impostazione visualizzazione multicolonna
Több oszlopos panel beállításai
Multikolonne visningsinnstillinger
Ustawienia wy¶wietlania wielokolumnowego
Inställning för visning med flera kolumner
;
MSG_MENU_MULTICOLUMNCHATDISPLAY_ITEM
Multi-column chat display
Mehrspaltige Chatanzeige
Ventana de charla en columnas
Affichage multi colonne
Multi-column chat display
Display chat multicolonna
Több oszlopos csevegõ panel
Multikolonne chat-visning
Wy¶wietlaj rozmowy w wielu kolumnach
Chat med flera kolumner
;
MSG_GRAPHICAL_NICKLIST_HELP_BUBBLE
Graphical Usermodes Key:\nGreen = Channel Operators or @\nBlue = HalfOps or \% \nYellow = Voiced Users or +
Grafische Nutzermodi:\nGrün = KanalOperator oder @\nBlau = HalbOperator oder %\nGelb = Sprachberechtigte Nutzer oder +
Colores según el tipo de usuario:\nVerde = operadores (@)\nAzul = medio-operadores (%)\nAmarillo = con voz (+)
Codes des modes utilisateurs graphiques :\nVert = Opérateur du canal ou @\nBleu = Demi-Opérateur ou \% \nJaune = Voice ou +
Graphical Usermodes Key:\nGreen = Channel Operators or @\nBlue = HalfOps or \% \nYellow = Voiced Users or +
Pulsante modalitÃ  di utilizzo grafico utente:\nVerde = Operatori canale o @\nBlu = MezziOperatori o \% \nGiallo = Voiced Users o +
Grafikus felhasználótípus kulcs:\nZöld = Csatorna Operátor vagy @\nKék = HalfOp vagy \% \nSárga = Voice felhasználó vagy +
Grafisk brukermodus nøkkel:\nGrønn = kanal-oper eller @\nBlå = halvoper eller \% \nGul = voice-brukere eller +
Graficzne tryby u¿ytkowników - podpowied¼:\nzielony = operator kana³u lub @\nniebieski = pó³operator kana³u lub \%\n¿ó³ty = u¿ytkownik z przyznanym g³osem lub +
Färgnyckel för grafiska användarlägen:\nGrön = Operator eller @\nBlå = HalfOp eller \% \nGul = Användare med röst eller +
;
MSG_GRAPHICAL_SMILEYS_PREVIEW
Graphical Smileys Preview
Vorschau der grafischen Smileys
Muestra emoticonos gráficos
Aperçu des Émoticônes
Graphical Smileys Preview
Anteprima emoticon
Graphical Smileys Preview
Forhåndsvisning grafiske smileys
Podgl±d emotikonek
Förhandsvisning Smileys
;
MSG_PREFS_NICKLIST_HORIZONTAL_WEIGHT
Nicklist horizontal weight
Breite der Spitznamensliste
Anchura de la lista de aliases (pixels)
Poids de la liste de pseudos
Nicklist horizontal weight
Ampiezza orizzontale Nicklist
Nicklist horizontal weight
Nicklistens horisontale vekt
Szeroko¶æ listy pseudonimów
Horisontell nicklista
;
MSG_PREFS_SMILEYS_THEME
Smileys theme
Smiliedarstellung
Emoticonos
Thème des Émoticônes
Smileys theme
Tema per gli emoticon
Smileys theme
Smileys-tema
Zestaw emotikonek
Tema Smileys
;
MSG_SERVER_CHARSET
Server charset
Zeichensatz des Servers
 Juego caracteres
Jeu de caractères du serveur
Server charset
Charset server
Server charset
Server karaktersett
Strona kodowa serwera
Server teckenuppsättning
;
MSG_LOCAL_CHARSET
Local charset
Lokaler Zeichensatz
Juego caracteres local
Jeu de caractères local
Local charset
Charset locale
Local charset
Lokalt karaktersett
Lokalna strona kodowa
Lokal teckenuppsättning
;
MSG_COLUMN_TITLE_CHARSET
Charset
Zeichensatz
J. Caracteres
Jeu de caractères
Charset
Charset
Charset
Karaktersett
Strona kodowa
Teckenuppsättning
;
MSG_MENU_ITEM_RESCAN_AREXX_DIR
<Rescan scripts directory>
<Skriptverzeichnis neu einlesen>
<Releer dir. scripts>
<Actualiser liste des scripts>
<Rescan scripts directory>
<Esplora directory script>
<Rescan scripts directory>
<Reskanne scripts-mappe>
<Przeskanuj katalog skryptów>
<Skanna om skriptkatalogen>
;
MSG_MENU_TITLE_AREXX
ARexx
ARexx
ARexx
ARexx
ARexx
ARexx
ARexx
Arexx
Skrypty Arexx'a
ARexx
;
MSG_ABOUT_WINDOW_TEXT
\033cProudly sponsored by Guru Meditation\nAmiga reseller and Linux consulting\nhttp://gurumeditation.se/
\ecUnterstützt von Guru Meditation\nAmiga Händler und Berater für Linux\nhttp://gurumeditation.se/
\033cEsponsorado con orgullo por Guru Meditation,\nvendedor de AMIGA y consultor Linux.\nhttp://gurumeditation.se/
\033cFièrement sponsorisé par Guru Meditation\nRevendeur Amiga et consultant Linux\nhttp://gurumeditation.se/
\033cProudly sponsored by Guru Meditation\nAmiga reseller and Linux consulting\nhttp://gurumeditation.se/
\033cGentilmente sponsorizzato da Guru Meditation\nAmiga reseller and Linux consulting\nhttp://gurumeditation.se/
\033cProudly sponsored by Guru Meditation\nAmiga reseller and Linux consulting\nhttp://gurumeditation.se/
\033cStolt sponsor: Guru Meditation\nForhandler av Amiga og Linux konsultatsjon\nhttp://gurumeditation.se/
\033cDzielnie wspierane przez Guru Meditation\nSprzedawcê Amigi i doradcê Linuksowego\nhttp://gurumeditation.se/
\033cStolt sponsrad av Guru Meditation\nAmiga återförsäljare och Linux konsultering\nhttp://gurumeditation.se/
;
MSG_ABOUT_WINDOW_TEXT_2
AREXX port:
ARexx Port:
\n
Port ARexx :
AREXX port:
Porta AREXX:
AREXX port:
ARexx-port:
Port Arexx'a:
AREXX port:
;
MSG_COLOUR_SETTINGS_LABEL_BACKGROUND
Background
Hintergrund
Fondo
Arrière-plan
Background
Sfondo
Background
Bakgrunn
T³o
Bakgrund
;
MSG_COLOUR_SETTINGS_LABEL_NICKLIST_BACKGROUND
Nicklist background
Namenslisten-Hintergrund
Lista aliases (fondo)
Arrière-plan liste pseudos
Nicklist background
Sfondo Nicklist
Nicklist background
Bakgrunn for nickliste
T³o listy pseudonimów
Bakgrund nicklista
;
MSG_COLOUR_SETTINGS_LABEL_JOIN
Join
Verbunden
Acceder
Join
Join
Entra
Join
Entre
Przy³±czenie
Anslut
;
MSG_COLOUR_SETTINGS_LABEL_PART
Part
Part
Abandonar
Part
Part
Part
Part
Forlater
Od³±czenie
Lämna
;
MSG_COLOUR_SETTINGS_LABEL_QUIT
Quit
Beenden
Salir
Quitter
Quit
Esci
Quit
Avslutter
Koniec
Avsluta
;
MSG_COLOUR_SETTINGS_LABEL_MODES
Modes
Modi
Modos
Modes
Modes
ModalitÃ 
Modes
Moduser
Zmiana trybu
Lägen
;
MSG_COLOUR_SETTINGS_LABEL_CTCP
CTCP
CTCP
 CTCP
CTCP
CTCP
CTCP
CTCP
CTCP
CTCP
CTCP
;
MSG_COLOUR_SETTINGS_LABEL_ACTIONS
Actions
Aktionen
Acciones
Actions
Actions
Azioni
Actions
Handlinger
Dzia³anie
Händelser
;
MSG_COLOUR_SETTINGS_LABEL_NORMAL
Normal
Normal
 Normal
Normal
Normal
Normale
Normal
Normal
Normalny
Normal
;
MSG_COLOUR_SETTINGS_LABEL_INFO
Info
Informationen
Información
Info
Info
Info
Info
Info
Informacja
Info
;
MSG_COLOUR_SETTINGS_LABEL_OWN_TEXT
Own text
Eigener Text
Texto propio
Texte personnel
Own text
Testo personale
Own text
Egen tekst
W³asny tekst
Egen text
;
MSG_COLOUR_SETTINGS_LABEL_HIGHLIGHT
Highlight
Hervorheben
Resaltar
Mise en évidence
Highlight
Evidenzia
Highlight
Høylys
Pod¶wietlenie
Framhäv
;
MSG_COLOUR_SETTINGS_LABEL_INVITE
Invite
Einladungen
Invitar
Invitation
Invite
Invita
Invite
Invitere
Zaproszenie
Bjud in
;
MSG_COLOUR_SETTINGS_LABEL_KICK
Kick
Rausschmiss
Expulsar
Kick
Kick
Kick
Kick
Kicke
Wyrzucenie
Kicka
;
MSG_COLOUR_SETTINGS_LABEL_NICK_CHANGE
Nick change
Namenswechsel
Cambiar alias
Changement de pseudo
Nick change
Cambia Nick
Nick change
Forandrer nick
Zmiana pseudonimu
Ändring nick
;
MSG_COLOUR_SETTINGS_LABEL_TOPIC
Topic
Thema
Temática
Sujet
Topic
Argomento
Topic
Topic
Temat
Ämne
;
MSG_COLOUR_SETTINGS_LABEL_WALLOPS
Wall ops
Wall ops
OPs
Wall ops
Wall ops
Wall ops
Wall ops
Wall ops
Informacje od operatorów
Wall ops
;
MSG_COLOUR_SETTINGS_LABEL_NOTICES
Notices
Servermeldungen
Noticias
Notices
Notices
Avvisi
Notices
Notices
Powiadomienia
Notiser
;
MSG_COLOUR_SETTINGS_LABEL_ACTIVITY
Activity
Tab-Aktivität
Actividad
Activité
Activity
AttivitÃ 
Activity
Aktivitet
Aktywno¶æ
Aktivitet
;
MSG_COLOUR_SETTINGS_LABEL_CHAT_ACTIVITY
Chat activity
Tab-Chatanzeige
Actividad (charla)
Discussion
Chat activity
AttivitÃ  Chat
Chat activity
Chatteaktivitet
Aktywno¶æ - rozmowa
Chat aktivitet
;
MSG_COLOUR_SETTINGS_LABEL_HIGHLIGHT_ACTIVITY
Highlight activity
Tab-Hervorhebeanzeige
Actividad (resaltar)
Mise en évidence
Highlight activity
Evidenzia attivitÃ 
Highlight activity
Høylys aktivitet
Aktywno¶æ - pod¶wietlenie
Framhäv aktivitet
;
MSG_COLOUR_SETTINGS_LABEL_NICKLIST_TEXT
Nicklist text
Namenslisten-Text
Lista aliases (texto)
Texte de la liste de pseudos
Nicklist text
Testo Nicklist
Nicklist text
Tekst nickliste
Pisak listy pseudonimów
Text nicklista
;
MSG_COLOUR_SETTINGS_LABEL_TABS_BACKGROUND
Tabs background
Tab-Hintergrund
 Lista cans./servs. (fondo)
Arrière plan des onglets
Tabs background
Sfondo Tab
Tabs background
Tabs bakgrunn
T³o zak³adek
Bakgrund flikar
;
MSG_COLOUR_SETTINGS_LABEL_TAGS_PEN
Tabs pen
Schriftfarbe der Tabs
 Lista cans./servs. (texto)
Texte des onglets
Tabs pen
Penna Tab
Tabs pen
Tabs penn
Pisak zak³adek
Penna flikar
;
MSG_SERVER
Server
Server
Servidor
Serveur
Server
Server
Server
Server
Serwer
Server
;
MSG_APPLY
Apply
Übernehmen
Aplicar
Appliquer
Apply
Applica
Apply
Utføre
Zastosuj
Applicera
;
MSG_AUTO_JOIN_CHANNELS
Automatically join these channels
Diese Kanäle automatisch betreten
 Canales acceder auto.:
Rejoindre ce canal automatiquement
Automatically join these channels
Connetti autmaticamente a questi canali
Automatically join these channels
Automatisk joine disse kanalene
Automatycznie przy³±czaj do tych kana³ów
Anslut automatiskt till dessa kanaler
;
MSG_NICK_REGISTER_PASSWORD
Nick registration password
Paßwort für Spitznamen
 Contraseña registrar alias:
Mot de passe d'enregistrement de pseudo
Nick registration password
Password per il NickName
Nick registration password
Passord for nickregistrering
Has³o rejestracji pseudonimu
Lösenord nickregistering
;
MSG_NICK_REGISTER_PASSWORD_SHORT_HELP
This option will only work with a registered nick on a network with a Nickserv service.
Diese Option macht nur dann Sinn, wenn man einen registrierten Spitznamen bei einem Netzwerk mit diesem Service verwendet.
Funciona sólo con alias registrados en una red con un servicio de servidor de alias (Nickserv).
Cette option ne fonctionne qu'avec un pseudo enregistré sur un réseau ayant un service de Nickserv.
This option will only work with a registered nick on a network with a Nickserv service.
Opzione disponibile Ã¨ disponibile solo ad un NickName registrato ad una rete con servizio Nickserv.
This option will only work with a registered nick on a network with a Nickserv service.
Denne muligheten vil bare virke med et registrert nick på et nettverk med en Nickserv service.
Opcja ta dzia³a tylko z zarejestrowanymi pseudonimami w sieciach z us³ug± Nickserv.
Detta val fungerar bara med ett registrerat nick på ett nätverk med Nickserv service.
;
MSG_ACCEPT_CHANGES
Accept Changes
Änderungen annehmen
Aceptar cambios
Accepter les modifications
Accept Changes
Accetta
Accept Changes
Utfør forandringer
Akceptuj zmiany
Acceptera ändringar
;
MSG_SOUND_SAMPLE_NEW_MSG
Sample to play when a new PRIVMSG arrives
Klang der beim Eintreffen einer privaten Nachricht abgespielt werden soll
Mensaje privado (PRIVMSG):
Échantillon à jouer lors de l'arrivée d'un message privé
Sample to play when a new PRIVMSG arrives
Suono da riprodurre quando rivevi un messaggio privato
Sample to play when a new PRIVMSG arrives
Sample å spille når en ny PRIVMSG ankommer
D¼wiêk odtwarzany przy nowej wiadomo¶ci prywatnej
Ljudklipp att spela när nytt PRIVMSG anländer
;
MSG_SOUND_SAMPLE_PLAY_OPTION
When private tab is not selected
Wenn das private Fenster nicht angewählt ist
Si la pestaña no está seleccionada
Lorsqu'un onglet privé n'est pas actif
When private tab is not selected
Suono da riprodurre quando il Tab privato non Ã¨ selezionato
When private tab is not selected
Når private tabs ikke er valgt
Gdy zak³adka rozmowy prywatnej nie jest wybrana
När privat flik inte är vald
;
MSG_UNABLE_TO_RECONNECT_TO_SERVER_2
Unable to connect!
Keine Verbindung möglich!
No se ha podido conectar!
Impossible de se connecter!
Unable to connect!
Riconnessione impossibile!
Unable to connect!
Kan ikke koble til!
Nie mo¿na uzyskaæ po³±czenia!
Kan inte ansluta!
;
MSG_RENAME
Rename
Rename
Rename
Rename
Rename
Rename
Rename
Nytt navn
Zmieñ nazwê
Rename
;
MSG_ICONIFY_TEXT_NEW_MESSAGES_WAITING
New Messages Waiting
New Messages Waiting
New Messages Waiting
New Messages Waiting
New Messages Waiting
New Messages Waiting
New Messages Waiting
Nye beskjeder venter
Oczekuj± nowe wiadomo¶ci
New Messages Waiting
;
MSG_UPLOAD_PATH
Upload Path
Upload Path
Upload Path
Upload Path
Upload Path
Upload Path
Upload Path
Upload sti
¦cie¿ka wysy³ania
Upload Path
;
MSG_ONLY_WHEN_TAB_IS_INACTIVE
Only when Tab is inactive
Only when Tab is inactive
Only when Tab is inactive
Only when Tab is inactive
Only when Tab is inactive
Only when Tab is inactive
Only when Tab is inactive
Bare når tab er inaktiv
Only when Tab is inactive
Only when Tab is inactive
;
MSG_INVITE
Invite
Einladen
Invite
Invite
Invite
Invite
Invite
Invite
Invite
Invite
;
MSG_EVENT
Event
Ereignis
Event
Event
Event
Event
Event
Event
Event
Event
;
MSG_USEWHEN
Use when
Benutzen wenn
Use when
Use when
Use when
Use when
Use when
Use when
Use when
Use when
;
MSG_AREXXSCRIPT
Arexx Script
Arexx Skript
Arexx Script
Arexx Script
Arexx Script
Arexx Script
Arexx Script
Arexx Script
Arexx Script
Arexx Script
;
MSG_TEXTTOSENDTOSCRIPT
Text to send to script
Text to send to script
Text to send to script
Text to send to script
Text to send to script
Text to send to script
Text to send to script
Text to send to script
Text to send to script
Text to send to script
;
MSG_NAME
Name
Name
Name
Name
Name
Name
Name
Name
Name
Name
;
MSG_COMMAND
Command
Kommando
Command
Command
Command
Command
Command
Command
Command
Command
;
MSG_NICKLISTBUTTONS
Nick List Buttons
Nick List Buttons
Nick List Buttons
Nick List Buttons
Nick List Buttons
Nick List Buttons
Nick List Buttons
Nick List Buttons
Nick List Buttons
Nick List Buttons
;
MSG_CHANGENICK
Change nick
Change nick
Change nick
Change nick
Change nick
Change nick
Change nick
Change nick
Change nick
Change nick
;
MSG_ENTERNEWNICK
Enter new nick
Enter new nick
Enter new nick
Enter new nick
Enter new nick
Enter new nick
Enter new nick
Enter new nick
Enter new nick
Enter new nick
;
MSG_ENTERNICKCANCEL
Cancel
Cancel
Cancel
Cancel
Cancel
Cancel
Cancel
Cancel
Cancel
Cancel
;
MSG_ENTERNICKCHANGE
Change
Change
Change
Change
Change
Change
Change
Change
Change
Change
;
MSG_CLIPBOARD
Clipboard
Clipboard
Clipboard
Clipboard
Clipboard
Clipboard
Clipboard
Clipboard
Clipboard
Clipboard
;
MSG_HIDEJOINANDPARTMESSAGES
Hide Join and Part messages in Channels
Hide Join and Part messages in Channels
Hide Join and Part messages in Channels
Hide Join and Part messages in Channels
Hide Join and Part messages in Channels
Hide Join and Part messages in Channels
Hide Join and Part messages in Channels
Hide Join and Part messages in Channels
Hide Join and Part messages in Channels
Hide Join and Part messages in Channels
;
MSG_HIDECHANELMODEBUTTONS
Hide the channel mode buttons at the top of the display
Hide the channel mode buttons at the top of the display
Hide the channel mode buttons at the top of the display
Hide the channel mode buttons at the top of the display
Hide the channel mode buttons at the top of the display
Hide the channel mode buttons at the top of the display
Hide the channel mode buttons at the top of the display
Hide the channel mode buttons at the top of the display
Hide the channel mode buttons at the top of the display
Hide the channel mode buttons at the top of the display
;
MSG_CY_LISTVIEW
Listview
Listview
Listview
Listview
Listview
Listview
Listview
Listview
Listview
Listview
;
MSG_CY_BUTTONS
Buttons
Buttons
Buttons
Buttons
Buttons
Buttons
Buttons
Buttons
Buttons
Buttons
;
MSG_METHODOFDISPLAYINGTABS
Method of displaying Tabs for channels and private messages
Method of displaying Tabs for channels and private messages
Method of displaying Tabs for channels and private messages
Method of displaying Tabs for channels and private messages
Method of displaying Tabs for channels and private messages
Method of displaying Tabs for channels and private messages
Method of displaying Tabs for channels and private messages
Method of displaying Tabs for channels and private messages
Method of displaying Tabs for channels and private messages
Method of displaying Tabs for channels and private messages
;
MSG_USERNAMEWHENTALK
Display usermodes beside a persons nick when they talk
Display usermodes beside a persons nick when they talk
Display usermodes beside a persons nick when they talk
Display usermodes beside a persons nick when they talk
Display usermodes beside a persons nick when they talk
Display usermodes beside a persons nick when they talk
Display usermodes beside a persons nick when they talk
Display usermodes beside a persons nick when they talk
Display usermodes beside a persons nick when they talk
Display usermodes beside a persons nick when they talk
;
MSG_USERLIST_ADD
Add
Add
Add
Add
Add
Add
Add
Add
Add
Add
;
MSG_USERLIST_REMOVE
Remove
Remove
Remove
Remove
Remove
Remove
Remove
Remove
Remove
Remove
;
MSG_USERLIST_MOVEUP
Move up
Move up
Move up
Move up
Move up
Move up
Move up
Move up
Move up
Move up
;
MSG_USERLIST_MOVEDOWN
Move down
Move down
Move down
Move down
Move down
Move down
Move down
Move down
Move down
Move down
;
MSG_USERLIST_HELP
\033lCodes:\n%s = selected nick, %h = hostname of the selected nick,\n%c = current channel, %n = your own nick
\033lCodes:\n%s = selected nick, %h = hostname of the selected nick,\n%c = current channel, %n = your own nick
\033lCodes:\n%s = selected nick, %h = hostname of the selected nick,\n%c = current channel, %n = your own nick
\033lCodes:\n%s = selected nick, %h = hostname of the selected nick,\n%c = current channel, %n = your own nick
\033lCodes:\n%s = selected nick, %h = hostname of the selected nick,\n%c = current channel, %n = your own nick
\033lCodes:\n%s = selected nick, %h = hostname of the selected nick,\n%c = current channel, %n = your own nick
\033lCodes:\n%s = selected nick, %h = hostname of the selected nick,\n%c = current channel, %n = your own nick
\033lCodes:\n%s = selected nick, %h = hostname of the selected nick,\n%c = current channel, %n = your own nick
\033lCodes:\n%s = selected nick, %h = hostname of the selected nick,\n%c = current channel, %n = your own nick
\033lCodes:\n%s = selected nick, %h = hostname of the selected nick,\n%c = current channel, %n = your own nick
;
MSG_USERLIST_HIDEALL
Hide all from the display
Hide all from the display
Hide all from the display
Hide all from the display
Hide all from the display
Hide all from the display
Hide all from the display
Hide all from the display
Hide all from the display
Hide all from the display
;
MSG_EVENTPLAYHOLDER_HELP
\033lCodes:\n%b = text being displayed in the window, %t = timestamp,\n%g = network name, %s = servername, %c = channel, %n = own nick, %m = nick
\033lCodes:\n%b = text being displayed in the window, %t = timestamp,\n%g = network name, %s = servername, %c = channel, %n = own nick, %m = nick
\033lCodes:\n%b = text being displayed in the window, %t = timestamp,\n%g = network name, %s = servername, %c = channel, %n = own nick, %m = nick
\033lCodes:\n%b = text being displayed in the window, %t = timestamp,\n%g = network name, %s = servername, %c = channel, %n = own nick, %m = nick
\033lCodes:\n%b = text being displayed in the window, %t = timestamp,\n%g = network name, %s = servername, %c = channel, %n = own nick, %m = nick
\033lCodes:\n%b = text being displayed in the window, %t = timestamp,\n%g = network name, %s = servername, %c = channel, %n = own nick, %m = nick
\033lCodes:\n%b = text being displayed in the window, %t = timestamp,\n%g = network name, %s = servername, %c = channel, %n = own nick, %m = nick
\033lCodes:\n%b = text being displayed in the window, %t = timestamp,\n%g = network name, %s = servername, %c = channel, %n = own nick, %m = nick
\033lCodes:\n%b = text being displayed in the window, %t = timestamp,\n%g = network name, %s = servername, %c = channel, %n = own nick, %m = nick
\033lCodes:\n%b = text being displayed in the window, %t = timestamp,\n%g = network name, %s = servername, %c = channel, %n = own nick, %m = nick
;
MSG_EVENTMODE
Use when
Use when
Use when
Use when
Use when
Use when
Use when
Use when
Use when
Use when
;
MSG_EVENTMODE_HELP
Choose when you want to execute this ARexx script
Choose when you want to execute this ARexx script
Choose when you want to execute this ARexx script
Choose when you want to execute this ARexx script
Choose when you want to execute this ARexx script
Choose when you want to execute this ARexx script
Choose when you want to execute this ARexx script
Choose when you want to execute this ARexx script
Choose when you want to execute this ARexx script
Choose when you want to execute this ARexx script
;
;
; All strings below are used and renamed properly,
; so after rework is complete all strings left
; on top of this line are obsolete
;
MSG_REQUESTER_TITLE
Wookiechat Request...
Wookiechat Meldung ...
Wookiechat Request...
Wookiechat Request...
Wookiechat Request...
Wookiechat Request...
Wookiechat Request...
Wookiechat Request...
Wookiechat Request...
Wookiechat Request...
;
MSG_REQUESTER_OK_GADGETS
Ok
Ok
Ok
Ok
Ok
Ok
Ok
Ok
Ok
Ok
;
MSG_ERROR_UNABLETOOPENLIBRARY
Unable to open library\n'%s' version %ld!
Konnte die Library '%s'\nmit der Version %ld nicht öffnen!
Unable to open library\n'%s' version %ld!
Unable to open library\n'%s' version %ld!
Unable to open library\n'%s' version %ld!
Unable to open library\n'%s' version %ld!
Unable to open library\n'%s' version %ld!
Unable to open library\n'%s' version %ld!
Unable to open library\n'%s' version %ld!
Unable to open library\n'%s' version %ld!
;
MSG_ERROR_UNABLETOCREATEMSGPORT
Unable to create MessagePort!
Unable to create MessagePort!
Unable to create MessagePort!
Unable to create MessagePort!
Unable to create MessagePort!
Unable to create MessagePort!
Unable to create MessagePort!
Unable to create MessagePort!
Unable to create MessagePort!
Unable to create MessagePort!
;
MSG_ERROR_UNABLETOCREATEIOREQUEST
Unable to create IORequest!
Unable to create IORequest!
Unable to create IORequest!
Unable to create IORequest!
Unable to create IORequest!
Unable to create IORequest!
Unable to create IORequest!
Unable to create IORequest!
Unable to create IORequest!
Unable to create IORequest!
;
MSG_ERROR_UNABLETOOPENTIMERDEVICE
Unable to open 'timer.device'!
Unable to open 'timer.device'!
Unable to open 'timer.device'!
Unable to open 'timer.device'!
Unable to open 'timer.device'!
Unable to open 'timer.device'!
Unable to open 'timer.device'!
Unable to open 'timer.device'!
Unable to open 'timer.device'!
Unable to open 'timer.device'!
;
; MUICLASS_APPLICATION
;
MSG_MUICLASS_APPLICATION_DESCRIPTION
IRC Client
IRC Klient
IRC Client
IRC Client
IRC Client
IRC Client
IRC Client
IRC Client
IRC Client
IRC Client
;
;
; MUICLASS_WINDOWCHAT
;
MSG_MUICLASS_WINDOWCHAT_TITLE
WookieChat V%ld.%ld
WookieChat V%ld.%ld
WookieChat V%ld.%ld
WookieChat V%ld.%ld
WookieChat V%ld.%ld
WookieChat V%ld.%ld
WookieChat V%ld.%ld
WookieChat V%ld.%ld
WookieChat V%ld.%ld
WookieChat V%ld.%ld
;
MSG_MUICLASS_WINDOWCHAT_CLOSETAB_GAD
X
X
X
X
X
X
X
X
X
X
;
MSG_MUICLASS_WINDOWCHAT_CLOSETAB_HELP
Close the current tab.
Diesen Tabulator Schliessen.
Cerrar la pestaña actual.
Fermer l'onglet courant.
Êëåßóéìï ôçò ôùñéíŞò êáñôİëáò.
Chiudi questo Tab.
Jelenlegi panelt bezár.
Steng denne taben.
Zamyka aktualn± zak³adkê.
Stäng flik.
;
MSG_MUICLASS_WINDOWCHAT_TOPIC_HELP
This is the channels current topic.
Das ist das Grundthema.
La temática actual del canal.
Sujet courant du canal.
Ôï èİìá ôïõ êáíáëéïı.
Questo Ã¨ l'oggetto del canale.
A csatornák jelenlegi témája.
Dette er kanalens overskrift.
To jest temat aktualnego kana³u.
Detta är kanalens nuvarande ämne (topic).
;
MSG_MUICLASS_WINDOWCHAT_MODET_GAD
T
T
T
T
T
T
T
T
T
T
;
MSG_MUICLASS_WINDOWCHAT_MODET_HELP
Topic protection. This setting prevents regular users from changing the channel topic.
Themenschutz.\nSie müssen Operator sein, um das Thema ändern zu können.
Proteger temática. Evita a usuarios regulares\nque cambien la temática del canal.
Mode « Protection du sujet ». Dans ce mode les utilisateurs normaux ne sont pas autorisés à changer le sujet du canal.
Ğñïöıëáîç èİìáôïò. ÁõôŞ ç åğéëïãŞ áğáãïñåıåé óôïõò áğëïıò ÷ñŞóôåò íá ôï áëëÜîïõí.
Protezione del titolo, Questa impostazione impedisce agli utenti normali di cambiare il titolo del canale.
Téma rögzítése. Ez a beállítás az általános felhasználók számára megakadályozza a témaváltoztatást.
Overskriftsbeskyttelse. Denne forhindrer ordinære brukere fra å forandre kanaloverskriften.
Zabezpieczenie tematu. To ustawienie zapobiega zmianom tematu przez normalnych u¿ytkowników.
Topic protection. Hindrar vanliga användare från att ändra kanalens ämne.
;
MSG_MUICLASS_WINDOWCHAT_MODEN_GAD
N
N
N
N
N
N
N
N
N
N
;
MSG_MUICLASS_WINDOWCHAT_MODEN_HELP
No messaging. This setting forces users to be in the channel to be able to talk in it.
Keine Nachrichten.\nNachrichten von Clients ausserhalb des Kanals verbieten.
No mensajes. Obliga a los usuarios del\ncanal a permitir hablar en el mismo.
Mode « Pas de message ». Dans ce mode les utilisateurs extérieurs ne peuvent pas envoyer de messages sur le canal.
×ùñßò ìçíıìáôá. ÁõôŞ ç åğéëïãŞ áíáãêÜæåé ôïõò ÷ñŞóôåò íá ğáñåõñßóêïíôáé óôï êáíÜëé ãéá íá ìéëïıí óå áõôü.
Niente messaggi. Questa impostazione permette agli utenti nel canale di chattare.
Nincs üzenõ. Csak a csatornában lévõ felhasználók szólhatnak bele.
Ingen beskjeder. Dette tvinger brukerne til å være i kanalen for å kunne snakke i den.
Przesy³anie wiadomo¶ci zabronione. To ustawienie wymusza na u¿ytkownikach obecno¶æ na kanale aby móc na nim rozmawiaæ.
No messaging. Kräver att användaren finns i kanalen för att kunna skriva till den.
;
MSG_MUICLASS_WINDOWCHAT_MODES_GAD
S
S
S
S
S
S
S
S
S
S
;
MSG_MUICLASS_WINDOWCHAT_MODES_HELP
Secret mode. If this is set, the channel is invisible until you have joined.
Geheim-Modus.\nWenn gesetzt, ist der Kanal bis zum Eintreten unsichtbar.
Modo secreto. Hace al canal "invisible"\nhasta que accede a él.
Mode « Secret ». Dans ce mode, le canal est invisible jusqu'à ce que vous l'ayez rejoint.
Êñõöü. Áí ïñéóôåß áõôŞ ç åğéëïãŞ, ôï êáíÜëé åßíáé êñõöü ìİ÷ñé íá ìğåé êÜğïéïò ìİóá.
ModalitÃ  segreta. Se abilitata, il canale rimarrÃ  invisibile fino a che non vi si entri.
Titkosított. Beállításával a csatorna láthatatlan amíg nem csatlakozol.
Hemmelig modus. Hvis denne er satt er kanalen usynlig inntil du har entret den.
Tajny. Je¶li ta opcja jest w³±czona, kana³ jest niewidoczny, dopóki u¿ytkownik siê do niego nie do³±czy.
Secret mode. Om satt, är kanalen osynlig tills du gått in i den.
;
MSG_MUICLASS_WINDOWCHAT_MODEI_GAD
I
I
I
I
I
I
I
I
I
I
;
MSG_MUICLASS_WINDOWCHAT_MODEI_HELP
Invite only. Only users who have been invited to the channel may join.
Nur für eingeladene Benutzer.
Sólo invitados. Permite acceder unicamente\na usuarios que han sido invitados.
Mode « Invitations seules ». Dans ce mode, seuls les utilisateurs y ayant été invités peuvent rejoindre le canal.
Ìİóù ğñüóêëçóçò. Ìğïñïıí íá ìğïõí ìüíï ïé ğñïóêåêëçìİíïé ÷ñŞóôåò.
Solo invito. Disponibile ai soli utenti invitati nel canale.
Csak meghívóval. Csatlakozni csak a meghívottak részére lehetséges.
Kun invitasjon. Kun brukere som er invitert til kanalen kan entre den.
Dostêpny tylko za zaproszeniem. Tylko u¿ytkownicy, którzy zostan± zaproszeni bêd± mogli siê do³±czyæ.
Invite only. Bara inbjudna användare får komma in.
;
MSG_MUICLASS_WINDOWCHAT_MODEP_GAD
P
P
P
P
P
P
P
P
P
P
;
MSG_MUICLASS_WINDOWCHAT_MODEP_HELP
Private mode. If set, channel topic is not shown on global channel lists.
Privat-Modus.\nWenn gesetzt, wird das Thema dieses Kanals nicht in der allgemeinen Kanalliste angezeigt.
Modo privado. La temática del canal no se\nmuestra en la lista global de canales.
Mode « Privé ». Dans ce mode, le sujet du canal n'apparaît pas dans les listes de canaux globales.
Ğñïóùğéêü. ÅÜí İ÷åé ïñéóèåß, ôï èİìá ôïõ êáíáëéïı äåí åìöáíßæåôáé óôéò ãåíéêİò ëßóôåò ôùí êáíáëéşí.
ModalitÃ Â privata. Se abilitata, il titolo del canale Ã¨ invisibile nell' elenco globale dei canali.
Privát. Beállításával a csatorna témája nem jeleneik meg a globális csatorna listában.
Privat modus. Hvis denne er satt vises ikke kanalen i globale kanallister.
Prywatny. Je¶li ta opcja jest w³±czona, temat kana³u nie zostanie wy¶wietlony na globalnej li¶cie kana³ów.
Private mode. Om satt, kommer kanalens ämne inte att visas i globala listningar.
;
MSG_MUICLASS_WINDOWCHAT_MODEM_GAD
M
M
M
M
M
M
M
M
M
M
;
MSG_MUICLASS_WINDOWCHAT_MODEM_HELP
Moderated mode. Only users with operator, halfop or voice status (thats @ % or +) may talk in channel when set.
Moderierter Kanal.\nNur Nutzer mit Operator, Halfop oder Voice Status (das sind @ % oder +) dürfen in diesem Kanal sprechen, wenn gesetzt.
Modo moderado. Pueden hablar solamente\noperadores, medio-operadores o con voz\n(op[@], halfop[%%], voice[+]).
Mode « Modéré ». Dans ce mode, seuls les utilisateurs avec le statut d'opérateur, demi-opérateur ou voice (i.e. @ % ou +) peuvent parler.
ÓõíôçñçôéêŞ êáôÜóôáóç. Ìüíï ïé äéá÷åéñéóôİò, çìéäéá÷åéñéóôİò êáé ïé ÷ñŞóôåò ğïõ İ÷ïõí ïìéëßá (ïé @ % Ş +) ìğïñïıí íá ìéëŞóïõí óå áõôü ôï êáíÜëé.
ModalitÃ Â operatore. Solo utenti con stato di operatore, semioperatore o voce (cioÃ¨ @ % o +) potranno parlare.
Moderált. Csak az operator, halfop vagy voice (vagyis @ % vagy +) státuszban lehet beszélni a csatornában ennek bekapcsolásával.
Moderert modus. Bare brukere som er operatør, halvoperatør eller som har voice-status (de med @ % eller +) kan prate i kanalen.
Tryb moderowany. Po w³±czeniu tej opcji, tylko u¿ytkownicy ze statusem operatora, pó³operatora lub g³osem (@ % lub +) mog± rozmawiaæ na kanale.
Moderated mode. Bara användare med operator, halfop eller voice (dvs. @ % or +) får tala i kanalen.
;
MSG_MUICLASS_WINDOWCHAT_MODEB_GAD
B
B
B
B
B
B
B
B
B
B
;
MSG_MUICLASS_WINDOWCHAT_MODEB_HELP
Click to view a list of all banmasks for this channel.
Klicke um eine komplette Liste aller Bannmasken für diesen Kanal zu sehen.
Pulse para ver la lista de todas\nlas prohibiciones en este canal.
Cliquez pour voir une liste de tous les masques de ban de ce canal.
ĞáôŞóôå åäş ãéá íá äåßôå üëåò ôéò ban ìÜóêåò ãéá ôï êáíÜëé.
Clicca per vedere la lista completa delle maschere di Ban per questo canale.
Kattints ide a csatornához tartozó teljes banmaszk listáért.
Klikk for å vise en liste over alle banmasker for denne kanalen.
Naci¶nij, aby zobaczyæ listê wszystkich masek blokad dla tego kana³u.
Klicka för att visa en lista över kanalens ban villkor.
;
MSG_MUICLASS_WINDOWCHAT_MODEK_GAD
K
K
K
K
K
K
K
K
K
K
;
MSG_MUICLASS_WINDOWCHAT_MODEK_HELP
Keyword mode. When this is set, users need to supply a keyword (password) to enter.
Passwort-Modus, wurde er gesetzt, benötigen Nutzer ein Passwort um einzutreten.
Modo palabra clave. El usuario\ndebe indicar una palabra clave\n(contraseña) para entrar.
Mode « Clef ». Dans ce mode, les utilisateurs doivent fournir une clef (mot de passe) pour entrer.
Ìå êùäéêü. ÅÜí İ÷åé ïñéóèåß, ïé ÷ñŞóôåò ğñİğåé íá åéóÜãïõí ìéá ëİîç êëåéäß (êùäéêü) ãéá íá åéóİëèïõí.
ModalitÃ  parola chiave. Se impostata, gli utenti potranno accedere solo tramite password.
Kulcsszóval. Beállításával a felhasználól egy kulcsszó (jelszó) megadására kötelezettek.
Passordmodus. Når denne er satt, må brukere gi et passord for å entre kanalen.
Tryb z has³em. Je¶li ta opcja jest w³±czona, u¿ytkownicy bêd± musieli podaæ has³o, aby móc siê do³±czyæ.
Keyword mode. Om satt, behöver användare ge ett lösenord för att komma in.
;
MSG_MUICLASS_WINDOWCHAT_MODEKEYWORD_HELP
Current channel keyword.
gegenwärtiges Kanal Kennwort.
Contraseña actual del canal.
Clef actuelle.
Ëİîç êëåéäß ôïõ êáíáëéïı.
Attuale parola chiave del canale.
Jelenlegi csatorna kulcsszava.
Aktiv kanals passord..
Aktualne has³o kana³u.
Gällande lösenord.
;
MSG_MUICLASS_WINDOWCHAT_MODEL_GAD
L
L
L
L
L
L
L
L
L
L
;
MSG_MUICLASS_WINDOWCHAT_MODEL_HELP
Enforce a limit to how many users can currently join the channel.
Limitiert die Anzahl der Nutzer für diesen Kanal.
Forzar el límite de usuarios\nposibles a acceder al canal.
Mode « Limite ». Dans ce mode, une limite au nombre d'utilisateurs pouvant actuellement rejoindre le canal est positionnée.
ÁëëáãŞ ôïõ ïñßïõ ôùí ÷ñçóôşí ğïõ ìğïñïıí íá åéóİëèïõí óôï êáíÜëé.
Limita il numero di utenti abilitati ad accedere al canale.
Korlátozás bevezetése a csatornához jelenleg csatlakoztatható létszámban.
Setter en grense på hvor mange brukere som kan entre kanalen.
Ustala limit maksymalnie po³±czonych u¿ytkowników.
Sätt en gräns för hur många användare som kan komma in i kanalen.
;
MSG_MUICLASS_WINDOWCHAT_MODELIMIT_HELP
Current user limit.
Gegenwärtiges Benutzerlimit.
Límite actual de usuarios.
Limite d'utilisateurs actuelle.
¼ñéï ÷ñçóôşí.
Limite utenti.
Létszámkorlát.
Aktuell grense på brukere.
Obecny limit u¿ytkowników.
Nuvarande max användarantal.
;
MSG_MUICLASS_WINDOWCHAT_NICKLIST_HELP
Graphical Usermodes Key:\nGreen = Channel Operators or @\nBlue = HalfOps or \% \nYellow = Voiced Users or +
Grafische Nutzermodi:\nGrün = KanalOperator oder @\nBlau = HalbOperator oder %\nGelb = Sprachberechtigte Nutzer oder +
Colores según el tipo de usuario:\nVerde = operadores (@)\nAzul = medio-operadores (%)\nAmarillo = con voz (+)
Codes des modes utilisateurs graphiques :\nVert = Opérateur du canal ou @\nBleu = Demi-Opérateur ou \% \nJaune = Voice ou +
Graphical Usermodes Key:\nGreen = Channel Operators or @\nBlue = HalfOps or \% \nYellow = Voiced Users or +
Pulsante modalitÃ  di utilizzo grafico utente:\nVerde = Operatori canale o @\nBlu = MezziOperatori o \% \nGiallo = Voiced Users o +
Grafikus felhasználótípus kulcs:\nZöld = Csatorna Operátor vagy @\nKék = HalfOp vagy \% \nSárga = Voice felhasználó vagy +
Grafisk brukermodus nøkkel:\nGrønn = kanal-oper eller @\nBlå = halvoper eller \% \nGul = voice-brukere eller +
Graficzne tryby u¿ytkowników - podpowied¼:\nzielony = operator kana³u lub @\nniebieski = pó³operator kana³u lub \%\n¿ó³ty = u¿ytkownik z przyznanym g³osem lub +
Färgnyckel för grafiska användarlägen:\nGrön = Operator eller @\nBlå = HalfOp eller \% \nGul = Användare med röst eller +
;
MSG_MUICLASS_WINDOWCHAT_MESSAGE_HELP
Type your message here!
Geben sie ihre Nachricht oder ihr Kommando hier ein!
Type your message here!
Type your message here!
Type your message here!
Type your message here!

MSG_PREFS_GENERAL_GROUPTITLE
Type your message here!
Type your message here!
;
;---------------------------------------------------------
; MAIN WINDOW MENU
;
MSG_MENU_PROJECT_TITLE
Project
Project
Project
Project
Project
Project
Project
Project
Project
Project
;
MSG_MENU_SELECTSERVER_ITEM
Select server...
Server auswählen ...
Elegir servidor...
Choisir un serveur...
ÅğéëïãŞ äéáêïìéóôŞ...
Scegli server...
Kiszolgáló választása ...
Velg server...
Wybierz serwer...
Välj server ...
;
MSG_MENU_SELECTSERVER_KEY
S
S
S
S
S
S
S
S
S
S
;
MSG_MENU_NEWSERVERTAB_ITEM
New server tab
Neuen Server Tab öffnen
Nueva pestaña
Nouvel onglet serveur
Íİá êáñôİëá äéáêïìéóôŞ
Apri un Tab "server"
Új kiszolgáló panel
Ny servertab
Nowa zak³adka serwera
Ny serverflik
;
MSG_MENU_NEWSERVERTAB_KEY
T
T
T
T
T
T
T
T
T
T
;
MSG_MENU_NEWGLOBALQUERYTAB_ITEM
Open global query tab
Globalen Tab öffnen
Abrir pestaña global
Ouvrir onglet de requête global
¶íïéãìá ãåíéêŞò êáñôİëáò
Apri un tab "globale"
Globális panel megnyitása
Åpne global spørretab
Otwórz globaln± zak³adkê rozmów
Öppna flik för global förfrågan
;
MSG_MENU_NEWGLOBALQUERYTAB_KEY
G
G
G
G
G
G
G
G
G
G
;
MSG_MENU_CLOSESERVERTAB_ITEM
Close current tab
Aktiven Tab schliessen
Cerrar pestaña actual
Fermer l'onglet actif
Êëåßóéìï êáñôİëáò
Chiudi questo Tab
Jelenlegi panel bezárása
Lukk aktiv tab
Zamknij aktualn± zak³adkê
Stäng gällande flik
;
MSG_MENU_CLOSESERVERTAB_KEY
Z
Z
Z
Z
Z
Z
Z
Z
Z
Z
;
MSG_MENU_HIDE_ITEM
Hide
Verbergen
Iconificar
Masquer
Hide
Nascondi
Elrejt
Gjem
Ikonifikuj
Göm
;
MSG_MENU_HIDE_KEY
H
H
H
H
H
H
H
H
H
H
;
MSG_MENU_HISTORYCLEAR_ITEM
Clear buffer
Fensterinhalt leeren
Limpiar búfer
Vider le tampon
ÅêêáèÜñéóç ğñïó. ìíŞìçò
Cancella il buffer
Buffer törlése
Rense buffer
Wyczy¶æ bufor
Töm bufferten
;
MSG_MENU_HISTORYCLEARALL_ITEM
Clear all buffers
Alle Fenster leeren
Limpiar todos los bufers
Effacer tous les tampons
Clear all buffers
Pulisci il buffer
Clear all buffers
Rense alle buffere
Wyczy¶æ wszystkie bufory
Rensa alla buffertar
;
MSG_MENU_HISTORYSAVE_ITEM
Save buffer as...
Fensterinhalt speichern als ...
Guardar búfer como...
Enregistrer le tampon sous...
ÁğïèŞêåõóç ğñïó. ìíŞìçò ùò...
Salva il buffer come...
Buffer mentése, mint...
Lagre buffer som...
Zapisz bufor jako...
Spara buffert som ...
;
MSG_MENU_ABOUT_ITEM
About...
Über ...
Acerca de...
À propos...
Ğİñé ôïõ...
Informazioni...
Névjegy...
Om...
O programie...
Om ...
;
MSG_MENU_ABOUT_KEY
?
?
?
?
?
?
?
?
?
?
;
MSG_MENU_QUIT_ITEM
Quit...
Programm beenden ...
Salir...
Quitter...
¸îïäïò...
Esci...
Kilépés...
Avslutte...
Skoñcz...
Avsluta ...
;
MSG_MENU_QUIT_KEY
Q
Q
Q
Q
Q
Q
Q
Q
Q
Q
;
MSG_MENU_EDIT_TITLE
Edit
Edit
Edit
Edit
Edit
Edit
Edit
Edit
Edit
Edit
;
MSG_MENU_CUT_ITEM
Cut
Cut
Cut
Cut
Cut
Cut
Cut
Cut
Cut
Cut
;
MSG_MENU_CUT_KEY
X
X
X
X
X
X
X
X
X
X
;
MSG_MENU_COPY_ITEM
Copy
Copy
Copy
Copy
Copy
Copy
Copy
Copy
Copy
Copy
;
MSG_MENU_COPY_KEY
C
C
C
C
C
C
C
C
C
C
;
MSG_MENU_PASTE_ITEM
Paste
Paste
Paste
Paste
Paste
Paste
Paste
Paste
Paste
Paste
;
MSG_MENU_PASTE_KEY
V
V
V
V
V
V
V
V
V
V
;
MSG_MENU_SETTINGS_TITLE
Settings
Einstellungen
Ajustes
Réglages
Ñõèìßóåéò
Impostazioni
Beállítások
Innstillinger
Ustawienia
Inställningar
;
MSG_MENU_SETTINGSMAIN_ITEM
WookieChat...
WookieChat ...
WookieChat...
WookieChat...
WookieChat...
WookieChat...
WookieChat...
WookieChat...
WookieChat...
WookieChat...
;
MSG_MENU_SETTINGSCOLOR_ITEM
Color settings...
Farbeinstellungen ...
Colores...
Réglages des couleurs...
×ñùìáôéêİò ñõèìßóåéò...
Impostazione colori...
Szín beállítások...
Fargeinnstillinger...
Ustawienia kolorów...
Färginställningar ...
;
MSG_MENU_SETTINGSMUI_ITEM
MUI settings...
MUI Einstellungen ...
Ajustes MUI...
Réglages MUI...
Ñõèìßóåéò MUI...
Impostazioni MUI...
MUI beállítások...
MUI-innstillinger...
Ustawienia MUI...
MUI-inställningar ...
;
MSG_MENU_MULITCOLUMNCHATDISPLAY_ITEM
Multi-column chat display
Extra Spalte für Anzeige
Múltiples columnas
Affichage Multi-colonnes
ÅìöÜíéóç ğïëëşí óôçëşí
Schermo chat multicolonna
Több-oszlopos csevegõ kijelzõ
Multikolonne snakke-visning
Wy¶wietlaj rozmowy w wielu kolumnach
Visning med flera kolumner
;
MSG_MENU_COLUMNMARKINGCLIPBOARD_ITEM
Column-marking Clipboard
Spaltenmarkierung Zwischenspeicher
Marcar/Copiar por columnas
Presse-papiers multi colonne
Column-marking Clipboard
Clipboard marcante a colonna
Kijelölés vágólapra
Kolonne-markerende utklippstavle
Kopiuj do schowka zaznaczane kolumny
Urklippshanterare med kolumnmarkering
;
MSG_MENU_MUTEALLSOUNDS_ITEM
Mute all sounds
Mute all sounds
Mute all sounds
Mute all sounds
Mute all sounds
Mute all sounds
Mute all sounds
Mute all sounds
Mute all sounds
Mute all sounds
;
MSG_MENU_SETTINGSSAVE_ITEM
Save settings
Einstellungen speichern
Guardar ajustes
Enregistrer les réglages
ÁğïèŞêåõóç ñõèìßóåùí
Salva impostazioni
Beállítások mentése
Lagre innstillinger
Zapisz ustawienia
Spara inställningar
;
MSG_MENU_WINDOWS_TITLE
Windows
Fenster
Ventanas
Fenêtres
ĞáñÜèõñá
Finestre
Ablakok
Vindu
Okna
Fönster
;
MSG_MENU_DCCINCOMING_ITEM
Incoming File Transfers...
Ankommender Dateitransfer ...
Recibir ficheros...
Transferts des fichiers entrants...
ÌåôáöïñÜ åéóåñ÷üìåíïõ áñ÷åßïõ...
Trasmissione dati in ricevimento...
Beérkezõ fájl átvitel...
Innkommende filoverføringer...
Pliki przychodz±ce...
Inkommande fil ...
;
MSG_MENU_DCCINCOMING_KEY
D
D
D
D
D
D
D
D
D
D
;
MSG_MENU_DCCOUTGOING_ITEM
Outgoing File Transfers...
Ausgehender Dateitransfer ...
Enviar ficheros...
Transferts des fichiers sortants...
ÌåôáöïñÜ åîåñ÷üìåíïõ áñ÷åßïõ...
Trasmissione dati uscita...
Kimenõ fájl átvitel...
Utgående filoverføringer...
Pliki wychodz±ce...
Utgående fil ...
;
MSG_MENU_DCCOUTGOING_KEY
R
R
R
R
R
R
R
R
R
R
;
MSG_MENU_URLGRABBER_ITEM
URL Grabber...
URL Greifer ...
Gestor URLs...
Récupérateur d'URL...
URL Grabber...
URL Grabber...
URL Gyûjtõ...
URL-fanger...
Kolekcjoner adresów URL...
URL-Grabber ...
;
MSG_MENU_URLGRABBER_KEY
U
U
U
U
U
U
U
U
U
U
;
MSG_MENU_IGNORELIST_ITEM
Ignore List...
Ignorieren Liste ...
Lista ignorar...
Liste d'ignorés...
Ëßóôá áãíüçóçò...
Lista dei contatti ignorati...
Mellõzött lista...
Ignore-liste...
Lista ignorowania...
Ignoreringslista ...
;
MSG_MENU_IGNORELIST_KEY
I
I
I
I
I
I
I
I
I
I
;
;
; MUI Classes
;
MSG_ERROR_UNABLETOSETUPMUICLASS
Unable to set-up MUI class!
Konnte MUI Klasse nicht erzeuge!
Unable to set-up MUI class!
Unable to set-up MUI class!
Unable to set-up MUI class!
Unable to set-up MUI class!
Unable to set-up MUI class!
Unable to set-up MUI class!
Unable to set-up MUI class!
Unable to set-up MUI class!
;
;
; MUICLASS_WINDOWABOUT
;
MSG_MUICLASS_WINDOWABOUT_TITLE
About
About
About
About
About
About
About
About
About
About
;
MSG_MUICLASS_WINDOWABOUT_AREXXPORT_GAD
AREXX port:\x20
ARexx Port:\x20
Port ARexx :\x20
AREXX port:\x20
Porta AREXX:\x20
AREXX port:\x20
ARexx-port:\x20
Port Arexx'a:\x20
AREXX port:\x20
AREXX port:\x20
;
;
;
; MUICLASS_WINDOWURLGRABBER
;
MSG_MUICLASS_WINDOWURLGRABBER_TITLE
URL Grabber - Double click URL to Open
URL Greifer - Doppelklick um URL zu öffnen
Gestor URLs - Doble pulsación para abrirla
Récupérateur d'URL - Double-clic pour ouvrir une URL
URL Grabber - Ìå äéğëü êëéê óôï URL ãéá Üíïéãìá
Ottieni URL - Doppio click sull'URL per attivarlo
URL Gyûjtõ - Dupla kattintás az URL megnyitásához
URL Fanger - Dobbelklikk URL for å åpne
Kolekcjoner adresów URL - aby otowrzyæ dwukliknij na adresie
URL Grabber - Dubbelklicka URL för att öppna
;
MSG_MUICLASS_WINDOWURLGRABBER_REMOVE_GAD
_Remove URL
_Entferne URL
_Remove URL
_Remove URL
_Remove URL
_Remove URL
_Remove URL
_Remove URL
_Remove URL
_Remove URL
;
MSG_MUICLASS_WINDOWURLGRABBER_REMOVE_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_WINDOWURLGRABBER_REMOVEALL_GAD
Clear _All
Lösche _alles
Limpiar todo
Supprimer tout
ÅêêáèÜñéóç üëùí
Cancella tutto
Mindet törli
Fjerne alle
Wyczy¶æ wszystko
Rensa allt
;
MSG_MUICLASS_WINDOWURLGRABBER_REMOVEALL_HELP
/
/
/
/
/
/
/
/
/
/
;
;
; MUICLASS_URLLIST
;
MSG_LV_URL
URL
URL
URL
URL
URL
URL
URL
URL
URL
URL
;
;
; MUICLASS_WINDOWIGNORELIST
;
MSG_MUICLASS_WINDOWIGNORELIST_TITLE
Ignore List...
Ignorieren Liste ...
Lista ignorar...
Liste d'ignorés...
Ëßóôá áãíüçóçò...
Lista dei contatti ignorati...
Mellõzött lista...
Ignore-liste...
Lista ignorowania...
Ignoreringslista ...
;
MSG_MUICLASS_WINDOWIGNORELIST_ADD_GAD
Add
Hinzufügen
Añadir
Ajouter
Ğñüóèåóç
Aggiungi
Hozzáad
Legge til
Dodaj
Lägg till
;
MSG_MUICLASS_WINDOWIGNORELIST_ADD_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_WINDOWIGNORELIST_REMOVE_GAD
Remove
Entfernen
Eliminar
Supprimer
Áöáßñåóç
Rimuovi
Eltávolít
Fjerne
Usuñ
Ta bort
;
MSG_MUICLASS_WINDOWIGNORELIST_REMOVE_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_WINDOWIGNORELIST_PATTERN_GAD
Pattern:
Pattern:
Ignorar:
Ignorer :
Êáôá÷şñçóç áãíüçóçò:
Elemento da ignorare
Mellõzõ bejegyzés
Ignoremaske:
Ignoruj:
Ignorera enligt:
;
MSG_MUICLASS_WINDOWIGNORELIST_PATTERN_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_WINDOWIGNORELIST_PRIVMSG_GAD
Text
Text
Texto
Texte
Êåßìåíï
Testo
Szöveg
Tekst
Tekst
Text
;
MSG_MUICLASS_WINDOWIGNORELIST_PRIVMSG_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_WINDOWIGNORELIST_CTCP_GAD
CTCP
CTCP
CTCP
CTCP
CTCP
CTCP
CTCP
CTCP
CTCP
CTCP
;
MSG_MUICLASS_WINDOWIGNORELIST_CTCP_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_WINDOWIGNORELIST_DCC_GAD
DCC
DCC
DCC
DCC
DCC
DCC
DCC
DCC
DCC
DCC
;
MSG_MUICLASS_WINDOWIGNORELIST_DCC_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_WINDOWIGNORELIST_INFO
Hostmarks look like this: nick!userid@hostname\ni.e.«Join» jahc (~jahc00@jahc.Amigaworld.Net)\njahc is the nick, jahc00 is the userid, and jahc.Amigaworld.Net is the hostname\nTo ignore this person, \
you could insert an ignore entry of: jahc#?\nThis will ignore all people with a nick starting with the letters jahc.\nThe use of wildcards makes this ignore command very powerful.\nThings like this \
are possible: guru#?editatio?!#?userid@#?.se\nor if you want to keep it simple, you could just use: jahc#?
Hostmarks look like this: nick!userid@hostname\ni.e.«Join» jahc (~jahc00@jahc.Amigaworld.Net)\njahc is the nick, jahc00 is the userid, and jahc.Amigaworld.Net is the hostname\nTo ignore this person, \
you could insert an ignore entry of: jahc*!*@*\nThis will ignore all people with a nick starting with the letters jahc.\nThe use of wildcards makes this ignore command very powerful.\nThings like this \
are possible: guru#?editatio?!#?userid@#?.se\nor if you want to keep it simple, you could just use: jahc#?
\033cLas marcas de anfitrión suelen ser: alias!usuarioID@anfitrión\nEjemplo: .. [Acceder] jahc (~jahc00@Amigaworld.Net) ..\n\033bjahc\033n es el alias, \033bjahc00\033n el usuarioID y \
\033bAmigaworld.Net\033n el anfitrión.\nPara ignorar a esta persona, puede insertar la cadena: \033bjahc#?!#?@#?\033n\nEl uso de comodines hace que este comando sea muy potente.\nPuede hacer cosas como: \
\033bguru#?editatio?!#?userid@#?.se\033n\no si desea hacerlo más simple: \033broberto#?\033n
Les masques d'hôte ressemblent à ceci : \npseudo!utilisateur@machine\ni.e. [Entrée] jahc (~jahc00@jahc.Amigaworld.Net)\njahc est le pseudo, jahc00 l'utilisateur et\njahc.Amigaworld.Net le nom d'hôte.\n\
Pour ignorer cette personne, vous pouvez insérer\nle masque jahc#?!#?, cela ignorera toute personne\ndont le pseudo commence par 'jahc'.\nL'utilisation de jokers rend cette commande très\npuissante, des \
choses telles que guru#?editatio?!#?userid@#?.se\nsont possibles ou pour faire plus simple vous pouvez\nutiliser roberto#?
Ôï ìáñêÜñéóìá ãßíåôáé êÜğùò İôóé: nick!userid@hostname\nğ.÷. «Join» jahc (~jahc00@jahc.Amigaworld.Net)\nôï jahc åßíáé ôï øåõäşíõìï, ôï jahc00 åßíáé ôï üíïìá ÷ñŞóôç, åíş ôï jahc.Amigaworld.Net åßíáé ôï \
hostname\nÃéá íá áãíïŞóåôå áõôü ôï Üôïìï, èá ìğïñïıóáôå íá åéóÜãåôå ìßá êáôá÷şñçóç áãíüçóçò üğùò: jahc#?!#?@#?\nÁõôü èá áãíïŞóåé üëïõò ìå øåõäşíõìï ğïõ áñ÷ßæåé ìå ôá ãñÜììáôá jahc.\nÇ ÷ñŞóç ôùí ìğáëáíôİñ \
êÜíåé áõôŞ ôç ëåéôïõñãåßá ğïëı äõíáôŞ. Ìğïñåßôå íá ÷ñçóéìïğïéŞóåôå ğñÜãìáôá üğùò: guru#?editatio?!#?userid@#?.se\nŞ áí èİëåôå íá ôï êñáôŞóåôå áğëü, ìğïñåßôå íá ãñÜøåôå áğëÜ: roberto#?
Le Hostmarks sono composte come segue: nick!userid@hostname\ni.e.[Join] jahc (~jahc00@jahc.Amigaworld.Net)\njahc Ã¨ il nick, jahc00 Ã¨ la userid e jahc.Amigaworld.Net Ã¨ la persona\nda ignorare \
dall'hostname, puoi inserire un ignore tipo: jahc#?!#?@#?\nQuesto ignorerÃ Â  tutte le persone con un nick che inizia con le lettere jahc.\nL'uso delle WildCards rende il comando ignore molto \
potente.\nSono possibili anche se seguenti combinazioni: guru#?editatio?!#?userid@#?.se\nno se vuoi renderti la vita piÃ¹ semplice potresti usare: roberto#?
A hoszt jelzõk így néznek ki: becenév!azonosító@hosztnév\ni.e«Join» jahc (~jahc00@jahc.Amigaworld.Net)\njahc az becenév, jahc00 az azonosító és jahc.Amigaworld.Net a hoszt név\nBeszúrva egy mellõzõ \
tagok, ez a személy mellõzhetõ: jahc#?!#?@#?\nEz mindenkit mellõzni fog akinek az álneve jahc, vagy azzal kezdõdik.\nA helyettesítõ karakterek használata igen hatékonnyá teszi ezt a parancsot.\nAkár a \
következõket is megtehetjük: guru#?editatio?!#?userid@#?.se\nvagy ha egyszerüsíteni akarjuk, akár csak enyivel: jahc#?
Hostmerkene ser slik ut: nick!userid@hostname\nfor eks. «Join» jahc (~jahc00@jahc.Amigaworld.Net)\njahc er nicket, jahc00 er brukerens id, og jahc.Amigaworld.Net er hostnavnet\nFor å overse denne \
personen, kan du sette en ignoremaske som dette: jahc#?!#?@#?\nDette vil overse alle som har et nick som starter med "jahc".\nBruken av wildcards gjør denne ignoremasken veldig kraftfull.\nTing som dette \
er mulig: guru#?editatio?!#?userid@#?.se\neller hvis du vil gjøre det enkelt, kan du bare bruke for eksempel: roberto#?
Maska wygl±da tak: pseudonim!identyfikator@wêze³\nnp. [Join] jahc (~jahc00@jahc.Amigaworld.net)\njahc to pseudonim, jahc00 to identyfikator\na jahc.Amigaworld.net to nazwa wêz³a.\nAby ignorowaæ tê \
osobê, mo¿esz dodaæ do listy jahc#?!#?@#?\nWookieChatten sposób bêd± ignorowane wszystkie osoby\nz pseudonimem zaczynaj±cym siê od liter jahc.\nU¿ywanie znaków zastêpczych pozwala na tworzenie z³o¿onych \
regu³.\nMo¿na np. utworzyæ tak± regu³ê: guru#?editatio?!#?userid@#?.se\nlub je¶li chcesz sobie upro¶ciæ ¿ycie, mo¿esz u¿yæ po prostu: jahc#?
Värdinfo ser ut så här: nick!userid@hostname\ndvs.Join jahc (~jahc00@jahc.Amigaworld.Net)\njahc är nicket, jahc00 är anvädarid och jahc.Amigaworld.Net är värdnamnet\nFör att ignorera användaren kan du \
skriva in jahc#?!#?@#?\nDetta ignorerar alla nick som börjar med bokstäverna jahc.\nMed wildcards (#?) blir detta kommando mycket kraftfullt.\nMöjliga kombinationer: guru#?editatio?!#?userid@#?.se\neller för \
att göra det enkelt:  roberto#?
;
;
; MUICLASS_IGNORELIST
;
MSG_MUICLASS_IGNORELIST_DEFAULTPATTERN
User#?
Benutzer#?
User#?
User#?
User#?
User#?
User#?
User#?
User#?
User#?
;
MSG_LV_PATTERN
Pattern
Pattern
Pattern
Pattern
Pattern
Pattern
Pattern
Pattern
Pattern
Pattern
;
MSG_LV_TEXT
Text
Text
Texto
Texte
Êåßìåíï
Testo
Szöveg
Tekst
Tekst
Text
;
MSG_LV_CTCP
CTCP
CTCP
CTCP
CTCP
CTCP
CTCP
CTCP
CTCP
CTCP
CTCP
;
MSG_LV_DCC
DCC
DCC
DCC
DCC
DCC
DCC
DCC
DCC
DCC
DCC
;
MSG_LV_ALLOW
Allow
Erlauben
Allow
Allow
Allow
Allow
Allow
Allow
Allow
Allow
;
MSG_LV_IGNORE
Ignore
Verhindern
Ignore
Ignore
Ignore
Ignore
Ignore
Ignore
Ignore
Ignore
;
;
; MUICLASS_WINDOWDCCINCOMING
;
MSG_MUICLASS_WINDOWDCCINCOMING_TITLE
Incoming File Transfers...
Ankommender Dateitransfer ...
Recibir ficheros...
Transferts des fichiers entrants...
ÌåôáöïñÜ åéóåñ÷üìåíïõ áñ÷åßïõ...
Trasmissione dati in ricevimento...
Beérkezõ fájl átvitel...
Innkommende filoverføringer...
Pliki przychodz±ce...
Inkommande fil ...
;
MSG_MUICLASS_WINDOWDCCINCOMING_DOWNLOADPATH_GAD
Download Path:
Pfad zum Laden der Dateien:
Descargas:
Chemin de téléchargement :
Download Path:
Percorso per il download
Letöltés helye:
Nedlastingssti
¦cie¿ka pobierania
Sökväg för nedladdning
;
MSG_MUICLASS_WINDOWDCCINCOMING_DOWNLOADPATH_HELP
Choose a path to save your downloads to.
Wähle einen Pfad um Deine Downloads zu speichern.
Senda para las descargas.
Choisissez un chemin pour enregistrer vos téléchargements.
Åğéëİîôå ôïí êáôÜëïãï üğïõ èá áğïèçêåıïíôáé üóá êáôåâÜæåôå.
Scegli il percorso dove salvare i file ricevuti.
Válaszd ki a letöltések helyét.
Velg stinavn for å lagre dine filer til.
Wybierz ¶cie¿kê dostêpu zapisywania pobieranych plików.
Välj sökväg att spara fil på.
;
MSG_MUICLASS_WINDOWDCCINCOMING_ACCEPT_GAD
Accept
Akzeptieren
Aceptar
Accepter les modifications
Áğïäï÷Ş
Accetta
Elfogad
Godta
Akceptuj
Acceptera
;
MSG_MUICLASS_WINDOWDCCINCOMING_ACCEPT_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_WINDOWDCCINCOMING_REJECT_GAD
Reject
Ablehnen
Cancelar
Annuler
Áêıñùóç
Annulla
Mégsem
Avbryt
Poniechaj
Avbryt
;
MSG_MUICLASS_WINDOWDCCINCOMING_REJECT_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_WINDOWDCCINCOMING_REMOVE_GAD
Remove
Entfernen
Eliminar
Supprimer
Áöáßñåóç
Rimuovi
Eltávolítás
Fjern
Usuñ
Ta bort
;
MSG_MUICLASS_WINDOWDCCINCOMING_REMOVE_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_LV_STATUS
Status
Status
Estado
Statut
ÊáôÜóôáóç
Stato
Állapot
Status
Status
Status
;
MSG_LV_SENDER
Sender
Sender
Remitente
Expéditeur
Áğïóôïëİáò
Mittente
Küldi
Avsender
Nadawca
Avsändare
;
MSG_LV_RECIEVER
Reciever
Empfänger
Destinatario
Destinataire
Áğïäï÷İáò
Destinatario
Fogadja
Mottaker
Odbiorca
Mottagare
;
MSG_LV_FILENAME
Filename
Dateiname
Fichero
Nom de fichier
¼íïìá áñ÷åßïõ
Nome file
Fájlnév
Filnavn
Nazwa pliku
Filnamn
;
MSG_LV_RECIEVED
Recieved
Empfangen
Recibido
Reçu
ĞáñåëŞöèçóáí
Ricevuto
Fogadott
Mottatt
Odebrano
Mottaget
;
MSG_LV_SENT
Amount sent
Bereits übertragen
Enviado
Envoyé
Áğåóôáëìİíï ìİãåèïò
QuantitÃ Â trasmessa
Elküldött mennyiség
Sendt
Wys³ano
Skickat
;
MSG_LV_SIZE
Size
Größe
Tamaño
Taille
Ìİãåèïò
Dimensione
Méret
Filstørrelse
Wielko¶æ
Storlek
;
MSG_LV_TIMEREMAINING
Time Left
Vergangene Zeit
Restan
Temps restant
×ñüíïò ğïõ áğïìİíåé
Tempo rimanente
Hátralévõ idõ
Gjenværende tid
Pozosta³o czasu
Tid kvar
;
;
; MUICLASS_SETTINGSCOLOR
;
MSG_MUICLASS_SETTINGSCOLOR_TITLE
Color settings...
Farbeinstellungen ...
Colores...
Réglages des couleurs...
×ñùìáôéêİò ñõèìßóåéò...
Impostazione colori...
Szín beállítások...
Fargeinnstillinger...
Ustawienia kolorów...
Färginställningar ...
;
MSG_MUICLASS_SETTINGSCOLOR_LOAD_GAD
Load...
Laden ...
Cargar...
Charger...
Load...
Carica...
Load...
Hente...
Wczytaj...
Ladda...
;
MSG_MUICLASS_SETTINGSCOLOR_LOAD_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSCOLOR_SAVE_GAD
Save
Speichern
Guardar
Enregistrer
Save
Salva
Save
Lagre
Zapisz
Spara
;
MSG_MUICLASS_SETTINGSCOLOR_SAVE_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSCOLOR_SAVEAS_GAD
Save as...
Speichern als ...
Guardar como...
Enregistrer sous...
Save as...
Salva come...
Save as...
Lagre som...
Zapisz jako...
Spara som...
;
MSG_MUICLASS_SETTINGSCOLOR_SAVEAS_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSCOLOR_CHOOSECOLOR_TITLE
Choose Color
Farbe wählen
Elija color
Sélectionnez la couleur
Choose Color
Scegli colore
Choose Color
Velg farge
Wybierz kolor
Välj färg
;
MSG_MUICLASS_SETTINGSCOLOR_CHANNELBG_GAD
Channel background:
Kanalhintergrund:
Fondo:
Arrière-plan:
Background:
Sfondo:
Background:
Bakgrunn:
Background:
Bakgrund:
;
MSG_MUICLASS_SETTINGSCOLOR_CHANNELBG_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSCOLOR_NICKLISTBG_GAD
Nicklist background:
Namenslisten-Hintergrund:
Lista aliases (fondo):
Arrière-plan liste pseudos:
Nicklist background:
Sfondo Nicklist:
Nicklist background:
Bakgrunn for nickliste:
Nicklist background:
Bakgrund nicklista:
;
MSG_MUICLASS_SETTINGSCOLOR_NICKLISTBG_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSCOLOR_TABLISTBG_GAD
Tabs background:
Tab-Hintergrund:
 Lista cans./servs. (fondo):
Arrière plan des onglets:
Tabs background:
Sfondo Tab:
Tabs background:
Tabs bakgrunn:
T³o zak³adek:
Bakgrund flikar:
;
MSG_MUICLASS_SETTINGSCOLOR_TABLISTBG_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSCOLOR_NORMAL_GAD
Normal:
Normal:
 Normal:
Normal:
Normal:
Normale:
Normal:
Normal:
Normalny:
Normal:
;
MSG_MUICLASS_SETTINGSCOLOR_NORMAL_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSCOLOR_NICKLISTTX_GAD
Nicklist text:
Namenslisten-Text:
Lista aliases (texto):
Texte de la liste de pseudos:
Nicklist text:
Testo Nicklist:
Nicklist text:
Tekst nickliste:
Pisak listy pseudonimów:
Text nicklista:
;
MSG_MUICLASS_SETTINGSCOLOR_NICKLISTTX_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSCOLOR_TABSPEN_GAD
Tabs pen:
Schriftfarbe der Tabs:
 Lista cans./servs. (texto):
Texte des onglets:
Tabs pen:
Penna Tab:
Tabs pen:
Tabs penn:
Pisak zak³adek:
Penna flikar:
;
MSG_MUICLASS_SETTINGSCOLOR_TABSPEN_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSCOLOR_JOIN_GAD
Join:
Verbunden:
Acceder:
Join:
Join:
Entra:
Join:
Entre:
Przy³±czenie:
Anslut:
;
MSG_MUICLASS_SETTINGSCOLOR_JOIN_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSCOLOR_PART_GAD
Part:
Part:
Abandonar:
Part:
Part:
Part:
Part:
Forlater:
Od³±czenie:
Lämna:
;
MSG_MUICLASS_SETTINGSCOLOR_PART_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSCOLOR_QUIT_GAD
Quit:
Beenden:
Salir:
Quitter:
Quit:
Esci:
Quit:
Avslutter:
Koniec:
Avsluta:
;
MSG_MUICLASS_SETTINGSCOLOR_QUIT_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSCOLOR_MODES_GAD
Modes:
Modi:
Modos:
Modes:
Modes:
ModalitÃ :
Modes:
Moduser:
Zmiana trybu:
Lägen:
;
MSG_MUICLASS_SETTINGSCOLOR_MODES_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSCOLOR_CTCP_GAD
CTCP:
CTCP:
 CTCP:
CTCP:
CTCP:
CTCP:
CTCP:
CTCP:
CTCP:
CTCP:
;
MSG_MUICLASS_SETTINGSCOLOR_CTCP_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSCOLOR_ACTIONS_GAD
Actions:
Aktionen:
Acciones:
Actions:
Actions:
Azioni:
Actions:
Handlinger:
Dzia³anie:
Händelser:
;
MSG_MUICLASS_SETTINGSCOLOR_ACTIONS_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSCOLOR_INFO_GAD
Info:
Informationen:
Información:
Info:
Info:
Info:
Info:
Info:
Informacja:
Info:
;
MSG_MUICLASS_SETTINGSCOLOR_INFO_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSCOLOR_OWNTEXT_GAD
Own text:
Eigener Text:
Texto propio:
Texte personnel:
Own text:
Testo personale:
Own text:
Egen tekst:
W³asny tekst:
Egen text:
;
MSG_MUICLASS_SETTINGSCOLOR_OWNTEXT_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSCOLOR_HIGHLIGHT_GAD
Highlight:
Hervorheben:
Resaltar:
Mise en évidence:
Highlight:
Evidenzia:
Highlight:
Høylys:
Pod¶wietlenie:
Framhäv:
;
MSG_MUICLASS_SETTINGSCOLOR_HIGHLIGHT_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSCOLOR_NOTICES_GAD
Notices:
Servermeldungen:
Noticias:
Notices:
Notices:
Avvisi:
Notices:
Notices:
Powiadomienia:
Notiser:
;
MSG_MUICLASS_SETTINGSCOLOR_NOTICES_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSCOLOR_INVITE_GAD
Invite:
Einladungen:
Invitar:
Invitation:
Invite:
Invita:
Invite:
Invitere:
Zaproszenie:
Bjud in:
;
MSG_MUICLASS_SETTINGSCOLOR_INVITE_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSCOLOR_KICK_GAD
Kick:
Rausschmiss:
Expulsar:
Kick:
Kick:
Kick:
Kick:
Kicke:
Wyrzucenie:
Kicka:
;
MSG_MUICLASS_SETTINGSCOLOR_KICK_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSCOLOR_NICKCHANGE_GAD
Nick change:
Namenswechsel:
Cambiar alias:
Changement de pseudo:
Nick change:
Cambia Nick:
Nick change:
Forandrer nick:
Zmiana pseudonimu:
Ändring nick:
;
MSG_MUICLASS_SETTINGSCOLOR_NICKCHANGE_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSCOLOR_TOPIC_GAD
Topic:
Thema:
Temática:
Sujet:
Topic:
Argomento:
Topic:
Topic:
Temat:
Ämne:
;
MSG_MUICLASS_SETTINGSCOLOR_TOPIC_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSCOLOR_WALLOPS_GAD
Wall ops:
Wall ops:
OPs:
Wall ops:
Wall ops:
Wall ops:
Wall ops:
Wall ops:
Informacje od operatorów:
Wall ops:
;
MSG_MUICLASS_SETTINGSCOLOR_WALLOPS_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSCOLOR_ACTIVITY_GAD
Activity:
Tab-Aktivität:
Actividad:
Activité:
Activity:
AttivitÃ :
Activity:
Aktivitet:
Aktywno¶æ:
Aktivitet:
;
MSG_MUICLASS_SETTINGSCOLOR_ACTIVITY_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSCOLOR_CHATACTIVITY_GAD
Chat activity:
Tab-Chatanzeige:
Actividad (charla):
Discussion:
Chat activity:
AttivitÃ  Chat:
Chat activity:
Chatteaktivitet:
Aktywno¶æ - rozmowa:
Chat aktivitet:
;
MSG_MUICLASS_SETTINGSCOLOR_CHATACTIVITY_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSCOLOR_HIGHLIGHTACTIVITY_GAD
Highlight activity:
Tab-Hervorhebeanzeige:
Actividad (resaltar):
Mise en évidence:
Highlight activity:
Evidenzia attivitÃ :
Highlight activity:
Høylys aktivitet:
Aktywno¶æ - pod¶wietlenie:
Framhäv aktivitet:
;
MSG_MUICLASS_SETTINGSCOLOR_HIGHLIGHTACTIVITY_HELP
/
/
/
/
/
/
/
/
/
/
;
;
; MUICLASS_WINDOWQUIT
;
MSG_MUICLASS_WINDOWQUIT_QUIT_TEXT
Do you really want to quit WookieChat?
Wollen sie wirklick WookieChat beenden?
Do you really want to quit WookieChat?
Do you really want to quit WookieChat?
Do you really want to quit WookieChat?
Do you really want to quit WookieChat?
Do you really want to quit WookieChat?
Do you really want to quit WookieChat?
Do you really want to quit WookieChat?
Do you really want to quit WookieChat?
;
MSG_MUICLASS_WINDOWQUIT_OK_GAD
_Ok
_Ok
_Ok
_Ok
_Ok
_Ok
_Ok
_Ok
_Ok
_Ok
;
MSG_MUICLASS_WINDOWQUIT_OK_HELP
/
/
/
/
/
/
/
/
/
/
;
;
MSG_MUICLASS_WINDOWQUIT_CANCEL_GAD
_Cancel
_Abbrechen
_Cancel
_Cancel
_Cancel
_Cancel
_Cancel
_Cancel
_Cancel
_Cancel
;
MSG_MUICLASS_WINDOWQUIT_CANCEL_HELP
/
/
/
/
/
/
/
/
/
/
;
;
; MUICLASS_WINDOWSETTINGS
;
MSG_MUICLASS_WINDOWSETTINGS_TITLE
Settings...
Einstellungen ...
Settings...
Settings...
Settings...
Settings...
Settings...
Settings...
Settings...
Settings...
;
MSG_PG_NICK
Nick
Nick
Nick
Nick
Nick
Nick
Nick
Nick
Nick
Nick
;
MSG_PG_SERVER
Server
Server
Servidores
Serveurs
Äéáêïìéóôİò
Server
Kiszolgálók
Servere
Serwery
Servrar
;
MSG_PG_GENERAL
General
Allgemein
General
Général
ÃåíéêÜ
Generale
Általános
Generell
G³ówne
Generellt
;
MSG_PG_COLOR
Color
Farben
Color
Color
Color
Color
Color
Color
Color
Color
;
MSG_PG_GUI
GUI
Oberfläche
GUI
GUI
GUI
GUI
GUI
GUI
GUI
GUI
;
MSG_PG_LOGGING
Logging
Mitschnitte
Registrar
Logging
ÊáôáãñáöŞ
Protocollo
Naplózás
Logging
Raportowanie
Logga
;
MSG_PG_DCC
DCC
DCC
DCC
DCC
DCC
DCC
DCC
DCC
DCC
DCC
;
MSG_PG_SOUNDS
Sounds
Klänge
Sonidos
Sons
¹÷ïé
Suoni
Hangok
Lyder
D¼wiêki
Ljud
;
MSG_PG_ALIASES
Aliases
Aliases
Aliases
Alias
Óõíşíõìá
Alias (sinonimi)
Helyettesítõk
Aliaser
Aliasy
Alias
;
MSG_PG_BUTTONS
Buttons
Tasten
Buttons
Buttons
Buttons
Buttons
Buttons
Buttons
Buttons
Buttons
;
MSG_PG_EVENTS
Events
Ereignisse
Events
Events
Events
Events
Events
Events
Events
Events
;
MSG_MUICLASS_SETTINGS_SAVE_GAD
_Save
_Save
_Save
_Save
_Save
_Save
_Save
_Save
_Save
_Save
;
MSG_MUICLASS_SETTINGS_SAVE_HELP
Store settings on disk.
Store settings on disk.
Store settings on disk.
Store settings on disk.
Store settings on disk.
Store settings on disk.
Store settings on disk.
Store settings on disk.
Store settings on disk.
Store settings on disk.
;
MSG_MUICLASS_SETTINGS_USE_GAD
_Use
_Use
_Use
_Use
_Use
_Use
_Use
_Use
_Use
_Use
;
MSG_MUICLASS_SETTINGS_USE_HELP
Use current settings.
Use current settings.
Use current settings.
Use current settings.
Use current settings.
Use current settings.
Use current settings.
Use current settings.
Use current settings.
Use current settings.
;
MSG_MUICLASS_SETTINGS_CANCEL_GAD
_Cancel
_Cancel
_Cancel
_Cancel
_Cancel
_Cancel
_Cancel
_Cancel
_Cancel
_Cancel
;
;
MSG_MUICLASS_SETTINGS_CANCEL_HELP
Restore settings to last used state.
Restore settings to last used state.
Restore settings to last used state.
Restore settings to last used state.
Restore settings to last used state.
Restore settings to last used state.
Restore settings to last used state.
Restore settings to last used state.
Restore settings to last used state.
Restore settings to last used state.
;
;
; MUICLASS_SETTINGSGENERAL
;
MSG_MUICLASS_SETTINGSGENERAL_MESSAGEWHEN_INFO
Message at...
Nachricht bei ...
Message at...
Message at...
Message at...
Message at...
Message at...
Message at...
Message at...
Message at...
;
MSG_MUICLASS_SETTINGSGENERAL_MESSAGEKICK_GAD
Kick:
Kick:
Kick:
Kick:
Kick:
Kick:
Kick:
Kick:
Kick:
Kick:
;
MSG_MUICLASS_SETTINGSGENERAL_MESSAGEKICK_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSGENERAL_MESSAGEQUIT_GAD
Quit:
Beenden
Quit:
Quit:
Quit:
Quit:
Quit:
Quit:
Quit:
Quit:
;
MSG_MUICLASS_SETTINGSGENERAL_MESSAGEQUIT_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSGENERAL_MESSAGEPART_GAD
Part:
Part:
Part:
Part:
Part:
Part:
Part:
Part:
Part:
Part:
;
MSG_MUICLASS_SETTINGSGENERAL_MESSAGEPART_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSGENERAL_DOUBLECLICKONNICKLIST_INFO
Double click on nick list causes...
Doppelklick auf Nickliste erzeugt ...
Double click on nick list causes...
Double click on nick list causes...
Double click on nick list causes...
Double click on nick list causes...
Double click on nick list causes...
Double click on nick list causes...
Double click on nick list causes...
Double click on nick list causes...
;
MSG_MUICLASS_SETTINGSGENERAL_OPENPRIVATEQUERY_GAD
private query.
private query.
private query.
private query.
private query.
private query.
private query.
private query.
private query.
private query.
;
MSG_MUICLASS_SETTINGSGENERAL_OPENPRIVATEQUERY_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSGENERAL_REQUESTWHOIS_GAD
requests whois information.
fordert Whois Informationen an.
requests whois information.
requests whois information.
requests whois information.
requests whois information.
requests whois information.
requests whois information.
requests whois information.
requests whois information.
;
MSG_MUICLASS_SETTINGSGENERAL_REQUESTWHOIS_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSGENERAL_EDITOR_GAD
Editor:
Editor:
Editor:
Editor:
Editor:
Editor:
Editor:
Editor:
Editor:
Editor:
;
MSG_MUICLASS_SETTINGSGENERAL_EDITOR_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSGENERAL_BROWSER_GAD
Browser:
Browser:
Browser:
Browser:
Browser:
Browser:
Browser:
Browser:
Browser:
Browser:
;
MSG_MUICLASS_SETTINGSGENERAL_BROWSER_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSGENERAL_NICKCOMPLETITION_GAD
Nick completition style:
Nick completition style:
Nick completition style:
Nick completition style:
Nick completition style:
Nick completition style:
Nick completition style:
Nick completition style:
Nick completition style:
Nick completition style:
;
MSG_MUICLASS_SETTINGSGENERAL_NICKCOMPLETITION_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_CY_MIRC
mIRC
mIRC
mIRC
mIRC
mIRC
mIRC
mIRC
mIRC
mIRC
mIRC
;
MSG_CY_AMIRC
AmIRC
AmIRC
AmIRC
AmIRC
AmIRC
AmIRC
AmIRC
AmIRC
AmIRC
AmIRC
;
MSG_MUICLASS_SETTINGSGENERAL_AUTOMATIC_INFO
Automatic...
Automatisch ...
Automatic...
Automatic...
Automatic...
Automatic...
Automatic...
Automatic...
Automatic...
Automatic...
;
MSG_MUICLASS_SETTINGSGENERAL_REJOIN_GAD
rejoin when kicked.
rejoin when kicked.
rejoin when kicked.
rejoin when kicked.
rejoin when kicked.
rejoin when kicked.
rejoin when kicked.
rejoin when kicked.
rejoin when kicked.
rejoin when kicked.
;
;
MSG_MUICLASS_SETTINGSGENERAL_REJOIN_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSGENERAL_OPENQUERY_GAD
open query when being messaged.
open query when being messaged.
open query when being messaged.
open query when being messaged.
open query when being messaged.
open query when being messaged.
open query when being messaged.
open query when being messaged.
open query when being messaged.
open query when being messaged.
;
MSG_MUICLASS_SETTINGSGENERAL_OPENQUERY_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSGENERAL_HIGHLIGHTPATTERN_GAD
Highlight Pattern:
Highlight Pattern:
Highlight Pattern:
Highlight Pattern:
Highlight Pattern:
Highlight Pattern:
Highlight Pattern:
Highlight Pattern:
Highlight Pattern:
Highlight Pattern:
;
MSG_MUICLASS_SETTINGSGENERAL_HIGHLIGHT_HELP
/
/
/
/
/
/
/
/
/
/
;
;
; MUICLASS_SETTINGSSOUND
;
MSG_MUICLASS_SETTINGSSOUND_ENABLED_GAD
Audio output
Tonausgabe aktiv
Audio output
Audio output
Audio output
Audio output
Audio output
Audio output
Audio output
Audio output
;
MSG_MUICLASS_SETTINGSSOUND_ENABLED_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSSOUND_PLAYSAMPLE_INFO
Play sample...
Ton abspielen ...
Play sample...
Play sample...
Play sample...
Play sample...
Play sample...
Play sample...
Play sample...
Play sample...
;
MSG_MUICLASS_SETTINGSSOUND_TABOPENING_GAD
on tab opening:
on tab opening:
on tab opening:
on tab opening:
on tab opening:
on tab opening:
on tab opening:
on tab opening:
on tab opening:
on tab opening:
;
MSG_MUICLASS_SETTINGSSOUND_TABOPENING_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSSOUND_TABOPENINGSAMPLE_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSSOUND_ONHIGHLIGHT_GAD
on highlight:
on highlight:
on highlight:
on highlight:
on highlight:
on highlight:
on highlight:
on highlight:
on highlight:
on highlight:
;
MSG_MUICLASS_SETTINGSSOUND_ONHIGHLIGHT_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSSOUND_ONHIGHLIGHTSAMPLE_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSSOUND_ONPRIVMSG_GAD
when private message arrives:
when private message arrives:
when private message arrives:
when private message arrives:
when private message arrives:
when private message arrives:
when private message arrives:
when private message arrives:
when private message arrives:
when private message arrives:
;
MSG_MUICLASS_SETTINGSSOUND_ONPRIVMSG_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSSOUND_CTCPSAMPLES_GAD
CTCP Samples:
CTCP Samples:
CTCP Samples:
CTCP Samples:
CTCP Samples:
CTCP Samples:
CTCP Samples:
CTCP Samples:
CTCP Samples:
CTCP Samples:
;
MSG_MUICLASS_SETTINGSSOUND_CTCPSAMPLES_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSSOUND_USESAMPLEPLAYER_GAD
Use external sample player:
Externes Programm zum Abspielen nutzen:
Use external sample player:
Use external sample player:
Use external sample player:
Use external sample player:
Use external sample player:
Use external sample player:
Use external sample player:
Use external sample player:
;
MSG_MUICLASS_SETTINGSSOUND_USESAMPLEPLAYER_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_CY_NEVER
Never
Never
Never
Never
Never
Never
Never
Never
Never
Never
;
MSG_CY_ALWAYS
Always
Always
Always
Always
Always
Always
Always
Always
Always
Always
;
MSG_CY_WINDOWISINACTIVE
when window is inactive
when window is inactive
when window is inactive
when window is inactive
when window is inactive
when window is inactive
when window is inactive
when window is inactive
when window is inactive
when window is inactive
;
MSG_CY_TABISINACTIVE
when tab is inactive
when tab is inactive
when tab is inactive
when tab is inactive
when tab is inactive
when tab is inactive
when tab is inactive
when tab is inactive
when tab is inactive
when tab is inactive
;
;
; MUICLASS_SETTINGSGUI
;
MSG_MUICLASS_SETTINGSGUI_TABS_INFO
Show tabs...
Zeige Tabs ...
Show tabs...
Show tabs...
Show tabs...
Show tabs...
Show tabs...
Show tabs...
Show tabs...
Show tabs...
;
MSG_MUICLASS_SETTINGSGUI_TABBUTTON_GAD
in button-style.
im Knopfstil.
in button-style.
in button-style.
in button-style.
in button-style.
in button-style.
in button-style.
in button-style.
in button-style.
;
MSG_MUICLASS_SETTINGSGUI_TABBUTTON_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSGUI_TABSERVER_GAD
for servers.
für Server.
for servers.
for servers.
for servers.
for servers.
for servers.
for servers.
for servers.
for servers.
;
MSG_MUICLASS_SETTINGSGUI_TABSERVER_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSGUI_NICKLIST_INFO
Nick list...
Nick list...
Nick list...
Nick list...
Nick list...
Nick list...
Nick list...
Nick list...
Nick list...
Nick list...
;
MSG_MUICLASS_SETTINGSGUI_NICKLISTWIDTH_GAD
Width:
Breite:
Width:
Width:
Width:
Width:
Width:
Width:
Width:
Width:
;
MSG_MUICLASS_SETTINGSGUI_NICKLISTWIDTH_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSGUI_NICKLISTHEIGHT_GAD
Height:
Höhe:
Width:
Width:
Width:
Width:
Width:
Width:
Width:
Width:
;
MSG_MUICLASS_SETTINGSGUI_NICKLISTHEIGHT_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSGUI_NICKLISTGFXINFO_GAD
with graphical info.
mit grafischen Zusatz.
with graphical info.
with graphical info.
with graphical info.
with graphical info.
with graphical info.
with graphical info.
with graphical info.
with graphical info.
;
MSG_MUICLASS_SETTINGSGUI_NICKLISTGFXINFO_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSGUI_NICKLISTREMOVECHARS_GAD
without '>' and '<' in nick.
ohne '>' und '<' im Nick.
without '>' and '<' in nick.
without '>' and '<' in nick.
without '>' and '<' in nick.
without '>' and '<' in nick.
without '>' and '<' in nick.
without '>' and '<' in nick.
without '>' and '<' in nick.
without '>' and '<' in nick.
;
MSG_MUICLASS_SETTINGSGUI_NICKLISTREMOVECHARS_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSGUI_SMILIES_GAD
Use graphical smilies with a
Grafische Smilies mit einem
Use graphical smilies with a
Use graphical smilies with a
Use graphical smilies with a
Use graphical smilies with a
Use graphical smilies with a
Use graphical smilies with a
Use graphical smilies with a
Use graphical smilies with a
;
MSG_MUICLASS_SETTINGSGUI_SMILIES_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSGUI_SMILIESTHEMEBACKGROUND_INFO
background.
Hintergrund benutzen.
background.
background.
background.
background.
background.
background.
background.
background.
;
MSG_MUICLASS_SETTINGSGUI_SMILIESTHEME_GAD
_t
_t
_t
_t
_t
_t
_t
_t
_t
_t
;
MSG_MUICLASS_SETTINGSGUI_SMILIESTHEME_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_CY_TRANSPARENT
transparent
transparenten
transparent
transparent
transparent
transparent
transparent
transparent
transparent
transparent
;
MSG_CY_WHITE
white
Weißen
white
white
white
white
white
white
white
white
;
MSG_CY_BLACK
black
schwarzen
black
black
black
black
black
black
black
black
;
MSG_SL_PIXEL
%ld Pixel
%ld Pixel
%ld Pixel
%ld Pixel
%ld Pixel
%ld Pixel
%ld Pixel
%ld Pixel
%ld Pixel
%ld Pixel
;
MSG_MUICLASS_SETTINGSGUI_CHANNELVIEW_INFO
Channel view...
Kanelansicht ...
Channel view...
Channel view...
Channel view...
Channel view...
Channel view...
Channel view...
Channel view...
Channel view...
;
MSG_MUICLASS_SETTINGSGUI_TIMESHOW_GAD
Show time in format
Zeige Uhrzeit im Format
Show time in format
Show time in format
Show time in format
Show time in format
Show time in format
Show time in format
Show time in format
Show time in format
;
MSG_MUICLASS_SETTINGSGUI_TIMESHOW_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSGUI_TIMEFORMAT_GAD
_f
_f
_f
_f
_f
_f
_f
_f
_f
_f
;
MSG_MUICLASS_SETTINGSGUI_TIMEFORMAT_HELP
Display format of timestamps! use these codes: %h for the hour, %m for the minute, and %s for the second. This is useful to see what time an event happened (such as a user joining, or speaking).
Anzeigeformat der Uhrzeit. %h für Stunden, %m für Minuten und %s für Sekunden. Das ist nützlich, um zu sehen, wann ein Ereignis stattfand.(wie Login/logout, oder sprechen).
Configurar el formato de la hora.\nÚtil para saber cuando pasó un evento\n(acceder a un canal, hablar,...).\n\033b%h\033n - hora; \033b%m\033n - minutos; \033b%s\033n - segundos.
Personnalisez l'horodatage. Utilisez ces codes %h pour l'heure, %m pour les minutes et %s pour les secondes. C'est utile pour retrouver à quelle heure un évènement est survenu (comme l'arrivée d'un \
utilisateur ou une annonce).
ÁëëÜîôå ôçí åìöÜíéóç ôùí åíäåßîåùí ÷ñüíïõ! ×ñçóéìïğïéŞóôå ôï %h ãéá ôçí şñá, ôï %m ãéá ôá ëåğôÜ êáé ôï %s ãéá ôá äåõôåñüëåğôá. ×ñŞóéìï, ãéá íá ğáñáêïëïõèåßôå ğüôå İãéíå İíá óõìâÜí, üğùò ğüôå ìğŞêå \
óôï êáíÜëé êÜğïéïò ÷ñŞóôçò Ş ğüôå åßğå êÜôé.
Customizza il formato della data e dell'ora! Utilizza %h per l'ora, %m per i minuti e %%s per i secondi. Ãˆ utile per vedere quando Ã¨ avvenuto un evento (p.es. l'accesso od un messaggio di un \
utente).
Idõlenyomat alakja! Használd órához a %h, perchez a %m és másodperchez a %s jelzõt. Hasznos, ha látható az események idõpontja (egy felhasználó mikor csatlakozott, beszélt).
Lag ditt eget tidsstempel! Bruk %h for timer, %m for minutter og %s for sekunder. Dette anvendes for å se tidspunktet for en spesiell hendelse (som f.eks. en bruker som entret eller snakket).
Spersonalizuj swoje oznaczenia czasowe! U¿yj %h dla godzin, %m - minut i %s - sekund. Jest to u¿yteczne, je¶li chcesz wiedzieæ kiedy co¶ siê wydarzy³o (np. u¿ytkownik siê do³±czy³ lub co¶ \
powiedzia³).
Modifiera tidstämplingen. Använd %h för timmar, %m för minuter, och %s för sekunder. Användbart för att se när något hände (såsom när en användare kom in, eller talade).
;
MSG_MUICLASS_SETTINGSGUI_CHANNELVIEWTIMEWIDTH_GAD
and a width of
und der Breite von
and a width of
and a width of
and a width of
and a width of
and a width of
and a width of
and a width of
and a width of
;
MSG_MUICLASS_SETTINGSGUI_CHANNELVIEWTIMEWIDTH_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSGUI_CHANNELVIEWNICKWIDTH_GAD
Nick width:
Nickbreite:
Nick width:
Nick width:
Nick width:
Nick width:
Nick width:
Nick width:
Nick width:
Nick width:
;
MSG_MUICLASS_SETTINGSGUI_CHANNELVIEWNICKWIDTH_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSGUI_CHANNELVIEWSPACING_GAD
Tab spacing:
Spaltenabstand:
Tab spacing:
Tab spacing:
Tab spacing:
Tab spacing:
Tab spacing:
Tab spacing:
Tab spacing:
Tab spacing:
;
MSG_MUICLASS_SETTINGSGUI_CHANNELVIEWSPACING_HELP
/
/
/
/
/
/
/
/
/
/
;
;
; MUICLASS_SETTINGSDCC
;
MSG_MUICLASS_SETTINGSDCC_GENERAL_INFO
General DCC
General DCC
General DCC
General DCC
General DCC
General DCC
General DCC
General DCC
General DCC
General DCC
;
MSG_MUICLASS_SETTINGSDCC_DCCLOCALADDRESS_GAD
Use local address
Lokale Adresse
Use local address
Use local address
Use local address
Use local address
Use local address
Use local address
Use local address
Usar dirección local
;
MSG_MUICLASS_SETTINGSDCC_DCCLOCALADDRESS_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSDCC_DCCENTERADDRESS_GAD
Specify address
Adresse eingeben
Dirección específica
Spécifier l'adresse
Specify address
Specifica indirizzo
Cím meghatározása
Spesifisere adresse
Okre¶l adres dla po³±czeñ
Specificera adress
;
MSG_MUICLASS_SETTINGSDCC_DCCENTERADDRESS_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSDCC_DCCADDRESS_GAD
_a
_d
_a
_a
_a
_a
_a
_a
_a
_a
;
MSG_MUICLASS_SETTINGSDCC_DCCADDRESS_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSDCC_DCCPORTRANGEFROM_GAD
Port range from
Port Bereich von
Port range from
Port range from
Port range from
Port range from
Port range from
Port range from
Port range from
Port range from
;
MSG_MUICLASS_SETTINGSDCC_DCCPORTRANGEFROM_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSDCC_DCCPORTRANGETO_GAD
to
bis
 a:
 à
İùò
fino a
vége
til
 do
DCC port-område slutar
;
MSG_MUICLASS_SETTINGSDCC_DCCPORTRANGETO_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSDCC_INCOMING_INFO
Incoming DCC
Incoming DCC
Incoming DCC
Incoming DCC
Incoming DCC
Incoming DCC
Incoming DCC
Incoming DCC
Incoming DCC
Incoming DCC
;
MSG_MUICLASS_SETTINGSDCC_DCCINACCEPT_GAD
Accept all
Alle akzeptieren
Accept all
Accept all
Accept all
Accept all
Accept all
Accept all
Accept all
Accept all
;
MSG_MUICLASS_SETTINGSDCC_DCCINACCEPT_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSDCC_DCCINPATH_GAD
Download path:
Download path:
Download path:
Download path:
Download path:
Download path:
Download path:
Download path:
Download path:
Download path:
;
MSG_MUICLASS_SETTINGSDCC_DCCINPATH_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSDCC_DCCINWRITEMODE_GAD
If file exists
Wenn die Datein existiert
If file exists
If file exists
If file exists
If file exists
If file exists
If file exists
If file exists
Download path:
;
MSG_MUICLASS_SETTINGSDCC_DCCINWRITEMODE_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_CY_OVERWRITE
overwrite
überschreiben
overwrite
overwrite
overwrite
overwrite
overwrite
overwrite
overwrite
overwrite
;
MSG_CY_RENAME
rename
rename
rename
rename
rename
rename
rename
rename
rename
rename
;
MSG_CY_ASK
ask
ask
ask
ask
ask
ask
ask
ask
ask
ask
;
MSG_CY_CANCEL
cancel
abbrechen
cancel
cancel
cancel
cancel
cancel
cancel
cancel
cancel
;
MSG_MUICLASS_SETTINGSDCC_OUTGOING_INFO
Outgoing DCC
Outgoing DCC
Outgoing DCC
Outgoing DCC
Outgoing DCC
Outgoing DCC
Outgoing DCC
Outgoing DCC
Outgoing DCC
Outgoing DCC
;
MSG_MUICLASS_SETTINGSDCC_DCCOUTPATH_GAD
Upload path:
Upload path:
Upload path:
Upload path:
Upload path:
Upload path:
Upload path:
Upload path:
Upload path:
Upload path:
;
MSG_MUICLASS_SETTINGSDCC_DCCOUTPATH_HELP
/
/
/
/
/
/
/
/
/
/
;
;
; MUICLASS_SETTINGSLOG
;
MSG_MUICLASS_SETTINGSLOG_LOGACTIVEFOR_INFO
Logging is active for...
Mitgeschneiden aktiv für...
Logging is active for...
Logging is active for...
Logging is active for...
Logging is active for...
Logging is active for...
Logging is active for...
Logging is active for...
Logging is active for...
;
MSG_MUICLASS_SETTINGSLOG_PUBLIC_GAD
public channels.
öffentliche Kanäle.
public channels.
public channels.
public channels.
public channels.
public channels.
public channels.
public channels.
public channels.
;
MSG_MUICLASS_SETTINGSLOG_PUBLIC_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSLOG_PRIVATE_GAD
private	channels.
private Kanäle.
private	channels.
private	channels.
private	channels.
private	channels.
private	channels.
private	channels.
private	channels.
private	channels.
;
MSG_MUICLASS_SETTINGSLOG_PRIVATE_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSLOG_LINESTORECOVERFROMLOG_INFO
Lines to be recovered from log...
Zeilen die aus dem Mittschnitt wiederhergestellt werden...
Lines to be recovered from log...
Lines to be recovered from log...
Lines to be recovered from log...
Lines to be recovered from log...
Lines to be recovered from log...
Lines to be recovered from log...
Lines to be recovered from log...
Lines to be recovered from log...
;
MSG_MUICLASS_SETTINGSLOG_LINESTORECOVERPUBLIC_GAD
in public channels:
in öffentliche Kanäle:
in public channels:
in public channels:
in public channels:
in public channels:
in public channels:
in public channels:
in public channels:
in public channels:
;
MSG_MUICLASS_SETTINGSLOG_LINESTORECOVERPUBLIC_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSLOG_LINESTORECOVERPRIVATE_GAD
in private channels:
in private Kanäle:
in private channels:
in private channels:
in private channels:
in private channels:
in private channels:
in private channels:
in private channels:
in private channels:
;
MSG_MUICLASS_SETTINGSLOG_LINESTORECOVERPRIVATE_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSLOG_LOGSPLIT_GAD
Logs get split into parts of
Mitschnitte werden in Abschnitte von
Logs get split into parts of
Logs get split into parts of
Logs get split into parts of
Logs get split into parts of
Logs get split into parts of
Logs get split into parts of
Logs get split into parts of
Logs get split into parts of
;
MSG_MUICLASS_SETTINGSLOG_LOGSPLIT_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSLOG_LOGSPLITLIMIT_GAD
KB.
KB Abschnitte geteilt.
KB.
KB.
KB.
KB.
KB.
KB.
KB.
KB.
;
MSG_MUICLASS_SETTINGSLOG_LOGSPLITLIMIT_HELP
/
/
/
/
/
/
/
/
/
/
;
;
; MUICLASS_SETTINGSALIAS
;
MSG_MUICLASS_SETTINGSALIAS_ADD_GAD
_Add
_Hinzufügen
_Add
_Add
_Add
_Add
_Add
_Add
_Add
_Add
;
MSG_MUICLASS_SETTINGSALIAS_ADD_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSALIAS_REMOVE_GAD
_Remove
_Entfernen
_Remove
_Remove
_Remove
_Remove
_Remove
_Remove
_Remove
_Remove
;
MSG_MUICLASS_SETTINGSALIAS_REMOVE_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSALIAS_ALIAS_GAD
A_lias
A_lias
A_lias
A_lias
A_lias
A_lias
A_lias
A_lias
A_lias
A_lias
;
MSG_MUICLASS_SETTINGSALIAS_ALIAS_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSALIAS_TEXT_GAD
_Name
_Name
_Name
_Name
_Name
_Name
_Name
_Name
_Name
_Name
;
MSG_MUICLASS_SETTINGSALIAS_TEXT_HELP
/
/
/
/
/
/
/
/
/
/
;
;
; MUICLASS_ALIASLIST
;
MSG_MUICLASS_ALIASLIST_DEFAULTALIAS
/short
/kurz
/short
/short
/short
/short
/short
/short
/short
/short
;
MSG_MUICLASS_ALIASLIST_DEFAULTTEXT
<this could be your text>
<Dies Könnte ihr Text sein>
<this could be your text>
<this could be your text>
<this could be your text>
<this could be your text>
<this could be your text>
<this could be your text>
<this could be your text>
<this could be your text>
;
MSG_LV_ALIAS
Alias
Alias
Alias
Alias
Alias
Alias
Alias
Alias
Alias
Alias
;
;
; MUICLASS_SETTINGSBUTTON
;
MSG_MUICLASS_SETTINGSBUTTON_INFO
\033lCodes:\n%s = selected nick, %h = hostname of the selected nick,\n%c = current channel, %n = your own nick
\033lCodes:\n%s = selected nick, %h = hostname of the selected nick,\n%c = current channel, %n = your own nick
\033lCodes:\n%s = selected nick, %h = hostname of the selected nick,\n%c = current channel, %n = your own nick
\033lCodes:\n%s = selected nick, %h = hostname of the selected nick,\n%c = current channel, %n = your own nick
\033lCodes:\n%s = selected nick, %h = hostname of the selected nick,\n%c = current channel, %n = your own nick
\033lCodes:\n%s = selected nick, %h = hostname of the selected nick,\n%c = current channel, %n = your own nick
\033lCodes:\n%s = selected nick, %h = hostname of the selected nick,\n%c = current channel, %n = your own nick
\033lCodes:\n%s = selected nick, %h = hostname of the selected nick,\n%c = current channel, %n = your own nick
\033lCodes:\n%s = selected nick, %h = hostname of the selected nick,\n%c = current channel, %n = your own nick
\033lCodes:\n%s = selected nick, %h = hostname of the selected nick,\n%c = current channel, %n = your own nick
;
MSG_MUICLASS_SETTINGSBUTTON_SHOWBUTTONS_GAD
Show _buttons in main window.
Zeige Tasten im Hauptfenster.
Show _buttons in main window.
Show _buttons in main window.
Show _buttons in main window.
Show _buttons in main window.
Show _buttons in main window.
Show _buttons in main window.
Show _buttons in main window.
Show _buttons in main window.
;
MSG_MUICLASS_SETTINGSBUTTON_SHOWBUTTONS_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSBUTTON_ADD_GAD
_Add
_Hinzufügen
_Add
_Add
_Add
_Add
_Add
_Add
_Add
_Add
;
MSG_MUICLASS_SETTINGSBUTTON_ADD_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSBUTTON_REMOVE_GAD
_Remove
_Entfernen
_Remove
_Remove
_Remove
_Remove
_Remove
_Remove
_Remove
_Remove
;
MSG_MUICLASS_SETTINGSBUTTON_REMOVE_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSBUTTON_NAME_GAD
_Name
_Name
_Name
_Name
_Name
_Name
_Name
_Name
_Name
_Name
;
MSG_MUICLASS_SETTINGSBUTTON_NAME_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSBUTTON_COMMAND_GAD
_Command
_Kommando
_Command
_Command
_Command
_Command
_Command
_Command
_Command
_Command
;
MSG_MUICLASS_SETTINGSBUTTON_COMMAND_HELP
/
/
/
/
/
/
/
/
/
/
;
;
;
; MUICLASS_BUTTONLIST
;
MSG_MUICLASS_BUTTONLIST_DEFAULTNAME
New
Neu
New
New
New
New
New
New
New
New
;
MSG_MUICLASS_BUTTONLIST_DEFAULTCOMMAND
<insert command>
<Kommando einfügen>
<insert command>
<insert command>
<insert command>
<insert command>
<insert command>
<insert command>
<insert command>
<insert command>
;
MSG_LV_NAME
Name
Name
Name
Name
Name
Name
Name
Name
Name
Name
;
MSG_LV_COMMAND
Command
Kommando
Command
Command
Command
Command
Command
Command
Command
Command
;
;
;
; MUICLASS_SETTINGSEVENT
;
MSG_MUICLASS_SETTINGSEVENT_INFO
\033lCodes:\n%b = text being displayed in the window, %t = timestamp,\n%g = network name, %s = servername, %c = channel, %n = own nick, %m = nick
\033lCodes:\n%b = text being displayed in the window, %t = timestamp,\n%g = network name, %s = servername, %c = channel, %n = own nick, %m = nick
\033lCodes:\n%b = text being displayed in the window, %t = timestamp,\n%g = network name, %s = servername, %c = channel, %n = own nick, %m = nick
\033lCodes:\n%b = text being displayed in the window, %t = timestamp,\n%g = network name, %s = servername, %c = channel, %n = own nick, %m = nick
\033lCodes:\n%b = text being displayed in the window, %t = timestamp,\n%g = network name, %s = servername, %c = channel, %n = own nick, %m = nick
\033lCodes:\n%b = text being displayed in the window, %t = timestamp,\n%g = network name, %s = servername, %c = channel, %n = own nick, %m = nick
\033lCodes:\n%b = text being displayed in the window, %t = timestamp,\n%g = network name, %s = servername, %c = channel, %n = own nick, %m = nick
\033lCodes:\n%b = text being displayed in the window, %t = timestamp,\n%g = network name, %s = servername, %c = channel, %n = own nick, %m = nick
\033lCodes:\n%b = text being displayed in the window, %t = timestamp,\n%g = network name, %s = servername, %c = channel, %n = own nick, %m = nick
\033lCodes:\n%b = text being displayed in the window, %t = timestamp,\n%g = network name, %s = servername, %c = channel, %n = own nick, %m = nick
;
MSG_MUICLASS_SETTINGSEVENT_MODE_GAD
Mode:
_Modus:
Mode:
Mode:
Mode:
Mode:
Mode:
Mode:
Mode:
Mode:
;
MSG_MUICLASS_SETTINGSEVENT_MODE_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSEVENT_SCRIPT_GAD
Script:
Script:
Script:
Script:
Script:
Script:
Script:
Script:
Script:
Script:
;
MSG_MUICLASS_SETTINGSEVENT_SCRIPT_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSEVENT_ARGUMENT_GAD
Argument:
Argument:
Argument:
Argument:
Argument:
Argument:
Argument:
Argument:
Argument:
Argument:
;
MSG_MUICLASS_SETTINGSEVENT_ARGUMENT_HELP
/
/
/
/
/
/
/
/
/
/
;
;
; MUICLASS_EVENTLIST
;
MSG_MUICLASS_EVENTLIST_TYPECONNECT
Connect
Connect
Connect
Connect
Connect
Connect
Connect
Connect
Connect
Connect
;
MSG_MUICLASS_EVENTLIST_TYPEDISCONNECT
Disconnect
Disconnect
Disconnect
Disconnect
Disconnect
Disconnect
Disconnect
Disconnect
Disconnect
Disconnect
;
MSG_MUICLASS_EVENTLIST_TYPEJOIN
Join
Join
Join
Join
Join
Join
Join
Join
Join
Join
;
MSG_MUICLASS_EVENTLIST_TYPEPART
Part
Part
Part
Part
Part
Part
Part
Part
Part
Part
;
MSG_MUICLASS_EVENTLIST_TYPEQUIT
Quit
Quit
Quit
Quit
Quit
Quit
Quit
Quit
Quit
Quit
;
MSG_MUICLASS_EVENTLIST_TYPEDCCRECEIVEOFFERED
DCC RECV Offered
DCC RECV Offered
DCC RECV Offered
DCC RECV Offered
DCC RECV Offered
DCC RECV Offered
DCC RECV Offered
DCC RECV Offered
DCC RECV Offered
DCC RECV Offered
;
MSG_MUICLASS_EVENTLIST_TYPEDCCRECEIVEDONE
DCC RECV Finished
DCC RECV Finished
DCC RECV Finished
DCC RECV Finished
DCC RECV Finished
DCC RECV Finished
DCC RECV Finished
DCC RECV Finished
DCC RECV Finished
DCC RECV Finished
;
MSG_MUICLASS_EVENTLIST_TYPEDCCSENDEDONE
DCC SEND Finished
DCC SEND Finished
DCC SEND Finished
DCC SEND Finished
DCC SEND Finished
DCC SEND Finished
DCC SEND Finished
DCC SEND Finished
DCC SEND Finished
DCC SEND Finished
;
MSG_MUICLASS_EVENTLIST_TYPECHANNELMESSAGE
Channel Message
Channel Message
Channel Message
Channel Message
Channel Message
Channel Message
Channel Message
Channel Message
Channel Message
Channel Message
;
MSG_MUICLASS_EVENTLIST_TYPEPRIVATEMESSAGE
Private Message
Private Message
Private Message
Private Message
Private Message
Private Message
Private Message
Private Message
Private Message
Private Message
;
MSG_MUICLASS_EVENTLIST_TYPENOTICE
Notice
Notice
Notice
Notice
Notice
Notice
Notice
Notice
Notice
Notice
;
MSG_MUICLASS_EVENTLIST_TYPECTCPREQUEST
CTCP Request
CTCP Request
CTCP Request
CTCP Request
CTCP Request
CTCP Request
CTCP Request
CTCP Request
CTCP Request
CTCP Request
;
MSG_MUICLASS_EVENTLIST_TYPEKICK
Kick
Kick
Kick
Kick
Kick
Kick
Kick
Kick
Kick
Kick
;
MSG_MUICLASS_EVENTLIST_TYPEMODECHANGE
Mode Change
Mode Change
Mode Change
Mode Change
Mode Change
Mode Change
Mode Change
Mode Change
Mode Change
Mode Change
;
MSG_MUICLASS_EVENTLIST_TYPEINVITE
Invite
Invite
Invite
Invite
Invite
Invite
Invite
Invite
Invite
Invite
;
MSG_MUICLASS_EVENTLIST_MODENEVER
Never
Never
Never
Never
Never
Never
Never
Never
Never
Never
;
MSG_MUICLASS_EVENTLIST_MODEALWAYS
Always
Always
Always
Always
Always
Always
Always
Always
Always
Always
;
MSG_MUICLASS_EVENTLIST_WINDOWISACTIVE
Window is active
Window is active
Window is active
Window is active
Window is active
Window is active
Window is active
Window is active
Window is active
Window is active
;
MSG_MUICLASS_EVENTLIST_WINDOWISINACTIVE
Window is inactive
Window is inactive
Window is inactive
Window is inactive
Window is inactive
Window is inactive
Window is inactive
Window is inactive
Window is inactive
Window is inactive
;
MSG_LV_EVENT
Event
Event
Event
Event
Event
Event
Event
Event
Event
Event
;
MSG_LV_MODE
Mode
Mode
Mode
Mode
Mode
Mode
Mode
Mode
Mode
Mode
;
MSG_LV_SCRIPT
Script
Skript
Script
Script
Script
Script
Script
Script
Script
Script
;
;
;
; MUICLASS_SETTINGSSERVER
;
MSG_PG_CONNECTIONS
Connections
Verbindungen
Connections
Connections
Connections
Connections
Connections
Connections
Connections
Connections
;
MSG_PG_ADDITIONAL
Additional
Additional
Additional
Additional
Additional
Additional
Additional
Additional
Additional
Additional
;
MSG_MUICLASS_SETTINGSSERVER_REALNAME_GAD
_Real Name
_Realname
 Nombre _real:
Nom _réel
Ïíïìáôåğşíõìï
Nome _reale
Valódi név
Virkelig navn
Prawdziwe imiê
Verkligt namn
;
MSG_MUICLASS_SETTINGSSERVER_REALNAME_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSSERVER_USERNAME_GAD
User _Name
Nutzer _Name
Usuario:
Nom d'utilisateur
¼íïìá ÷ñŞóôç
Nome utente
Felhasználónév
Brukernavn
Nazwa u¿ytkownika
Användarnamn
;
MSG_MUICLASS_SETTINGSSERVER_USERNAME_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSSERVER_RECONNECT_GAD
Reconnects:
Neuverbindungen:
Reconnects:
Reconnects:
Reconnects:
Reconnects:
Reconnects:
Reconnects:
Reconnects:
Reconnects:
;
;
MSG_MUICLASS_SETTINGSSERVER_RECONNECT_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSSERVER_RECONNECTDELAY_GAD
Reconnect Interval:
Neuverbindungsinterval:
Reconnect Interval:
Reconnect Interval:
Reconnect Interval:
Reconnect Interval:
Reconnect Interval:
Reconnect Interval:
Reconnect Interval:
Reconnect Interval:
;
MSG_MUICLASS_SETTINGSSERVER_RECONNECTDELAY_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSSERVER_DUPEDETECTION_GAD
Dupe detection
Doppelte Erkennen
Dupe detection
Dupe detection
Dupe detection
Dupe detection
Dupe detection
Dupe detection
Dupe detection
Dupe detection
;
MSG_MUICLASS_SETTINGSSERVER_DUPEDETECTION_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSSERVER_ADD_GAD
Add
Add
Add
Add
Add
Add
Add
Add
Add
Add
;
MSG_MUICLASS_SETTINGSSERVER_ADD_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSSERVER_REMOVE_GAD
Remove
Remove
Remove
Remove
Remove
Remove
Remove
Remove
Remove
Remove
;
MSG_MUICLASS_SETTINGSSERVER_REMOVE_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSSERVER_AUTOCONNECT_GAD
Auto connect
Automatisch verbinden
Auto connect
Auto connect
Auto connect
Auto connect
Auto connect
Auto connect
Auto connect
Auto connect
;
MSG_MUICLASS_SETTINGSSERVER_AUTOCONNECT_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSSERVER_NAME_GAD
Name:
Name:
Name:
Name:
Name:
Name:
Name:
Name:
Name:
Name:
;
MSG_MUICLASS_SETTINGSSERVER_NAME_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSSERVER_ADDRESS_GAD
Address:
Adresse:
Address:
Address:
Address:
Address:
Address:
Address:
Address:
Address:
;
MSG_MUICLASS_SETTINGSSERVER_ADDRESS_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSSERVER_PORT_GAD
Port:
Port:
Port:
Port:
Port:
Port:
Port:
Port:
Port:
Port:
;
MSG_MUICLASS_SETTINGSSERVER_PORT_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSSERVER_PASSWORD_GAD
Password:
Password:
Password:
Password:
Password:
Password:
Password:
Password:
Password:
Password:
;
MSG_MUICLASS_SETTINGSSERVER_PASSWORD_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSSERVER_CHARSET_GAD
Charset:
Zeichensatz:
Charset:
Charset:
Charset:
Charset:
Charset:
Charset:
Charset:
Charset:
;
MSG_MUICLASS_SETTINGSSERVER_CHARSET_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSSERVER_NICKADD_GAD
Add
Add
Add
Add
Add
Add
Add
Add
Add
Add
;
MSG_MUICLASS_SETTINGSSERVER_NICKADD_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSSERVER_NICKREMOVE_GAD
Remove
Remove
Remove
Remove
Remove
Remove
Remove
Remove
Remove
Remove
;
MSG_MUICLASS_SETTINGSSERVER_NICKREMOVE_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSSERVER_NICKNAME_GAD
Name:
Name:
Name:
Name:
Name:
Name:
Name:
Name:
Name:
Name:
;
MSG_MUICLASS_SETTINGSSERVER_NICKNAME_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSSERVER_NICKPASSWORD_GAD
Password:
Password:
Password:
Password:
Password:
Password:
Password:
Password:
Password:
Password:
;
MSG_MUICLASS_SETTINGSSERVER_NICKPASSWORD_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSSERVER_CHANNELADD_GAD
Add
Add
Add
Add
Add
Add
Add
Add
Add
Add
;
MSG_MUICLASS_SETTINGSSERVER_CHANNELADD_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSSERVER_CHANNELREMOVE_GAD
Remove
Remove
Remove
Remove
Remove
Remove
Remove
Remove
Remove
Remove
;
MSG_MUICLASS_SETTINGSSERVER_CHANNELREMOVE_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSSERVER_CHANNELNAME_GAD
Name:
Name:
Name:
Name:
Name:
Name:
Name:
Name:
Name:
Name:
;
MSG_MUICLASS_SETTINGSSERVER_CHANNELNAME_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_SETTINGSSERVER_CHANNELPASSWORD_GAD
Password:
Password:
Password:
Password:
Password:
Password:
Password:
Password:
Password:
Password:
;
MSG_MUICLASS_SETTINGSSERVER_CHANNELPASSWORD_HELP
/
/
/
/
/
/
/
/
/
/
;
;
; MUICLASS_SERVERLIST
;
MSG_MUICLASS_SERVERLIST_DEFAULTNAME
<name me>
<name me>
<name me>
<name me>
<name me>
<name me>
<name me>
<name me>
<name me>
<name me>
;
MSG_MUICLASS_SERVERLIST_DEFAULTADDRESS
irc.xxxx.yy
irc.xxxx.yy
irc.xxxx.yy
irc.xxxx.yy
irc.xxxx.yy
irc.xxxx.yy
irc.xxxx.yy
irc.xxxx.yy
irc.xxxx.yy
irc.xxxx.yy
;
MSG_LV_SERVER
Server
Server
Server
Server
Server
Server
Server
Server
Server
Server
;
MSG_LV_PORT
Port
Port
Port
Port
Port
Port
Port
Port
Port
Port
;
MSG_LV_CONNECT
Connect
Verbinden
Connect
Connect
Connect
Connect
Connect
Connect
Connect
Connect
;
MSG_LV_CHARSET
Charset
Zeichensatz
Charset
Charset
Charset
Charset
Charset
Charset
Charset
Charset
;
;
; MUICLASS_NICKLIST
;
MSG_MUICLASS_NICKLIST_DEFAULTNAME
Nickname
Nickname
Nickname
Nickname
Nickname
Nickname
Nickname
Nickname
Nickname
Nickname
;
MSG_LV_NICK
Nick
Nick
Nick
Nick
Nick
Nick
Nick
Nick
Nick
Nick
;
MSG_LV_PASSWORD
Password
Password
Password
Password
Password
Password
Password
Password
Password
Password
;
;
;
; MUICLASS_CHANNELLIST
;
MSG_MUICLASS_CHANNELLIST_DEFAULTNAME
#channelname
#kanalname
#channelname
#channelname
#channelname
#channelname
#channelname
#channelname
#channelname
#channelname
;
MSG_LV_CHANNEL
Channel
Channel
Channel
Channel
Channel
Channel
Channel
Channel
Channel
Channel
;
MSG_LV_YES
Yes
Ja
Sí
Oui
Íáé
Si
Igen
Ja
Tak
Ja
;
MSG_LV_NO
No
NEIN
No
Non
¼÷é
No
Nem
Nei
Nie
Nej
;
;
; MUICLASS_NETWORK
;
MSG_MUICLASS_NETWORK_PART
Part
Part
Abandonar
Sortie
Ìİñïò
Part
Szétbomlás
forlater
Part
Lämna
;
MSG_MUICLASS_NETWORK_JOIN
Join
Verbunden
Acceder
Entrée
¸íôáîç
Entra
Csatlakozás
ankommer
Join
Anslut
;
MSG_MUICLASS_NETWORK_QUIT
Quit
Beenden
Salir
Quitte
¸îïäïò
Esci
Kilépés
Avslutte
Quit
Avsluta
;
MSG_MUICLASS_NETWORK_TOPIC
Topic
Topic
Topic
Topic
Topic
Topic
Topic
Topic
Topic
Topic
;
MSG_MUICLASS_NETWORK_NAMES
Names
Namen
Names
Names
Names
Names
Names
Names
Names
Names
;
MSG_MUICLASS_NETWORK_NOTICE
Notice
Mitteilung
Notice
Notice
Notice
Notice
Notice
Notice
Notice
Notice
;
MSG_MUICLASS_NETWORK_SERVER
Server
Server
Server
Server
Server
Server
Server
Server
Server
Server
;
MSG_MUICLASS_NETWORK_TOPICSETBY
set by\x20
von\x20
establecida por\x20
entré par\x20
ìğŞêå áğü ôïí\x20
di\x20
beállította:\x20
satt av\x20
ustawiony przez\x20
satt av\x20
;
MSG_MUICLASS_NETWORK_TOPICSETONDATE
\x20on\x20
\x20am\x20
\x20el\x20
\x20le\x20
\x20óôéò\x20
\x20al\x20
\x20ekkor:\x20
\x20på\x20
\x20dnia\x20
\x20den\x20
;
MSG_MUICLASS_NETWORK_TOPICNOTSET
not set!
wurde nicht gesetzt!
not set!
not set!
not set!
not set!
not set!
not set!
not set!
not set!
;
MSG_MUICLASS_NETWORK_USERSINCHANNEL
Users in channel
Anwesende im Kanal
Usuarios en el canal
Utilisateurs dans le canal
×ñŞóôåò óôï êáíÜëé
Utenti nel canale
Csatorna felhasználói
Brukere i kanal
U¿ytkownicy na kanale
Användare i kanalen
;
; MUICLASS_QUICKSETUP
;
MSG_MUICLASS_WINDOWQUICKSETUP_TITLE
WookieChat Quick Set-Up
WookieChar Schnellstarter
WookieChat Quick Set-Up
WookieChat Quick Set-Up
WookieChat Quick Set-Up
WookieChat Quick Set-Up
WookieChat Quick Set-Up
WookieChat Quick Set-Up
WookieChat Quick Set-Up
WookieChat Quick Set-Up
;
MSG_MUICLASS_WINDOWQUICKSETUP_INFO
\033cThis is the first time you run WookieChat!\033n\n \nTo finish the automatic configuration\nfill out the field below and continue.
Dies ist das erste Mal, dass wie WookieChat starten!\n \nUm eine automatische Konfiguration zu vervollständigen\nist es nötig die unteren Felder vollstängig auszufüllen\nund den Vorgang fortzusetzen.
\033cThis is the first time you run WookieChat!\033n\n \nTo finish the automatic configuration\nfill out the field below and continue.
\033cThis is the first time you run WookieChat!\033n\n \nTo finish the automatic configuration\nfill out the field below and continue.
\033cThis is the first time you run WookieChat!\033n\n \nTo finish the automatic configuration\nfill out the field below and continue.
\033cThis is the first time you run WookieChat!\033n\n \nTo finish the automatic configuration\nfill out the field below and continue.
\033cThis is the first time you run WookieChat!\033n\n \nTo finish the automatic configuration\nfill out the field below and continue.
\033cThis is the first time you run WookieChat!\033n\n \nTo finish the automatic configuration\nfill out the field below and continue.
\033cThis is the first time you run WookieChat!\033n\n \nTo finish the automatic configuration\nfill out the field below and continue.
\033cThis is the first time you run WookieChat!\033n\n \nTo finish the automatic configuration\nfill out the field below and continue.
;
MSG_MUICLASS_WINDOWQUICKSETUP_REALNAME_GAD
_Real Name:
_Realname:
 Nombre _real:
Nom _réel:
Ïíïìáôåğşíõìï:
Nome _reale:
Valódi név:
Virkelig navn:
Prawdziwe imiê:
Verkligt namn:
;
MSG_MUICLASS_WINDOWQUICKSETUP_REALNAME_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_WINDOWQUICKSETUP_USERNAME_GAD
_User Name:
N_utzer Name:
Usuario:
Nom d'utilisateur:
¼íïìá ÷ñŞóôç:
Nome utente:
Felhasználónév:
Brukernavn:
Nazwa u¿ytkownika:
Användarnamn:
;
MSG_MUICLASS_WINDOWQUICKSETUP_USERNAME_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_WINDOWQUICKSETUP_NICKNAME_GAD
_Nick Name:
_Nick Name:
_Nick Name:
_Nick Name:
_Nick Name:
_Nick Name:
_Nick Name:
_Nick Name:
_Nick Name:
_Nick Name:
;
MSG_MUICLASS_WINDOWQUICKSETUP_NICKNAME_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_WINDOWQUICKSETUP_CONTINUE_GAD
_Continue
_Fortsetzen
_Continue
_Continue
_Continue
_Continue
_Continue
_Continue
_Continue
_Continue
;
MSG_MUICLASS_WINDOWQUICKSETUP_CONTINUE_HELP
/
/
/
/
/
/
/
/
/
/
;
MSG_MUICLASS_WINDOWQUICKSETUP_QUIT_GAD
_Quit
_Beenden
_Quit
_Quit
_Quit
_Quit
_Quit
_Quit
_Quit
_Quit
;
MSG_MUICLASS_WINDOWQUICKSETUP_QUIT_HELP
/
/
/
/
/
/
/
/
/
/
;

