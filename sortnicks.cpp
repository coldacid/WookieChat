    ...
    if(SORT)
    {
        ...
        do
        {
            ...
            for (indx2 = entries-1; indx2 >= indx; --indx2)
            {
                ...
                if(stricmp((p_in)status_conductor->conductor->nicklist[indx2].name,(p_in)status_conductor->conductor->nicklist[indx2-1].name) < 0)
                {
                    ...
                    strcpy(status_conductor->conductor->nicklist[indx2-1].name, status_conductor->conductor->nicklist[indx2].name);
                    strcpy(status_conductor->conductor->nicklist[indx2-1].modes, status_conductor->conductor->nicklist[indx2].modes);
                    //if(!status_conductor->conductor->nicklist[indx2-1].hostname) status_conductor->conductor->nicklist[indx2-1].hostname=new char[HOSTNAME_STRING_SIZE];
...
{
    if(my_settings.sort_tabs_alphabetically==0) return;

    if(DoMethod((Object*)status_conductor->GR_server1_buttons,MUIM_Group_InitChange))
	{

    for(num_nodes=0,status_conductor->conductor=status_conductor->root; status_conductor->conductor; status_conductor->conductor=status_conductor->conductor->next)
    {
        num_nodes++;
    ...
    for(count=1,status_conductor=status_root;
        status_conductor && status_conductor!=work_status; //status_conductor!=status_current;
    ...
    for(status_conductor->conductor=status_conductor->root,status_conductor->iv=(count*100+1); status_conductor->conductor; status_conductor->conductor=status_conductor->conductor->next)
    {
        if(status_conductor->conductor)
        {
            status_conductor->iv++;
        ...
        for(status_conductor->conductor=status_conductor->root; status_conductor->conductor;
            status_conductor->conductor=status_conductor->conductor->next)
            {
                if(status_conductor->conductor->name[0]=='$')
                {
                    ...
                    for(dcc_chat_conductor=dcc_chat_root; dcc_chat_conductor; dcc_chat_conductor=dcc_chat_conductor->next)
                    {
