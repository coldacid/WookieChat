...
BOOL TimerWait_Open(void)
{
                        	...
                        	if ((Timer5IO = (struct timerequest *) CreateIORequest(Timer5MP,sizeof(struct timerequest))))
                            {
                            	if (!(error=OpenDevice((c_in)TIMERNAME,UNIT_WAITUNTIL,(struct IORequest *)Timer5IO,0L)))
                            	{
                                	...
                                	if ((Timer4IO = (struct timerequest *) CreateIORequest(Timer4MP,sizeof(struct timerequest))))
                                    {
                                    	...
                                    	if (!(error=OpenDevice((c_in)TIMERNAME,UNIT_WAITUNTIL,(struct IORequest *)Timer4IO,0L)))
                                    	{
                                            	...
                                            	if ((Timer2IO = (struct timerequest *) CreateIORequest(Timer2MP,sizeof(struct timerequest))) != NULL )
                                                {
                                                	if (!(error=OpenDevice((c_in)TIMERNAME,UNIT_WAITUNTIL,(struct IORequest *)Timer2IO,0L)))
                                                	{
                                                        ...
                                                        {
                                                        	if (!(error=OpenDevice((c_in)TIMERNAME,UNIT_VBLANK,(struct IORequest *)TimerIO,0L)))
                                                        	{
    ...
    if(!Timer1_WaitCheck) { /*TimerWait_Close();*/ return FALSE; } else return TRUE;
...
BOOL TimerWait_Close(void)
{
    ...
    if(Timer2_WaitCheck)
    {
        ...
        if(Timer2MP) DeletePort(Timer2MP);
        ...
        if(Timer1_WaitCheck)
        {
            ...
            if(Timer4_WaitCheck)
            {
                ...
                if(Timer4MP) DeletePort(Timer4MP);
                ...
                if(Timer5_WaitCheck)
                {
                    ...
                    if(Timer5MP) DeletePort(Timer5MP);
