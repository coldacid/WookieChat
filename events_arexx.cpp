...
#define MAX_CODES_TO_CONVERT 7
    ...
    for(int codes_count=0; codes_count<MAX_CODES_TO_CONVERT; codes_count++)
    {
        ...
        while((search=strstr(buffer4,html_codes[codes_count])))
        {
            ...
            if(codes_count==0) sprintf(buffer4,"%s%s%s",start_of_string,buffer3,end_of_string);
            else if(codes_count==1) sprintf(buffer4,"%s%s%s",start_of_string,timestamp,end_of_string);
            else if(codes_count==2 && status_conductor) sprintf(buffer4,"%s%s%s",start_of_string,status_conductor->networkname,end_of_string);
            else if(codes_count==3 && status_conductor) sprintf(buffer4,"%s%s%s",start_of_string,status_conductor->servername,end_of_string);
            else if(codes_count==4 && status_conductor->conductor) sprintf(buffer4,"%s%s%s",start_of_string,status_conductor->conductor->name,end_of_string);
            else if(codes_count==5) sprintf(buffer4,"%s%s%s",start_of_string,status_conductor->nick,end_of_string);
            else if(codes_count==6) sprintf(buffer4,"%s%s%s",start_of_string,target_nick,end_of_string);
            else sprintf(buffer4,"%s%s",start_of_string,end_of_string);
            //printf("buffer4:%s\n",buffer4);
        }
    }
