...
void load_colours(char *load_this_theme)
{
    ...
    if(!open_file)
    {
        ...
        if(work2[0]=='r')
        {
            memset(background2,'\0',63); strcpy(background2," "); strcat(background2,work2);
        ...
        if(work3[0]=='r')
        {
            memset(background2,'\0',63); strcpy(background2," "); strcat(background2,work3);
            background2[0]='2'; background2[1]=':';
        ...
        if(work4[0]=='r')
        {
            memset(background2,'\0',63); strcpy(background2," "); strcat(background2,work4);
            background2[0]='2'; background2[1]=':';
    ...
    sprintf(colour_settings_title,"%s %s",GetCatalogStr(catalog,102,"Colour Settings..."),my_settings.default_colours_theme);
    ...
    len2=(char*)FGets(open_file,(l_in)work2,1024); work2[strlen(work2)-1]='\0';
    if(!len2) strcpy(work2,"2:00000000,00000000,00000000");
    if(work2[0]=='r')
    {
        memset(background2,'\0',63); strcpy(background2," "); strcat(background2,work2);
        background2[0]='2'; background2[1]=':';
    ...
    len2=(char*)FGets(open_file,(l_in)work3,1024); work3[strlen(work3)-1]='\0';
    if(!len2) strcpy(work3,"2:284E284E,3D983D98,91919191");
    if(work3[0]=='r')
    {
        memset(background2,'\0',63); strcpy(background2," "); strcat(background2,work3);
        background2[0]='2'; background2[1]=':';
    ...
    len2=(char*)FGets(open_file,(l_in)work,32); work[strlen(work)-1]='\0';
    if(len2) strcpy(pendisplay_specs[3]->buf, work);
    else strcpy(pendisplay_specs[3]->buf,"r521D521D,FFFFFFFF,2D9C2D9C");

    len2=(char*)FGets(open_file,(l_in)work,32); work[strlen(work)-1]='\0';
    if(len2) strcpy(pendisplay_specs[4]->buf, work);
    else strcpy(pendisplay_specs[4]->buf,"r209C209C,A1AFA1AF,1A271A27");

    len2=(char*)FGets(open_file,(l_in)work,32); work[strlen(work)-1]='\0';
    if(len2) strcpy(pendisplay_specs[5]->buf, work);
    else strcpy(pendisplay_specs[5]->buf,"rE7D8E7D8,25A425A4,FFFFFFFF");

    len2=(char*)FGets(open_file,(l_in)work,32); work[strlen(work)-1]='\0';
    if(len2) strcpy(pendisplay_specs[6]->buf, work);
    else strcpy(pendisplay_specs[6]->buf,"rFFFFFFFF,8FC08FC0,12D112D1");

    len2=(char*)FGets(open_file,(l_in)work,32); work[strlen(work)-1]='\0';
    if(len2) strcpy(pendisplay_specs[7]->buf, work);
    else strcpy(pendisplay_specs[7]->buf,"rFFFFFFFF,0D620D62,00000000");

    len2=(char*)FGets(open_file,(l_in)work,32); work[strlen(work)-1]='\0';
    if(len2) strcpy(pendisplay_specs[8]->buf, work);
    else strcpy(pendisplay_specs[8]->buf,"rFFFF0000,FFFF0000,FFFF0000");

    len2=(char*)FGets(open_file,(l_in)work,32); work[strlen(work)-1]='\0';
    if(len2) strcpy(pendisplay_specs[9]->buf, work);
    else strcpy(pendisplay_specs[9]->buf,"rFFFDFFFD,FFFBFFFB,FFFFFFFF");

    len2=(char*)FGets(open_file,(l_in)work,32); work[strlen(work)-1]='\0';
    if(len2) strcpy(pendisplay_specs[10]->buf, work);
    else strcpy(pendisplay_specs[10]->buf,"rFFFFFFFF,AEACAEAC,D159D159");

    len2=(char*)FGets(open_file,(l_in)work,32); work[strlen(work)-1]='\0';
    if(len2) strcpy(pendisplay_specs[11]->buf, work);
    else strcpy(pendisplay_specs[11]->buf,"r7C3B7C3B,FFFFFFFF,F8CEF8CE");

    len2=(char*)FGets(open_file,(l_in)work,32); work[strlen(work)-1]='\0';
    if(len2) strcpy(pendisplay_specs[12]->buf, work);
    else strcpy(pendisplay_specs[12]->buf,"rFFFFFFFF,FBCCFBCC,25A225A2");

    len2=(char*)FGets(open_file,(l_in)work,32); work[strlen(work)-1]='\0';
    if(len2) strcpy(pendisplay_specs[13]->buf, work);
    else strcpy(pendisplay_specs[13]->buf,"rFFFFFFFF,1A591A59,25292529");

    len2=(char*)FGets(open_file,(l_in)work,32); work[strlen(work)-1]='\0';
    if(len2) strcpy(pendisplay_specs[14]->buf, work);
    else strcpy(pendisplay_specs[14]->buf,"rFFFFFFFF,12D012D0,168E168E");

    len2=(char*)FGets(open_file,(l_in)work,32); work[strlen(work)-1]='\0';
    if(len2) strcpy(pendisplay_specs[15]->buf, work);
    else strcpy(pendisplay_specs[15]->buf,"rFFFFFFFF,24CE24CE,12D212D2");

    len2=(char*)FGets(open_file,(l_in)work,32); work[strlen(work)-1]='\0';
    if(len2) strcpy(pendisplay_specs[16]->buf, work);
    else strcpy(pendisplay_specs[16]->buf,"rFFFFFFFF,8F098F09,1E1D1E1D");

    len2=(char*)FGets(open_file,(l_in)work,32); work[strlen(work)-1]='\0';
    if(len2) strcpy(pendisplay_specs[17]->buf, work);
    else strcpy(pendisplay_specs[17]->buf,"rFFFFFFFF,96409640,34B234B2");

    len2=(char*)FGets(open_file,(l_in)work,32); work[strlen(work)-1]='\0';
    if(len2) strcpy(pendisplay_specs[18]->buf, work);
    else strcpy(pendisplay_specs[18]->buf,"r61E061E0,FFFFFFFF,8CDC8CDC");

    len2=(char*)FGets(open_file,(l_in)work,32); work[strlen(work)-1]='\0';
    if(len2) strcpy(pendisplay_specs[19]->buf, work);
    else strcpy(pendisplay_specs[19]->buf,"r43C043C0,508F508F,FFFFFFFF");

    len2=(char*)FGets(open_file,(l_in)work,32); work[strlen(work)-1]='\0';
    if(len2) strcpy(pendisplay_specs[20]->buf, work);
    else strcpy(pendisplay_specs[20]->buf,"rFFFFFFFF,07850785,0B360B36");

    len2=(char*)FGets(open_file,(l_in)work,32); work[strlen(work)-1]='\0';
    if(len2) strcpy(pendisplay_specs[21]->buf, work);
    else strcpy(pendisplay_specs[21]->buf,"r54F454F4,EBCAEBCA,299B299B");

    len2=(char*)FGets(open_file,(l_in)work,32); work[strlen(work)-1]='\0';
    if(len2) strcpy(pendisplay_specs[22]->buf, work);
    else strcpy(pendisplay_specs[22]->buf,"rFFFDFFFD,FFFCFFFC,FFFFFFFF");

    len2=(char*)FGets(open_file,(l_in)work4,1024); work4[strlen(work4)-1]='\0';
    if(!len2) strcpy(work4,"2:DE50DE50,DE4ADE4A,DE47DE47");
    if(work4[0]=='r')
    {
        memset(background2,'\0',63); strcpy(background2," "); strcat(background2,work4);
        background2[0]='2'; background2[1]=':';
    ...
    len2=(char*)FGets(open_file,(l_in)work,32); work[strlen(work)-1]='\0';
    if(len2) strcpy(pendisplay_specs[24]->buf, work);
    else strcpy(pendisplay_specs[24]->buf,"2:00000000,00000000,00000000");

    colour=8;

    if(open_file) Close(open_file);
...
void save_colours(void)
{
    ...
    FPuts(save_file,(l_in)popimage_background); FPuts(save_file,(l_in)"\n");

    FPuts(save_file,(l_in)popimage_nicklistbackground); FPuts(save_file,(l_in)"\n");
    ...
    */
    ...
    FPuts(save_file,(l_in)pendisplay_specs[3]->buf); FPuts(save_file,(l_in)"\n");

    FPuts(save_file,(l_in)pendisplay_specs[4]->buf); FPuts(save_file,(l_in)"\n");

    FPuts(save_file,(l_in)pendisplay_specs[5]->buf); FPuts(save_file,(l_in)"\n");

    FPuts(save_file,(l_in)pendisplay_specs[6]->buf); FPuts(save_file,(l_in)"\n");

    FPuts(save_file,(l_in)pendisplay_specs[7]->buf); FPuts(save_file,(l_in)"\n");

    FPuts(save_file,(l_in)pendisplay_specs[8]->buf); FPuts(save_file,(l_in)"\n");

    FPuts(save_file,(l_in)pendisplay_specs[9]->buf); FPuts(save_file,(l_in)"\n");

    FPuts(save_file,(l_in)pendisplay_specs[10]->buf); FPuts(save_file,(l_in)"\n");

    FPuts(save_file,(l_in)pendisplay_specs[11]->buf); FPuts(save_file,(l_in)"\n");

    FPuts(save_file,(l_in)pendisplay_specs[12]->buf); FPuts(save_file,(l_in)"\n");

    FPuts(save_file,(l_in)pendisplay_specs[13]->buf); FPuts(save_file,(l_in)"\n");

    FPuts(save_file,(l_in)pendisplay_specs[14]->buf); FPuts(save_file,(l_in)"\n");

    FPuts(save_file,(l_in)pendisplay_specs[15]->buf); FPuts(save_file,(l_in)"\n");

    FPuts(save_file,(l_in)pendisplay_specs[16]->buf); FPuts(save_file,(l_in)"\n");

    FPuts(save_file,(l_in)pendisplay_specs[17]->buf); FPuts(save_file,(l_in)"\n");

    FPuts(save_file,(l_in)pendisplay_specs[18]->buf); FPuts(save_file,(l_in)"\n");

    FPuts(save_file,(l_in)pendisplay_specs[19]->buf); FPuts(save_file,(l_in)"\n");

    FPuts(save_file,(l_in)pendisplay_specs[20]->buf); FPuts(save_file,(l_in)"\n");

    FPuts(save_file,(l_in)pendisplay_specs[21]->buf); FPuts(save_file,(l_in)"\n");

    FPuts(save_file,(l_in)pendisplay_specs[22]->buf); FPuts(save_file,(l_in)"\n");

    //FPuts(save_file,(l_in)pendisplay_specs[23]->buf); FPuts(save_file,(l_in)"\n");
    FPuts(save_file,(l_in)popimage_tabsbackground); FPuts(save_file,(l_in)"\n");

    FPuts(save_file,(l_in)pendisplay_specs[24]->buf); FPuts(save_file,(l_in)"\n");
