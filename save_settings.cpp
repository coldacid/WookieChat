...
void load_colours_choice()
{
    ...
    if(save_file)
    {
        ...
        {
            if(DEBUG) printf("loaded default colour theme:%s\n",my_settings.default_colours_theme);
        }
        else
...
void save_ignore_list()
{
    ...
    if(save_file)
    {
        ...
        for(unsigned int count=0; count<entries && count<100; count++)
        {
            ...
            if(my_settings.totalignores==100) break;
...
void load_ignore_list()
{
    ...
    if(ignore_file)
    {
        while(FGets(ignore_file,(b_in)urlgrabber_str, 2000))
        {
            ...
            for(unsigned int count=0; count<strlen(urlgrabber_str); count++) urlgrabber_str[count]=ToLower(urlgrabber_str[count]);
...
//    char *a=new char[800];
    ...
    sprintf(output_string,"#the next two time values are added together for the paste delay (seconds + microseconds)\n");
    FPuts(save_file,(l_in)output_string);
    ...
    sprintf(output_string,"#for the setting below, choose 1 to sort tabs alphabetically when they open, ot 0 to NOT sort tabs alphabetically\n");
    FPuts(save_file,(l_in)output_string);
...
void retrieve_settings()
{
    ...
    if(a) my_settings.display_server_window_on_startup=1; else my_settings.display_server_window_on_startup=0;
    ...
    if(a) my_settings.hide_user_list_buttons=1; else my_settings.hide_user_list_buttons=0;

    getmacro((Object*)WookieChat->CH_clone_detection,MUIA_Selected,&a);
    if(a) my_settings.clone_detection=1; else my_settings.clone_detection=0;


    getmacro((Object*)WookieChat->CH_remove_brackets,MUIA_Selected,&a);
    if(a) my_settings.remove_brackets=1; else my_settings.remove_brackets=0;

    //getmacro((Object*)WookieChat->CH_tabs,MUIA_Selected,&a);
    getmacro((Object*)WookieChat->CYCLE_tabs,MUIA_Cycle_Active,&a);
    if(a) my_settings.use_nlistobject_for_tabs=0; else my_settings.use_nlistobject_for_tabs=1;

    getmacro((Object*)WookieChat->CH_autoaccept,MUIA_Selected,&a);
    if(a) my_settings.dcc_recv_auto_accept=1; else my_settings.dcc_recv_auto_accept=0;

    getmacro((Object*)WookieChat->CH_graphical_nicklist,MUIA_Selected,&a);
    if(a) my_settings.graphical_nicklist=1; else my_settings.graphical_nicklist=0;

    getmacro((Object*)WookieChat->CH_graphical_smileys,MUIA_Selected,&a);
    if(a) my_settings.graphical_smilies=1; else my_settings.graphical_smilies=0;
    ...
    if(a) my_settings.auto_rejoin_on_kick=1; else my_settings.auto_rejoin_on_kick=0;

    getmacro((Object*)WookieChat->CH_auto_open_query_tabs_when_msged,MUIA_Selected,&a);
    if(a) my_settings.open_query_on_privmsg=1; else my_settings.open_query_on_privmsg=0;
    ...
    if(a) my_settings.reconnect_to_same_server=1; else my_settings.reconnect_to_same_server=0;
    ...
    if(a) my_settings.splitup_logfiles=1; else my_settings.splitup_logfiles=0;
    ...
    if(a) my_settings.no_server_tabs=1; else my_settings.no_server_tabs=0;

    getmacro((Object*)WookieChat->CH_sound_replayer,MUIA_Selected,&a);
    if(a) my_settings.use_external_sound_replayer=1; else my_settings.use_external_sound_replayer=0;

    getmacro((Object*)WookieChat->PA_sound_replayer,MUIA_String_Contents,&b);
    if(b) strcpy(my_settings.external_sound_replayer,b);


    getmacro((Object*)WookieChat->CH_hide_joins_parts,MUIA_Selected,&a);
    if(a) my_settings.show_joins_parts=0; else my_settings.show_joins_parts=1;
    ...
    if(a) my_settings.hide_channel_mode_buttons=1; else my_settings.hide_channel_mode_buttons=0;


    getmacro((Object*)WookieChat->CH_user_modes,MUIA_Selected,&a);
    if(a) my_settings.user_modes_beside_nicks=1; else my_settings.user_modes_beside_nicks=0;
...
void set_settings()
{
    ...
    {
        DoMethod((Object*)WookieChat->LV_alias,MUIM_NList_InsertSingle,my_settings.command_alias[count],MUIV_NList_Insert_Bottom);
    ...
    if(my_settings.ipaddr_use) setmacro((Object*)WookieChat->CH_ipaddr_dcc,MUIA_Selected,TRUE);
        else setmacro((Object*)WookieChat->CH_ipaddr_dcc,MUIA_Selected,FALSE);
    ...
    setmacro((Object*)WookieChat->STR_sound_sample_tabopen,MUIA_String_Contents,my_settings.sound_sample_tabopen);

    setmacro((Object*)WookieChat->STR_sound_sample_newmsg,MUIA_String_Contents,my_settings.sound_sample_newmsg);

    setmacro((Object*)WookieChat->STR_sound_sample_highlight,MUIA_String_Contents,my_settings.sound_sample_highlight);
    ...
    setmacro((Object*)WookieChat->RA_sound_sample_highlight,MUIA_Cycle_Active,my_settings.play_sound_when_highlight);


    setmacro((Object*)WookieChat->RA_autoaccept,MUIA_Cycle_Active,my_settings.dcc_recv_auto_accept_if_exists_action);
    ...
    if(status_current->current_query && my_settings.use_column_display==1) setmacro((Object*)status_current->current_query->LV_channel,MUIA_NList_Format,listview_format);
...
void load_settings()
{
    ...
    while(1)
    {
        ...
        else if(!stricmp(which_setting,"SORT_TABS_ALPHABETICALLY"))
        {
            ...
            else my_settings.sort_tabs_alphabetically=1;
        ...
        else if(!stricmp(which_setting,"USE_EXTERNAL_SOUND_REPLAYER"))
        {
            ...
            else my_settings.use_external_sound_replayer=0;

        }
        else if(!stricmp(which_setting,"EXTERNAL_SOUND_REPLAYER"))
        {
            if(setting_value) strcpy(my_settings.external_sound_replayer,setting_value);
                else strcpy(my_settings.external_sound_replayer,"Unconfigured_please_choose_a_player");

        }
        else if(!stricmp(which_setting,"TIMESTAMP_COLUMN_WIDTH"))
        {
            if(setting_value) my_settings.timestamp_column_width=atoi(setting_value);

            if(my_settings.timestamp_column_width==-1) my_settings.timestamp_column_width=90;


        }
        else if(!stricmp(which_setting,"NICK_COLUMN_WIDTH"))
        {
            if(setting_value) my_settings.nick_column_width=atoi(setting_value);

            if(my_settings.nick_column_width==-1) my_settings.nick_column_width=150;
        ...
        else if(!stricmp(which_setting,"RELOAD_CHAN"))
        {
            ...
            if(setting_value) my_settings.how_many_lines_to_reload_channel=atoi(setting_value);
                else my_settings.how_many_lines_to_reload_channel=15;
        ...
        else if(!stricmp(which_setting,"RELOAD_PRIVATE"))
        {
            ...
            if(setting_value) my_settings.how_many_lines_to_reload_private=atoi(setting_value);
                else my_settings.how_many_lines_to_reload_private=15;
        }
        else if(!stricmp(which_setting,"DCC_START_PORT"))
        {
            //set((Object*)WookieChat->STR_port1_dcc,MUIA_String_Contents,setting_value);
            if(setting_value) strcpy(my_settings.start_port,setting_value);
                else strcpy(my_settings.start_port,"1025");

        }
        else if(!stricmp(which_setting,"DCC_END_PORT"))
        {
            //set((Object*)WookieChat->STR_port2_dcc,MUIA_String_Contents,setting_value);
            if(setting_value) strcpy(my_settings.end_port,setting_value);
                else strcpy(my_settings.end_port,"1035");

        }
        else if(!stricmp(which_setting,"HIGHLIGHT"))
        {
            //set((Object*)WookieChat->STR_highlight_label,MUIA_String_Contents,setting_value);
            if(setting_value) strcpy(my_settings.highlight,setting_value);
        ...
        {
            if(setting_value) strcpy(my_settings.ipaddr,setting_value);
                else strcpy(my_settings.ipaddr,"");
        ...
        {
            if(setting_value) strcpy(my_settings.sound_sample_tabopen,setting_value);

        }
        else if(!stricmp(which_setting,"SAMPLE_NEWMSG"))
        {
            if(setting_value) strcpy(my_settings.sound_sample_newmsg,setting_value);
        ...
        else if(!stricmp(which_setting,"SAMPLE_HIGHLIGHT"))
        {
            ...
            if(setting_value) strcpy(my_settings.sound_sample_highlight,setting_value);
        ...
        {
            if(setting_value) my_settings.nick_completion=atoi(setting_value);
        ...
        {
            if(setting_value) my_settings.delta=atoi(setting_value);
        }
        ...
        else if(!stricmp(which_setting,"PASTE_DELAY_MICROSECONDS"))
        {
            ...
            if(my_settings.paste_delay_microseconds<250000) my_settings.paste_delay_microseconds=250000;

        }
        else if(!stricmp(which_setting,"SHOW_JOINS_PARTS"))
        {

            if(atoi(setting_value)) my_settings.show_joins_parts=1; else my_settings.show_joins_parts=0;

        }
        else if(!stricmp(which_setting,"HIDE_USER_LIST_BUTTONS"))
        {

            if(atoi(setting_value)) my_settings.hide_user_list_buttons=1; else my_settings.hide_user_list_buttons=0;

        }
        else if(!stricmp(which_setting,"HIDE_CHANNEL_MODE_BUTTONS"))
        {

            if(atoi(setting_value)) my_settings.hide_channel_mode_buttons=1; else my_settings.hide_channel_mode_buttons=0;

        }
        else if(!stricmp(which_setting,"USER_MODES_BESIDE_NICKS"))
        {

            if(atoi(setting_value)) my_settings.user_modes_beside_nicks=1; else my_settings.user_modes_beside_nicks=0;
...
void get_events_settings(void)
{
        ...
        {
            if(DEBUG) printf("retrieving event %d..\n",count);
            ...
            if(work_buffer)
            {
                ...
                if((string1=strtok(NULL," "))) my_settings.events[count].use_when=atoi(string1);

                if((string1=strtok(NULL,"\033"))) stccpy(my_settings.events[count].arexx_script,string1,800);
...
int save_events_settings(void)
{
        ...
        for(unsigned int count=0; count<MAX_EVENTS; count++)
        {
            if(DEBUG) printf("saving event %d..\n",count);
        ...
        if(DEBUG) printf("finished saving events, closing save file..\n");
...
int load_events_settings(void)
{
    ...
    char insert_string2[800];
        ...
        for(int count=0; count<MAX_EVENTS; count++)
        {
            ...
            {
                if((string1=strtok(work_buffer," "))) my_settings.events[count].use_when=atoi(string1);
            ...
            else
            {
                ...
                sprintf(insert_string,"%d %s\033%s",my_settings.events[count].use_when, my_settings.events[count].arexx_script, my_settings.events[count].arexx_script2);
...
int create_user_list_buttons(void)
{
    //if(DoMethod((Object*)WookieChat->GR_click_user_list_buttons,MUIM_Group_InitChange))
    {
        ...
        for(unsigned int count2=0; count2<entries && count2<MAX_BUTTONS; count2++)
        {
            ...
            DoMethod((Object*)WookieChat->GR_click_user_list_buttons, OM_ADDMEMBER, (Object*)buttons[count2].BT_click);
...
int save_user_list_buttons_config(void)
{
    ...
    BPTR save_file;
    if(DEBUG) printf("saving user list buttins config..\n");
    ...
    if(save_file)
    {
    if(DEBUG) printf("save file has been opened..\n");

        for(unsigned int count=0; count<entries && count<MAX_BUTTONS; count++)
        {
    if(DEBUG) printf("saving entry %d..\n",count);
        ...
        strcpy(output_string,"\n");
        FPuts(save_file,(l_in)output_string);
    if(DEBUG) printf("finished saving, closing save file..\n");

        Close(save_file);
    if(DEBUG) printf("finished saving, file closed!\n");
